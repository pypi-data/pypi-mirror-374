...
def requires_confirmation(action: str, ttl: int = 300, skip_private: bool = False):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(ctx: Context, *args, **kwargs) -> Output:
            ...
            # Get package info for token generation
            try:
                pkg = ctx.package
            except ValueError:
                # If no package can be determined, use root name
                pkg_name = ctx.name
                pkg_version = ctx.version
            else:
                pkg_name = pkg.name
            ...
            if not provided or not verify_token(pkg_name, action, provided):
                new_token = generate_token(pkg_name, action, ttl)

                # Build details based on action
                details = [
                    {"type": "text", "content": f"Package: {pkg_name} v{pkg_version}"},
                    {
                        "type": "text",
...
def requires_review(review_type: str, review_commands: list[str], ttl: int = 600):
    ...
    def decorator(func: Callable) -> Callable:
        @wraps(func)
        def wrapper(ctx: Context, *args, **kwargs) -> Output:
            # Review tokens are always workspace/project-wide, not package-specific
            # This ensures tokens generated by 'git log' work for any package bump
            token_name = ctx.name  # Always use root/project name for review tokens
            ...
            if not provided or not verify_token(token_name, review_action, provided):
                return Output(
                    success=False,
...
def requires_clean_dist(func: Callable) -> Callable:
    ...
    def wrapper(ctx: Context, *args, **kwargs) -> Output:
        ...
        try:
            dist_path = ctx.package.dist_path
        except ValueError:
            # No package context, use root dist
