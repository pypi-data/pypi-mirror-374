...
def validate_conventional_commit(message: str) -> Tuple[bool, Optional[str]]:
    ...
    if not match:
        ...
        wrong_types = {
            ...
            "refactoring": "refactor", "tests": "test",
        }
    ...
    if len(description) < 10:
        return False, f"Description too short (minimum 10 characters)"
...
def git_wrapper(ctx: Context, *git_args) -> Output:
        ...
        if not any(flag in args for flag in ['-l', '--list', '-d', '--delete']):
            return Output(
                ...
                details=[
                    ...
                    {"type": "text", "content": "This ensures version, changelog, and tag stay in sync"},
                ],
            )
    ...
    if args[0] == "commit":
        ...
        if not token or not verify_content_token(ctx.name, "review_staged", tree_hash, token):
            return Output(
                success=False,
    ...
    if args[0] in ["log", "diff", "status", "show"]:
        ...
        if proc.returncode == 0 and proc.stdout.strip():
            ...
            if args[0] == "log":
                ...
                tree_hash = get_staged_tree_hash(ctx)
                if tree_hash:
                    token = generate_content_token(ctx.name, "review_staged", tree_hash, ttl=600)
