...
def a5_metrics(res, unit: str = "m"):                          # length unit is m, area unit is m2
    """
    ...
    avg_cell_area = cell_area(res)  # cell_area returns area in m²
...
def a5stats(unit: str = "m"):       
    ...
    for res in range(min_res, max_res + 1):
        num_cells, avg_edge_len, avg_cell_area, cls = a5_metrics(res, unit=unit)       # length unit is m, area unit is m2
...
def a5_norm_area_hist(a5_gpd):
    ...
    a5_gpd_filtered = a5_gpd[~a5_gpd['crossed']]
    
    # Create the histogram with color ramp
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # Get histogram data
    counts, bins, patches = ax.hist(a5_gpd_filtered['norm_area'], bins=50, alpha=0.7, edgecolor='black')
    
    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = a5_gpd_filtered['norm_area'].min(), a5_gpd_filtered['norm_area'].max(), 1
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
    ...
    stats_text = f'Mean: {a5_gpd_filtered["norm_area"].mean():.3f}\nStd: {a5_gpd_filtered["norm_area"].std():.3f}\nMin: {a5_gpd_filtered["norm_area"].min():.3f}\nMax: {a5_gpd_filtered["norm_area"].max():.3f}'
...
def a5_compactness_hist(a5_gpd):
    """
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(a5_gpd_filtered['ipq'], bins=50, alpha=0.7, edgecolor='black')
    ...
    # Add statistics text box
    stats_text = f'Mean: {a5_gpd_filtered["ipq"].mean():.3f}\nStd: {a5_gpd_filtered["ipq"].std():.3f}\nMin: {a5_gpd_filtered["ipq"].min():.3f}\nMax: {a5_gpd_filtered["ipq"].max():.3f}'
    ax.text(0.02, 0.98, stats_text, transform=ax.transAxes, fontsize=12, verticalalignment='top', 
             bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
