...
def isea3h_metrics(res, unit: str = "m"):                   # length unit is km, area unit is km2
    """
    ...
    avg_edge_len = math.sqrt((2 * avg_cell_area) / (3 * math.sqrt(3))) # edge length in km
...
def isea3hstats(unit: str = "m"):                   # length unit is km, area unit is km2
    ...
    for res in range(min_res, max_res + 1):
        num_cells, avg_edge_len, avg_cell_area, cls = isea3h_metrics(res, unit=unit)       # length unit is km, area unit is km2
...
def isea3h_norm_area_hist(isea3h_gpd):
    """
    ...
    isea3h_gpd_filtered = isea3h_gpd[~isea3h_gpd['crossed']]
    
    # Create the histogram with color ramp
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # Get histogram data
    counts, bins, patches = ax.hist(isea3h_gpd_filtered['norm_area'], bins=50, alpha=0.7, edgecolor='black')
    
    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = isea3h_gpd_filtered['norm_area'].min(), isea3h_gpd_filtered['norm_area'].max(), 1
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
    ...
    stats_text = f'Mean: {isea3h_gpd_filtered["norm_area"].mean():.3f}\nStd: {isea3h_gpd_filtered["norm_area"].std():.3f}\nMin: {isea3h_gpd_filtered["norm_area"].min():.3f}\nMax: {isea3h_gpd_filtered["norm_area"].max():.3f}'
...
def isea3h_compactness_hist(isea3h_gpd):
    """
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(isea3h_gpd_filtered['ipq'], bins=50, alpha=0.7, edgecolor='black')
    ...
    ax.axvline(x=0.907, color='red', linestyle='--', linewidth=2, label='Ideal Hexagon (IPQ = 0.907)')
    
    # Add statistics text box
    stats_text = f'Mean: {isea3h_gpd_filtered["ipq"].mean():.3f}\nStd: {isea3h_gpd_filtered["ipq"].std():.3f}\nMin: {isea3h_gpd_filtered["ipq"].min():.3f}\nMax: {isea3h_gpd_filtered["ipq"].max():.3f}'
    ax.text(0.02, 0.98, stats_text, transform=ax.transAxes, fontsize=12, verticalalignment='top', 
             bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
