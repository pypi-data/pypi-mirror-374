...
import warnings
warnings.filterwarnings("ignore", message="driver ESRI Shapefile does not support open option DRIVER", category=RuntimeWarning)
...
def dggsinspect():
    ...
    dggs_configs = {
        'h3': {
            'inspect_func': h3inspect,
            'param_name': 'res',
            'cell_id_col': 'h3'
        },
        's2': {
            'inspect_func': s2inspect,
            'param_name': 'res',
            'cell_id_col': 's2'
        },
        'a5': {
            'inspect_func': a5inspect,
            'param_name': 'res',
            'cell_id_col': 'a5'
        },
        'isea4t': {
    ...
    for dggs_type, config in dggs_configs.items():
        ...
        for res in range(min_res, max_res + 1):
            try:
                ...
                if dggs_type.startswith('dggrid_'):
                    ...
                    gdf = config['inspect_func'](dggrid_instance, dggs_type=config['dggs_type'], **{config['param_name']: res})
                elif 'dggs_type' in config:
                    # For dggal functions that need dggs_type parameter
                    gdf = config['inspect_func'](dggs_type=config['dggs_type'], **{config['param_name']: res})
                else:
        ...
        if dggs_type_gdfs:
            ...
            print(f"  ✓ Successfully saved {len(combined_gdf)} records to {output_file}")
...
def dggsinspect_cli():
    ...
    parser = argparse.ArgumentParser(description="Multi-DGGS inspection tool using DGGS_INSPECT configuration")
    
    args = parser.parse_args()
    
    try:
        results = dggsinspect()
        # Files are already saved in the main function, just return results
        return results
    except Exception as e:
        print(f"Error: {e}")
        return None
...
def dggsboxplot(parquet_file: str, y_column: str = "norm_area", y_lim: tuple = (0.5, 1.3)) -> pd.DataFrame:
    """
    ...
    # Plot the boxplots
    chart = sns.boxplot(x='dggs_type', y=y_column, data=df, palette="viridis", 
                       saturation=0.9, showfliers=True, flierprops=outlier_design)
    ...
    summary = df.groupby('dggs_type')[y_column].agg(['count', 'mean', 'std', 'min', 'max'])
    print(summary)
...
def dggsboxplot_cli():
    ...
    parser = argparse.ArgumentParser(description="Create boxplots from DGGS inspection data")
    parser.add_argument('-input','--input', type=str, required=True,
                       help='Input parquet file path containing DGGS inspection data')
    parser.add_argument('-y','--y-column', type=str, default="norm_area",
                       help='Column name to plot on y-axis (default: norm_area)')
    parser.add_argument('-ymin','--ymin', type=float, default=0.5,
                       help='Minimum y-axis value (default: 0.5)')
    parser.add_argument('-ymax','--ymax', type=float, default=1.3,
...
def dggs_dggridinspect(dggs_type: str = None) -> gpd.GeoDataFrame:
    ...
    if dggs_type is not None:
        if dggs_type not in dggs_types:
            raise ValueError(f"Invalid dggs_type: {dggs_type}. Valid types: {list(dggs_types.keys())}")
        ...
        for res in range(min_res, max_res + 1):
            try:
                ...
                if gdf is None or len(gdf) == 0:
                    print(f"Warning: dggridinspect returned empty result for {dggs_type_key} at resolution {res}")
                    continue
                ...
                print(f"✓ Completed dggridinspect for {dggs_type_key} at resolution {res}")
...
def dggs_dggridinspect_cli():
    ...
    parser = argparse.ArgumentParser(description="Multi-DGGS inspection tool using DGGS_INSPECT configuration")
    parser.add_argument('-dggs', '--dggs_type', dest='dggs_type', type=str, default=None,
    ...
    try:
        ...
        if args.dggs_type:
            print(f"Generated inspection data for {len(result)} cells for DGGS type: {args.dggs_type}")
...
def dggsboxplot_folder(folder: str = ".", y_column: str = "norm_area", y_lim: tuple = (0.5, 1.3)) -> pd.DataFrame:
    """
    ...
    print(f"Combined data with {len(df)} cells across DGGS types: {df['dggs_type'].unique()}")
    print(f"Resolution range: {df['resolution'].min()}-{df['resolution'].max()}")
...
def dggsboxplot_folder_cli():
    ...
    parser = argparse.ArgumentParser(description="Create boxplots from DGGS inspection data files in a folder")
    parser.add_argument('-folder','--folder', type=str, default=".",
...
def hilbert_curve(dggs_type, resolution, bbox=None, output_format="gpd", figsize=(12, 8), save_plot=True):
    """
    ...
    try:
        ...
        raise ImportError("matplotlib and numpy are required for hilbert curve visualization")
    
    # Map DGGS types to their grid generation functions
    dggs_grid_functions = {
        'h3': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.h3grid', fromlist=['h3grid']).h3grid(res, bbox, output_format),
        's2': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.s2grid', fromlist=['s2grid']).s2grid(res, bbox, output_format),
        'a5': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.a5grid', fromlist=['a5grid']).a5grid(res, bbox, output_format),
        'rhealpix': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.rhealpixgrid', fromlist=['rhealpixgrid']).rhealpixgrid(res, bbox, output_format),
        'isea4t': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.isea4tgrid', fromlist=['isea4tgrid']).isea4tgrid(res, bbox, output_format),
        'isea3h': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.isea3hgrid', fromlist=['isea3hgrid']).isea3hgrid(res, bbox, output_format),
        'ease': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.easegrid', fromlist=['easegrid']).easegrid(res, bbox, output_format),
        'qtm': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.qtmgrid', fromlist=['qtmgrid']).qtmgrid(res, bbox, output_format),
        'olc': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.olcgrid', fromlist=['olcgrid']).olcgrid(res, bbox, output_format),
        'geohash': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.geohashgrid', fromlist=['geohashgrid']).geohashgrid(res, bbox, output_format),
        'georef': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.georefgrid', fromlist=['georefgrid']).georefgrid(res, bbox, output_format),
        'mgrs': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.mgrsgrid', fromlist=['mgrsgrid']).mgrsgrid(res, bbox, output_format),
        'tilecode': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.tilecodegrid', fromlist=['tilecodegrid']).tilecodegrid(res, bbox, output_format),
        'quadkey': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.quadkeygrid', fromlist=['quadkeygrid']).quadkeygrid(res, bbox, output_format),
        'maidenhead': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.maidenheadgrid', fromlist=['maidenheadgrid']).maidenheadgrid(res, bbox, output_format),
        'gars': lambda res, bbox=None, output_format="gpd": __import__('vgrid.generator.garsgrid', fromlist=['garsgrid']).garsgrid(res, bbox, output_format),
    }
    
    dggs_type = dggs_type.lower()
    if dggs_type not in dggs_grid_functions:
        raise ValueError(f"Unsupported DGGS type: {dggs_type}. Supported types: {list(dggs_grid_functions.keys())}")
    ...
    if grid_gdf is None or len(grid_gdf) == 0:
        raise ValueError(f"No grid cells generated for {dggs_type} at resolution {resolution}")
    ...
    grid_gdf.plot(ax=ax1, column=f"{dggs_type}", cmap='viridis', legend=True, 
                  legend_kwds={'orientation': 'horizontal', 'shrink': 0.8})
    ax1.set_title(f'{dggs_type.upper()} Grid - Resolution {resolution}\nCell Ordering by ID')
    ...
    scatter = ax2.scatter(lon_norm, lat_norm, c=range(len(grid_gdf)), 
    ...
    ax2.set_title(f'Hilbert Curve Visualization\n{dggs_type.upper()} Resolution {resolution}')
...
def hilbert_curve_cli():
    ...
    parser = argparse.ArgumentParser(description="Visualize Hilbert curve for DGGS grids.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-save",
        action="store_true",
        help="Don't save the plot to file"
    )
