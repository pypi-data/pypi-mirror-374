...
from vgrid.utils.constants import AUTHALIC_AREA, VMIN_TRI, VMAX_TRI, VCENTER_TRI, DGGS_TYPES
...
def isea4t_metrics(res, unit: str = "m"):                       # length unit is km, area unit is km2
    """
...
def isea4tstats(unit: str = "m"):                       # length unit is km, area unit is km2
    ...
    for res in range(min_res, max_res + 1):
        num_cells, avg_edge_len, avg_cell_area, cls = isea4t_metrics(res, unit=unit)       # length unit is km, area unit is km2
...
def isea4t_norm_area_hist(isea4t_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(isea4t_gpd_filtered['norm_area'], bins=50, alpha=0.7, edgecolor='black')
    
    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = isea4t_gpd_filtered['norm_area'].min(), isea4t_gpd_filtered['norm_area'].max(), 1
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
...
def isea4t_compactness_hist(isea4t_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(isea4t_gpd_filtered['ipq'], bins=50, alpha=0.7, edgecolor='black')
    ...
    ax.axvline(x=0.604, color='red', linestyle='--', linewidth=2, label='Ideal Triangle (IPQ = 0.604)')
