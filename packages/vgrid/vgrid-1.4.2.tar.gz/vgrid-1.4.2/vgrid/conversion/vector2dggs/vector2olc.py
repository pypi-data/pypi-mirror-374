"""
OLC Grid Conversion Module

This module provides comprehensive functionality for converting vector geometries to OLC (Open Location Code) grid cells.
OLC is a hierarchical geospatial indexing system that divides the Earth's surface into cells of
varying resolutions, from 2 (coarsest) to 15 (finest).

Key Features:
- Convert points, lines, and polygons to OLC grid cells
...
- Topology preservation mode to ensure disjoint features have disjoint OLC cells
- OLC compact mode to reduce cell count for polygons
...
The module uses the OLC library for hierarchical geospatial indexing and supports
...
from vgrid.utils.io import validate_olc_resolution, convert_to_output_format, process_input_data_vector, DGGS_TYPES
...
def point2olc(      
    ...
    Returns:
        list: List of dictionaries representing OLC cells containing the point(s)
