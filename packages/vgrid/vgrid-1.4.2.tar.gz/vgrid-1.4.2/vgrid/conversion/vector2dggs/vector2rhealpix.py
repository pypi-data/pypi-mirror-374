"""
rHEALPix Grid Conversion Module

This module provides comprehensive functionality for converting vector geometries to rHEALPix grid cells.
rHEALPix is a hierarchical geospatial indexing system that divides the Earth's surface into cells of
varying resolutions, from 0 (coarsest) to 30 (finest).

Key Features:
- Convert points, lines, and polygons to rHEALPix grid cells
...
- Topology preservation mode to ensure disjoint features have disjoint rHEALPix cells
- rHEALPix compact mode to reduce cell count for polygons
...
def point2rhealpix(
    ...
    Args:
        ...
        compact (bool, optional): Enable rHEALPix compact mode (not used for points)
    ...
    Returns:
        list: List of dictionaries representing rHEALPix cells containing the point(s)
