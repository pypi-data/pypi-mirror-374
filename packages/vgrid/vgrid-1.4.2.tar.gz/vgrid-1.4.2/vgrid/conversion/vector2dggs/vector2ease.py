"""
EASE Grid Conversion Module

This module provides comprehensive functionality for converting vector geometries to EASE (Equal-Area Scalable Earth) grid cells.
EASE is a hierarchical geospatial indexing system that divides the Earth's surface into equal-area cells of
varying resolutions, from 0 (coarsest) to 6 (finest).

Key Features:
- Convert points, lines, and polygons to EASE grid cells
...
- Topology preservation mode to ensure disjoint features have disjoint EASE cells
- EASE compact mode to reduce cell count for polygons
...
The module uses the EASE-DGGS library for hierarchical equal-area geospatial indexing and supports
...
from vgrid.utils.io import validate_ease_resolution, process_input_data_vector, convert_to_output_format, DGGS_TYPES
...
def point2ease(
    ...
    Args:
        feature (shapely.geometry.Point): Point geometry to convert
    ...
    if topology:
        ...
        if points_list:
            ...
            if shortest_distance > 0:
                for res in range(min_res, max_res+1):  # EASE resolution range is [0..6]
