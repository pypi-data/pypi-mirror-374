...
class VGRID:
    ...
    def __init__(self, cell_size: float, aperture: int = 4):
        ...
        if self.resolution < -MAX_HIERARCHY_LEVEL:
            raise ValueError(f"Calculated resolution {self.resolution} is too negative (minimum: {-MAX_HIERARCHY_LEVEL})")
        if self.resolution > MAX_HIERARCHY_LEVEL:
            raise ValueError(f"Calculated resolution {self.resolution} exceeds maximum {MAX_HIERARCHY_LEVEL}")
    ...
    def get_latlon(self, vgrid_id: int) -> tuple[float, float]:
        ...
        if resolution != self.resolution:
            raise ValueError(f"VGrid ID resolution {resolution} doesn't match VGRID instance resolution {self.resolution}")
    ...
    def tiles_for_bounding_box(
        ...
        assert bottom <= top, "Bottom latitude must be less than or equal to top latitude"
        ...
        if left > right:
            east = self.tiles_for_bounding_box(left, bottom, 180.0, top, target_resolution)
            west = self.tiles_for_bounding_box(-180.0, bottom, right, top, target_resolution)
            return east + west
        ...
        for x in range(int(left / target_cell_size_lon), int(right / target_cell_size_lon) + 1):
            # For each row (latitude)
            for y in range(int(bottom / target_cell_size_lat), int(top / target_cell_size_lat) + 1):
                # Ensure we don't exceed grid bounds
                if x >= 0 and x < target_total_columns and y >= 0 and y < target_total_rows:
