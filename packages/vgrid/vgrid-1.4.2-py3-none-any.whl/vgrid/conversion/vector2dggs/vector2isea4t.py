"""
ISEA4T Grid Conversion Module

This module provides comprehensive functionality for converting vector geometries to ISEA4T grid cells.
ISEA4T is a hierarchical geospatial indexing system that divides the Earth's surface into cells of
varying resolutions, from 0 (coarsest) to 30 (finest).

Key Features:
- Convert points, lines, and polygons to ISEA4T grid cells
...
- Topology preservation mode to ensure disjoint features have disjoint ISEA4T cells
- ISEA4T compact mode to reduce cell count for polygons
...
from vgrid.utils.io import validate_isea4t_resolution, process_input_data_vector, convert_to_output_format, DGGS_TYPES
import platform
...
def point2isea4t(
    ...
    Args:
        ...
        compact (bool, optional): Enable ISEA4T compact mode (not used for points)
    ...
    Returns:
        list: List of dictionaries representing ISEA4T cells containing the point(s)
...
def vector2isea4t_cli():
    ...
    if platform.system() == "Windows":
        ...
        print("ISEA4T conversion is only supported on Windows systems.", file=sys.stderr)
        sys.exit(1)
