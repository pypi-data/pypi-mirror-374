...
This module provides comprehensive functionality for converting vector geometries to Tilecode grid cells.
Tilecode is a hierarchical geospatial indexing system based on Web Mercator tiles that divides the Earth's surface into cells of
varying resolutions, from 0 (coarsest) to 29 (finest).

Key Features:
- Convert points, lines, and polygons to Tilecode grid cells
...
- Topology preservation mode to ensure disjoint features have disjoint Tilecode cells
- Tilecode compact mode to reduce cell count for polygons
- Multiple output formats (GeoJSON, GPKG, Parquet, CSV, Shapefile)
- Command-line interface for batch processing
- Automatic resolution calculation for topology preservation mode
...
The module uses the mercantile library for Web Mercator tile calculations and supports
...
def point2tilecode(
    ...
    Args:
        feature (shapely.geometry.Point or shapely.geometry.MultiPoint): Point geometry to convert
    ...
    Returns:
        list: List of dictionaries representing Tilecode cells containing the point(s)
...
def polygon2tilecode(
    ...
    if compact and tilecode_rows:
        ...
        compacted_gdf = tilecodecompact(temp_gdf, tilecode_id="tilecode", output_format="gpd")
    ...
    if topology:
        ...
        if points_list:
            ...
            if shortest_distance > 0:
                tilecode_cell_sizes = [40075016.68557849 / (2**z) for z in range(min_res, max_res+1)]
