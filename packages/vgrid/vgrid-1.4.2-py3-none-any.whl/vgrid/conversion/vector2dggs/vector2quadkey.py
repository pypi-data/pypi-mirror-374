...
This module provides comprehensive functionality for converting vector geometries to Quadkey grid cells.
Quadkey is a hierarchical geospatial indexing system based on Web Mercator tiles that divides the Earth's surface into cells of
varying resolutions, from 0 (coarsest) to 29 (finest).

Key Features:
- Convert points, lines, and polygons to Quadkey grid cells
...
- Topology preservation mode to ensure disjoint features have disjoint Quadkey cells
- Quadkey compact mode to reduce cell count for polygons
...
from vgrid.utils.io import validate_quadkey_resolution, process_input_data_vector, convert_to_output_format, DGGS_TYPES
...
def point2quadkey(
    ...
    Returns:
        list: List of dictionaries representing Quadkey cells containing the point(s)
...
def polygon2quadkey(
    ...
    if compact and quadkey_rows:
        ...
        compacted_gdf = quadkeycompact(temp_gdf, quadkey_id="quadkey", output_format="gpd")
    ...
    if topology:
        ...
        if points_list:
            ...
            if shortest_distance > 0:
                quadkey_cell_sizes = [40075016.68557849 / (2**z) for z in range(min_res, max_res+1)]
