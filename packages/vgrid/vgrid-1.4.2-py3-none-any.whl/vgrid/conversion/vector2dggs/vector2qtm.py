"""
QTM Grid Conversion Module

This module provides comprehensive functionality for converting vector geometries to QTM (Quaternary Triangular Mesh) grid cells.
QTM is a hierarchical geospatial indexing system that divides the Earth's surface into triangular cells of
varying resolutions, from 1 (coarsest) to 24 (finest).

Key Features:
- Convert points, lines, and polygons to QTM grid cells
...
- Topology preservation mode to ensure disjoint features have disjoint QTM cells
- QTM compact mode to reduce cell count for polygons
...
The module uses the QTM library for hierarchical triangular geospatial indexing and supports
...
from vgrid.utils.io import validate_qtm_resolution, process_input_data_vector, convert_to_output_format, DGGS_TYPES
    ...
    if topology:
        ...
        if points_list:
            ...
            if shortest_distance > 0:
                for res in range(min_res, max_res+1):  # QTM resolution range is [1..24]
