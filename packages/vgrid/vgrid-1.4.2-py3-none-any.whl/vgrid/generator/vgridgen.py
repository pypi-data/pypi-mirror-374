...
Key Functions:
- vgrid_grid(): Main grid generation function with bounding box support
...
def vgrid_gen(vgrid_instance: VGRID, resolution: int = 0, bbox: list[float] = None) -> gpd.GeoDataFrame:
    """
    ...
    tiles = vgrid_instance.tiles_for_bounding_box(bbox[0], bbox[1], bbox[2], bbox[3], resolution)
    
    vgrid_records = []
    
    with tqdm(total=len(tiles), desc=f"Generating VGRID resolution {resolution}", unit=" cells") as pbar:
        for res, tile_index in tiles:
            ...
            cell_polygon = Polygon([
                ...
                (min(180, lon + target_cell_size_lon), min(90, lat + target_cell_size_lat)),
...
def vgridgen(vgrid_instance: VGRID, resolution: int = 0, bbox: list[float] = None, output_format: str = "gpd"):
    """
...
def vgridgen_cli():
    ...
    try:
        ...
        result = vgridgen(vgrid_instance, args.resolution, args.bbox, args.output_format)
