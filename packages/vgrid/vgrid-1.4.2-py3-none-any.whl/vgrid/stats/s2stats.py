...
def s2_metrics(res, unit: str = "km"):       # length unit is km, area unit is km2
    """
...
def s2stats(unit: str = "m"):               # length unit is km, area unit is km2
    ...
    for res in range(min_res, max_res + 1):
        num_cells, avg_edge_len, avg_cell_area, cls = s2_metrics(res, unit=unit)       # length unit is km, area unit is km2
...
def s2_norm_area_hist(s2_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(s2_gpd_filtered['norm_area'], bins=50, alpha=0.7, edgecolor='black')
    
    # Create color ramp using the same normalization as the map function
    vmin, vmax, vcenter = s2_gpd_filtered['norm_area'].min(), s2_gpd_filtered['norm_area'].max(), 1
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
...
def s2_compactness_hist(s2_gpd):
    ...
    # Get histogram data
    counts, bins, patches = ax.hist(s2_gpd_filtered['ipq'], bins=50, alpha=0.7, edgecolor='black')
    ...
    ax.axvline(x=1.0, color='red', linestyle='--', linewidth=2, label='Ideal Square (IPQ = 1.0)')
