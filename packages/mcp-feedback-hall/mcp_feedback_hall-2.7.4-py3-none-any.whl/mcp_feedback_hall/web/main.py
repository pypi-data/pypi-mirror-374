        ...
        if env_port:
            try:
                ...
                if custom_port == 0:
                    ...
                    debug_log(
                        f"MCP_WEB_PORT 值無效 ({custom_port})，必須在 1024-65535 範圍內或為 0，使用預設端口 9877"
                    )
            except ValueError:
                debug_log(
                    f"MCP_WEB_PORT 格式錯誤 ({env_port})，必須為數字，使用預設端口 9877"
                )
        else:
    ...
    async def launch_desktop_app(self, url: str) -> bool:
        ...
        try:
            ...
            def import_desktop_app():
                # 首先嘗試從發佈包位置導入
                try:
                    from mcp_feedback_hall.desktop_app import (
...
我已成功為 **mcp-feedback-hall** 專案實現了 Markdown 語法顯示功能！
