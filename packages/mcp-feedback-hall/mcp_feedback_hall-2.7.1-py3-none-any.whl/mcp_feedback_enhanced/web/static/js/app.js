...
(function() {
    ...
    FeedbackApp.prototype.initDebounceHandlers = function() {
        ...
        // 為 WebSocket 訊息處理添加防抖（深度 CPU 優化）
    ...
    /**
     * 恢復當前會話狀態（修復 Cursor 重啟後會話消失問題）
     */
    FeedbackApp.prototype.restoreCurrentSession = function() {
        console.log('🔄 正在恢復當前會話狀態...');
        ...
        fetch('/api/current-session')
            ...
            .then(function(sessionData) {
                if (!sessionData || !sessionData.session_id) {
                    console.log('📭 沒有活躍會話需要恢復');
                    return;
                }

                console.log('✅ 發現活躍會話，正在恢復:', sessionData.session_id);

                // 恢復會話 ID
                self.currentSessionId = sessionData.session_id;

                // 更新 UI 狀態
                if (self.uiManager) {
                    // 根據會話狀態設定 UI 狀態
                    const uiState = sessionData.status === 'feedback_submitted' ? 
                        window.MCPFeedback.Utils.CONSTANTS.FEEDBACK_SUBMITTED : 
                        window.MCPFeedback.Utils.CONSTANTS.FEEDBACK_WAITING;
                    
                    self.uiManager.setFeedbackState(uiState, sessionData.session_id);
                    console.log('🔄 UI 狀態已恢復:', uiState);
                }

                // 更新 AI 摘要內容
                if (sessionData.summary) {
                    self.updateSummaryContent(sessionData.summary);
                }

                // 更新專案目錄
                if (sessionData.project_directory) {
                    self.updateProjectDirectory(sessionData.project_directory);
                }

                // 觸發會話更新事件
                if (self.sessionManager && self.sessionManager.dataManager) {
                    self.sessionManager.dataManager.updateCurrentSession(sessionData);
                }

                console.log('✅ 會話狀態恢復完成');
            })
            .catch(function(error) {
                console.warn('⚠️ 恢復會話狀態時發生錯誤:', error);
                // 不影響正常功能，只記錄警告
            });
    };
...
`;
    ...
    FeedbackApp.prototype.setupUserActivityListeners = function() {
        ...
        // 定義需要監聽的活動事件（減少 mousemove 和 scroll 以優化 CPU）
        const activityEvents = ['click', 'keypress', 'touchstart'];
        
        // 防抖處理，避免過於頻繁地重置計時器（深度 CPU 優化）
        ...
        }, 15000, false); // 15秒內的連續活動只重置一次，大幅減少處理頻率
