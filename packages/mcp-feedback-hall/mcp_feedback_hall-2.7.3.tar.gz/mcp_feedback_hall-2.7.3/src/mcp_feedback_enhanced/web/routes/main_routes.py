...
def _merge_and_deduplicate_sessions(existing_sessions: list, new_sessions: list) -> list:
    """
    智能合併會話列表，去重並保持時間順序
    
    Args:
        existing_sessions: 現有的會話列表
        new_sessions: 新的會話列表
    
    Returns:
        合併後的會話列表，按創建時間排序（最新的在前）
    """
    # 建立以 session_id 為鍵的字典來去重
    session_dict = {}
    ...
    # 合併新會話（新會話會覆蓋同 ID 的舊會話）
    for session in new_sessions:
        session_id = session.get("session_id")
        if session_id:
            # 如果是更新現有會話，保留更多信息
            if session_id in session_dict:
                existing_session = session_dict[session_id]
                # 合併 user_messages（保持新的，但不丟失舊的）
                existing_messages = existing_session.get("user_messages", [])
                new_messages = session.get("user_messages", [])
                ...
                for msg in sorted(all_messages, key=lambda x: x.get("timestamp", 0)):
                    msg_timestamp = msg.get("timestamp")
                    if msg_timestamp not in seen_timestamps:
                        unique_messages.append(msg)
                        seen_timestamps.add(msg_timestamp)
                
                # 更新會話資料
                session["user_messages"] = unique_messages
                session["last_user_interaction"] = max(
                    existing_session.get("last_user_interaction", 0),
                    session.get("last_user_interaction", 0)
                )
            
            session_dict[session_id] = session
    
    # 轉換回列表並按創建時間排序（最新的在前）
    merged_sessions = list(session_dict.values())
...
def setup_routes(manager: "WebUIManager"):
    ...
    async def save_session_history(request: Request):
        ...
        try:
            ...
            if history_file.exists():
                try:
                    with open(history_file, "r", encoding="utf-8") as f:
                        ...
                        debug_log(f"載入了 {len(existing_sessions)} 個現有會話記錄")
                except (json.JSONDecodeError, Exception) as e:
                    debug_log(f"讀取現有歷史檔案失敗，將建立新檔案: {e}")
                    existing_sessions = []

            # 取得新的會話資料
            new_sessions = data.get("sessions", [])
            debug_log(f"收到 {len(new_sessions)} 個新會話記錄")

            # 智能合併會話：去重並保持時間順序
            merged_sessions = _merge_and_deduplicate_sessions(existing_sessions, new_sessions)
            debug_log(f"合併後共有 {len(merged_sessions)} 個會話記錄")
            ...
            debug_log(f"成功保存 {session_count} 個會話記錄（合併模式）")
