...
## [v2.7.2] - 2025-09-07 - 會話歷史合併修復

### 🐛 問題修復
- 🔄 **修復會話歷史覆蓋問題**: 解決重新啟動 Cursor 時會話歷史被完全覆蓋的問題
- 🔗 **智能會話合併**: 新增 `_merge_and_deduplicate_sessions` 函數實現智能會話合併
- 📝 **訊息去重**: 實現按時間戳去重合併用戶訊息，避免重複記錄
- 💾 **歷史保存邏輯**: 修改 `/api/save-session-history` API 先讀取現有歷史再合併保存

### 🔧 技術改進
- 📊 **會話保留機制**: 確保設定的保留期限（如30天）正確運作
- 🛡️ **錯誤處理**: 加強歷史檔案讀取的錯誤處理機制
- 📋 **詳細日誌**: 增加合併過程的詳細日誌記錄，便於除錯
