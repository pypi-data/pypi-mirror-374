...
class UsageSession:
    """Capture delta usage for a block and return JSON data."""
    def __init__(self, pricing_table, label=None):
...
def track_usage(pricing_table, label=None):
    """Decorator to track usage for a single function call."""
    def _decorator(fn):
        def _wrapped(*args, **kwargs):
            with UsageSession(pricing_table, label=label) as sess:
                rv = fn(*args, **kwargs)
