import requests, json, time
from typing import Any, Callable, Optional

class StateScope:
    def __init__(self, org_id, project_id=None, env_id=None):
        self.org_id = org_id
        self.project_id = project_id
        self.env_id = env_id

class StateClientBuilder:
    def __init__(self, request):
        meta = request["metadata"]
        self.base_url = meta["stateStoreUrl"]
        self.token = meta["stateStoreToken"]
        self.org_id = meta["organizationID"]
        self.project_id = meta.get("projectID")
        self.env_id = meta.get("environmentID")

    def with_org_scope(self):
        return StateClient(self.base_url, self.token, self.org_id, self.project_id, self.env_id, StateScope(self.org_id))

    def with_project_scope(self):
        return StateClient(self.base_url, self.token, self.org_id, self.project_id, self.env_id, StateScope(self.org_id, self.project_id))

    def with_env_scope(self):
        return StateClient(self.base_url, self.token, self.org_id, self.project_id, self.env_id, StateScope(self.org_id, self.project_id, self.env_id))

class StateClient:
    def __init__(self, base_url, token, org_id, project_id, env_id, scope: StateScope, timeout=5):
        self.base_url = base_url.rstrip("/")
        self.token = token
        self.scope = scope
        self.timeout = timeout
        self.org_id = org_id
        self.project_id = project_id
        self.env_id = env_id

    # ---------- Helpers ----------
    def _headers(self):
        headers = {
            "X-Eaas-State-Token": self.token,
            "X-Organization-ID": self.org_id,
            "X-Project-ID": self.project_id,
            "X-Environment-ID": self.env_id,
            "Content-Type": "application/json",
        }
        return headers
    
    # ...implement methods using self.scope...

    def _get_raw(self, key: str):
        params = {"key": key, "organization_id": self.org_id}
        if self.scope.project_id: params["project_id"] = self.scope.project_id
        if self.scope.env_id: params["environment_id"] = self.scope.env_id

        resp = requests.get(self.base_url, headers=self._headers(),
                            params=params, timeout=self.timeout)
        if resp.status_code == 404:
            return None, ""
        resp.raise_for_status()
        data = resp.json()
        return data["value"], data.get("version", "")

    # ---------- Public API ----------
    def Get(self, key: str) -> Any:
        raw, version = self._get_raw(key)
        return raw, version

    def SetKV(self, key: str, value: str, version: int) -> None:
        """Create/update without OCC retry and let consumer handle conflicts"""
        body = {
            "scope": self.scope.__dict__,
            "key": key,
            "value": value,
            "version": version
        }
        resp = requests.put(self.base_url,
                                headers=self._headers(),
                                json=body, timeout=self.timeout)

        resp.raise_for_status()
        return

    def Set(self, key: str, update_fn: Callable[[Any], Any], max_retries: int = 5) -> None:
        """Create/update with OCC retry. update_fn takes old_value -> new_value"""
        for attempt in range(max_retries):
            old_value, version = self._get_raw(key)
            new_value = update_fn(old_value)

            body = {
                "scope": self.scope.__dict__,
                "key": key,
                "value": new_value,
                "version": version
            }
            resp = requests.put(self.base_url,
                                 headers=self._headers(),
                                 json=body, timeout=self.timeout)
            if resp.status_code == 409:
                # OCC conflict, retry
                continue
            resp.raise_for_status()
            return
        raise Exception(f"Set failed after {max_retries} retries due to OCC conflicts")

    def Delete(self, key: str) -> None:
        body = {
                "scope": self.scope.__dict__,
                "key": key
            }
        resp = requests.delete(self.base_url,
                               headers=self._headers(),
                               json=body, timeout=self.timeout)
        if resp.status_code != 200 and resp.status_code != 404:
            resp.raise_for_status()