...
SPAMHAUS_API_KEY_ENV_VAR = 'SECRET_SPAMHAUS_API_KEY'

SPAMHAUS_API_MAX_REQUEST_ONE_MINUTE = 15
SPAMHAUS_API_REMAINING_CALLS = SPAMHAUS_API_MAX_REQUEST_ONE_MINUTE
SPAMHAUS_API_TIMETOWAIT = 60
...
common_group.add_argument('-k', '--api-key', help='API key (could either be provided in the "SECRET_SPAMHAUS_API_KEY" env var)', type = str)
...
def make_api_submit_request(url_endpoint, req_data, options):
...
def spamhaus_get_actions(options):
    ...
    if action:
        ...
        req = requests.get(url_endpoint, headers=options.api_key)
        ...
        with open(options.input_file, mode='r', encoding='utf-8') as fd_input:
            for entry in fd_input:
                ...
                if req_data:
                    ...
                    req = make_api_submit_request(url_endpoint, req_data, options)
                    if req.ok:
...
def main():
    ...
    if not(api_key):
        if SPAMHAUS_API_KEY_ENV_VAR in os.environ:
            api_key = os.environ[SPAMHAUS_API_KEY_ENV_VAR]
        else:
            parser.error('[!] No API key has been provided, exiting.')
            
    options.api_key = { "Authorization": "Bearer " + api_key }
