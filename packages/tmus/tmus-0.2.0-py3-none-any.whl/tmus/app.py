...
import os
import vlc
from tmus.music_scanner import flatten_album, scan_music
...
def main_ui(stdscr, path):
    ...
    curses.curs_set(0)
    selected_artist = 0
    library = update_library_cache(path, scan_music)
    artist_offset = 0
    selected_song = 0
    song_offset = 0
    curr_song = None
    ...
    while True:
        ...
        if curr_song and curr_artist:
            ...
            stdscr.addstr(height - 3, 1, "[", curses.A_BOLD)
            stdscr.addstr(height - 3,  bar_width, "]", curses.A_BOLD)
