...
def parse_anchor_date(
    ...
    if m:
        ...
        y = now.year + 1 if wants_next_year else (now.year if wants_this_year or True else now.year)
        d = _clamp_day(y, mo, d)
        return date(y, mo, d)
    ...
    # next week / 下週（可帶週幾；沒帶則預設下週一）
    if ("next week" in lower) or ("下週" in text) or ("下周" in text) or ("下星期" in text):
        # 嘗試抓目標週幾
        ...
        # 強制下一週：先推一週，再對齊目標週幾
        days_to_target_this_week = (target_wd - today_wd) % 7
        days_ahead = 7 + days_to_target_this_week
...
def parse_natural_datetime(text: str) -> Optional[datetime]:
    ...
    # 3) 組成時區感知的 datetime
    dt = datetime(anchor.year, anchor.month, anchor.day, hour, minute, tzinfo=TZ)
    return dt
