...
SourceType = tuple[str, str | None, t.Callable[[], bool] | None]
    ...
    async def load_async(
        ...
        if bcc is not None:
            try:
                ...
                checksum = hashlib.sha256(source_str.encode("utf-8")).hexdigest()

                # Try to get existing bytecode
                code = bucket.get_bytecode(checksum)

                if code is None:
                    # Compile template
                    code = environment.compile(source_str, name, path)
                    bucket.set_bytecode(checksum, code)
            except Exception:
