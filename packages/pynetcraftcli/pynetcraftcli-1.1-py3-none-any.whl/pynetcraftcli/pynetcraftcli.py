...
NETCRAFT_API_KEY_ENV_VAR = 'SECRET_NETCRAFT_API_KEY'

# Options definition
parser = argparse.ArgumentParser(description="version: " + VERSION)
common_group = parser.add_argument_group('common parameters')
common_group.add_argument('-k', '--api-key', help='API key (could either be provided in the "SECRET_NETCRAFT_API_KEY" env var)', type = str)
    ...
    if os.path.isfile(options.input_file):
        ...
        if malicious_url:
            ...
            for sublist in chunks(malicious_url, 1000):
                req_data = { "email": options.api_key,
                             "urls": [] }
...
def main():
    ...
    if not(api_key):
        if NETCRAFT_API_KEY_ENV_VAR in os.environ:
            api_key = os.environ[NETCRAFT_API_KEY_ENV_VAR]
        else:
