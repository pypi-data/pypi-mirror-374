    ...
    def load_csv(
        ...
        **kwargs
        ...
        Args:
            ...
            sort: If True, sort by entity_col and time_col
            **kwargs: Additional arguments passed to pd.read_csv() (ignored if source is DataFrame)
        
        Returns:
            Validated and cleaned DataFrame with datetime conversion
        ...
        if isinstance(source, (str, Path)):
            try:
                ...
                raise IOError(f"Failed to read CSV from {source}: {e}")
        elif isinstance(source, pd.DataFrame):
            df = source.copy()
        else:
            raise TypeError(f"Expected str, Path, or DataFrame, got {type(source)}")

        # Standardize column names only if exactly 3 columns and no match
        if len(df.columns) == 3 and not all(col in df.columns for col in [entity_col, time_col, target_col]):
            original_cols = df.columns.tolist()
            # Warn if original names are meaningful
            if not set(original_cols).intersection([entity_col, time_col, target_col]):
                print(f"Warning: Renaming columns {original_cols} → [{entity_col}, {time_col}, {target_col}]")
        ...
        # Sort by entity and time (important for time-series models)
        if sort:
