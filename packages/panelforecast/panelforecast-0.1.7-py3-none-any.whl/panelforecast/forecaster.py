...
class PanelForecaster:
    ...
    def load_data(self, source: Union[str, pd.DataFrame], **kwargs) -> 'PanelForecaster':
        ...
        """
        entity_col = kwargs.get('entity_col', 'Entity')
        time_col = kwargs.get('time_col', 'Date')
        target_col = kwargs.get('target_col', 'Value')
        
        # Use updated loader that accepts both str and DataFrame
        self.data = self.loader.load_data(source, entity_col=entity_col, 
                                        time_col=time_col, target_col=target_col, **kwargs)
