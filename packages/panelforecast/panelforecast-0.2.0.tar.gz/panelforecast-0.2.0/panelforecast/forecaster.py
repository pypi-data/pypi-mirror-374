...
class PanelForecaster:
    ...
    def evaluate_by_entity(self, loader=None) -> Dict[Any, Tuple[float, float, float, float, float]]:
        """
        Evaluate model performance separately for each entity.
        
        Args:
            loader: DataLoader to evaluate (default: test_loader)
            
        Returns:
            Dictionary mapping entity ID to (MAE, MSE, RMSE, MAPE, R²)
        """
        if not self.is_fitted:
            raise ValueError("Model must be fitted before evaluation")
        ...
        with torch.no_grad():
            for X, y, e in loader:
                ...
                all_pred.extend(pred)
                all_true.extend(y.cpu().numpy().flatten())
                all_entity_ids.extend(e[:, 0].cpu().numpy())
        
        # Convert to numpy arrays
        all_true = np.array(all_true)
        all_pred = np.array(all_pred)
        all_entity_ids = np.array(all_entity_ids)
        
        # Inverse transform to original scale
        true_original = self.preprocessor.inverse_transform_target(all_true, all_entity_ids)
        pred_original = self.preprocessor.inverse_transform_target(all_pred, all_entity_ids)
        
        # Use PanelEvaluator for per-entity metrics
        evaluator = PanelEvaluator()
        entity_metrics = evaluator.entity_wise_metrics(true_original, pred_original, all_entity_ids)
        
        # Optional: map encoded IDs back to original labels (e.g., 0 → "store_A")
        if hasattr(self.preprocessor, 'entity_encoder'):
            try:
                label_map = {
                    i: self.preprocessor.entity_encoder.inverse_transform([i])[0]
                    for i in entity_metrics.keys()
                }
                labeled_metrics = {
                    label_map[enc_id]: metrics for enc_id, metrics in entity_metrics.items()
                }
                return labeled_metrics
            except Exception:
                pass  # fallback to encoded IDs
