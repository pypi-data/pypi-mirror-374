...
from typing import Union, Protocol, Any, TYPE_CHECKING
        ...
        for item_name, data in event_items.items():
            try:
                ...
                try:
                    ...
                    NewType = msgspec.defstruct(name=class_name, fields=fields, bases=(Item,), tag=new_type_name)
...
class EventMixin:
    ...
    def handle_connection_event(self: EventMixinProtocol, event: events.ConnectionEvent, client: "GameClient") -> None:
        assert self.character_id == event.player.id

        if event.items:
            items = parse_items(event.items)
            client.set_items_and_constants(items, event.constants)
...
class InstanceCharacter(EventMixin):
    ...
    def handle_deposited_event(self, event: events.DepositedEvent) -> None:
    ...
    def handle_withdrew_event(self, event: events.WithdrewEvent) -> None:
    ...
    def handle_spell_casted_event(self, event: events.SpellCastedEvent) -> None:
        """Handle successful spell casting."""
        logger.info(f"Spell '{event.spell}' casted by {event.unitId}")
        if event.target:
            logger.info(f"Spell '{event.spell}' targeted at {event.target}")
        if event.damage:
            logger.info(f"Spell '{event.spell}' dealt {event.damage} damage")
        if event.heal:
            logger.info(f"Spell '{event.spell}' healed for {event.heal}")

    def handle_spell_failed_event(self, event: events.SpellFailedEvent) -> None:
        """Handle failed spell casting."""
        reason = event.reason or "unknown reason"
        logger.warning(f"Spell '{event.spell}' failed for {event.unitId}: {reason}")

    def handle_spell_interrupted_event(self, event: events.SpellInterruptedEvent) -> None:
        """Handle interrupted spell casting."""
        reason = event.reason or "unknown reason"
        logger.warning(f"Spell '{event.spell}' interrupted for {event.unitId}: {reason}")

    def handle_event(self, event: BaseEvent, client: "GameClient") -> None:
        match event:
            ...
            case events.ConnectionEvent():
                self.handle_connection_event(event, client)
            ...
            case events.SpellCastedEvent():
                self.handle_spell_casted_event(event)
            case events.SpellFailedEvent():
                self.handle_spell_failed_event(event)
            case events.SpellInterruptedEvent():
                self.handle_spell_interrupted_event(event)
