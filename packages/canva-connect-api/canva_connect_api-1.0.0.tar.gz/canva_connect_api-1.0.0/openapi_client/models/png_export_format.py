...
class PngExportFormat(BaseModel):
    """
    ...
    lossless: Optional[StrictBool] = Field(default=True, description="If set to `true` (default), the PNG is exported without compression. If set to `false`, the PNG is compressed using a lossy compression algorithm. Lossy PNG compression is only available to users on a Canva plan that has premium features, such as Canva Pro. If the user is on the Canva Free plan and this parameter is set to `false`, the export operation will fail.")
    transparent_background: Optional[StrictBool] = Field(default=False, description="If set to `true`, the PNG is exported with a transparent background. This option is only available to users on a Canva plan that has premium features, such as Canva Pro. If the user is on the Canva Free plan and this parameter is set to `true`, the export operation will fail.")
    as_single_image: Optional[StrictBool] = Field(default=False, description="When `true`, multi-page designs are merged into a single image. When `false` (default), each page is exported as a separate image.")
    ...
    __properties: ClassVar[List[str]] = ["type", "export_quality", "height", "width", "lossless", "transparent_background", "as_single_image", "pages"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "width": obj.get("width"),
            "lossless": obj.get("lossless") if obj.get("lossless") is not None else True,
            "transparent_background": obj.get("transparent_background") if obj.get("transparent_background") is not None else False,
            "as_single_image": obj.get("as_single_image") if obj.get("as_single_image") is not None else False,
            "pages": obj.get("pages")
        })
        return _obj
