...
class MentionSuggestionEventType(BaseModel):
    """
    Event type for a mention in a reply to a suggestion.
    """ # noqa: E501
    type: StrictStr
    reply_url: StrictStr = Field(description="A URL to the design, focused on the suggestion reply.")
    reply: Reply
    __properties: ClassVar[List[str]] = ["type", "reply_url", "reply"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['mention']):
            raise ValueError("must be one of enum values ('mention')")
        return value
