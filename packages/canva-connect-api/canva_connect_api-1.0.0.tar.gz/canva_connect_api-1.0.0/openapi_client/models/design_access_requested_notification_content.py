...
class DesignAccessRequestedNotificationContent(BaseModel):
    """
    ...
    grant_access_url: StrictStr = Field(description="A URL, which is scoped only to the user that can grant the requested access to the design, that approves the requested access.")
    __properties: ClassVar[List[str]] = ["type", "triggering_user", "receiving_team_user", "design", "grant_access_url"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['design_access_requested']):
            raise ValueError("must be one of enum values ('design_access_requested')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "grant_access_url": obj.get("grant_access_url")
        })
        return _obj
