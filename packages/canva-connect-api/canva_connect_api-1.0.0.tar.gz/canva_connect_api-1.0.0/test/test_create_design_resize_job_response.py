# coding: utf-8

"""
    Canva Connect API

    API for building integrations with Canva via a REST api

    The version of the OpenAPI document: 2024-06-18
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.create_design_resize_job_response import CreateDesignResizeJobResponse

class TestCreateDesignResizeJobResponse(unittest.TestCase):
    """CreateDesignResizeJobResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CreateDesignResizeJobResponse:
        """Test CreateDesignResizeJobResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CreateDesignResizeJobResponse`
        """
        model = CreateDesignResizeJobResponse()
        if include_optional:
            return CreateDesignResizeJobResponse(
                job = openapi_client.models.design_resize_job.DesignResizeJob(
                    id = 'bbd8dfcd-ead1-4871-81d5-962bfec82274', 
                    status = 'success', 
                    result = openapi_client.models.design_resize_job_result.DesignResizeJobResult(
                        design = openapi_client.models.design_summary.DesignSummary(
                            id = 'DAFVztcvd9z', 
                            title = 'My summer holiday', 
                            url = 'https://www.canva.com/design/DAFVztcvd9z/edit', 
                            thumbnail = openapi_client.models.thumbnail.Thumbnail(
                                width = 595, 
                                height = 335, 
                                url = 'https://document-export.canva.com/Vczz9/zF9vzVtdADc/2/thumbnail/0001.png?<query-string>', ), 
                            urls = openapi_client.models.design_links.DesignLinks(
                                edit_url = 'https://www.canva.com/api/design/eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIiwiZXhwaXJ5IjoxNzQyMDk5NDAzMDc5fQ..GKLx2hrJa3wSSDKQ.hk3HA59qJyxehR-ejzt2DThBW0cbRdMBz7Fb5uCpwD-4o485pCf4kcXt_ypUYX0qMHVeZ131YvfwGPIhbk-C245D8c12IIJSDbZUZTS7WiCOJZQ.sNz3mPSQxsETBvl_-upMYA/edit', 
                                view_url = 'https://www.canva.com/api/design/eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIiwiZXhwaXJ5IjoxNzQyMDk5NDAzMDc5fQ..GKLx2hrJa3wSSDKQ.hk3HA59qJyxehR-ejzt2DThBW0cbRdMBz7Fb5uCpwD-4o485pCf4kcXt_ypUYX0qMHVeZ131YvfwGPIhbk-C245D8c12IIJSDbZUZTS7WiCOJZQ.sNz3mPSQxsETBvl_-upMYA/view', ), 
                            created_at = 1377396000, 
                            updated_at = 1692928800, 
                            page_count = 3, ), ), 
                    error = openapi_client.models.design_resize_error.DesignResizeError(
                        code = 'thumbnail_generation_error', 
                        message = '', ), )
            )
        else:
            return CreateDesignResizeJobResponse(
                job = openapi_client.models.design_resize_job.DesignResizeJob(
                    id = 'bbd8dfcd-ead1-4871-81d5-962bfec82274', 
                    status = 'success', 
                    result = openapi_client.models.design_resize_job_result.DesignResizeJobResult(
                        design = openapi_client.models.design_summary.DesignSummary(
                            id = 'DAFVztcvd9z', 
                            title = 'My summer holiday', 
                            url = 'https://www.canva.com/design/DAFVztcvd9z/edit', 
                            thumbnail = openapi_client.models.thumbnail.Thumbnail(
                                width = 595, 
                                height = 335, 
                                url = 'https://document-export.canva.com/Vczz9/zF9vzVtdADc/2/thumbnail/0001.png?<query-string>', ), 
                            urls = openapi_client.models.design_links.DesignLinks(
                                edit_url = 'https://www.canva.com/api/design/eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIiwiZXhwaXJ5IjoxNzQyMDk5NDAzMDc5fQ..GKLx2hrJa3wSSDKQ.hk3HA59qJyxehR-ejzt2DThBW0cbRdMBz7Fb5uCpwD-4o485pCf4kcXt_ypUYX0qMHVeZ131YvfwGPIhbk-C245D8c12IIJSDbZUZTS7WiCOJZQ.sNz3mPSQxsETBvl_-upMYA/edit', 
                                view_url = 'https://www.canva.com/api/design/eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIiwiZXhwaXJ5IjoxNzQyMDk5NDAzMDc5fQ..GKLx2hrJa3wSSDKQ.hk3HA59qJyxehR-ejzt2DThBW0cbRdMBz7Fb5uCpwD-4o485pCf4kcXt_ypUYX0qMHVeZ131YvfwGPIhbk-C245D8c12IIJSDbZUZTS7WiCOJZQ.sNz3mPSQxsETBvl_-upMYA/view', ), 
                            created_at = 1377396000, 
                            updated_at = 1692928800, 
                            page_count = 3, ), ), 
                    error = openapi_client.models.design_resize_error.DesignResizeError(
                        code = 'thumbnail_generation_error', 
                        message = '', ), ),
        )
        """

    def testCreateDesignResizeJobResponse(self):
        """Test CreateDesignResizeJobResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
