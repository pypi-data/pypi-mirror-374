...
class Debugger:
    def __init__(self, program: AST.Program, source: str) -> None:
        ...
        # Attach hooks
        self.interp.on_node_start = self._on_node_start  # type: ignore[assignment]
        self.interp.on_node_end = self._on_node_end  # type: ignore[assignment]
    ...
    def _interactive(self) -> None:
        while self.pause:
            ...
            if op in {"c", "cont", "continue"}:
                ...
                print("Commands: c(continue), s(step), n(next), b <line>, p <name>, l(list), q(quit)")
    ...
    def _format_where(self) -> str:
        ...
        if isinstance(line, int) and 1 <= line <= len(self.source_lines):
            return f"Line {line}: {self.source_lines[line - 1]}"
        return "Line ?"
