...
from graphiti_core_ecolink.cross_encoder.client import CrossEncoderClient
from graphiti_core_ecolink.cross_encoder.openai_reranker_client import OpenAIRerankerClient
from graphiti_core_ecolink.driver.driver import GraphDriver
from graphiti_core_ecolink.driver.neo4j_driver import Neo4jDriver
...
from graphiti_core_ecolink.embedder import EmbedderClient, OpenAIEmbedder
from graphiti_core_ecolink.graphiti_types import GraphitiClients
from graphiti_core_ecolink.helpers import (
    get_default_group_id,
    semaphore_gather,
    validate_excluded_entity_types,
    validate_group_id,
)
from graphiti_core_ecolink.llm_client import LLMClient, OpenAIClient
from graphiti_core_ecolink.nodes import (
    CommunityNode,
    EntityNode,
    EpisodeType,
    EpisodicNode,
...
from graphiti_core_ecolink.search.search import SearchConfig, search
from graphiti_core_ecolink.search.search_config import DEFAULT_SEARCH_LIMIT, SearchResults
from graphiti_core_ecolink.search.search_config_recipes import (
    COMBINED_HYBRID_SEARCH_CROSS_ENCODER,
    EDGE_HYBRID_SEARCH_NODE_DISTANCE,
    EDGE_HYBRID_SEARCH_RRF,
)
from graphiti_core_ecolink.search.search_filters import SearchFilters
from graphiti_core_ecolink.search.search_utils import (
    RELEVANT_SCHEMA_LIMIT,
    get_edge_invalidation_candidates,
    get_mentioned_nodes,
    get_relevant_edges,
)
from graphiti_core_ecolink.telemetry import capture_event
from graphiti_core_ecolink.utils.bulk_utils import (
    RawEpisode,
...
from graphiti_core_ecolink.utils.maintenance.edge_operations import (
...
from graphiti_core_ecolink.utils.maintenance.graph_data_operations import (
    EPISODE_WINDOW_LEN,
    build_indices_and_constraints,
    retrieve_episodes,
)
from graphiti_core_ecolink.utils.maintenance.node_operations import (
    extract_attributes_from_nodes,
    extract_nodes,
    resolve_extracted_nodes,
)
from graphiti_core_ecolink.utils.ontology_utils.entity_types_utils import validate_entity_types
...
class Graphiti:
    ...
    async def build_indices_and_constraints(self, delete_existing: bool = False):
        ...
        `graphiti_core_ecolink.utils.maintenance.graph_data_operations` module to perform
    ...
    async def retrieve_episodes(
        ...
        from the `graphiti_core_ecolink.utils` module.
        """
    ...
    async def add_episode(
        ...
        try:
            ...
            extracted_nodes = await extract_nodes(
                self.clients, #graphiti连接实例
                episode, #当前要处理的文档/消息片段
                previous_episodes, #历史文档片段
                entity_types, #实体类型定义
                excluded_entity_types #要排除的实体类型
            )
            print(extracted_nodes,'extracted_nodesextracted_nodesextracted_nodes')
            # Extract edges and resolve nodes （semaphore_gather：并发控制包装器，限制运行协程数量；防止api限流错误，429）
            (nodes, uuid_map, node_duplicates), extracted_edges = await semaphore_gather(
                ...
                extract_edges(
                    self.clients, #graphiti连接实例
                    episode, #当前要处理的文档/消息片段
                    extracted_nodes, #新提取的实体
                    previous_episodes, #历史文档片段
                    edge_type_map or edge_type_map_default, #实体类型到关系类型的映射
                    group_id, #组id
                    edge_types, #关系类型定义
                ),
            ...
            #根据大模型判断
            # 认为是重复的情况：
            # 语义等价: 如果现有实体中的描述性标签明确指向上下文中的命名实体
            # 同一现实对象: 指代相同的真实世界对象或概念
            # 不认为是重复的情况：
            # 相关但不同的实体
            # 相似名称或用途但指代不同实例或概念
            #resolved_edges: 解析完成的实体节点列表
            #invalidated_edges: 因冲突而失效的关系列表
            #hydrated_nodes: 属性丰富的节点列表
            (resolved_edges, invalidated_edges), hydrated_nodes = await semaphore_gather(
                resolve_extracted_edges(
                    ...
                    episode, #文档片段
                    nodes, #实体列表
                    edge_types or {}, #关系类型定义
                    edge_type_map or edge_type_map_default, #关系类型映射
                ),
                #从文档中提取实体的属性信息，丰富实体的描述信息，更新实体的特征数据
                extract_attributes_from_nodes(
                    self.clients,
                    nodes,
                    episode,
