...
class FileFormatConverter:
    ...
    class LablemeHelper:
        ...
        def labelme2YoloPolygon(json_file_path, class_file_path, txt_file_path):
            ...
            with open(json_file_path, 'r', encoding='utf8') as fp:
                ...
                image_height = json_data.get("imageHeight")
                shapes_array = json_data.get("shapes")
                # 标注内容的类别、归一化后的x坐标，归一化后的y坐标（此处归一化指的是除以图片宽和高）
                for shape_obj in shapes_array:
                    class_name = FileFormatConverter.LablemeHelper.handle_label_name(shape_obj.get("label"))
                    points_array = shape_obj.get("points")
                    label = classes_dict[class_name]
                    ...
                    for point in points_array:
                        # 归一化
                        tmpx = point[0]/image_width
                        tmpy = point[1]/image_height
        ...
        def build_labelme_json_by_binary_image(label_name, mask_pic_file_path, labelme_json_file_path):
            ...
            if len(shapes_array) > 0:
                json_file_dict = FileFormatConverter.LablemeHelper.__build_lableme_json_dict(shapes_array,
                                                                                             mask_pic_file_path)
                JsonHelper.write_json_obj_into_file(json_file_dict, labelme_json_file_path)
            else:
                print("当前二值图的标签数据为空")
    ...
    class KITTIHelper:
        ...
        @staticmethod
        def kitti2labelme(txt_path, category_to_label, image_height, image_width, json_path):
            #         # 定义类别ID到标签的映射
            #         category_to_label = {
            #             0: "ignored-regions",
            #             1: "pedestrian",
            ...
            #         }
            # 从txt文件名获取图片名
            image_name = os.path.basename(txt_path).replace('.txt', '.png')
            # 初始化JSON结构
            json_data = {
                "version": "0.2.2",
                "flags": {},
            ...
            for line in lines:
                ...
                # 解析数据
                bbox_left, bbox_top, bbox_width, bbox_height, _, object_category, _, _ = map(float, line.split(','))
                ...
                y2 = bbox_top
                x3 = bbox_left + bbox_width
                y3 = bbox_top + bbox_height
                x4 = bbox_left
                y4 = bbox_top + bbox_height

                # 获取标签名
                label = category_to_label.get(object_category, "others")
    ...
    class CocoHelper:
        ...
        # coco8转pascalVOC
        @staticmethod
        def coco82pascalVoc(txt_file_path, xml_file_path, pic_file_path, class_file_path):
            classes_dict = {}
            # class_file_path= "classes.names"
            with open(class_file_path, encoding='utf-8') as f:
                for idx, line in enumerate(f.readlines()):
        ...
        @staticmethod
        def coco2017hbb2Labelme(coco_path,labelme_path):
            # 加载COCO格式的数据
...
class SampleTools:
    ...
    # 批量图片转换为视频
    @staticmethod
    def create_video_from_images(image_folder, output_video, fps=0.5, img_format='jpg', num_images=None,
                                 random_select=False):
        """
        将图片文件夹中的图像合成为视频
        :param image_folder: 图片文件夹路径
        :param output_video: 输出视频路径 (必须以.mp4结尾)
        :param fps: 帧率 (默认0.5帧/秒，即每帧2秒)
        :param img_format: 图片格式 (默认jpg)
        :param num_images: 使用的图片数量 (None表示使用所有图片)
        :param random_select: 是否随机选择图片
        """
        # 确保路径正确格式化
        ...
        # 获取所有图片文件 - 使用字节路径避免编码问题
        pattern = os.path.join(image_folder, f"*.{img_format.lower()}")
        ...
        if sys.platform == 'win32':
            ...
            images = [f.decode('utf-8') for f in glob.glob(pattern_bytes)]

            # 如果小写找不到，尝试大写扩展名
            if not images:
                pattern = os.path.join(image_folder, f"*.{img_format.upper()}")
                pattern_bytes = pattern.encode('utf-8')
                images = [f.decode('utf-8') for f in glob.glob(pattern_bytes)]
        else:
            # 非Windows系统使用普通路径
            images = glob.glob(pattern, recursive=False)
            # 尝试大写扩展名
            if not images:
                pattern = os.path.join(image_folder, f"*.{img_format.upper()}")
                images = glob.glob(pattern, recursive=False)
        ...
        if not images:
            print(f"错误: 在 {image_folder} 中没有找到 {img_format} 格式的图片")
            return
        ...
        # 读取第一张图片获取尺寸 - 使用新的读取方法
        sample = SampleTools.read_image_with_chinese_path(selected_images[0])
        if sample is None:
            print(f"无法读取图片: {selected_images[0]}")
            return
        h, w, _ = sample.shape
        ...
        if not video_writer.isOpened():
            print("无法创建视频文件，请检查路径和权限")
            return

        # 处理每张图片 - 使用新的读取方法
        for i, img_path in enumerate(selected_images):
            frame = SampleTools.read_image_with_chinese_path(img_path)
        ...
        print(
            f"视频时长: {len(selected_images) * (1 / fps):.2f} 秒 ({len(selected_images)} 张图片 * {1 / fps:.1f} 秒/张)")
    ...
    def split_train_val_data_of_mm(sample_pic_dir_path, label_json_dir_path, mm_json_dir_path,
        ...
        # 语义分割
            ...
            FileFormatConverter.LablemeHelper.labelme2Citycapes(mm_train_dir, label_suffix, mm_train_dir, alg_type,
                                                                category_name)
            FileFormatConverter.LablemeHelper.labelme2Citycapes(mm_val_dir, label_suffix, mm_val_dir, alg_type,
                                                                category_name)
