...
_error_file_path_default = os.getenv("AIKO_ERROR_LOG_FILE", os.path.join(os.getcwd(), "aiko_errors.log"))
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            ...
            if record.exc_info:
                ...
                event["traceback"] = "".join(traceback.format_exception(etype, evalue, etb))
...
def enable_logfmt(
    *, level: int = logging.ERROR, stream=sys.stderr, file: str = ""
    ...
    """
    allow = os.getenv("AIKO_ALLOW_LOGFMT", "0").strip().lower() in {"1", "true", "yes", "on"}
    if not allow:
        return
    if file:
...
_DEFAULT_ENDPOINT = os.getenv(
    "AIKO_MONITOR_ENDPOINT", "http://localhost:8080/monitor"
)
...
def safe_execute_context(block_name: str = "anonymous"):
    try:
        ...
        logger.error(f"Monitoring error in block '{block_name}': {str(e)}", exc_info=True)
...
class _HTTPTransport:
    ...
    def send(self, payload: Dict[str, Any]) -> None:
        ...
        logger.debug(
            "queue_put",
            extra={"endpoint": payload.get('endpoint', 'unknown'), "queue_size": self._queue.qsize()},
        )
    ...
    def _run(self) -> None:
        ...
        while True:
            try:
                ...
                logger.debug("queue_get", extra={"endpoint": item.get('endpoint', 'unknown'),"total_processed": total_processed})
                ...
                try:
                    ...
                    identity = extract_user_id_sync(request_headers=headers,
                                                    jwt_field_cache=self._jwt_cache)
    ...
    def _mark_unhealthy_and_probe(self):
        ...
        t = threading.Thread(target=self._health_check_loop, name="aiko-health", daemon=True)
        t.start()
    ...
    def _flush(self, item: Dict):
        ...
        sig = _sign(self.secret, body) # project key, signature, Authorization, Bearer token
        headers = {
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug("http_post_success", extra={"status": response.status_code})
                return True # Success, exit retry loop
            except Exception as exc:
                logger.warning("http_post_failed", extra={"attempt": attempt + 1, "error": str(exc)})
    ...
    def capture_http_call(
        ...
        try:
            ...
            payload = {
                ...
                "url": req_meta["url"],
                "extracted_user": user_id if user_id is not None else "unknown", # user id can now be overwritten since the extract_identity fn runs in after __enter__ right now. while init is in the other wsgi/asgi layer
            }
            '''
            logger.debug("payload_created", extra={"endpoint": req_meta['endpoint'], "status": req_meta['status']})
