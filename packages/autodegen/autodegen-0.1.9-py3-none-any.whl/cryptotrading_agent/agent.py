...
class TradingAgent:
    ...
    async def analyze(
        ...
        try:
            ...
            prompt = self._create_analysis_prompt_with_technical_data(symbol, timeframe, technical_data)
    ...
    def _create_analysis_prompt_with_technical_data(self, symbol: str, timeframe: str, tech_data: TechnicalData) -> str:
...
- Senkou Span A: ${tech_data.ichimoku_senkou_a:,.2f if tech_data.ichimoku_senkou_a else 'N/A'}
- Senkou Span B: ${tech_data.ichimoku_senkou_b:,.2f if tech_data.ichimoku_senkou_b else 'N/A'}
...
- Market Phase: {tech_data.market_phase.upper() if tech_data.market_phase else 'N/A'}
        ...
        {{
          ...
          "fibonacci_levels": "From High ${tech_data.high_24h:,.2f} to Low ${tech_data.low_24h:,.2f}: " + "{fibonacci_text}",
          ...
          "trading_setup": "Entry: Consider ${tech_data.current_price * 0.995:,.2f} (long) / ${tech_data.current_price * 1.005:,.2f} (short) | Stop Loss: {' / '.join([f'${level:,.2f}' for level in tech_data.support_levels[:2]])} (long) | Take Profit: {' / '.join([f'${level:,.2f}' for level in tech_data.resistance_levels[:2]])} (long)",
          "risk_assessment": "Risk Level: {'HIGH' if tech_data.atr and tech_data.atr > tech_data.current_price * 0.05 else 'MODERATE' if tech_data.atr and tech_data.atr > tech_data.current_price * 0.02 else 'LOW'} | Volatility (ATR): {tech_data.atr:.2f if tech_data.atr else 'N/A'} | Trend Confidence: {tech_data.trend_strength.upper() if tech_data.trend_strength else 'UNKNOWN'}",
          "outlook": "{tech_data.trend_direction.upper() if tech_data.trend_direction else 'NEUTRAL'} bias with {tech_data.trend_strength} trend strength. Key levels: Support {tech_data.support_levels[0]:,.2f if tech_data.support_levels else 'N/A'} | Resistance {tech_data.resistance_levels[0]:,.2f if tech_data.resistance_levels else 'N/A'}"
        }}
        ...
        Timeframe: {timeframe}
        """.replace("{fibonacci_text}", 
                   " | ".join([f"{level}%: ${price:,.2f}" for level, price in tech_data.fibonacci_levels.items()]) 
                   if tech_data.fibonacci_levels else "Levels calculated from recent range")
