...
class LLMCore:
    ...
    async def complete_structured(
        ...
        # Enhanced system prompt for structured output (full schema)
        base_prompt = """You are a professional crypto market analyst. Produce ONLY a JSON object matching this exact schema:
{
  "title": "**COMPREHENSIVE TECHNICAL ANALYSIS: [SYMBOL]**",
  "market_state": "Current market conditions with price, 24h change, volume and phase",
  "trend_indicators": "ADX/DI, Ichimoku, Moving Averages, MACD alignment",
  "oscillators": "RSI, Stochastic, Williams %R, CCI summary",
  "volume_indicators": "OBV, MFI/CMF, Volume Profile interpretation",
  "fibonacci_levels": "Key retracements and extensions (e.g., 0.382, 0.618, 1.618)",
  "elliott_wave": "Wave count, context and next likely movement",
  "smart_money_concepts": "Order blocks, FVGs, liquidity zones",
  "trading_levels": "Support/resistance, trading range and key levels",
  "indicator_analysis": "Concise indicator summary with numbers",
  "timeframe_analysis": "1h/4h/1d trends and strengths",
  "trading_setup": "Entry, stop-loss, take-profit and invalidation",
  "risk_assessment": "Risk level, position sizing and R/R",
  "outlook": "Concise directional bias and triggers"
}

CRITICAL RULES:
1. Output MUST be valid JSON only (no markdown, no preamble, no meta commentary)
2. Do NOT include phrases like "The key will be", "I'll", "Hmm", "The user wants", or any <think> blocks
3. Title MUST start with **COMPREHENSIVE TECHNICAL ANALYSIS:** or **MARKET ANALYSIS:**
4. Every field should contain concrete, specific details (numbers, levels, percentages) where available
5. If a field genuinely has no signal, set its value to "No data" (do not omit the field)
...
- Do not omit any computed technical indicators"""
    ...
    async def complete_with_fallback(
        ...
        **kwargs
        ...
        if report:
            # Convert structured report back to rich formatted text
            parts = [report.title, ""]
            parts.append(f"### INITIAL OVERALL MARKET STATE\n{report.market_state}")
            parts.append(f"### 📈 TREND INDICATORS\n{report.trend_indicators or 'No data'}")
            parts.append(f"### ⚡ OSCILLATORS\n{report.oscillators or 'No data'}")
            parts.append(f"### 📊 VOLUME INDICATORS\n{report.volume_indicators or 'No data'}")
            parts.append(f"### 🎯 FIBONACCI LEVELS\n{report.fibonacci_levels or 'No data'}")
            parts.append(f"### 🌊 ELLIOTT WAVE\n{report.elliott_wave or 'No data'}")
            parts.append(f"### 💎 SMART MONEY CONCEPTS\n{report.smart_money_concepts or 'No data'}")
            parts.append(f"### KEY TRADING LEVELS\n{report.trading_levels}")
            parts.append(f"### MULTI-TIMEFRAME ANALYSIS\n{report.timeframe_analysis}")
            parts.append(f"### ADVANCED INDICATOR ANALYSIS\n{report.indicator_analysis}")
            parts.append(f"### 🎯 TRADING SETUP\n{report.trading_setup or 'No data'}")
            parts.append(f"### ⚠️ RISK ASSESSMENT\n{report.risk_assessment or 'No data'}")
            parts.append(f"### CONCISE OUTLOOK\n{report.outlook}")
            return "\n\n".join(parts)
