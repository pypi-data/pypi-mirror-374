...
class Farfield:
    ...
    def set_theta_component(self, e_theta: np.array):
        self.samples[:,:,0] = e_theta

    def get_theta_component(self) -> np.array:
        return self.samples[:,:,0]

    def set_phi_component(self, e_phi: np.array):
        self.samples[:,:,1] = e_phi

    def get_phi_component(self) -> np.array:
        return self.samples[:,:,1]

    def get_sample_at(self, theta_index: int, phi_index: int) -> np.array:
        return self.samples[theta_index,phi_index,:]

    def init_samples_with_zeros(self, num_theta_samples: int, num_phi_samples: int):
    ...
    def set_z_axis(self, z_axis: np.array):
        self.z_axis = z_axis

    def get_z_axis(self) -> np.array:
        return self.z_axis

    def set_x_axis(self, x_axis: np.array):
        self.x_axis = x_axis

    def get_x_axis(self) -> np.array:
        return self.x_axis

    def set_radiated_power(self, power: float):
        self.radiated_power = power

    def get_radiated_power(self) -> float:
        return self.radiated_power

    def set_accepted_power(self, power: float):
        self.accepted_power = power

    def get_accepted_power(self) -> float:
        return self.accepted_power

    def set_stimulated_power(self, power: float):
...
class Parser:
    ...
    def _has_lines_to_process(self):
        return self.current_line < len(self.lines)

    def _get_line_rel(self, relative_line_index):
