...
def test_base_init_valid_params():
    ...
    assert mp_base.velocidad_inicial.magnitude == 10.0
    assert mp_base.angulo_radianes.magnitude == math.radians(45)
    assert mp_base.gravedad.magnitude == 9.81
    assert mp_base.velocidad_inicial_x.magnitude == pytest.approx(
        10.0 * math.cos(math.radians(45))
    )
    assert mp_base.velocidad_inicial_y.magnitude == pytest.approx(
...
def test_base_init_invalid_angulo():
    with pytest.raises(
        ValueError, match="El ángulo de lanzamiento debe estar entre 0 y 90 grados."
    ):
...
def test_base_posicion():
    mp_base = MovimientoParabolicoBase(
        velocidad_inicial=10.0, angulo_grados=30, gravedad=10.0
    )  # Usar g=10 para cálculos más simples
    # x = v0x * t
    # y = v0y * t - 0.5 * g * t^2
    ...
    assert pos_x.magnitude == pytest.approx(8.660254037844387)
    assert pos_y.magnitude == pytest.approx(0.0)

    # t = 0s
    pos_x_0, pos_y_0 = mp_base.posicion(tiempo=0.0)
    assert pos_x_0.magnitude == pytest.approx(0.0)
    assert pos_y_0.magnitude == pytest.approx(0.0)
...
def test_base_velocidad():
    ...
    assert vel_x.magnitude == pytest.approx(8.660254037844387)
    assert vel_y.magnitude == pytest.approx(-5.0)

    # t = 0s
    vel_x_0, vel_y_0 = mp_base.velocidad(tiempo=0.0)
    assert vel_x_0.magnitude == pytest.approx(8.660254037844387)
    assert vel_y_0.magnitude == pytest.approx(5.0)
...
def test_analisis_tiempo_vuelo():
    ...
    # T = 2 * v0y / g = 2 * 5 / 10 = 1
    assert mp_analisis.tiempo_vuelo().magnitude == pytest.approx(1.0)

    mp_base_horizontal = MovimientoParabolicoBase(
        velocidad_inicial=10.0, angulo_grados=0, gravedad=10.0
    )
    mp_analisis_horizontal = MovimientoParabolicoAnalisis(mp_base_horizontal)
    assert mp_analisis_horizontal.tiempo_vuelo().magnitude == pytest.approx(0.0)
...
def test_analisis_altura_maxima():
    ...
    assert mp_analisis.altura_maxima().magnitude == pytest.approx(1.25)

    mp_base_horizontal = MovimientoParabolicoBase(
        velocidad_inicial=10.0, angulo_grados=0, gravedad=10.0
    )
    mp_analisis_horizontal = MovimientoParabolicoAnalisis(mp_base_horizontal)
    assert mp_analisis_horizontal.altura_maxima().magnitude == pytest.approx(0.0)
...
def test_analisis_alcance_maximo():
    ...
    mp_analisis = MovimientoParabolicoAnalisis(mp_base)
    # R = v0x * T = 8.66 * 1 = 8.66
    assert mp_analisis.alcance_maximo().magnitude == pytest.approx(8.660254037844387)

    mp_base_horizontal = MovimientoParabolicoBase(
        velocidad_inicial=10.0, angulo_grados=0, gravedad=10.0
    )
    mp_analisis_horizontal = MovimientoParabolicoAnalisis(mp_base_horizontal)
    assert mp_analisis_horizontal.alcance_maximo().magnitude == pytest.approx(0.0)
