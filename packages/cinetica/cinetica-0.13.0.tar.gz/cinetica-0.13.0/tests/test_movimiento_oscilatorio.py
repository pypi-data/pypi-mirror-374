...
def test_mas_init():
    mas = MovimientoArmonicoSimple(
        amplitud=0.5, frecuencia_angular=2 * math.pi, fase_inicial=math.pi / 2
    )
...
def test_mas_posicion():
    ...
    mas_fase = MovimientoArmonicoSimple(
        amplitud=1, frecuencia_angular=math.pi, fase_inicial=math.pi / 2
    )  # x(t) = cos(pi*t + pi/2) = -sin(pi*t)
...
def test_mas_velocidad():
    mas = MovimientoArmonicoSimple(
        amplitud=1, frecuencia_angular=math.pi
    )  # v(t) = -pi * sin(pi*t)
...
def test_mas_energia_total():
    ...
    assert mas.energia_total(masa, k).magnitude == pytest.approx(
        0.5 * k * mas.amplitud.magnitude**2
    )
