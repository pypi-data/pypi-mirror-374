...
_uiautomation is shared under the Apache Licene 2.0.
...
2, You need to use an UIAutomationInitializerInThread object if use _uiautomation in a thread, see demos/uiautomation_in_thread.py'''.format(ex), ConsoleColor.Yellow)
                    raise ex
                    ...
                    if pebBaseAddress:
                        ...
                        if isok:
                            ...
                            if isok:
                                ...
                                if isok:
                                    pinfo.exePath = exePath.set_value
                                ...
                                if isok:
                                    pinfo.cmdLine = cmdLine.set_value
...
def SetGlobalSearchTimeout(seconds: float) -> None:
    """
    seconds: float.
    To make this available, you need explicitly import _uiautomation:
        from _uiautomation import _uiautomation as auto
...
def RunByHotKey(keyFunctions: Dict[Tuple[int, int], Callable], stopHotKey: Optional[Tuple[int, int]] = None, exitHotKey: Tuple[int, int] = (ModifierKey.Control, Keys.VK_D), waitHotKeyReleased: bool = True) -> None:
    ...
    keyFunctions: Dict[Tuple[int, int], Callable], such as {(_uiautomation.ModifierKey.Control, _uiautomation.Keys.VK_1) : function}
    ...
    _uiautomation.RunByHotKey({(_uiautomation.ModifierKey.Control, _uiautomation.Keys.VK_1) : main}
                        , (_uiautomation.ModifierKey.Control | _uiautomation.ModifierKey.Shift, _uiautomation.Keys.VK_2))
    """
    import traceback
