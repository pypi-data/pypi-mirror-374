...
import _pyautogui
from _pyautogui import LEFT, MIDDLE, RIGHT
...
The *KB dictionaries in _pyautogui map a string that can be passed to keyDown(),
keyUp(), or press() into the code used for the OS-specific keyboard function.

They should always be lowercase, and the same keys should be used across all OSes."""
keyboardMapping = dict([(key, None) for key in _pyautogui.KEY_NAMES])
...
def _normalKeyEvent(key, upDown):
    ...
    try:
        if _pyautogui.isShiftCharacter(key):
            ...
            time.sleep(_pyautogui.DARWIN_CATCH_UP_TIME)
...
def _dragTo(x, y, button):
    if button == LEFT:
        ...
        assert False, "button argument not in ('left', 'middle', 'right')"
    time.sleep(_pyautogui.DARWIN_CATCH_UP_TIME) # needed to allow OS time to catch up.
