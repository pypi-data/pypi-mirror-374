...
def func_sig(f):
    ...
    for param in sig.parameters.values():
        if param.kind is inspect.Parameter.POSITIONAL_OR_KEYWORD:
            ...
            raise XlwingsError("_xlwings does not support UDFs with keyword arguments")
...
async def custom_functions_call(data, module, sio=None):
    ...
    if data["version"] != __version__:
        raise XlwingsError(
            "_xlwings version mismatch: please restart Excel or "
    ...
    if inspect.isasyncgenfunction(func):
        ...
        async def task():
            try:
                async for result in func(*args):
                    ...
                    await sio.emit(
                        f"_xlwings:set-result-{task_key}",
                        {"result": result},
                    )
        ...
        if task_key not in background_tasks:
            mytask = asyncio.create_task(task(), name=f"_xlwings-{task_key}")
...
def custom_functions_code(
    module, custom_functions_call_path="/_xlwings/custom-functions-call"
):
...
async def sio_disconnect(sid):
    ...
    active_tasks = [
        ...
        if task.get_name().startswith("_xlwings")
    ]
    logger.info(f"Active _xlwings tasks:" f"{active_tasks}")
