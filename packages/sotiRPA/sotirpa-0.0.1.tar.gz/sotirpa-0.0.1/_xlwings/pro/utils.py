...
_xlwings PRO is dual-licensed under one of the following licenses:

* PolyForm Noncommercial License 1.0.0 (for noncommercial use):
  https://polyformproject.org/licenses/noncommercial/1.0.0
* _xlwings PRO License (for commercial use):
...
import _xlwings  # To prevent circular imports
...
class LicenseHandler:
    ...
    def get_cipher():
        try:
            ...
            raise _xlwings.LicenseError(
                "Couldn't validate _xlwings license key."
            ) from None
    ...
    def get_license():
        ...
        try:
            sheet_license_key = read_config_sheet(_xlwings.Book.caller()).get(
                "LICENSE_KEY"
            )
            ...
            pass
        # User config file
        config_file = _xlwings.USER_CONFIG_FILE
        ...
        raise _xlwings.LicenseError("Couldn't find an _xlwings license key.")
    ...
    def validate_license(product, license_type=None):
        ...
        if key.startswith("gA") and not Fernet:
            ...
            raise ImportError(
                "You are using a legacy _xlwings license key that requires the "
            ...
            try:
                ...
                raise _xlwings.LicenseError("Invalid _xlwings license key.") from None
        else:
            signature = hmac.new(
        ...
        try:
            if (
                ...
                raise _xlwings.LicenseError(
                    "You need a paid _xlwings license key for this action."
                )
        except KeyError:
            raise _xlwings.LicenseError(
                "You need a paid _xlwings license key for this action."
        ...
        if (
            ...
            raise _xlwings.LicenseError("Invalid _xlwings license key format.") from None
        license_valid_until = dt.datetime.strptime(
            license_info["valid_until"], "%Y-%m-%d"
        ).date()
        if dt.date.today() > license_valid_until:
            raise _xlwings.LicenseError(
                "Your _xlwings license expired on {}.".format(
                    license_valid_until.strftime("%Y-%m-%d")
                )
            ) from None
        if product not in license_info["products"]:
            raise _xlwings.LicenseError(
                f"Your _xlwings license key isn't valid for the '{product}' "
                "functionality."
            ) from None
        if (
            "version" in license_info.keys()
            and license_info["version"] != _xlwings.__version__
        ):
            raise _xlwings.LicenseError(
                "Your _xlwings deploy key is only valid for v{0}. To use a different "
                "version of _xlwings, re-release your tool or generate a new deploy key "
                "via '_xlwings license deploy'.".format(license_info["version"])
            ) from None
        if (license_valid_until - dt.date.today()) < dt.timedelta(days=30):
            warnings.warn(
                f"Your _xlwings license key expires in "
...
def get_embedded_code_temp_dir():
    tmp_base_path = os.path.join(tempfile.gettempdir(), "_xlwings")
