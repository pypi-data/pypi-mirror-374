    ...
    def __setattr__(self, key, value):
        ...
        while True:
            try:
                ...
                # characters would be tried forever, causing _xlwings to hang (they
...
class Apps(base_classes.Apps):
    ...
    def cleanup():
        ...
        active_pids = {app.pid for app in _xlwings.apps}
...
class App(base_classes.App):
    def __init__(self, spec=None, add_book=True, xl=None, visible=None):
        ...
        if xl is None:
            ...
            try:
                self._xl = GetActiveObject("Excel.Application")
            except com_error:
                self._xl = DispatchEx("Excel.Application")
            # ========================
            if add_book:
...
class Book(base_classes.Book):
    ...
    def fullname(self):
        ...
        try:
            ...
            win32gui.ShowWindow(book_hwnd, win32con.SW_NORMAL)
            win32gui.SetForegroundWindow(book_hwnd)
            # 模拟按下 ESC 键
            win32api.keybd_event(win32con.VK_ESCAPE, 0)
            # 模拟释放 ESC 键
            win32api.keybd_event(win32con.VK_ESCAPE, 0, win32con.KEYEVENTF_KEYUP)
        while True:
            try:
                if self.xl.FullName:
                    break
