...
from _uiautomation import OPERATION_WAIT_TIME, Logger, MAX_MOVE_SECOND, SetCursorPos, GetCursorPos
...
def wait_for_locator(locator: uia.Control, timeout):
    """等待定位器"""
    if not locator.Exists(maxSearchSeconds=timeout):
        raise LookupError('Find Control Timeout({}s): {}'.format(timeout, locator.GetSearchPropertiesStr()))

    rect = locator.BoundingRectangle
    print(rect)
    print(rect.height(), rect.width())
    if not rect.height() and not rect.height():
        raise '元素不可见'




def set_active(locator: uia.Control):
    """激活元素窗口"""
    try:
        locator: uia.WindowControl
        top_control = locator.GetTopLevelControl()
        top_control.SetActive()
    except AttributeError:
        pass


def min_for_active(name=None, class_name=None):
    """
    最小化激活
    ...
    import win32gui
    import win32con
    hwnd = win32gui.FindWindow(name, class_name)
