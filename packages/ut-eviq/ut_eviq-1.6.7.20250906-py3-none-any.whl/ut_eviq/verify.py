...
class EvinVfyAdm:
    ...
    def vfy_duns(
        ...
        Verify DUNS number
        """
        _key: TnStr = Dic.loc(d_cfg, 'Utils', 'evin_key', 'duns')
        ...
        if not _val.isdigit():
            ...
            d_sw['duns'] = False
            return
        if Dic.loc(d_cfg, 'Utils', 'd_sw_adm', 'vfy_duns_is_unique'):
            if dod[_key][_val] > 1:
    ...
    def vfy_cpydinm(
        ...
        """
        _key: TnStr = Dic.loc(d_cfg, 'Utils', 'evin_key', 'cpydinm')
    ...
    def vfy_regno(
        ...
        """
        _key: TnStr = Dic.loc(d_cfg, 'Utils', 'evin_key', 'regno')
        _val = Dic.get(d_evin, _key)
        if not _val:
    ...
    def vfy_coco(
        ...
        Verify Country Code
        """
        _key: TyStr = Dic.loc(d_cfg, 'Utils>', 'evin_key', 'coco')
        _val = Dic.get(d_evin, _key)
        if not _val:
    ...
    def vfy_objectid(
        ...
        Verify Country Code
        """
        _key: TyStr = Dic.loc(d_cfg, 'Utils', 'evin_key', 'objectid')
        _val = Dic.get(d_evin, _key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.objectid_is_empty, d_evin)
            d_sw['objectid'] = False
            return
        if Dic.loc(d_cfg, 'Utils', 'd_sw_adm', 'vfy_objectid_is_unique'):
            if dod[_key][_val] > 1:
    ...
    def vfy_town(
        ...
        """
        _key_town: TyStr = Dic.loc(d_cfg, 'Utils', 'evin_key', 'town')
        _val_town: TnStr = Dic.get(d_evin, _key_town)
        if not _val_town:
            DoA.append_unique_by_key(doaod_vfy, cls.town_is_empty, d_evin)
            d_sw['town'] = False
            return
        if not Dic.loc(d_cfg, 'Utils', 'd_sw_adm', 'vfy_town_with_coco'):
            d_sw['town'] = True
            return
        _key_coco: TyStr = Dic.loc(d_cfg, 'Utils', 'evin_key', 'coco')
    ...
    def vfy_poco(
        ...
        Verify Postal Code
        """
        _key_poco: TyStr = Dic.loc(d_cfg, 'Utils', 'evin_key', 'poco')
    ...
    def vfy_d_evin(
        ...
        _d_sw_adm: TyDic = Dic.loc(d_cfg, 'Utils', 'd_sw_adm')
    ...
    def sh_dod(
        ...
        _sw_vfy_objectid_is_unique: TyBool = Dic.loc(
                d_cfg, 'Utils', 'd_sw_adm', 'vfy_objectid_is_unique')
...
class EvinVfyDel:
    ...
    def vfy_objectid(
        ...
        Verify Country Code
        """
        _key: TnStr = Dic.loc(d_cfg, 'Utils', 'evin_key', 'objectid')
        _val = Dic.get(d_evin, _key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.objectid_is_empty, d_evin)
            d_sw['objectid'] = False
            return
        if Dic.loc(d_cfg, 'Utils', 'd_sw_del', 'vfy_objectid_is_unique'):
            if dod[_key][_val] > 1:
                DoA.append_unique_by_key(
                        doaod_vfy, cls.objectid_isnot_unique, d_evin)
                d_sw['objectid'] = False
    ...
    def vfy_iq_id(
        ...
        Verify IQ_ID
        """
        _key = Dic.loc(d_cfg, 'Utils', 'evin_key_iq_id')
        _val = Dic.get(d_evin, _key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.iq_id_is_empty, d_evin)
            d_sw['iq_id'] = False
            return
        if Dic.loc(d_cfg, 'Utils', 'd_sw_del', 'vfy_iq_id_is_unique'):
            if dod[_key][_val] > 1:
    ...
    def vfy_d_evin(
        ...
        if not Dic.loc(d_cfg, 'Utils', 'd_sw_del', 'vfy'):
            return True

        # Set verification summary switch
        _d_sw: TyDoB = {}

        if Dic.loc(d_cfg, 'Utils', 'd_sw_del', 'vfy_objectid'):
            # Verify ObjectID
            cls.vfy_objectid(_d_sw, d_evin, dod, doaod_vfy, d_cfg)

        if Dic.loc(d_cfg, 'Utils', 'd_sw_del', 'vfy_iq_id'):
