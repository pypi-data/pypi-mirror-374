...
class EvinVfyAdm:
    ...
    def vfy_duns(
        ...
        Verify DUNS number
        """
        _key: TnStr = Dic.loc(d_cfg, '_Utils', 'evin_key', 'duns')
        ...
        if not _val.isdigit():
            ...
            d_sw['duns'] = False
            return
        if Dic.loc(d_cfg, '_Utils', 'd_sw_adm', 'vfy_duns_is_unique'):
            if dod[_key][_val] > 1:
    ...
    @classmethod
    def vfy_cpydinm(
            cls, d_sw: TyDoB, d_evin: TyDic, doaod_vfy: TyDoAoD, d_cfg: TyDic
    ) -> None:
        """
        Verify Company display name
        """
        _key: TnStr = Dic.loc(d_cfg, '_Utils', 'evin_key', 'cpydinm')
    ...
    def vfy_regno(
        ...
        """
        _key: TnStr = Dic.loc(d_cfg, '_Utils', 'evin_key', 'regno')
        _val = Dic.get(d_evin, _key)
        if not _val:
    ...
    def vfy_coco(
        ...
        Verify Country Code
        """
        _key: TyStr = Dic.loc(d_cfg, '_Utils>', 'evin_key', 'coco')
        _val = Dic.get(d_evin, _key)
        if not _val:
    ...
    @classmethod
    def vfy_objectid(
            cls, d_sw: TyDoB, d_evin: TyDic, dod: TyDoD, doaod_vfy: TyDoAoD, d_cfg: TyDic
    ) -> None:
        """
        Verify Country Code
        """
        _key: TyStr = Dic.loc(d_cfg, '_Utils', 'evin_key', 'objectid')
        _val = Dic.get(d_evin, _key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.objectid_is_empty, d_evin)
            d_sw['objectid'] = False
            return
        if Dic.loc(d_cfg, '_Utils', 'd_sw_adm', 'vfy_objectid_is_unique'):
            if dod[_key][_val] > 1:
    ...
    def vfy_town(
        ...
        """
        _key_town: TyStr = Dic.loc(d_cfg, '_Utils', 'evin_key', 'town')
        _val_town: TnStr = Dic.get(d_evin, _key_town)
        if not _val_town:
            DoA.append_unique_by_key(doaod_vfy, cls.town_is_empty, d_evin)
            d_sw['town'] = False
            return
        if not Dic.loc(d_cfg, '_Utils', 'd_sw_adm', 'vfy_town_with_coco'):
            d_sw['town'] = True
            return
        _key_coco: TyStr = Dic.loc(d_cfg, '_Utils', 'evin_key', 'coco')
    ...
    def vfy_poco(
        ...
        Verify Postal Code
        """
        _key_poco: TyStr = Dic.loc(d_cfg, '_Utils', 'evin_key', 'poco')
    ...
    @classmethod
    def vfy_d_evin(
            cls, d_evin: TyDic, dod, doaod_vfy: TyDoAoD, d_cfg: TyDic
    ) -> TyBool:
        _d_sw_adm: TyDic = Dic.loc(d_cfg, '_Utils', 'd_sw_adm')
        if not _d_sw_adm['vfy']:
            return True

        # Set verification summary switch
        _d_sw: TyDoB = {}

        if Dic.loc(_d_sw_adm, 'vfy_duns'):
            # Verify DUNS
            cls.vfy_duns(_d_sw, d_evin, dod, doaod_vfy, d_cfg)
        if Dic.loc(_d_sw_adm, 'vfy_cpydinm'):
            # Verify Company display name
            cls.vfy_cpydinm(_d_sw, d_evin, doaod_vfy, d_cfg)
        if Dic.loc(_d_sw_adm, 'vfy_regno'):
            # Verify Country display name
            cls.vfy_regno(_d_sw, d_evin, doaod_vfy, d_cfg)
        if Dic.loc(_d_sw_adm, 'vfy_coco'):
            # Verify Country Code
            cls.vfy_coco(_d_sw, d_evin, doaod_vfy, d_cfg)
        if Dic.loc(_d_sw_adm, 'vfy_objectid'):
            # Verify ObjectID
            cls.vfy_objectid(_d_sw, d_evin, dod, doaod_vfy, d_cfg)
        if Dic.loc(_d_sw_adm, 'vfy_town'):
            # Verify Town in Country
            cls.vfy_town(_d_sw, d_evin, doaod_vfy, d_cfg)
        if Dic.loc(_d_sw_adm, 'vfy_poco'):
            # Verify Postal Code
            cls.vfy_poco(_d_sw, d_evin, doaod_vfy, d_cfg)

        if Dic.loc(_d_sw_adm, 'use_duns'):
    ...
    @staticmethod
    def sh_dod(
            aod_evin: TnAoD, d_cfg: TyDic) -> TyDoD:
        _dod: TyDoD = {}
        if not aod_evin:
            return _dod
        _sw_vfy_duns_is_unique: TyBool = Dic.loc(
                d_cfg, '_Utild', 'd_sw_adm', 'vfy_duns_is_unique')
        _sw_vfy_objectid_is_unique: TyBool = Dic.loc(
                d_cfg, '_Utils', 'd_sw_adm', 'vfy_objectid_is_unique')
        for _d_evin in aod_evin:
            if _sw_vfy_duns_is_unique:
    ...
    @classmethod
    def vfy_aod_evin(
            cls, aod_evin: TnAoD, d_cfg: TyDic
    ) -> tuple[TyAoD, TyDoAoD]:
        _aod_evin: TyAoD = []
        _doaod_vfy: TyDoAoD = {}
        if not aod_evin:
            return _aod_evin, _doaod_vfy
        _dod = cls.sh_dod(aod_evin, d_cfg)
        for _d_evin in aod_evin:
            _sw: bool = cls.vfy_d_evin(_d_evin, _dod, _doaod_vfy, d_cfg)
            if _sw:
...
class EvinVfyDel:
    ...
    def vfy_objectid(
            cls,
            d_sw: TyDoB, d_evin: TyDic, dod: TyDoD, doaod_vfy: TyDoAoD, d_cfg: TyDic
    ) -> None:
        """
        Verify Country Code
        """
        _key: TnStr = Dic.loc(d_cfg, '_Utils', 'evin_key', 'objectid')
        _val = Dic.get(d_evin, _key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.objectid_is_empty, d_evin)
            d_sw['objectid'] = False
            return
        if Dic.loc(d_cfg, '_Utils', 'd_sw_del', 'vfy_objectid_is_unique'):
            if dod[_key][_val] > 1:
                DoA.append_unique_by_key(
                        doaod_vfy, cls.objectid_isnot_unique, d_evin)
                d_sw['objectid'] = False
    ...
    def vfy_iq_id(
        ...
        Verify IQ_ID
        """
        _key = Dic.loc(d_cfg, '_Utils', 'evin_key_iq_id')
        _val = Dic.get(d_evin, _key)
        if not _val:
            DoA.append_unique_by_key(doaod_vfy, cls.iq_id_is_empty, d_evin)
            d_sw['iq_id'] = False
            return
        if Dic.loc(d_cfg, '_Utils', 'd_sw_del', 'vfy_iq_id_is_unique'):
            if dod[_key][_val] > 1:
    ...
    def vfy_d_evin(
            cls,
            d_evin: TyDic, dod: TyDoD, doaod_vfy: TyDoAoD, d_cfg: TyDic
    ) -> TyBool:
        if not Dic.loc(d_cfg, '_Utils', 'd_sw_del', 'vfy'):
            return True

        # Set verification summary switch
        _d_sw: TyDoB = {}

        if Dic.loc(d_cfg, '_Utils', 'd_sw_del', 'vfy_objectid'):
            # Verify ObjectID
            cls.vfy_objectid(_d_sw, d_evin, dod, doaod_vfy, d_cfg)

        if Dic.loc(d_cfg, '_Utils', 'd_sw_del', 'vfy_iq_id'):
            # Verify EcoVadis IQ Id
            cls.vfy_iq_id(_d_sw, d_evin, dod, doaod_vfy, d_cfg)
    ...
    def vfy_aod_evin(
        ...
        _dod = cls.sh_dod(aod_evin, d_cfg)
        for _d_evin in aod_evin:
            if cls.vfy_d_evin(_d_evin, _dod, _doaod_vfy, d_cfg):
                _aod_evin.append(_d_evin)
        return _aod_evin, _doaod_vfy
