...
class TaskTmpIn:
    ...
    def evupadm(
        ...
        _in_path = PathNm.sh_path(Dic.loc(_d_cfg, 'InPath', 'evup_tmp'), kwargs)
        _sheet_adm = Dic.loc(_d_cfg, 'Utils', 'sheet', 'adm')
        _wb: TnOpWb = OpIoiPathWb.sh_wb_adm(_in_path, aod_evup_adm, _sheet_adm)
        return _wb
    ...
    def evupdel(
        ...
        _d_cfg: TyDic = Dic.loc(kwargs, 'd_cfg')
        _in_path = PathNm.sh_path(Dic.loc(_d_cfg, 'InPath', 'evup_tmp'), kwargs)
        _sheet_del = Dic.loc(_d_cfg, 'Utils', 'sheet', 'del')
        _wb: TnOpWb = OpIoiPathWb.sh_wb_del(_in_path, aod_evup_del, _sheet_del)
        return _wb
    ...
    def evupreg(
        ...
        _wb: TnOpWb = OpIoiPathWb.sh_wb_reg(
           _in_path, aod_evup_adm, aod_evup_del, _sheet_adm, _sheet_del)
        return _wb
...
class TaskIn:
    ...
    def evupdel(
        ...
        if Dic.loc(_d_cfg, 'Utils', 'sw_del', 'use_evex'):
    ...
    def evdomap(
        ...
        """
        _d_ecv_iq2umh_iq = Dic.loc(kwargs, 'd_cfg', 'Utils', 'd_ecv_iq2umh_iq')
