...
class Evex:
    ...
    def sh_d_evup_del_from_dic(d_evex: TnDic) -> TnDic:
        ...
        if d_evex is None:
            return d_evup
        _d_del_evup2evex = Cfg._Utils.d_del_evup2evex
...
class Evin:
    ...
    def read_wb_adm_to_df(cls, kwargs) -> TnPdDf:
        _sheet = Cfg.sheet_adm
        _path = PathNm.sh_path(Cfg.InPath.evin, kwargs)
    ...
    def read_wb_del_to_aod(cls, kwargs) -> TnAoD:
        _path = PathNm.sh_path(Cfg.InPath.evin, kwargs)
        _sheet = Cfg.sheet_del
        _aod: TnAoD = IoiPathWb.read_wb_to_aod(_path, _sheet, **cls.kwargs_wb)
        return _aod

    @staticmethod
    def sh_d_evup_adm(d_evin: TyDic) -> TyDic:
        _d_evup2const = Cfg._Utils.d_evup2const
        _d_evup2evin = Cfg._Utils.d_evup2evin
        d_evup: TyDic = {}
...
class EvinEvex:
    ...
    def query_with_key(
        ...
        if not _evin_value:
            ...
            return None
        _evex_key_coco = Cfg._Utils.evex_key_coco
        _d_evin2evex_keys = Cfg._Utils.d_evin2evex_keys
    ...
    def query_with_keys(
        ...
        _evin_key_coco = Cfg._Utils.evin_key_coco
        _evin_val_coco = d_evin.get(_evin_key_coco)
        if not _evin_val_coco:
            Log.error(cls.msg_evin.format(K=_evin_key_coco, D=d_evin))
            return None
        for _evin_key in Cfg._Utils.a_evin_key.keys():
            df = cls.query_with_key(d_evin, df_evex, _evin_key, _evin_val_coco)
            if df is not None:
                return df
        return None
    ...
    def query(cls, d_evin: TyDic, df_evex: TnPdDf) -> TyDic:
        _evin_key_objectid = Cfg._Utils.evin_key_objectid
        _evin_key_duns = Cfg._Utils.evin_key_duns
        _d_evin2evex_keys = Cfg._Utils.d_evin2evex_keys
    ...
    def join_adm(cls, aod_evin: TnAoD, df_evex: TnPdDf) -> TyDoAoD:
        ...
        _evin_key_objectid = Cfg._Utils.evin_key_objectid
        _d_evin2evex_keys = Cfg._Utils.d_evin2evex_keys
        _sw_adm_use_evex = Cfg.DoSwAdm.use_evex
    ...
    def join_del(cls, aod_evin: TnAoD, df_evex: TnPdDf) -> TyAoD:
        ...
        for _d_evin in aod_evin:
            _df_evex_row: TnPdDf = PdDf.query_with_key(
                    df_evex, _d_evin,
                    dic_key=_evin_key_objectid, d_key2key=_d_evin2evex_keys)
    ...
    def sh_d_evup_adm(d_evin: TyDic, d_evex: TyDic) -> tuple[str, TyDic]:
        ...
        _d_evup2const: TyDic = Cfg._Utils.d_evup2const
        _d_evup2evex: TyDic = Cfg._Utils.d_evup2evex
        _d_evup2evin: TyDic = Cfg._Utils.d_evup2evin
...
class EvexEvin:
    ...
    def join_del(cls, aod_evex: TnAoD, df_evin: TnPdDf) -> TyAoD:
        ...
        if not aod_evex or df_evin is None:
            return _aod_evup
        _evin_key_objectid = Cfg._Utils.evin_key_objectid
        _d_evex2evin_keys = Cfg._Utils.d_evex2evin_keys
