...
def generate_ml_report(
    ...
    preprocessed = preprocess_data(df, target_column=target_column)
    df_clean = preprocessed["processed_df"]
    preprocessing_steps = preprocessed["steps"]

    # Step 5: Model selection, evaluation, and explainability
    ...
    for model in models:
        ...
        shap_output = explain_model(model, df_clean, target_column)
        explanations.append({
            "model": model.__class__.__name__,
    ...
    # Step 7: Return all outputs for programmatic use
    return {
        "summary": summary,
