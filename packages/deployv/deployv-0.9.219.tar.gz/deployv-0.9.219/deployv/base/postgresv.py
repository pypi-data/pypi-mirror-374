...
class PostgresConnector:
    ...
    def execute(self, sql_str, *args, **kwargs):
        ...
        if kwargs.pop('with_as_write', False):
            sql_sentences += ('with', )
        change = sql_str.lower().startswith(sql_sentences)
        try:
    ...
    * dropdb
    ...
    def __init__(self, config):
        ...
        for executable in ['pg_dump', 'pg_restore', 'psql']:
            custom_exec = "/opt/orchestsh/bin/%s" % executable
            # Negative expression used to receive a return code of 1 (True) if it exists.
            exec_exists = shell.run(
                shlex.split("test ! -f %s" % custom_exec),
                allow_error=True
            ).return_code
            self.__config.update({executable: custom_exec if exec_exists else executable})
    ...
    def dump(self, database, destiny_file, jobs=False, exclude=False):
        ...
        dump_cmd = '{pg_dump} -d {dbname} -w -O -x -f {0}'
        if jobs:
            self.__config.update({'num': jobs})
            dump_cmd = '{pg_dump} -w  -j {num} -F d -d {dbname} -f {0} -x'
        if exclude:
    ...
    def restore(self, database_name, dump_name, jobs=False):
        ...
        restore_cmd = '{psql} -d {dbname} -w -f {0}'
        if jobs:
