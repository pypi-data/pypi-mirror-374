...
class KernelAgent(Kernel):
    """
    Remote HTTP kernel “agent” that proxies notebook cell execution to a gateway.
    NOTE: Configuration is passed via KernelAgent.boot (set by CLI) or, secondarily, env.
    We DO NOT mutate environment variables in this module.
    """
    ...
    # Populated by CLI before launch; do not rely on env writes.
    boot: Dict[str, Any] = {}

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        cfg = getattr(KernelAgent, "boot", {}) or {}
        ...
        # Session & UX
        self.session_id: str = cfg.get("session_id") or os.environ.get("KERNEL_AGENT_SESSION_ID") or str(uuid.uuid4())
        self.display_name: str = cfg.get("display_name") or os.environ.get("KERNEL_AGENT_DISPLAY_NAME") or "Super Kernel"

        # Auth
        self.api_key: Optional[str] = cfg.get("api_key") or os.environ.get("KERNEL_AGENT_API_KEY") or None

        # Timeouts & behavior
        try:
            self.timeout: float = float(cfg.get("timeout") or os.environ.get("KERNEL_AGENT_TIMEOUT") or "300")
        except Exception:
            self.timeout = 300.0
        self.on_disconnect: str = (cfg.get("on_disconnect")
                                   or os.environ.get("KERNEL_AGENT_ON_DISCONNECT")
                                   or "hold").lower()

        # Channel: ALWAYS default to "default" if not set anywhere
        self.channel: str = (cfg.get("channel")
                             or os.environ.get("KERNEL_AGENT_CHANNEL")
                             or "default")

        # SSE toggle (default off unless explicitly enabled)
        sse_flag = (cfg.get("sse") or os.environ.get("KERNEL_AGENT_SSE") or "off").lower()
        self.use_sse: bool = (sse_flag == "on")
    ...
    def _rpc(self, action: str, **extra) -> Dict[str, Any]:
        ...
        payload = {"action": action, "session_id": self.session_id, "channel": self.channel, **extra}
        return self._post_json(self.endpoint, payload, self.api_key, timeout=self.timeout, channel=self.channel)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--channel", default=None, help="Logical routing channel to include in every request")
    return p
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Stash boot config directly on the class (do NOT modify environment variables).
    KernelAgent.boot = {
        ...
        "sse": args.sse,                         # "on" or "off" or None
        "channel": args.channel or "default",    # always default to "default"
    }
