...
class KernelAgent(Kernel):
    ...
    def _maybe_magic(self, code: str) -> bool:
        ...
        if s in {"%info"}:
            try:
                resp = self._rpc("info")  # calls POST / with {"action": "info", ...}
                md = [
                    "### Torch Cluster Info",
                    f"- **World Size**: {resp.get('world_size')}",
                    f"- **Rank**: {resp.get('rank')}",
                    f"- **CUDA Available**: {resp.get('cuda')}",
                    f"- **Device Count**: {resp.get('device_count')}",
                ...
                self._write_stream("stderr", f"[kernel-agent] %info failed: {e}\n")
            return True        
        if s in {"%reset"}:
            try:
                self._rpc("reset")
