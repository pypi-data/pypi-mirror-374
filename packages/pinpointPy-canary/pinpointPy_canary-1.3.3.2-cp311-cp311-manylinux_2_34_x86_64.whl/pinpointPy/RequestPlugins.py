...
class RequestPlugin(Common.PinTrace):
    ...
    def onBefore(self, parentId, *args, **kwargs):
        ...
        # Debug printing for incoming request headers
        debug_enabled = os.environ.get('PINPOINT_DEBUG_HEADERS', 'false').lower() == 'true'
        if debug_enabled:
            print(f"[PINPOINT-INBOUND] 📨 Incoming Request Headers:")
            print(f"  🌐 Path: {request.path}")
            print(f"  🏠 Remote: {request.remote_addr} -> Host: {request.host}")
            
            # Print all Pinpoint related headers
            pinpoint_headers = {}
            for header_name in [
                ...
                if header_name in request.headers:
                    pinpoint_headers[header_name] = request.headers[header_name]
            
            if pinpoint_headers:
                print(f"  📋 Pinpoint Headers Found:")
                for name, value in pinpoint_headers.items():
                    icon = "🏷️" if "CANARY" in name else "🔍"
                    print(f"    {icon} {name}: {value}")
            else:
                print(f"  ❌ No Pinpoint headers found - starting new trace")
            print()
        ...
        if Defines.PP_HTTP_CANARY_TAG in request.headers:
            self.canary_tag = request.headers[Defines.PP_HTTP_CANARY_TAG]
            pinpoint.add_context('canary_tag', self.canary_tag, trace_id)
            pinpoint.add_trace_header('canary_tag', self.canary_tag, trace_id)
        ...
        if Defines.PP_HEADER_CANARY_TAG in request.headers:
            self.canary_tag = request.headers[Defines.PP_HEADER_CANARY_TAG]
            pinpoint.add_context('canary_tag', self.canary_tag, trace_id)
            pinpoint.add_trace_header('canary_tag', self.canary_tag, trace_id)
        ...
        if debug_enabled:
            canary_tag = getattr(self, 'canary_tag', None)
            print(f"[PINPOINT-INBOUND] 🆔 Generated Trace Info:")
            print(f"  🔗 TraceID: {self.tid}")
            print(f"  🎯 SpanID: {self.sid}")
