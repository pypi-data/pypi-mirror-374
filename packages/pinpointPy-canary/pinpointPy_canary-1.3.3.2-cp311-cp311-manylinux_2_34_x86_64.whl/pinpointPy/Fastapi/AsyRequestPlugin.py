...
class AsyRequestPlugin(AsyncPinTrace):
    ...
    def onBefore(self, parentId, *args, **kwargs):
        ...
        if Defines.PP_HTTP_CANARY_TAG in request.headers:
            canary_tag = request.headers[Defines.PP_HTTP_CANARY_TAG]
            pinpoint.add_context('canary_tag', canary_tag, traceId)
            pinpoint.add_trace_header('canary_tag', canary_tag, traceId)
        ...
        if Defines.PP_HEADER_CANARY_TAG in request.headers:
            canary_tag = request.headers[Defines.PP_HEADER_CANARY_TAG]
            pinpoint.add_context('canary_tag', canary_tag, traceId)
            pinpoint.add_trace_header('canary_tag', canary_tag, traceId)
