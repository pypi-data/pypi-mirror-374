...
class DocumentMerger:
    def __init__(self, config_path=None, base_path=None):
        # Resolve config path first, then force base_path to its parent to guarantee
        # all outputs stay inside the project folder initialized by `init`.
        cwd = Path.cwd()
        resolved_base = Path(base_path).resolve() if base_path else cwd
        cfg = Path(config_path).resolve() if config_path else (resolved_base / "config.yaml")

        if not cfg.exists():
            raise FileNotFoundError(
                f"Config file not found at {cfg}.\n"
                f"Please run 'doc-tracker init <folder>' then: cd <folder> && doc-tracker watch"
            )

        self.config_path = cfg
        # Always tie base_path to the config file's directory
    ...
    def aggregate_documents(self):
        ...
        for doc in documents_data:
            ...
            # Append content lines, removing empty/whitespace-only lines
