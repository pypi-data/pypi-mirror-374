...
def main():
    ...
    parser.add_argument(
        ...
        help=('init: create project folder with config.yaml and master-doc.md\n'
              'watch: aggregate and watch; in current folder uses local config.yaml,\n'
              '       from parent folder auto-detects the only subfolder that has\n'
              '       config.yaml and its configured output file')
    )
