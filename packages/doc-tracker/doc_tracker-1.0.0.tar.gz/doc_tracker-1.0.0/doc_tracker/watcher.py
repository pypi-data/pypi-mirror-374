...
def _read_output_file(dir_path: Path) -> Optional[str]:
    ...
    try:
        ...
        out = data.get('output_file', 'MASTER_DOCUMENTATION.md')
        if os.path.isabs(out):
            # invalid per spec; ignore this folder
            return None
        return out
    except Exception:
        return None
...
def _find_project_base(start_dir: Path) -> Optional[Path]:
    # If current dir has config.yaml, treat it as project root (even if output_file is missing)
    if (start_dir / 'config.yaml').exists():
        return start_dir
    ...
    for p in start_dir.iterdir():
        if not p.is_dir():
            continue
        out = _read_output_file(p)
        if out and (p / out).exists():
            candidates.append(p)
...
def watch(config_path=None, base_path=None):
    ...
    # Monitor base directory (for config.yaml changes)
    observer.schedule(event_handler, str(event_handler.base_path), recursive=True)
    
    # Monitor each directory containing tracked files
    watched_dirs = set()
    for doc in event_handler.merger.tracked_docs:
        doc_path_str = doc['path']
        if not Path(doc_path_str).is_absolute():
        ...
        # Add the directory containing this file to watch list
        parent_dir = doc_path.parent
        if parent_dir not in watched_dirs and parent_dir != event_handler.base_path:
            watched_dirs.add(parent_dir)
            print(f"📁 Also watching: {parent_dir}")
