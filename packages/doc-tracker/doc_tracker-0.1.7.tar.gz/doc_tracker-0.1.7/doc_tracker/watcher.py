...
def _read_output_file(dir_path: Path) -> str | None:
...
def _find_project_base(start_dir: Path) -> Path | None:
    # Prefer a directory that has both config.yaml and its output_file present
    out_here = _read_output_file(start_dir)
    if out_here:
        if (start_dir / out_here).exists():
            return start_dir
