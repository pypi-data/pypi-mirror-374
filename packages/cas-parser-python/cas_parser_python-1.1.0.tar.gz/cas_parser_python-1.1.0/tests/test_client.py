...
from cas_parser import CasParser, AsyncCasParser, APIResponseValidationError
from cas_parser._types import Omit
from cas_parser._utils import asyncify
from cas_parser._models import BaseModel, FinalRequestOptions
from cas_parser._exceptions import APIStatusError, CasParserError, APITimeoutError, APIResponseValidationError
from cas_parser._base_client import (
    DEFAULT_TIMEOUT,
...
class TestCasParser:
            ...
            for frame in diff.traceback:
                if any(
                    ...
                    for fragment in [
                        ...
                        "cas_parser/_legacy_response.py",
                        "cas_parser/_response.py",
                        # pydantic.BaseModel.model_dump || pydantic.BaseModel.dict leak memory for some reason.
                        "cas_parser/_compat.py",
    ...
    def test_validate_headers(self) -> None:
        ...
        with pytest.raises(CasParserError):
            with update_env(**{"CAS_PARSER_API_KEY": Omit()}):
                client2 = CasParser(base_url=base_url, api_key=None, _strict_response_validation=True)
            _ = client2
    ...
    def test_multipart_repeating_array(self, client: CasParser) -> None:
        request = client._build_request(
    ...
    def test_base_url_setter(self) -> None:
        client = CasParser(base_url="https://example.com/from_init", api_key=api_key, _strict_response_validation=True)
    ...
    @pytest.mark.parametrize(
        "client",
        [
            CasParser(base_url="http://localhost:5000/custom/path/", api_key=api_key, _strict_response_validation=True),
            CasParser(
    ...
    def test_base_url_trailing_slash(self, client: CasParser) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="/foo",
    ...
    def test_base_url_no_trailing_slash(self, client: CasParser) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="/foo",
    ...
    def test_absolute_request_url(self, client: CasParser) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
    ...
    def test_client_max_retries_validation(self) -> None:
        with pytest.raises(TypeError, match=r"max_retries cannot be None"):
            CasParser(base_url=base_url, api_key=api_key, _strict_response_validation=True, max_retries=cast(Any, None))
    ...
    def test_received_text_for_expected_json(self, respx_mock: MockRouter) -> None:
        ...
        strict_client = CasParser(base_url=base_url, api_key=api_key, _strict_response_validation=True)
    ...
    def test_omit_retry_count_header(
        self, client: CasParser, failures_before_success: int, respx_mock: MockRouter
    ) -> None:
        client = client.with_options(max_retries=4)
...
class TestAsyncCasParser:
    ...
    def test_multipart_repeating_array(self, async_client: AsyncCasParser) -> None:
    ...
    async def test_received_text_for_expected_json(self, respx_mock: MockRouter) -> None:
        ...
        strict_client = AsyncCasParser(base_url=base_url, api_key=api_key, _strict_response_validation=True)
