...
class FincoreSubjectRating(AutoUpper):
    STRONG_BUY = auto()  # type: ignore
    BUY = auto()  # type: ignore
    HOLD = auto()  # type: ignore
    SELL = auto()  # type: ignore
    STRONG_SELL = auto()  # type: ignore
...
class FincoreCategoryDetailed(AutoLower):
    ...
    MIXED = auto()  # type: ignore  # Used when multiple categories apply, e.g., a stock that is also in an ETF
