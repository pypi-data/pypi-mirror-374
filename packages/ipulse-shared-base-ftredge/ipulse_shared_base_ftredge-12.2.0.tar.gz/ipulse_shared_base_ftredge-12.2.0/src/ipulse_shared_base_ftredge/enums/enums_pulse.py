...
class Sector(AutoLower): 
    #1 Sector = 1 Target Subject Type
    # --- TARGETS ARE ASSETS ----
    ...
    # --- TARGETS ARE SUBJECTS , about ABOUT STATE OF THE WORLD, NOT ASSETS : ECONOMICS AND GEOPOLITICAL INDICATORS, NOT ASSETS ------
    MACROECONOMY=auto()  # type: ignore # # ;eg. US , Automobile etc. includes economic indicators and geopolitical events
...
class SectorRecordsCategory(AutoLower):
    # FACT DATA
    MARKET           = auto()  # type: ignore   #  relates to $$ ie prices, trades, quotes; structured time series data
    ...
    FUNDAMENTAL      = auto()  # type: ignore   #  financial statements , revenue, earnings, etc. Can be structured (annual revenues) or unstructured - balance sheet, income statement, etc.
    EVENT           = auto()  # type: ignore   #  dividends, splits, M&A, flood, tariff announcement , declaration of war etc.
    NEWSFEED         = auto()  # type: ignore   #  news, tweets, social media posts
    RISK            = auto()  # type: ignore   #  Descriptive Risks usually in Class/Text/Verdict like High Debt. If it is a quantitative risk, then most probably will be in INDICATOR
    ...
    # DIMENSIONAL and GOVERNANCE DATA
    REFERENCE        = auto()  # type: ignore   # static ids, calendars
    SPECIFICATIONS  = auto()  # type: ignore   # technical specifications, used for describing data and its structure
    CATALOG          = auto()  # type: ignore   #  Catalogs, used for cataloging data and metadata
    SCHEMAS          = auto()  # type: ignore  # Dataset used for schema definitions
...
class SectorCategory(AutoLower):
