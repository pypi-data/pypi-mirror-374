...
from bear_dereth.tools.logger.config import ConsoleOptions, Container
from bear_dereth.tools.logger.protocols.handler import Handler, HandlerModes

if TYPE_CHECKING:
    from collections.abc import Callable

    from bear_dereth.tools.di._wiring import Provider
...
class ConsoleHandler[Handler_Type: TextIO | IO](Handler):
    ...
    def __init__(
        ...
        *,
        ...
        self.callback: Callable[..., Any] = error_callback
    ...
    def emit(self, msg: object, style: str, level: LogLevel, **kwargs) -> None:
        ...
        if self.caller and self.should_emit(level):
            try:
                ...
                self.callback(f"ConsoleHandler emit error: {e}")
