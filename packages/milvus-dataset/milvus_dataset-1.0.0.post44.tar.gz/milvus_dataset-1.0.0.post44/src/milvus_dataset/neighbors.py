    ...
    def compute_neighbors(  # noqa
        ...
        def process_batch(test_batch):
            ...
            if self.use_gpu:
                ...
                logger.info(f"CPU: Using metric '{self.metric_type}' for distance computation")

                if self.metric_type == "inner_product":
                    logger.debug(
                        "CPU: Computing inner product distances using matrix multiplication"
                    )
                    distance = -1 * (train_emb @ test_emb.T)
                else:
                    logger.debug(
                        f"CPU: Computing pairwise distances using sklearn with metric '{self.metric_type}' and n_jobs=-1"
                    )
                    distance = pairwise_distances(
                        train_emb, Y=test_emb, metric=self.metric_type, n_jobs=-1
                    )

                logger.debug("CPU: Transposing and converting distance matrix to contiguous array")
                distance = np.array(distance.T, order="C", dtype=np.float32)
                logger.debug(f"CPU: Distance matrix shape after transpose: {distance.shape}")

                logger.debug(f"CPU: Sorting distances to find top-{self.top_k} neighbors")
                ...
                logger.info(
                    f"CPU: Batch computation completed successfully - processed {len(test_idx)} test vectors"
                )
                logger.debug(
                    f"CPU: Results - indices shape: {indices.shape}, distances shape: {distances.shape}"
                )
                return indices, distances, test_idx, True
