...
from siotls.iana import CertificateType, ContentType, ExtensionType, HandshakeType, TLSVersion
...
class ServerWaitClientHello(State):
    ...
    def _send_server_certificate(self):
        ...
        if self.nconfig.server_certificate_type == CertificateType.RAW_PUBLIC_KEY:
        ...
        elif self.nconfig.server_certificate_type == CertificateType.X509:
            ...
            e = "unknown server certificate type: "
            raise NotImplementedError(e + str(self.nconfig.server_certificate_type))
    ...
    def _negociate_signature_algorithms(self, sa_ext):
        ...
        sign_algos = TLSSignatureScheme.for_key_algo(self.config.asn1_public_key['algorithm'])
        for sign_algo in sign_algos:
            if sign_algo.iana_id in sa_ext.supported_signature_algorithms:
                self._signature = sign_algo(private_key=self.config.private_key)
                self.nconfig.signature_algorithm = sign_algo.iana_id
                return [], []
