...
class TLSCipherSuite(ICipher, metaclass=RegistryMeta):
    ...
    usage_limit: int
    """
    How many messages and how many blocks can safely be encrypted using
    a same secret key according to `draft-irtf-cfrg-aead-limits`_
    (Usage Limits on AEAD Algorithms; Draft RFC).

    .. _draft-irtf-cfrg-aead-limits: https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-aead-limits
    """
    ...
    def should_rekey(self) -> bool:
        """
        Whether we are dangerously approaching :attr:`usage_limit` and
        that there's a risk in continuing using this secret key.
        """
        return False if self._write_seq is None else (
...
class Aes128GcmMixin:
    ...
    :attr:`~TLSCipherSuite.usage_limit`,
