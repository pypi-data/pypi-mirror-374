...
from siotls.language import TLSIO, TLSBufferError, TooMuchDataError
...
class TLSConnection:
    ...
    def close_receiving_end(self):
        ...
        if not self.is_post_handshake():
            ...
            self._fail()
            return
        if self.is_connected():  # ragged EOF
            # https://tlswg.org/tls13-spec/draft-ietf-tls-rfc8446bis.html#name-closure-alerts
            # > If a transport-level close is received prior to a
            # > "close_notify", the receiver cannot know that all the
