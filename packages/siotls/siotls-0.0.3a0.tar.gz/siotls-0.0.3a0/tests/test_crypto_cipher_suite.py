...
class TestCrytoCipherSuite(TestCase):
    ...
    def test_cipher_early_secret(self):
        ...
        with self.assertLogs('siotls.keylog', logging.INFO) as capture:
            client_cipher.derive_early_secrets(pre_shared_key, 'external', transcript_hash)
        ...
        server_cipher = ChaPoly('server', client_unique, log_keys=True)
        with self.assertLogs('siotls.keylog', logging.INFO) as capture:
            server_cipher.derive_early_secrets(pre_shared_key, 'external', transcript_hash)
            self.assertEqual(capture.output, [
                f'INFO:siotls.keylog:CLIENT_EARLY_TRAFFIC_SECRET {client_unique.hex()} '
                    'fc3ab38713cde6d40f65f709e740339997f5fc5004ce46cf820a51ddcd81f8c8'
            ])
        ...
        self.assertEqual(server_cipher.state, CipherState.EARLY)
        self.assertTrue(server_cipher.must_decrypt, "client will send encrypted early data")
        self.assertFalse(server_cipher.must_encrypt, "server will send clear server hello")
        self.assertFalse(server_cipher.should_rekey)
    ...
    def test_cipher_handshake_after_init(self):
        ...
        with self.assertLogs('siotls.keylog', logging.INFO) as capture:
            ...
            self.assertEqual(capture.output, [
                f'INFO:siotls.keylog:CLIENT_HANDSHAKE_TRAFFIC_SECRET {client_unique.hex()} '
                    'dbefec40bafcc3a522a2b3f5a654cb2968c3d455536d2580bb1f12c7222fa229',
                f'INFO:siotls.keylog:SERVER_HANDSHAKE_TRAFFIC_SECRET {client_unique.hex()} '
        ...
        self.assertTrue(client_cipher.must_decrypt, "server will send encrypted extensions")
        self.assertTrue(client_cipher.must_encrypt, "client will send encrypted finished")
        self.assertFalse(client_cipher.should_rekey)
        ...
        self.assertTrue(server_cipher.must_encrypt, "server will send encrypted extensions")
        self.assertTrue(server_cipher.must_decrypt, "client will send encrypted finished")
        self.assertFalse(server_cipher.should_rekey)
    ...
    def test_cipher_handshake_after_early(self):
        ...
        # client side
        client_cipher = ChaPoly('client', client_unique, log_keys=True)
        client_cipher.derive_early_secrets(pre_shared_key, 'external', ch_transcript_hash)
        with self.assertLogs('siotls.keylog', logging.INFO) as capture:
        ...
        # server side
        server_cipher = ChaPoly('server', client_unique, log_keys=True)
        server_cipher.derive_early_secrets(pre_shared_key, 'external', ch_transcript_hash)
        with self.assertLogs('siotls.keylog', logging.INFO) as capture:
    ...
    def test_cipher_application(self):
        ...
        with self.assertLogs('siotls.keylog', logging.INFO) as capture:
            ...
            self.assertEqual(capture.output, [
                f'INFO:siotls.keylog:CLIENT_TRAFFIC_SECRET_0 {client_unique.hex()} '
                    'b3779f71acf1ed121518da01abaec9eed9fd48eb2f6dbccf1a29f0203e1c12eb',
                f'INFO:siotls.keylog:SERVER_TRAFFIC_SECRET_0 {client_unique.hex()} '
                    'f529b987cbe46f37e517d9ae8f92c1eeba82c051acff1a4557fddd3a229900e1',
                f'INFO:siotls.keylog:EXPORTER_SECRET {client_unique.hex()} '
        ...
        self.assertTrue(client_cipher.must_encrypt, "client to send encrypted application data")
        self.assertTrue(client_cipher.must_decrypt, "server to send encrypted application data")
        self.assertFalse(client_cipher.should_rekey)
        ...
        self.assertTrue(server_cipher.must_encrypt, "server to send encrypted application data")
        self.assertTrue(server_cipher.must_decrypt, "client to send encrypted application data")
        self.assertFalse(server_cipher.should_rekey)
