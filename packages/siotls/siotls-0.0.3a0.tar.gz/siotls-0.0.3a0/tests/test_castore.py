...
class TestCAStore(TestCase):
    ...
    def test_castore_certifi(self):
        self.assertIsCAStore(castore.get_certifi_ca_certificates())

    @unittest.skipUnless(is_debian_like, "need debian-like")
    def test_castore_debian_ca_certificates(self):
        m = r"^detected system like (debian|ubuntu), using /etc/ssl/certs/ca-certificates.crt$"
        with (
            patch.object(castore, 'ssl', None),
            self.assertLogs('siotls.trust', 'INFO', log_pattern=m),
        ):
            self.assertIsCAStore(castore.get_system_ca_certificates())
    ...
    @unittest.skipUnless(platform.system() == 'Windows' and ssl, "need windows with stdlib ssl")
    ...
    def test_castore_unknown_system(self):
        ...
        with (
            ...
            castore.get_system_ca_certificates()
