...
class TestNegociationServerCipherSuite(TestNegociationServer):
    def setUp(self):
        try:
            ...
            if "algorithms are not available" in exc.args[0]:
    ...
    def test_negociation_server_cipher_suite_empty(self):
...
class TestNegociationServerSupportedGroups(TestNegociationServer):
    def setUp(self):
        super().setUp()
        try:
            self.replace_config(key_exchanges=[NamedGroup.x25519, NamedGroup.secp256r1])
        except ValueError as exc:
            if "algorithms are not available" in exc.args[0]:
                self.skipTest("incompatible crypto backend")
            raise
...
class TestNegociationServerClientCertificateType(TestNegociationServer):
    ...
    def test_negociation_server_client_certificate_type(
        self, trust_store, trusted_pubkey, user_types, type_, response,
    ):
        self.replace_config(
            x509verifier=test_x509verifier if trust_store else None,
            trusted_public_keys=test_trusted_public_keys if trusted_pubkey else (),
        )
...
class TestNegociationServerServerCertificateType(TestNegociationServer):
    ...
    def test_negociation_server_server_certificate_type(
        self, cert, pubkey, user_types, type_, response,
    ):
        self.replace_config(
            certificate_chain=[server_der_cert, ca_der_cert] if cert else [],
            public_key=server_der_pubkey if pubkey else None,
        )
...
class TestNegociationServerKeyShare(TestNegociationServer):
    ...
    def test_negociation_server_key_share_corrupted_x(self):
        if NamedGroup.x25519 not in TLSKeyExchange:
            self.skipTest("incompatible crypto backend")
        self.server.nconfig.key_exchange = NamedGroup.x25519
    ...
    def test_negociation_server_key_share_corrupted_ffdhe(self):
        if NamedGroup.ffdhe2048 not in TLSKeyExchange:
            self.skipTest("incompatible crypto backend")
...
class TestNegociationServerALPN(TestNegociationServer):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.replace_config(alpn=[b'h2', b'http/1.1'])
    ...
    def test_negociation_server_alpn_unconfigured(self):
        ...
        clears, crypts = self._negociate_alpn(ALPN([b'http/1.1']))
        self.assertEqual(clears, [])
        self.assertEqual(crypts, [])
        self.assertIsNone(self.server.nconfig.alpn)
    ...
    def test_negociation_server_alpn_pick_only_available(self):
        clears, crypts = self._negociate_alpn(ALPN([b'http/1.1']))
        self.assertEqual(clears, [])
        self.assertEqual(crypts, [ALPN([b'http/1.1'])])
        self.assertEqual(self.server.nconfig.alpn, b'http/1.1')

    def test_negociation_server_alpn_pick_server_best(self):
        clears, crypts = self._negociate_alpn(ALPN([b'http/1.1', b'h2']))
        self.assertEqual(clears, [])
        self.assertEqual(crypts, [ALPN([b'h2'])])
        self.assertEqual(self.server.nconfig.alpn, b'h2')
...
class TestNegociationClient(TestCase):
    @classmethod
    def setUpClass(cls):
        cls.client = TLSConnection(client_config, server_domain)
...
class TestNegociationClientALPN(TestNegociationClient):
    ...
    def test_negociation_client_alpn_missing_offered(self):
        self.replace_config(alpn=[b'h2', b'http/1.1'])
        self.client._state._negociate_alpn(None)
        self.assertEqual(self.client.nconfig.alpn, None)
    ...
    def test_negociation_client_alpn_present_not_offered(self):
        ...
        with self.assertRaises(alerts.IllegalParameter, error_msg=e):
            self.client._state._negociate_alpn(ALPN([b'h2']))
    ...
    def test_negociation_client_alpn_too_many_protocols_present(self):
        ...
        with self.assertRaises(alerts.IllegalParameter, error_msg=e):
            self.client._state._negociate_alpn(ALPN([b'h2', b'http/1.1']))

    def test_negociation_client_alpn_present_valid(self):
        self.replace_config(alpn=[b'h2', b'http/1.1'])
        with self.subTest(msg="server picked h2"):
            self.client._state._negociate_alpn(ALPN([b'h2']))
            self.assertEqual(self.client.nconfig.alpn, b'h2')
        with self.subTest(msg="server picked h1"):
            self.client._state._negociate_alpn(ALPN([b'http/1.1']))
            self.assertEqual(self.client.nconfig.alpn, b'http/1.1')
