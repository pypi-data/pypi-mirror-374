...
class ClientHello(Handshake, SerializableBody):
    """
    Client Hello handshake as defined in :rfc:`8446#section-4.1.2`.
    Conveys the client's supported cryptographic algorithms, along with
    additional extensions to negotiate optional features.

    **Note**: this handshake, together with
    :class:`~siotls.contents.handshakes.server_hello.ServerHello`, are
    ...
    """
    32 bytes generated by a secure random number generator. Uniquely
    identify this connection.
    """

    legacy_session_id: bytes = dataclasses.field(default=b'', repr=False)

    cipher_suites: list[CipherSuites | int]  # type: ignore[misc]
    """
    The list of ciphers that this client support.

    See also :attr:`~siotls.configuration.TLSConfiguration.cipher_suites`.
    """
    ...
    are used to negotiate cryptographic settings and all additionnal
    ...
    def parse_body(cls, stream, **kwargs):
        legacy_version = try_cast(TLSVersion, stream.read_int(2))
        if legacy_version != TLSVersion.TLS_1_2:
            e = f"expected {TLSVersion.TLS_1_2!r} but {legacy_version!r} found"
