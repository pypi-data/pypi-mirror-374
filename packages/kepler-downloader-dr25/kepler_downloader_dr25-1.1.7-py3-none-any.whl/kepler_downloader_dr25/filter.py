...
class KeplerFilter:
    ...
    def check_mode_compatibility(self) -> Tuple[bool, str]:
        ...
        if self.config.force_mode:
            if self.source_job_info.mode != self.config.target_mode:
                self.logger.warning(f"Mode mismatch: Source is {self.source_job_info.mode.value}, "
                                  f"target is {self.config.target_mode.value} (forced)")
            return True, "Mode conversion forced by user"
        
        # Check compatibility
        if self.source_job_info.mode != self.config.target_mode:
            reason = (f"Mode mismatch: Source job is {self.source_job_info.mode.value}, "
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (
                self.source_job_info.mode.value,
    ...
    def download_from_mast(self, kic_id: int) -> dict:
        ...
        try:
            ...
            mask = ['_llc.fits' in str(row['productFilename']) or '_dvt.fits' in str(row['productFilename']) for row in fits_products]
            if not any(mask):
            ...
            if kic_list:
                ...
                if 'download_records' in table_names:
                    ...
                    records = source_conn.execute(
                        f"SELECT * FROM download_records WHERE kic IN ({placeholders})",
                        kic_list
                    ).fetchall()
        ...
        try:
            ...
            if self.missing_kics and self.config.download_missing:
                ...
                with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
                    future_to_kic = {executor.submit(self.download_from_mast, kic_id): kic_id for kic_id in self.missing_kics}
                    for future in tqdm(as_completed(future_to_kic), total=len(self.missing_kics), desc="Downloading KICs"):
                        future.result()
...
Examples:
  ...
  python kepler_filter.py --input-csv input/koi.csv --source-job output/job-20250906 --no-validate-dvt
    ...
    parser.add_argument('--input-csv', required=True,
                       help='Input CSV file containing KIC IDs')
    parser.add_argument('--source-job', required=True,
                       help='Source job folder to copy data from')
    parser.add_argument('--no-exominer', action='store_true',
                       help='Use Standard MAST format instead of ExoMiner (ExoMiner is default)')
    parser.add_argument('--output-dir',
                       help='Output directory for filtered data')
    parser.add_argument('--force-mode', action='store_true',
                       help='Force mode conversion even if incompatible')
    parser.add_argument('--no-validate-dvt', action='store_true',
                       help='Disable DVT validation for ExoMiner mode')
