...
def regenerate_health_report(job_dir):
    ...
    kics_without_dvt = conn.execute("SELECT COUNT(DISTINCT kic) FROM download_records WHERE has_dvt = 0 AND success = 1").fetchone()[0]
...
def main():
    if len(sys.argv) > 1:
        ...
        job_dirs = sorted([os.path.join(base_dir, d) for d in os.listdir(base_dir) 
                          if d.startswith('job-')])
