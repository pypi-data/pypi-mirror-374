...
from ppio_sandbox.core.sandbox.commands.command_handle import CommandExitException
...
def test_internet_access_enabled(template):
    ...
    try:
        ...
        result = sbx.commands.run(
            "curl -s -o /dev/null -w '%{http_code}' https://sandbox.ppio.cn/health"
        )
...
def test_internet_access_disabled(template):
    ...
    try:
        ...
        with pytest.raises(CommandExitException) as exc_info:
            sbx.commands.run(
                "curl --connect-timeout 3 --max-time 5 -Is https://sandbox.ppio.cn/health"
            )
