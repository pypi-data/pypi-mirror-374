...
from minjiang_client.com.plugin import (create_plugin, list_plugins, upload_plugin_version,
                                        get_plugin_detail, list_plugin_versions)
from minjiang_client.utils.local import get_cache_dir, get_plugin_dir, get_default_config_dir
from minjiang_client.com.oss import add_resource, get_resource_info
...
class PluginManager(object):
    ...
    async def submit_plugin_release(plugin_id: int, plugin_file: str):
        ...
        rid = add_resource("", "zip", json.dumps(uri), "plugin", "")["resource_id"]
    ...
    @staticmethod
    def create_plugin(plugin_name: str, description: str, must_be_installed: bool, auto_upgrade: bool):
        return create_plugin(plugin_name, description, must_be_installed, auto_upgrade)
