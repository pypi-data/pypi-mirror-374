...
class Database(object):
    ...
    def __init__(self, auto_create: bool = True, db_name: str = "fusion.db"):

        self.db_name = str(Path(db_name).resolve())
    ...
    def has_data(self) -> bool:
        return self.select_count('history') > 0
    ...
    def insert_history(self, source: str, data: str, stack_trace: str = ''):
        from frida_fusion.config import Configuration

        try:
            self.insert_one('history',
            ...
            Color.pl('{!} {R}Error inserting history data:{O} %s{W}' % str(e))

    @classmethod
    def scrub(cls, input_string):
