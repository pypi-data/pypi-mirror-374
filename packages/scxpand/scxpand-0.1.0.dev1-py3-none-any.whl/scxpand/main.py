...
python -m scxpand.main predict --model_type autoencoder --model_path results/autoencoder --data_path new_data.h5ad
python -m scxpand.main predict --model_type mlp --model_path results/mlp --data_path new_data.h5ad --evaluate_metrics false
"""
...
from scxpand.hyperopt.model_registry import MODEL_REGISTRY
...
def optimize_all(
    ...
    **kwargs,
    ...
    if model_types is None:
        ...
        models_to_optimize = list(MODEL_REGISTRY.keys())
    else:
...
def predict(
    ...
    Args:
        ...
        model_url: Direct URL to model ZIP file (for any external model, e.g., Zenodo)
        model_type: Type of model to use for prediction (auto-detected if not provided)
        ...
        evaluate_metrics: Whether to compute evaluation metrics (requires ground truth labels in data)
    ...
    Examples:
        ...
        predict --data_path my_data.h5ad --model_name autoencoder_pan_cancer

        # Direct URL inference (any external model)
        predict --data_path my_data.h5ad --model_url "https://zenodo.org/records/7625517/files/model.zip?download=1"
    """
...
def list_pretrained_models() -> None:
    ...
    for name, info in PRETRAINED_MODELS.items():
        ...
        logger.info(f"  Type: {info.model_type.value}")
        logger.info(f"  Version: {info.version}")
        logger.info(f"  URL configured: {'Yes' if info.url else 'No'}")
        logger.info(f"  Checksum available: {'Yes' if info.sha256 else 'No'}")
        logger.info("")
