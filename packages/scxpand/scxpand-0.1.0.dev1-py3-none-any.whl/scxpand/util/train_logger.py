...
class TrainLogger:
    def __init__(
        ...
        with (self.save_path / "run_info.json").open("w") as f:
    ...
    def save_checkpoint(
        ...
        ckpt_save_path = self.save_path / "last_ckpt.pt"
        ...
        if self.best_model_score is None or model_score > self.best_model_score:
            ...
            best_ckpt_path = self.save_path / "best_ckpt.pt"
            shutil.copy(ckpt_save_path, best_ckpt_path)
            logger.info(f"Saved checkpoint to {best_ckpt_path}")

            if dev_set_metrics:
                json_path = self.save_path / "best_model_dev_set_metrics.json"
            ...
            with (self.save_path / "best_model_info.json").open("w") as f:
    ...
    def load_best_model(
        ...
        if best_ckpt_path.exists():
            ...
            logger.info(f"Loaded best model from {self.save_path / 'best_ckpt.pt'}")
        else:
    ...
    def resume_from_checkpoint(
        ...
        checkpoint_path = ckpt_dir / "last_ckpt.pt"
    ...
    def save_final_summary(self):
        ...
        with (self.save_path / "summary_info.json").open("w") as f:
            json.dump(self.run_info, f, indent=4)
...
def has_checkpoint(path: Path | str):
    path = Path(path)
    return path.exists() and (path / "last_ckpt.pt").exists()
