...
DEFAULT_EPS = 1e-10  # Small constant to prevent division by zero
...
def extract_is_expanded(obs: pd.DataFrame | pd.Series | dict[str, pd.Series]) -> np.ndarray[int]:
    ...
    if isinstance(obs, pd.Series):
        ...
        if "expansion" in obs.index:
            expansion_labels = obs["expansion"]
        else:
            expansion_labels = obs  # Assume obs is the expansion series itself
    elif isinstance(obs, dict):
        expansion_labels = obs["expansion"].to_numpy()
    else:
        # DataFrame case
        expansion_labels = obs["expansion"].to_numpy()
