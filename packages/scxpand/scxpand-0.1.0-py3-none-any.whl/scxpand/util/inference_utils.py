...
def load_model(model_type: ModelType | str, results_path: Path, device: torch.device | str) -> BaseEstimator | Module:
    """Load a trained model from disk.

    Args:
        model_type: Type of model to load
        results_path: Path to directory containing the trained model
    ...
    logger.info(f"Loading {model_type_enum.value} model from {results_path}")

    if model_type_enum == ModelType.AUTOENCODER:
        model = load_ae_model(results_path=results_path, device=device)
    elif model_type_enum == ModelType.MLP:
        model = load_nn_model(results_path=results_path, device=device)
    elif model_type_enum in [ModelType.LIGHTGBM, ModelType.LOGISTIC, ModelType.SVM]:
        model = load_sklearn_model(results_path=results_path)
    else:
...
def setup_inference_environment(
    ...
    # Load model
    model = load_model(model_type=model_type_enum, results_path=model_path, device=device)
