...
def configure_logistic_trial_params(trial: optuna.Trial) -> dict:
    ...
    # Note: shuffle is less relevant with partial_fit(), but keeping for consistency
...
def configure_lightgbm_trial_params(trial: optuna.Trial) -> dict:
    ...
    objective = trial.suggest_categorical("lgbm_objective", ("binary",))
    metric = trial.suggest_categorical("lgbm_metric", ("binary_logloss", "auc", "binary_error"))
    verbose = trial.suggest_categorical("lgbm_verbose", (-1, 0))  # -1: silent, 0: warning
