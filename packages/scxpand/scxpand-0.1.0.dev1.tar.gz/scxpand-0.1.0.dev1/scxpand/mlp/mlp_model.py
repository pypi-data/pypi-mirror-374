...
def load_nn_model(results_path: Path, device: torch.device | str) -> MLPModel:
    logger.info(f"Loading nn model components from: {results_path}")
    with (results_path / "parameters.json").open("r") as f:
        prm_dict = json.load(f)
        prm = MLPParam(**prm_dict)
    data_format = load_data_format(results_path / "data_format.json")
    model = MLPModel(prm=prm, device=device, data_format=data_format)
    log_manager = TrainLogger(save_path=results_path)
    model = log_manager.load_best_model(model=model, device_name=str(device))
    model.eval()
