...
def load_ae_model(
    ...
    """Load a trained autoencoder model from saved checkpoint.

    Reconstructs the autoencoder architecture and loads trained weights from
    the results directory. Also loads the associated data format for preprocessing.

    Args:
        results_path: Path to directory containing model files ('model.pt',
                    'parameters.json', 'data_format.json').
    ...
    """
    logger.info(f"Loading Autoencoder model from {results_path}")
    ...
    try:
        ...
        # Load parameters
        params_path = results_path / "parameters.json"
        with params_path.open("r") as f:
            prm_dict = json.load(f)
            prm = AutoEncoderParams(**prm_dict)

        # Create model architecture
        model = create_ae_model(data_format=data_format, prm=prm, device=device)

        # If model_path is a directory, look for model.pt file
        ...
        logger.error(f"Invalid JSON in parameter file: {e}")
        raise
