"""Moondream provider implementation."""
...
class MoondreamProvider(Provider[AsyncOpenAI]):
    """Provider for Moondream API."""
    ...
    def base_url(self) -> str:
        ...
        return os.environ.get("MOONDREAM_BASE_URL", "https://api.moondream.ai/v1")

    @property
    def client(self) -> AsyncOpenAI:
        """Get a client configured for Moondream."""
        return self._client
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize provider for Moondream.

        Args:
            api_key: The API key to use for authentication.
                     If not provided, the `MOONDREAM_API_KEY`
        ...
        """  # noqa: E501
        api_key = api_key or os.environ.get("MOONDREAM_API_KEY")

        if api_key is None and openai_client is None:
            msg = (
                "Set the `MOONDREAM_API_KEY` environment variable or pass it via "
                "`MoondreamProvider(api_key=...)` to use the Moondream provider."
            )
        ...
        if openai_client is not None:
            ...
            if http_client is None:
                # Create HTTP client with Moondream headers
                http_client = cached_async_http_client()

                # Add Moondream-specific headers
