...
def unit_step(n: int, show: bool = True):
    ...
    if show:
        _stem(np.arange(n), x, title=f"Unit Step u[n] (length={n})", xlabel="n", ylabel="amplitude")
    return x

def unit_impulse(n: int, k: int = 0, show: bool = True):
    """
    Generates a unit impulse sequence δ[n-k] of length n with impulse at index k (default 0).
    """
    if n <= 0:
        raise ValueError("n must be positive")
    if not (0 <= k < n):
    ...
    x[k] = 1.0
    if show:
        _stem(np.arange(n), x, title=f"Unit Impulse δ[n-{k}] (length={n})", xlabel="n", ylabel="amplitude")
    return x
...
def ramp_signal(n: int, show: bool = True):
    ...
    if show:
        _stem(np.arange(n), x, title=f"Ramp r[n] (length={n})", xlabel="n", ylabel="amplitude")
    return x
