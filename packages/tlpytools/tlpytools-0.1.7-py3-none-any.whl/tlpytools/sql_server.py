...
class ms_sql_tables:
    ...
    def download_encrypted(cls, con, flag_csv="ms_sql.sys.tables_flag.csv"):
        ...
        try:
            ...
            print("You need to build a flagged table of ms_sql.sys.tables")
        ...
        if plain_salt == None:
            ...
            print(
                "Save salt to your environment variable 'ms_sql_salt': \n{}".format(
                    salt
                )
            )
        ...
        if folder == None:
            ...
            print("No ms_sql_path has been specified, using current directory instead.")
            print("Specify ms_sql_path environment variable after download")
...
class azure_td_tables(ms_sql_tables):
    # get azure server url
    azure_sql_svr_uri = os.environ.get(
        "TLPT_AZURE_SQL_URI", "glcprdsqlinolab06.database.windows.net"
    )
    # create engine
    engine = sql.create_engine(
        "mssql+pyodbc:///?odbc_connect="
