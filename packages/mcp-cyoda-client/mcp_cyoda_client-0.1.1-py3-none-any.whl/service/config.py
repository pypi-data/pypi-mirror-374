...
from common.config.config import CYODA_TOKEN_URL, CYODA_CLIENT_ID, CYODA_CLIENT_SECRET
...
def get_service_config() -> Dict[str, Any]:
    ...
    logger.info(f"  - Repository type: {'In-Memory' if config['repository']['use_in_memory'] else 'Cyoda'}")
...
def validate_configuration() -> Dict[str, Any]:
    ...
    if not config['authentication']['client_id']:
        validation['warnings'].append("CYODA_CLIENT_ID not set - authentication will not work")
    
    if not config['authentication']['client_secret']:
        validation['warnings'].append("CYODA_CLIENT_SECRET not set - authentication will not work")
    
    if not config['authentication']['token_url']:
        validation['warnings'].append("CYODA_TOKEN_URL not set - authentication will not work")
    
    # Check repository configuration
    if is_in_memory_repository():
        validation['warnings'].append("Using in-memory repository - data will not persist")
