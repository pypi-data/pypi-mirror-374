...
Usage:
    ide-rules copy --ide windsurf --target /path/to/project
    ide-rules copy --ide cursor --target /path/to/project
    ide-rules copy --ide windsurf  # copies to current directory
"""
...
def get_rules_files(source_dir: Path | None = None) -> list[Path]:
    """Get all rules files from the rules/ subdirectory."""
    if source_dir is None:
        # Use rules directory relative to this package
        package_dir = Path(__file__).parent
        rules_dir = package_dir / "rules"
        
        # If not found in package directory, try parent directory (development mode)
        if not rules_dir.exists():
            rules_dir = package_dir.parent / "ide_rules" / "rules"
    else:
...
def copy(
    ...
    source: Annotated[
        ...
        typer.Option(
            "--source",
            "-s",
            help="Source directory containing rules/ folder (defaults to package rules)",
            exists=True,
            file_okay=False,
            dir_okay=True,
        )
    ...
    Examples:
        ide-rules copy --ide windsurf --target /path/to/project
        ide-rules copy --ide cursor
        ide-rules copy --dry-run --ide windsurf
    """
    # Validate IDE choice
    ...
    # If no source specified, use package rules; otherwise use specified source
    if source is None:
        source_display = "package rules"
        rules_files = get_rules_files()  # Use package rules
    else:
        source_display = str(source)
        rules_files = get_rules_files(source)  # Use specified source
    ...
    # Check if rules files were found
    if not rules_files:
        search_location = f"{source}/rules/" if source else "package rules directory"
        console.print(f"[yellow]No rules files found in {search_location}[/yellow]")
        return
...
def list_rules(
    ...
    Examples:
        ide-rules list --ide windsurf
        ide-rules list --ide cursor --target /path/to/project
    """
    if ide.lower() not in ["windsurf", "cursor"]:
        console.print(f"[red]Error: Invalid IDE '{ide}'. Must be 'windsurf' or 'cursor'[/red]")
        raise typer.Exit(1)
...
def clean(
    ...
    Examples:
        ...
        ide-rules clean --ide cursor --dry-run
    """
    if ide.lower() not in ["windsurf", "cursor"]:
        console.print(f"[red]Error: Invalid IDE '{ide}'. Must be 'windsurf' or 'cursor'[/red]")
        raise typer.Exit(1)
