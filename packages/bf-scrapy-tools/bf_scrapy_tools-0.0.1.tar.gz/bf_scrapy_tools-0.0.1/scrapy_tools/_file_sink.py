...
def generate_rename_path(root, ext, creation_time):
    ...
    date = FileDateFormatter(creation_datetime).__format__('%Y-%m-%d_%H')

    kibana_special_path = os.path.splitext(root)
    if kibana_special_path[1]:
        ext = kibana_special_path[1] + ext
        root = kibana_special_path[0]
