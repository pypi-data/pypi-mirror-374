...
class UnifiedRegistryManager:
    ...
    def _load_workspace_registry(self, workspace_id: str, workspace_path: Path):
        ...
        try:
            ...
            registry_file = workspace_path / "src" / "cursus_dev" / "registry" / "workspace_registry.py"
            ...
            if 'developer_id' in workspace_metadata:
                ...
                if actual_workspace_id != workspace_id:
                    ...
                    self._workspace_steps[actual_workspace_id] = self._workspace_steps.pop(workspace_id)
                    self._workspace_overrides[actual_workspace_id] = self._workspace_overrides.pop(workspace_id)
                    self._workspace_metadata[actual_workspace_id] = self._workspace_metadata.pop(workspace_id)
                    workspace_id = actual_workspace_id
            
            logger.debug(f"Loaded workspace '{workspace_id}': {len(self._workspace_steps[workspace_id])} local steps, {len(self._workspace_overrides[workspace_id])} overrides")
        ...
        with self._lock:
            if include_source:
                ...
                for workspace_id in self._workspace_steps:
                    ...
                    workspace_steps.update(self._workspace_overrides[workspace_id].keys())
    ...
    def has_step(self, step_name: str, workspace_id: str = None) -> bool:
        ...
        with self._lock:
            if workspace_id and workspace_id in self._workspace_steps:
                return (step_name in self._workspace_steps[workspace_id] or 
                        step_name in self._workspace_overrides[workspace_id] or 
                        step_name in self._core_steps)
            else:
                # Check all workspaces and core
                if step_name in self._core_steps:
                    return True
                for ws_id in self._workspace_steps:
                    if (step_name in self._workspace_steps[ws_id] or 
                        step_name in self._workspace_overrides[ws_id]):
                        return True
                return False
