...
class VideoElement(VideoBase):
    ...
    def _get_frame_at_time(self, video_time: float) -> Optional[np.ndarray]:
        ...
        # Resize if needed with high-resolution rendering
        effective_scale = self.scale * self.render_scale
        if effective_scale != 1.0:
            new_width = int(self.original_width * effective_scale)
            new_height = int(self.original_height * effective_scale)
        ...
        if self.render_scale > 1.0:
            final_width = int(pil_frame.size[0] / self.render_scale)
            final_height = int(pil_frame.size[1] / self.render_scale)
            pil_frame = pil_frame.resize((final_width, final_height), Image.Resampling.LANCZOS)
