...
class MasterScene:
    ...
    def __init__(self, width: int = 1920, height: int = 1080, fps: int = 60, quality: str = 'medium'):
        ...
        # Quality settings for render_scale
        self.quality_settings = {
            'low': 1.0,      # Normal resolution
            'medium': 2.0,   # 2x resolution
            'high': 3.0,     # 3x resolution  
            'ultra': 4.0     # 4x resolution
        }
        self.quality = quality
        self.render_scale = self.quality_settings.get(quality, 2.0)
    ...
    def add(self, scene: Scene):
        ...
        # Apply quality settings to all elements in the scene
        self._apply_quality_to_scene(scene)
        
        return self
    
    def set_quality(self, quality: str):
        """Set rendering quality level for all elements.
        ...
        if quality in self.quality_settings:
            self.quality = quality
            self.render_scale = self.quality_settings[quality]
            
            # Apply new quality settings to all existing scenes
            for scene in self.scenes:
                self._apply_quality_to_scene(scene)
                
    def _apply_quality_to_scene(self, scene):
        """Apply quality settings to all elements in a scene."""
        from .text_element import TextElement
        from .image_element import ImageElement  
        from .video_element import VideoElement
        
        for element in scene.elements:
            if isinstance(element, (TextElement, ImageElement, VideoElement)):
                element.set_render_quality(self.render_scale)
