...
class VideoBase:
    ...
    def __init__(self) -> None:
        ...
        self.render_scale: float = 2.0  # Internal rendering scale multiplier for quality
    ...
    def get_actual_render_position(self) -> Tuple[float, float, float, float]:
        ...
        Returns:
            Tuple of (actual_x, actual_y, element_width, element_height)
        """
    ...
    def set_render_quality(self: VideoBaseT, render_scale: float) -> VideoBaseT:
        """Set internal rendering scale for quality improvement.
        
        Args:
            render_scale: Scale multiplier for internal texture resolution (1.0 = normal, 2.0 = 2x quality, etc.)
                         Higher values improve quality but use more memory
        
        Returns:
            Self for method chaining
        """
        self.render_scale = max(1.0, render_scale)  # Minimum 1.0x scale
        # Mark texture for recreation
    ...
    def _apply_corner_radius_to_image(self, img: Image.Image) -> Image.Image:
        """Apply corner radius clipping to image content with high-quality antialiasing.
        ...
        # 高解像度マスクを作成（アンチエイリアシング用）
        mask_scale = int(self.render_scale * 2)  # Extra scaling for smoother curves
        ...
        mask_draw.rounded_rectangle([0, 0, mask_width, mask_height], radius=mask_radius, fill=255)
        
        # マスクを元のサイズにダウンサンプリング（アンチエイリアシング効果）
