...
# Create a comprehensive figure with multiple subplots - INCREASED HEIGHT
fig = plt.figure(figsize=(20, 18))  # Increased from 16 to 18 to provide more space
...
plt.xticks(range(len(metrics_names)), metrics_names, rotation=0, ha='center', fontsize=10)
...
# INCREASED BOTTOM MARGIN AND ADJUSTED TEXT BOX POSITIONS
# Adjust layout with much more space at bottom
plt.tight_layout(rect=[0, 0.15, 1, 0.88])  # Increased bottom margin from 0.08 to 0.15

# Add algorithm ranking text box (LEFT SIDE) - MOVED DOWN
...
plt.figtext(0.02, 0.08, ranking_text, fontsize=11,  # Moved down from 0.02 to 0.08
           bbox=dict(boxstyle="round,pad=0.5", facecolor="lightblue", alpha=0.8))

# Add performance summary text box (RIGHT SIDE) - MOVED DOWN
...
plt.figtext(0.98, 0.08, summary_text, fontsize=11, ha='right',  # Moved down from 0.02 to 0.08
...
print(f"\nFixed overlap comprehensive visualization saved to: {output_path}")
print("Layout fixes applied:")
print("- Increased figure height from 16 to 18")
print("- Increased bottom margin in tight_layout from 0.08 to 0.15")
print("- Moved text boxes down from y=0.02 to y=0.08") 
print("- Adjusted title positions to accommodate taller figure")
print("- Maintained flat/horizontal heatmap labels")
print("- Text boxes no longer overlap with heatmap")
