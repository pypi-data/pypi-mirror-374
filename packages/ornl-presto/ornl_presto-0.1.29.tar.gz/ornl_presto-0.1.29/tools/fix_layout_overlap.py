...
fig = plt.figure(figsize=(20, 18))  # Made figure taller to prevent overlap
...
# 9. Similarity metrics heatmap - ADJUSTED POSITION TO PREVENT OVERLAP
ax9 = plt.subplot(4, 4, (13, 15))  # Changed from (13, 16) to (13, 15) to make it narrower
similarity_data = []
metrics_names = ['KS Stat', 'JS Div', 'Pearson']  # Shortened names to fit better
...
# Create heatmap with lighter colors to ensure black text is visible
similarity_array = np.array(similarity_data)
im = plt.imshow(similarity_array, cmap='Blues', aspect='auto', vmin=0, vmax=1)
cbar = plt.colorbar(im, ax=ax9, shrink=0.6)  # Made colorbar smaller
cbar.set_label('Metric Value', fontsize=9)

# Add labels with better spacing
plt.xticks(range(len(metrics_names)), metrics_names, rotation=0, fontsize=9)  # No rotation, smaller font
plt.yticks(range(len(algorithms)), [algo.capitalize() for algo in algorithms], fontsize=10)
plt.title('Similarity Metrics Heatmap', fontsize=12, fontweight='bold', pad=15)  # Smaller title
...
fig.suptitle('PRESTO: Privacy-Preserving Energy Data Analysis', 
             fontsize=22, fontweight='bold', y=0.96)  # Moved up slightly
plt.figtext(0.5, 0.92, 'Top-3 Unique Privacy Algorithms Evaluation and Comparison', 
            ha='center', fontsize=15, style='italic')

# Add algorithm ranking text box with updated results - MOVED POSITION
...
plt.figtext(0.02, 0.05, ranking_text, fontsize=10,  # Moved up and made smaller
           bbox=dict(boxstyle="round,pad=0.5", facecolor="lightblue", alpha=0.8))

# Add performance summary - MOVED POSITION AND MADE SMALLER
...
plt.figtext(0.75, 0.05, summary_text, fontsize=10, ha='left',  # Changed position and alignment
           bbox=dict(boxstyle="round,pad=0.5", facecolor="lightgreen", alpha=0.8))

# Adjust layout to prevent overlap with more space at bottom
plt.tight_layout(rect=[0, 0.12, 1, 0.90])  # More space at bottom and top
...
print(f"\nFixed layout comprehensive visualization saved to: {output_path}")
print("Layout fixes applied:")
print("- Made figure taller (20x18) to prevent overlap")
print("- Narrowed heatmap from (13,16) to (13,15)")
print("- Shortened metric names: 'KS Stat', 'JS Div', 'Pearson'")
print("- Moved text boxes to better positions")
print("- Increased bottom margin in tight_layout")
print("- All text remains black and clearly visible")
