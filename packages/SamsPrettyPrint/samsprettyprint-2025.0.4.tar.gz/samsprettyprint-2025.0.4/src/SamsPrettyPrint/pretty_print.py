...
def align(
    ...
    padding_length = width - len(remove_ascii_colour_tags(text))
    repeat_fill = (fill*(padding_length//len(fill) + 1))[:padding_length]
    match alignment:
        case '<':
            return text + repeat_fill
        case '^':
            return repeat_fill[:math.floor(padding_length/2)] + text + repeat_fill[-math.ceil(padding_length/2):]
        case '>':
            return repeat_fill + text
        case _:
...
def print_table(
    ...
    _print(
        table(
            rows, title, prefix, suffix, column_headers, column_max_widths, column_min_widths,
            column_alignment, column_separator, header_underline, title_underline, newline,
            number_format, format_numeric_str, strip_whitespace
        ),
        copy_to_clipboard
    )
...
def table(
    ...
    :param title: Displayed centered at the top of the table.
    :param prefix: Inserted at the beginning of every line.
    :param prefix: Inserted at the end of every line.
    :param suffix: Displayed centered at the top of the table.
    ...
    :param title_underline: This string is repeated as fill characters either side of the title.
    ...
    # Tile
    if title:
        full_width = np.sum(column_widths) + len(column_separator)*(len(column_widths) - 1)
        title = align(f' {title} ', full_width, title_underline, '^', False)
        title = ' '*len(prefix) + title + ' '*len(suffix) + '\n'
    else:
        title = ''

    # Join the text and return the tabel
    return title + newline.join(prefix + column_separator.join(row) + suffix for row in rows)
