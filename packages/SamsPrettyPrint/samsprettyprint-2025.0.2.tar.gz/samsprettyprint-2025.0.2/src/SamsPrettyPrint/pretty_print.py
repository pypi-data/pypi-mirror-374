...
def print_table(
    ...
    _print(table(rows, column_headers, column_max_widths, column_min_widths, column_alignment, column_separator,
                 header_underline, newline, number_format, format_numeric_str), copy_to_clipboard)


# //==================================================\\
# ||                   print_title                    ||
# \\==================================================//
def print_title(text: str, width=54, prefix='', bold=False, level: int = 3, copy_to_clipboard: bool = False):
    """
    ...
    match level:
        case 1:
            _print(title1(text, width, prefix), copy_to_clipboard)
        case 3:
            _print(title3(text, width, prefix), copy_to_clipboard)
        case 5:
            _print(title5(text, width, prefix), copy_to_clipboard)
        case _:
...
def table(
    ...
    """
    Returns a nearly formated table.

    | Parameter          | Description                                                                         |
    |------------------- |-------------------------------------------------------------------------------------|
    | rows               | The object row[i][j] will be cast to a string and arranged in row i column j.       |
    | column_headers     | An optional extra                                                                   |
    | column_max_widths  | Defines the maximum number of characters each column may take.                      |
    | column_min_widths  | Defines the maximum number of characters each column may take.                      |
    | column_alignment   | One of '<' left, '^' center or '>' right alignment                                  |
    | column_separator   | This string is added as a separator between each column                             |
    | header_underline   | This string is repeated underneath each header. Set to None to remove this feature. |
    | newline            | This string (e.g. '\n') will be appended to the end of each row.                    |
    | number_format      | Any according to #formatspec                                                        |
    | format_numeric_str | If true, numeric strings (e.g.'1.2') will be formated according to number_format    |
    """
    ...
    for i, row in enumerate(rows):
        for j, item in enumerate(row):
            rows[i][j] = align(item, column_widths[j], fill=' ', alignment=column_alignment[j])
