...
from test_project_3 import TestProject3, AsyncTestProject3, DefaultAioHttpClient
from test_project_3._utils import is_dict
...
logging.getLogger("test_project_3").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[TestProject3]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with TestProject3(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncTestProject3]:
