...
from BoschRpaMagicBox.sharepoint_functions import *
...
class MiniRpaFunction(MiniRPACore):
    ...
    def upload_file_to_sharepoint(self, from_folder_path: str, from_file_name: str, sharepoint_site_url: str, sharepoint_folder_url: str, sharepoint_file_name):
        """
        Upload a file to SharePoint.

        Args:
            from_folder_path(str): This is the folder path where the file is located
            from_file_name(str): This is the name of the file to be uploaded
            sharepoint_site_url(str): This is the SharePoint site URL
            sharepoint_folder_url(str): This is the SharePoint folder URL where the file will be uploaded
            sharepoint_file_name(str): This is the name of the file in SharePoint after upload
        """
        share_point_client = SharePointClient(sharepoint_site_url, self.user_name, self.user_password)
        from_file_path = from_folder_path + os.sep + from_file_name
        file_obj = smb_load_file_obj(self.user_name, self.user_password, self.server_name, self.share_name, from_file_path, self.port)
        share_point_client.upload_file_to_sharepoint(sharepoint_folder_url, sharepoint_file_name, file_obj=file_obj)

    def batch_copy_files_within_public_folders(self, from_server_name, from_share_name, from_folder_path, file_key_word, update_server_name, update_share_name, update_folder_path,
                                               copy_mode='contain_key_word'):
        """ This function is used to copy file between public folder.
        ...
        traverse_result_list = smb_traverse_remote_folder(self.user_name, self.user_password, from_server_name, from_share_name, from_folder_path, self.port)
        file_name_dict = {file_item['name']: str(file_item['name']).upper() for file_item in traverse_result_list if file_item['is_file']}
