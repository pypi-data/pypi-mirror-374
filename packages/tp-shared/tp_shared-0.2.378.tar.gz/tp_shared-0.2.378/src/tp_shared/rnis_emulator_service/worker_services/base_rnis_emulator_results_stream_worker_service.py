...
from tp_shared.rnis_emulator_service.repos.base_rnis_emulator_results_stream_queue_repo import (
    BaseRnisEmulatorResultsStreamQueueRepo,
)
...
class BaseRnisEmulatorResultsStreamQueueWorkerService(
    BaseRnisEmulatorResultsStreamQueueRepo, BaseWorkerService
):
    def __init__(
        self,
        redis_client: Redis,
        ...
        BaseRnisEmulatorResultsStreamQueueRepo.__init__(self, redis_client=redis_client)
    ...
    async def add(self, message: RnisEmulatorResultMessage) -> None:
        await BaseRnisEmulatorResultsStreamQueueRepo.add(self, message)
    ...
    async def pop(
        ...
        return await BaseRnisEmulatorResultsStreamQueueRepo.pop(
            self,
    ...
    async def ack(self, message_id: str):
        await BaseRnisEmulatorResultsStreamQueueRepo.ack(
            self, self.group_name, message_id
        )
    ...
    async def claim_reassign(
        ...
        return await BaseRnisEmulatorResultsStreamQueueRepo.claim_reassign(
            self,
            group_name=self.group_name,
            consumer_name=self.consumer_name,
            min_idle_time=min_idle_time,
    ...
    async def create_consumer_group(self, create_stream: bool = True):
        await BaseRnisEmulatorResultsStreamQueueRepo.create_consumer_group(
            self,
            group_name=self.group_name,
    ...
    async def trim_by_age(self, retention: timedelta) -> int:
        return await BaseRnisEmulatorResultsStreamQueueRepo.trim_by_age(self, retention)
    ...
    async def delete_all(self) -> None:
        await BaseRnisEmulatorResultsStreamQueueRepo.delete_all(self)
