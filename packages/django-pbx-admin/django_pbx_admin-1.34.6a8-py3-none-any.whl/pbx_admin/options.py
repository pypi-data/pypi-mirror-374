...
class ModelAdmin:
    ...
    def get_breadcrumbs(self, request, obj=None, **kwargs):
        ...
        qs = getattr(request, "_current_queryset", None) or self.queryset._clone()

        if self.parent_field:
            p_obj = self.parent.get_object(request, **list_kwargs)
            parent_filter = self.parent_field.get_forward_related_filter(p_obj)

            if hasattr(request, "_current_queryset") and not parent_filter:
                parent_qs = request._current_queryset
            else:
                parent_qs = self.queryset._clone()
                if parent_filter:
                    parent_qs = parent_qs.filter(**parent_filter)
        ...
        if (
            hasattr(self, "_object_count")
            and hasattr(request, "_current_queryset")
            and str(request._current_queryset.query) == str(parent_qs.query)
        ):
            objects_count = self._object_count
        else:
            if (
                hasattr(request, "_current_queryset")
                and request._current_queryset.model == parent_qs.model
                and hasattr(self, "_object_count")
                and not self.parent_field
            ):
                objects_count = self._object_count
            else:
                objects_count = self.get_cached_count(parent_qs)
    ...
    def get_cached_count(self, queryset, timeout=30):
        """Shared cache for object counting using CACHEOPS"""
        try:
            ...
            def _count_objects():
                if self.use_estimated_count:
                    count = count_estimate(
                        queryset, precision=max(admin_settings.PAGINATION_CHOICES)
                    )
                else:
    ...
    def invalidate_count_cache(self):
        """Invalidate cache for object counting - CACHEOPS handles this automatically"""

    def _get_objects_count(self, qs):
        if hasattr(self, "_object_count"):
            delattr(self, "_object_count")

        if hasattr(qs, "_result_cache"):
            qs._result_cache = None

        count = self.get_cached_count(qs)
