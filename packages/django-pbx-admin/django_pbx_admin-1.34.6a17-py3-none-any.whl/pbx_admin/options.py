...
class ModelAdmin:
    ...
    def get_breadcrumbs(self, request, obj=None, **kwargs):
        ...
        objects_count = self._get_objects_count(parent_qs)
    ...
    def _get_objects_count(self, qs):
        """Optimized objects count calculation with caching"""
        return self.get_cached_count(qs)
        # qs_key = str(qs.query)
        # cache_attr = f"_object_count_{hash(qs_key)}"

        # if hasattr(self, cache_attr):
        #     return getattr(self, cache_attr)

        # count = self.get_cached_count(qs)
        # setattr(self, cache_attr, count)
        # return count
