...
def run_tr_assembly( method, bam_file, genome_file = None, gtf_file = None, \
    ...
    start = time.time()
    if method == 'stringtie':
        file_out = run_stringtie( bam_file, gtf_file, out_dir, p)
        file_out_ti, df = get_tr_assembly_info( file_out )
        
    elif method == 'cufflinks':
        file_out = run_cufflinks( bam_file, gtf_file, out_dir, p)
        file_out_ti, df = get_tr_assembly_info( file_out )
        
    elif method == 'strawberry':
        file_out = run_strawberry( bam_file, gtf_file, out_dir, p)
        file_out_ti, df = get_tr_assembly_info( file_out )
        
    elif method.lower() == 'trinity':
        file_out = run_trinity( bam_file, out_dir, p)
        
    elif method == 'scallop':
        file_out = run_scallop( bam_file, out_dir)
        file_out_ti, df = get_tr_assembly_info( file_out )
    ...
    if (genome_file is not None) & (method.lower() != 'trinity'):
        ...
        file_out_fa = GFFread( file_out, genome_file )
