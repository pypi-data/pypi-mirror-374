...
class TestLoadAndRegisterPrompts:
    ...
    @patch('devops_mcps.prompt_management.Path')
    @patch('devops_mcps.prompt_management.open', new_callable=mock_open, 
    ...
    @patch('devops_mcps.prompt_management.logger')
    async def test_dynamic_prompt_function_required_variable_missing(self, mock_logger, mock_file, mock_path):
    ...
    @patch('devops_mcps.prompt_management.open', new_callable=mock_open, 
           read_data='{"test_prompt": {"description": "Test", "template": "Hello {name}", "variables": {"name": {"required": false, "default": "World"}}}}')
    @patch('devops_mcps.prompt_management.logger')
    async def test_dynamic_prompt_function_with_default_value(self, mock_logger, mock_file, mock_path):
    ...
    @patch('devops_mcps.prompt_management.logger')
    async def test_dynamic_prompt_function_with_provided_variable(self, mock_logger, mock_file, mock_path):
...
class TestValidatePromptConfig:
    ...
    def test_missing_description(self, mock_logger):
        ...
        mock_logger.error.assert_called_with("Missing required field 'description' in prompt configuration")
    ...
    def test_missing_template(self, mock_logger):
        ...
        assert result is False
        mock_logger.error.assert_called_with("Missing required field 'template' in prompt configuration")
    ...
    def test_invalid_variable_config_type(self, mock_logger):
        ...
        assert result is False
        mock_logger.error.assert_called_with("Variable 'name' configuration must be a dictionary")
...
class TestGetAvailablePrompts:
    ...
    def test_get_available_prompts_with_custom_path(self, mock_logger):
        ...
        with patch('devops_mcps.prompt_management.open', 
                   new_callable=mock_open, 
                   read_data='{"custom_prompt": {"description": "Custom"}}') as mock_file:
            # Execute
    ...
    @patch('devops_mcps.prompt_management.Path')
    @patch('devops_mcps.prompt_management.open', new_callable=mock_open, read_data='invalid json')
    @patch('devops_mcps.prompt_management.logger')
...
class TestEdgeCasesAndIntegration:
    ...
    @patch('devops_mcps.prompt_management.logger')
    def test_complex_prompt_with_multiple_variables(self, mock_logger, mock_file, mock_path):
    ...
    def test_unicode_handling(self, mock_logger, mock_path):
        ...
        with patch('devops_mcps.prompt_management.open', 
                   new_callable=mock_open, 
                   read_data=unicode_content) as mock_file:
            mock_mcp = MagicMock()
            mock_path_instance = MagicMock()
            mock_path_instance.exists.return_value = True
            mock_path.return_value.parent.__truediv__.return_value = mock_path_instance
