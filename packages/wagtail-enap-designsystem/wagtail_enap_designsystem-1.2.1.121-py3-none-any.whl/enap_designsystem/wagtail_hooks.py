...
class VotoRegistradoViewSet(SnippetViewSet):
    model = VotoRegistrado
    menu_label = 'Votos Registrados'
    icon = 'tick'
    menu_order = 600
...
# Registrar o ViewSet
register_snippet(VotoRegistrado, viewset=VotoRegistradoViewSet)


# Hook personalizado para adicionar estatísticas no admin
@hooks.register('construct_main_menu')
def add_votacao_stats_menu_item(request, menu_items):
    """
    Adiciona estatísticas de votação no menu principal
    """
    ...
    try:
        ...
        pass  # Se der erro na query, ignora


# Hook para personalizar o dashboard
@hooks.register('construct_homepage_panels')
def add_votacao_dashboard_panel(request, panels):
    """
    Adiciona painel de estatísticas no dashboard
    """
    from django.template.loader import render_to_string
    from wagtail.admin.ui.components import Component
    
    try:
        from .models import CategoriaVotacao, ProjetoVotacao
        ...
        panels.append(Component(html))
        
    except Exception as e:
        # Se der erro, não quebra o admin
        pass