...
async def get_client(ip_address, protocol, show_responses) -> Optional[HarmonyAPI]:
    ...
    if show_responses:
        listen_callback = Handler(
            handler_obj=output_response, handler_name="output_response", once=False
        )
    ...
    try:
        if await client.connect():
            print(
                f"Connected to HUB {client.name} ({ip_address}) with firmware version {client.fw_version} and HUB ID {client.hub_id} using protocol {client.protocol}"
            )
            return client
...
async def just_listen(client, args):
    ...
    def output_response(message):
        print(
            f"{datetime.today().strftime('%Y-%m-%d %H:%M:%S')} {client.name}: {message}"
        )

    print(
        f"Starting to listen on HUB {client.name} with firmware version {client.fw_version}"
    )
...
async def start_activity(client, args):
    ...
    if (args.activity.isdigit()) or (args.activity == "-1"):
        ...
        if activity_id:
            print(
                f"HUB: {client.name} Found activity named {args.activity} ({activity_id})"
            )
    if activity_id:
...
async def send_command(client, args):
    ...
    snd_cmmnd = SendCommandDevice(
        device=device_id, command=args.command, delay=args.hold_secs
    )
    ...
    if result_list:
        for result in result_list:
            print(
                f"HUB: {client.name} Sending of command {result.command.command} to device {result.command.device} failed with code {result.code}: "
                f"{result.msg}"
            )
    else:
...
async def send_commands(client, args):
    ...
    for single_command in words:
        ...
        snd_cmmnd = SendCommandDevice(
            device=device_id, command=single_command, delay=args.hold_secs
        )
        snd_cmmnd_list.append(snd_cmmnd)
        if args.delay_secs > 0:
            snd_cmmnd_list.append(args.delay_secs)
...
async def execute_per_hub(hub, args):
    ...
    try:
        ...
        hub_client = await get_client(hub, args.protocol, args.show_responses)
...
async def run():
    ...
    required_flags.add_argument(
        "--discover", action="store_true", help="Scan for Harmony devices."
    )
    ...
    show_responses_parser = parser.add_mutually_exclusive_group(required=False)
    show_responses_parser.add_argument(
        "--show_responses",
        dest="show_responses",
        action="store_true",
        help="Print out responses coming from HUB.",
    )
    show_responses_parser.add_argument(
        "--no-show_responses",
        dest="show_responses",
        action="store_false",
        help="Do not print responses coming from HUB.",
    )
    ...
    show_detailed_config_parser = subparsers.add_parser(
        "show_detailed_config", help="Print the detailed Harmony device configuration."
    )
    ...
    listen_parser = subparsers.add_parser(
        "listen",
        help="Output everything HUB sends out. Use in combination with --wait.",
    )
    ...
    command_parser.add_argument(
        "--device_id", help="Specify the device id to which we will send the command."
    )
    ...
    command_parser.add_argument(
        ...
        type=float,
        default=0,
        help='Number of seconds to "hold" before releasing. Defaults to 0.4seconds',
    )
