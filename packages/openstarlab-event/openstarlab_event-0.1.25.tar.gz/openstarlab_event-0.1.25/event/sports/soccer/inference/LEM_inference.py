...
from ..utils.LEM_cost_function import LEM_cost_function
from ..trainers.LEM_train import one_hot_encode_from_continuous
...
def LEM_inference(train_df, data, model_name, model_path, model_config, num_workers=4, batch_size=64, device=None, min_max_dict_path=None):
    ...
    with torch.no_grad():
        ...
        for i, (input_seq, gt, end_idx) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            input_seq = torch.cat((inputs_one_hot, input_seq[:,:,1:]), dim=2)
            ...
            loss, loss_components= LEM_cost_function(gt, output,  min_dict=min_dict, max_dict=max_dict, device=device,config=config)
...
def LEM_simulation_possession(train_df, data, model_name, model_path, model_config, num_workers=4, batch_size=64, device=None, random_selection=False, max_iter=200, min_max_dict_path=None):
        # Load the training configuration 
    with open(model_config, "r") as f:
        config = json.load(f)
        ...
        for i, (input_seq, end_idx) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            while end_idx:
                ...
                if len(config['other_features'])>0:
                    ...
                    append_tensor = torch.zeros((prev_timestep.shape[0],len(config['features'])+config['num_actions']-1))
                    ...
                    if config['seq_len']==1:
                        ...
                        input_seq = torch.cat((input_seq[:, len(config['features'])-1+config['num_actions']:], append_tensor.flatten(1)), dim=-1) 
                else:
...
def LEM_simulation_match(train_df, data, model_name, model_path, model_config, num_workers=4, batch_size=64, device=None, random_selection=False, max_iter=200, min_max_dict_path=None, simulation_time=45,match_id=None): 
        # Load the training configuration 
    with open(model_config, "r") as f:
        config = json.load(f)
            ...
            while end_idx:
                ...
                for j, idx in enumerate(end_idx):
                    ...
                    if 'home_score' in config['features'] and 'away_score' in config['features']:
                        ...
                        home_score_j = input_seq[j][home_score_idx]
                        away_score_j = input_seq[j][away_score_idx]
                ...
                if len(config['other_features'])>0:
                    # other_features = ['team','home_team','success','seconds','deltaX','deltaY','distance','dist2goal','angle2goal']
                    #get the previous timestep
                    prev_timestep = input_seq[:,seq_len:] if len(input_seq.shape) == 3 else input_seq
                    #create a tensor with the same shape as the prev_timestep
                    append_tensor = torch.zeros((prev_timestep.shape[0],len(config['features'])+config['num_actions']-1))
                    #convert the prev_timestep to a numpy array
                    prev_timestep = prev_timestep.cpu().numpy() if device != "cpu" else prev_timestep.numpy()
...
def load_model(model_name:str,model_path:str,model_config:str):
    if model_name == "LEM":
        ...
        input_size_action = config["input_size_action"]
        hidden_size_action = config["hidden_size_action"]
        output_size_action = config["output_size_action"]
        activation = config["activation"]
        activation_action = config["activation_action"]
...
if __name__ == "__main__":
    model_path = os.getcwd()+"/test/model/LEM/out/optuna/20241009_010549/run_1/_model_1.pth"
    model_config = os.getcwd()+"/test/model/LEM/out/optuna/20241009_010549/run_1/hyperparameters.json"
    ...
    save_path = os.getcwd()+"/test/inference/LEM/"
    os.makedirs(save_path, exist_ok=True)

    #testing inference for LEM
    # inferenced_data, loss_df = LEM_inference(train_path, valid_path, "LEM", model_path, model_config)
    # inferenced_data, loss_df = LEM_inference(None, valid_path, "LEM", model_path, model_config, min_max_dict_path=min_max_dict_path)
