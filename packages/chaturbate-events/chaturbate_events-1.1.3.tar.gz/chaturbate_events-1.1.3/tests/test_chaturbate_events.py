"""Condensed tests for the Chaturbate Events API wrapper."""
...
def test_event_model(event_data: dict[str, Any], expected_type: EventType) -> None:
    """Test Event model creation and type mapping."""
...
async def test_client_poll_and_auth(credentials, mock_http_get, mocker):
    """Test polling events and handling authentication errors."""
...
async def test_client_multiple_events(credentials, multiple_events, mocker):
    """Test client handling of multiple events in a single poll."""
...
async def test_client_cleanup(credentials):
    """Test client cleanup via context manager and explicit close."""
    client = EventClient(
        username=credentials["username"],
        token=credentials["token"],
        use_testbed=credentials["use_testbed"],
...
async def test_router_dispatch(event_type):
    """Test EventRouter dispatches to correct handlers and on_any."""
    router = EventRouter()
    handler = AsyncMock()
