...
class LockIn:
    ...
    def __init__(self, FsIn, Fc, BufferSize, Filter, DownFactor, nRow=0, Index=0, RowGain=None, TIAGain=None):
        self.nRow = nRow  # Index of the row in input buffer
        self.Index = Index  # Index of the output for output stacking
        self.Fs = FsIn
        self.Ts = 1 / FsIn
        self.BufferSize = BufferSize

        # Store the gain of the rows and TIAs
        if RowGain is not None:
            self.RowGain = RowGain
        if TIAGain is not None:
            self.TIAGain = TIAGain
    ...
    def CalcData(self, NewData):
        ...
        # Return the complex value, Index and the gain to simplify postprocessing stacking
        return r + i * 1j, self.Index, (self.TIAGain, self.RowGain)
