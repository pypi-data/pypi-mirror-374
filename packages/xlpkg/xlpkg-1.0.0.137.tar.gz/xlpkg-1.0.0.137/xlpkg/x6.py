...
try:
    ...
    reqs = None
    logging.warning("requests library not available. Some features may not work.")
try:
    from elasticsearch import Elasticsearch as ES
except ImportError:
    ES = None
    logging.warning("elasticsearch library not available. Some features may not work.")
...
def f_widget_geometry(widget):
    ...
    return (widget.objectName(), rect.x(), rect.y(), rect.width(), rect.height())
...
def f_dtmin(dt, min=365, max=0):
    dttm.setMinimumDate(QDate.currentDate().addDays(min * -1))  # 设置QDateTimeEdit控件可显示的最小日期
    # 设置最大日期
    dttm.setMaximumDate(QDate.currentDate().addDays(max))  # 设置QDateTimeEdit控件可显示的最大日期
    pass
...
# 时间戳转日期时间
def f_ts2dt(ts,n=0):
    dt1 = dttm.datetime.fromtimestamp(ts)
    ret = ''
    match n:
        case 0:
...
def f_str2dt(dt_str,n = 0):
    ...
    match n:
        case 0:
            fmt = '%Y-%m-%d %H:%M:%S'
            ret = dttm.datetime.strptime(dt_str,fmt)
        case 1:
            fmt = '%Y-%m-%d'
            ret = dttm.datetime.strptime(dt_str, fmt)
        case 2:
...
# 日期时间转时间戳
def f_dt2ts(dt,n = 0):
    ts = dttm.datetime.fromtimestmp(dt)
    ret = ''
    match n:
        case 0:
            ret = dttm.strftie('%Y-%m-%d %H:%M:%S')
        case 1:
            ret = dttm.strftie('%Y-%m-%d')
        case 2:
            ret = dttm.strftie('%H:%M:%S')
    return(ret)
    pass
...
try:
    ...
    logging.warning(f"Failed to create SSH tunnel: {e}")
...
def f_dt(n=0,fmt=0):
    ...
    curr=dttm.datetime.now()+dttm.timedelta(days=n)
    match fmt:
        case 0:
...
def f_dttz(dt):
    ret=('T'.join(dttm.split(' '))+'Z').replace('.','-')
    return(ret)
...
def f_removedup(lst, m=0):
    ...
    """
    # 输入验证
    if not isinstance(lst, (list, tuple, __builtins__.set)):
