...
#
# ----------------------------------------------------------------------
def create_qr(qr_path, lines, width=5):
...
def cli(lines, do_print, device, fmt, width, gray, font_size, qr_path, qr_create):
    """
    ...
    if not do_print:
        # display with cv2
        cvim = pil_to_cv2(im)
        cv2.imshow("Preview - press any key to close", cvim)
        cv2.waitKey(0)
...
# from importlib_resources import files, as_file
# import shlex # split correctly CMD
# import subprocess as sp
# import glob
# import tempfile
# from console import fg, bg
...
#     source = files('data').joinpath('ql570')
...
#     """
#     print("D...", "_"*(90-len(CMDi)), fg.yellow, CMDi, fg.default) # Just show on terminal
...
#     main()
################################################################################
# # Compose QR (left) + text block (right)
# def compose_qr_and_text(qr_path, lines, total_width=714, pad=10, gap=10, font_path=None, font_size=36, qr_max_size=350):
#     if not (1 <= len(lines) <= 3):
#         raise ValueError("Provide 1..3 lines.")
#     if not os.path.exists(qr_path):
#         raise FileNotFoundError(qr_path)
#     # load font
#     try:
...
#     # measure text lines
#     tmp_h = 4000
#     tmp = Image.new("L", (total_width, tmp_h), 255)
#     draw_tmp = ImageDraw.Draw(tmp)
#     line_sizes = []
#     for ln in lines:
#         bbox = draw_tmp.textbbox((0, 0), ln, font=font)
...
#         h = bbox[3] - bbox[1]
#         line_sizes.append((w, h))
#     spacing = max(int(0.2 * (line_sizes[0][1] if line_sizes else 12)), 4)
#     text_block_w = max(w for w, _ in line_sizes)
#     text_block_h = sum(h for _, h in line_sizes) + spacing * (len(lines) - 1)
#     # open qr and possibly scale
#     qr = Image.open(qr_path).convert("RGB")
#     # constrain QR to qr_max_size and to available width
#     available_for_qr = total_width - (text_block_w + gap + pad*2)
#     if available_for_qr <= 0:
#         # if no room, allocate half-half
#         available_for_qr = min(qr_max_size, total_width // 2 - pad - gap)
#     qr_target_h = min(qr_max_size, max(text_block_h, qr.height))
#     # scale QR to match text_block_h (if bigger) but not exceed available width/height
#     # compute scale preserving aspect ratio
#     resample = getattr(Image, "Resampling", Image).LANCZOS
#     scale_h = text_block_h
#     # ensure we don't exceed available_for_qr width
#     # try target height = text_block_h, compute width
#     w_ratio = qr.width / qr.height if qr.height else 1
#     qr_w_by_h = int(scale_h * w_ratio)
#     if qr_w_by_h > available_for_qr:
#         # reduce height to fit available width
#         qr_w = available_for_qr
#         qr_h = max(1, int(qr_w / w_ratio))
#     else:
#         qr_w = qr_w_by_h
#         qr_h = max(1, int(scale_h))
#     # also ensure not exceeding qr_max_size
#     if qr_h > qr_max_size:
#         qr_h = qr_max_size
#         qr_w = max(1, int(qr_h * w_ratio))
#     qr = qr.resize((qr_w, qr_h), resample=resample)
#     # recompute text area width to fill remaining space
#     text_area_w = total_width - (pad*2 + gap + qr_w)
#     if text_area_w < 1:
#         text_area_w = max(1, total_width - (pad*2) - qr_w - gap)
#     # create canvas height as max(qr_h, text_block_h) + pad*2
#     canvas_h = max(qr_h, text_block_h) + pad*2
#     canvas = Image.new("RGB", (total_width, canvas_h), "white")
#     # paste QR at left, vertically centered
#     qr_y = pad + (canvas_h - pad*2 - qr_h) // 2
#     canvas.paste(qr, (pad, qr_y))
#     # draw text block on right of QR
#     draw = ImageDraw.Draw(canvas)
#     text_x = pad + qr_w + gap
#     # vertical starting position to center the text block
#     text_y = pad + (canvas_h - pad*2 - text_block_h) // 2
#     for (w, h), ln in zip(line_sizes, lines):
#         x = text_x + max((text_area_w - w) // 2, 0)
#         draw.text((x, text_y), ln, font=font, fill=0)
#         text_y += h + spacing
#     return canvas
