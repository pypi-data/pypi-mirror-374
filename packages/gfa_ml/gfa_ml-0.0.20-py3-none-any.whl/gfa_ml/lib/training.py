...
from gfa_ml.data_model.common import RunConfig, TransformerConfig, LSTMConfig
...
def build_lstm_model(input_size: int, ml_model_config: LSTMConfig):
    try:
...
def build_transformer_model(input_size: int, ml_model_config: TransformerConfig):
    try:
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name, nested=True) as run:
            ...
            if model_path:
                ...
                run_path = os.path.join(model_path, f"model_{mlflow_run_id}")
                os.makedirs(run_path, exist_ok=True)
                scaler_file_path = os.path.join(run_path, "scalers.pkl")
                ml_model_path = os.path.join(run_path, "ml_model")
                os.makedirs(ml_model_path, exist_ok=True)
                wrapped_model_path = os.path.join(run_path, "wrapped-model")
                os.makedirs(wrapped_model_path, exist_ok=True)
            ...
            # if mlflow_enable:
            #     try:
            #         mlflow.log_artifact(scaler_file_path, artifact_path="preprocessing")
            #     except Exception as e:
            #         logging.error(f"Error logging MLflow scaler in {model_type}: {e}")
            # data_spec_dict = {"input_cols": input_cols, "output_col": [output_col]}
            # save_yaml(dictionary=data_spec_dict, path="data_spec.yaml")
            # mlflow.log_artifact("data_spec.yaml", artifact_path="preprocessing")
            ...
            if mlflow_enable:
                try:
                    ...
                    try:
                        joblib.dump(
                            {"input": x_scalers, "output": y_scaler}, scaler_file_path
                        )
                        model_info = mlflow.pytorch.save_model(
                            mlmodel, input_example=input_example, path=ml_model_path
                        )
                        mlflow.pyfunc.save_model(
                            path=wrapped_model_path,
