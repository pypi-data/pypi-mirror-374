...
class BubblewrapProvider(CodeExecutionProvider):
    """
    A code execution provider that uses Bubblewrap for sandboxed execution on Linux systems.
    
    This provider executes Python code and shell commands within a Bubblewrap sandbox for enhanced security.
    It only works on Linux systems and provides similar functionality to SeatbeltProvider for macOS.
    """
    ...
    def __init__(
        ...
        bubblewrap_profile: Optional[str] = None,
        bubblewrap_profile_path: Optional[str] = None,
        python_env_path: Optional[str] = None,
        authorized_imports: list[str] | None = None,
        authorized_functions: list[str] | None = None,
        check_string_obfuscation: bool = True,
        bypass_shell_safety: bool = True,  # Default to True for BubblewrapProvider
        additional_safe_shell_commands: Optional[List[str]] = None,
        additional_safe_control_operators: Optional[List[str]] = None,
        additional_read_dirs: Optional[List[str]] = None,  # Additional read directories
        additional_write_dirs: Optional[List[str]] = None,  # Additional write directories
        environment_variables: Optional[Dict[str, str]] = None,  # Environment variables
        **kwargs
    ):
        """
        Initialize the BubblewrapProvider.
        ...
        self.logger = None
        if log_manager:
            self.logger = log_manager.get_logger('tinyagent.code_agent.providers.bubblewrap_provider')
        ...
        if platform.system() != "Linux":
            raise RuntimeError("BubblewrapProvider only works on Linux systems")
        
        # Check if bubblewrap is available
        if not self._check_bubblewrap_availability():
            raise RuntimeError("Bubblewrap (bwrap) is not available on this system. Please install bubblewrap package.")
        ...
        try:
            self.sandbox_tmp_dir = os.path.join("/tmp", f"tinyagent_bw_{os.getpid()}")
        ...
        # Log initialization
        if self.logger:
            self.logger.info("Initialized BubblewrapProvider with sandbox temp dir: %s", self.sandbox_tmp_dir)
    ...
    def _check_bubblewrap_availability(self) -> bool:
        """
    ...
    def _get_sandbox_environment(self) -> Dict[str, str]:
        ...
        base_env = {
            ...
            'HOME': self.sandbox_tmp_dir,  # Use sandbox temp dir as HOME
    ...
    def _build_bubblewrap_command(
        ...
        additional_binds: Optional[Dict[str, str]] = None,
        enable_network: bool = False,
        working_dir: Optional[str] = None
    ) -> List[str]:
        """
        Build a complete bubblewrap command with security settings.
        
        Args:
            exec_command: The command to execute inside the sandbox
            additional_binds: Additional bind mounts as {host_path: sandbox_path}
            enable_network: Whether to enable network access
            working_dir: Working directory inside the sandbox
            
        Returns:
            Complete bubblewrap command as list of arguments
        """
        cmd = ['bwrap']
        ...
        # Create new namespaces for security isolation
        cmd.extend([
            '--unshare-user',
            '--unshare-pid',
            '--unshare-ipc',
        ...
        # Create a new /tmp inside the sandbox
        cmd.extend(['--tmpfs', '/tmp'])
        
        # Bind essential system directories as read-only
        essential_ro_dirs = ['/usr', '/lib', '/lib64', '/bin', '/sbin', '/etc']
        for dir_path in essential_ro_dirs:
            if os.path.exists(dir_path):
                cmd.extend(['--ro-bind', dir_path, dir_path])
        
        # Handle /lib32 on 64-bit systems
        if os.path.exists('/lib32'):
            cmd.extend(['--ro-bind', '/lib32', '/lib32'])
        
        # Bind /proc filesystem
        cmd.extend(['--proc', '/proc'])
        
        # Bind essential devices
        essential_devices = ['/dev/null', '/dev/zero', '/dev/urandom', '/dev/random']
        for device in essential_devices:
            if os.path.exists(device):
                cmd.extend(['--dev-bind', device, device])
        
        # Create a minimal /dev/pts for pseudo-terminals
        ...
        cmd.extend(['--bind', current_dir, current_dir])
        
        # Bind additional read directories
        for read_dir in self.additional_read_dirs:
            if os.path.exists(read_dir):
                cmd.extend(['--ro-bind', read_dir, read_dir])
                if self.logger:
                    self.logger.debug("Added read-only bind: %s", read_dir)
        
        # Bind additional write directories
        for write_dir in self.additional_write_dirs:
            if os.path.exists(write_dir):
                cmd.extend(['--bind', write_dir, write_dir])
                if self.logger:
                    self.logger.debug("Added read-write bind: %s", write_dir)
        
        # Bind sandbox temp directory
        if self.sandbox_tmp_dir and os.path.exists(self.sandbox_tmp_dir):
            cmd.extend(['--bind', self.sandbox_tmp_dir, self.sandbox_tmp_dir])
        
        # Add any additional bind mounts
        if additional_binds:
            for host_path, sandbox_path in additional_binds.items():
                if os.path.exists(host_path):
                    cmd.extend(['--bind', host_path, sandbox_path])
        
        # Set working directory inside sandbox
        if working_dir:
            cmd.extend(['--chdir', working_dir])
        else:
            cmd.extend(['--chdir', current_dir])
...
""")
        ...
        try:
            ...
            bwrap_cmd = self._build_bubblewrap_command([python_cmd, code_file_path])
            
            if self.logger:
                self.logger.debug("Executing Python code in bubblewrap: %s", " ".join(bwrap_cmd[:5]) + "...")
            ...
            if self.logger:
                self.logger.error("Error executing Python in bubblewrap: %s", str(e))
            return {
                "printed_output": "",
                "return_value": None,
    ...
    async def _prepare_git_sandbox_command(self, command: List[str]) -> List[str]:
        """
        Prepare a specialized bubblewrap command for git operations.
        
        Args:
            command: Git command to execute
            
        Returns:
            Complete bubblewrap command for git operations
        """
...
""")
        ...
        # Build bubblewrap command with additional bind mounts for git operations
        additional_binds = {temp_dir: temp_dir}
        bwrap_cmd = self._build_bubblewrap_command(
            command,
            additional_binds=additional_binds,
            enable_network=True  # Git operations need network access
        )
        
        # Store environment for later use
        self._git_env = final_env
        
        return bwrap_cmd
    
    async def execute_shell(self, command: List[str], timeout: int = 10, workdir: Optional[str] = None) -> Dict[str, Any]:
        """
        Execute a shell command securely within a bubblewrap sandbox and return the result.
        ...
        """
        if self.logger:
            self.logger.debug("Executing shell command in bubblewrap: %s", " ".join(command))
        ...
        try:
            ...
            if len(command) > 0 and command[0] == "git":
                bwrap_cmd = await self._prepare_git_sandbox_command(command)
                sandbox_env = getattr(self, '_git_env', self._get_sandbox_environment())
            ...
            # Special handling for bash login shell to avoid profile loading errors
                ...
                bwrap_cmd = self._build_bubblewrap_command(
                    ["bash", "-c", command[2]],
                    working_dir=workdir or os.getcwd()
                )
                sandbox_env = bash_env
                temp_dir = None
            ...
            try:
                ...
                if temp_dir and hasattr(self, '_temp_git_dir'):
                    try:
                        ...
                        if hasattr(self, '_git_env'):
                            delattr(self, '_git_env')
                    except Exception:
                        pass
        
        except Exception as e:
            if self.logger:
                self.logger.error("Error executing shell command in bubblewrap: %s", str(e))
            response = {
                "stdout": "",
    ...
    def is_supported(cls) -> bool:
        """
        Check if the current system supports bubblewrap sandboxing.
        
        Returns:
            True if the system supports bubblewrap (Linux), False otherwise
        """
        if platform.system() != "Linux":
            return False
        ...
        try:
            subprocess.run(["which", "bwrap"], check=True, capture_output=True)
            return True
