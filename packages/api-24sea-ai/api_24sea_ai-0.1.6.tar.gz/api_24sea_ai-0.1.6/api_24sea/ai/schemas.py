...
class AIPIPredictionsInputSchema(U.BaseModel):
    ...
    - `models` can be a string or list of strings; coerced to a non-empty list of
      stripped strings.
    - `data` can be a pandas DataFrame or a list of dictionaries; validated and
      exposed as a JSON-serializable payload via `payload()`.
    ...
    def validate_data(cls, v):
        ...
        if isinstance(v, list):
            ...
            if not all("timestamp" in item for item in v):
                raise ValueError(
                    "Each dictionary in `data` list must contain a 'timestamp' key."
                )
            return v
        raise ValueError(
            "`data` must be a pandas DataFrame or a list of dictionaries."
    ...
    def payload(self) -> Any:
        ...
        if isinstance(self.data, pd.DataFrame):
            ...
            # Drop 'timestamp' column coming from DataFrame as server may infer it separately
            normalized: List[Dict[str, Any]] = [
                {str(k): v for k, v in rec.items() if str(k) != "timestamp"}
                for rec in records
            ]
        else:
            # For list input, preserve the dictionaries as-is (including 'timestamp')
            normalized = [
