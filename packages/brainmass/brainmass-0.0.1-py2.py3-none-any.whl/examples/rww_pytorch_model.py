    ...
    def __init__(self, model_name, **kwargs):
        if model_name == 'WWD':
            param = {
                ...
                "k1": [2.38, 0],
                "k2": [2.0, 0],
                "k3": [0.48, 0],  # adjust this number from 0.48 for BOLD fluctuate around zero
                "V": [.02, 0],
                "E0": [0.34, 0],
                "tau_s": [0.65, 0],
                "tau_f": [0.41, 0],
...
class RNNWWD(torch.nn.Module):
    ...
    def __init__(self, input_size: int, node_size: int,
        ...
        self.hidden_size = np.asarray(tr / step_size, dtype=int)
        self.batch_size = batch_size  # size of the batch used at each step
        self.node_size = node_size  # num of ROI
        self.sc = sc  # matrix node_size x node_size structure connectivity
        self.dist = torch.tensor(dist, dtype=torch.float32)
        ...
        for var in vars:
            if np.any(getattr(param, var)[1] > 0):
                setattr(
                    ...
                    Parameter(
                        torch.tensor(
                            getattr(param, var)[0] + 1 / np.sqrt(getattr(param, var)[1]) * np.random.randn(1)[0],
                            dtype=torch.float32
                        )
                    )
                )
...
class Model_fitting:
    ...
    def __init__(self, model, ts, num_epoches, cost, lr):
        ...
        # placeholder for output(EEG and history of model parameters and loss)
        self.output_sim = OutputNM(
            self.model.model_name,
            self.model.node_size,
            self.model.param,
            self.model.fit_gains_flat,
            self.model.fit_lfm_flat
        )
    ...
    def train(self, u=0):
        ...
        if self.model.model_name == 'WWD':
            # initial state
            X = torch.tensor(
                0.45 * np.random.uniform(0, 1, (self.model.node_size, self.model.state_size)) +
                np.array([0, 0, 0, 1.0, 1.0, 1.0]),
                dtype=torch.float32
            )
        elif self.model.model_name == 'JR':
            X = torch.tensor(
                np.random.uniform(state_lb, state_ub, (self.model.node_size, self.model.state_size)),
                dtype=torch.float32
            )
        hE = torch.tensor(
            np.random.uniform(state_lb, state_ub, (self.model.node_size, delays_max)),
            dtype=torch.float32
        )
        ...
        # define num_batches
        num_batches = np.asarray(self.ts.shape[2] / self.model.batch_size, dtype=int)
        ...
        for i_epoch in range(self.num_epoches):
            ...
            external = torch.tensor(
                np.zeros([self.model.node_size, self.model.hidden_size, self.model.batch_size]),
                dtype=torch.float32
            )
            ...
            for i_batch in range(num_batches):
                ...
                noise_in = torch.tensor(
                    np.random.randn(self.model.node_size, self.model.hidden_size,
                                    self.model.batch_size, self.model.input_size),
                    dtype=torch.float32
                )
                noise_out = torch.tensor(
                    np.random.randn(self.model.node_size, self.model.batch_size),
                    dtype=torch.float32
                )
                ...
                for var in variables_p:
                    ...
                    if np.any(getattr(self.model.param, var)[1] > 0) and var != 'std_in' and var not in exclude_param:
                        ...
                        loss_prior.append(torch.sum(self.model.get_parameter(dict_np['v']) * \
                                                    (self.model.get_parameter(var) - self.model.get_parameter(
                                                        dict_np['m'])) ** 2) \
                                          + torch.sum(-torch.log(self.model.get_parameter(dict_np['v']))))
                # total loss
                if self.model.model_name == 'WWD':
                    loss = (0.1 * w_cost * self.cost.cost_eff(next_batch['bold_batch'], ts_batch) +
                            sum(loss_prior) + loss_EI)
                elif self.model.model_name == 'JR':
