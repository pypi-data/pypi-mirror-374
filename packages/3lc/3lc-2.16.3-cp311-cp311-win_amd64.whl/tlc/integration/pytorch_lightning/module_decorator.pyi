...
def lightning_module(structure: _SampleTypeStructure | None = None, dataset_prefix: str = '', run_name: str | None = None, run_description: str | None = None, project_name: str | None = None, root_url: Url | str | None = None, if_dataset_exists: Literal['raise', 'reuse', 'rename', 'overwrite'] = 'reuse', if_run_exists: Literal['raise', 'reuse', 'rename', 'overwrite'] = 'rename', add_weight_column: bool = True, all_arrays_are_fixed_size: bool = True, exclude_zero_weights_train: bool = True, weighted: bool = True, use_latest: bool = True, map_functions: list[Callable[[object], object]] | Callable[[object], object] | None = None, predictor_args: PredictorArgs = ..., metrics_collectors: MetricsCollectorType | None = None, collect_metrics_on_train_end: bool = True, collect_metrics_on_train_start: bool = False, metrics_collection_interval: int = 0, metrics_collection_start: int = 0, metrics_collection_exclude_zero_weights: bool = True, metrics_collection_map_functions: list[Callable[[object], object]] | Callable[[object], object] | None = None) -> Callable[[type[_LightningModule]], type[_LightningModule]]:
    '''Decorator for using 3LC with PyTorch Lightning.

    Adding this decorator to a `pytorch_lightning.LightningModule` subclass definition will create tables from the
    datasets used within the module, as well as collect metrics and create a run when used with a `Trainer`.
    ...
    :param exclude_zero_weights_train: If True, rows with a weight of zero will be excluded from the sampler during
        training. This is useful for adjusting the length of the sampler, and thus the length of an epoch, to the
        number of non-zero weighted rows in the table.
    ...
    :param metrics_collection_start: The epoch to start collecting metrics. Metrics will be collected at the end of
