...
import argparse
import zipfile
#import libarchive # (hopefully) faster replacement for zipfile
...
def compress_buffer(buffdata, method, nthreads=4, clevel=8, use_dict=True, **kwargs):
    assert method.lower() in ['blosc2','none'], f'supported compression methods: BLOSC2 and None, received {method}.'
...
def writer_function(nwriters: int, comm: MPI.Intracomm, rank: int, archive_basename: str, cmethod: str, usezip: bool=False):
    ...
    if not usezip: 
        ...
        #ctx=libarchive.file_writer(h5filename, 'zip')
