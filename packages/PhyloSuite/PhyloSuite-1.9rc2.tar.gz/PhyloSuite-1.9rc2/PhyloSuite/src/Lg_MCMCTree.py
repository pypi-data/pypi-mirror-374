...
def run_code(popen_=None, log2="", log_prefix="", queque=None, runPath=None):
    ...
    while True:
        ...
        if re.search(r"ERROR", out_line, re.IGNORECASE) and ("multifurcating".upper() in out_line.upper()):
            ...
            break
        elif re.search(r"ERROR", out_line, re.IGNORECASE):
            is_error = True
    ...
    if mode == "start":
        ...
        if dict_args["use_data"] in ["no data (prior)", "seq like (exact likelihood) SLOW"]:
            ...
            if dict_args["hessian_program"] == "Baseml":
                basemlEXE = os.path.join(os.path.dirname(dict_args["mcmctreeEXE"]), "baseml.exe")
                shutil.copy(basemlEXE, os.path.join(runPath, "baseml.exe"))
                codemlEXE = os.path.join(os.path.dirname(dict_args["mcmctreeEXE"]), "codeml.exe")
                shutil.copy(codemlEXE, os.path.join(runPath, "codeml.exe"))
                replace_ctl(ctl_file,
                            r"usedata\s*=\s*\d+",
                            "usedata = 3")
                mcmctree_popen = factory.init_popen(command, stdin_=True)
            ...
            if dict_args["hessian_program"]=="Baseml":
                codemlEXE = os.path.join(os.path.dirname(dict_args["mcmctreeEXE"]), "codeml.exe")
                dat_folder = os.path.dirname(os.path.dirname(codemlEXE))
...
method = 1'''
                ...
                for ctl_file in ctl_files:
                    ...
                    codeml_command = f"{codemlEXE} {ctl_file}"
        ...
        r8s_popen = factory.init_popen(command, stdin_=True)
...
class MCMCTracer(QMainWindow,Ui_MCMCTracer,object):
    ...
    def parse_data(self, files, use_folder_name=False):
        try:
            ...
            for row, file in enumerate(files):
                ...
                sample_frequency = df['Gen'].iloc[-1] - df['Gen'].iloc[-2]
                self.table1_data[base] = [os.path.abspath(file), states_value, sample_frequency]
                # ▶ 提前计算并缓存所有变量的统计量
                analysis_result = {}
    ...
    def chooseConvergenceFoler(self):
        self.resultsPath = self.workPath + os.sep + "MOLDAT_results"
...
class MCMCTree(QDialog,Ui_MCMCTreeGUI,object):
    ...
    def __init__(
        ...
        self.comboBox_3.activated[str].connect(self.usedata_changed)
        self.usedata_changed(self.comboBox_3.currentText())
        self.comboBox_14.activated.connect(self.ctl_mix_models)
        self.ctl_mix_models(self.comboBox_14.currentIndex())
        ...
        self.comboBox_14.setCurrentText(self.comboBox_14.currentText())
        # 恢复用户的设置
        self.guiRestore()
        # 给开始按钮添加菜单
        menu = QMenu(self)
    ...
    def on_pushButton_continue_clicked(self):
        ...
        resultsPath = None
        ##choose work folder
        if os.path.exists(self.workPath + os.sep + "MOLDAT_results"):
            list_result_dirs = sorted([i for i in os.listdir(self.workPath + os.sep + "MOLDAT_results")
                                       if os.path.isdir(self.workPath + os.sep + "MOLDAT_results" + os.sep + i)],
                                      key=lambda x: os.path.getmtime(self.workPath + os.sep + "MOLDAT_results" + os.sep + x), reverse=True)
            if list_result_dirs:
                item, ok = QInputDialog.getItem(self, "Choose previous results",
                                                "Previous results:", list_result_dirs, 0, False)
                if ok and item:
                    resultsPath = self.workPath + os.sep + "MOLDAT_results" + os.sep + item
        else:
            QMessageBox.information(
                self,
                "MDGUI",
        ...
        if (not self.isRunning()) and (mode=="direct click"):
            ...
            if os.path.exists(self.workPath + os.sep + "MOLDAT_results"):
                ...
                resultsPath = QFileDialog.getExistingDirectory(
                    ...
                    self.workPath + os.sep + "MOLDAT_results",
    ...
    def ctrlModel(self, text):
        ...
        if (text in ["JC69", "K80", "F81", "F84", "HKY", "T92", "TN93", "GTR", "UNREST", "REVu", "UNRESTu"]) and \
            ...
            return
        elif (text in ["cpREV10", "cpREV64", "dayhoff", "dayhoff-dcmut", "g1975a", "g1975c", "g1975p", "g1975v",
    ...
    def run_command(self, rerun=False):
        try:
            ...
            self.exportPath = self.factory.creat_dirs(self.workPath +
                                                 os.sep + "MOLDAT_results" + os.sep + self.output_dir_name) if \
    ...
    def run_command_r8s(self, cfg=None):
        try:
            self.output_dir_name = self.factory.fetch_output_dir_name(self.dir_action)
            self.exportPath = self.factory.creat_dirs(self.workPath +
                                                      os.sep + "MOLDAT_results" + os.sep + self.output_dir_name)
            self.time_start = datetime.datetime.now()
            self.startButtonStatusSig.emit(
                [
                    self.pushButton_5,
            ...
            if platform.system().lower() != "windows":
                ...
                command_list = [f"{self.r8sEXE}{' -b' if self.batch_process else ''} -f {r8s_cmd_data_file}"]
                # 修改
    ...
    def end_run(self,
        ...
        with open(self.exportPath + os.sep + "PhyloSuite_MCMCTREE.log") as f:
            content = f.read()
        rgx_version = re.compile(r"(?m)^MCMCTREE.+?(\d+\.\d+\.\d+)")
    ...
    def guiRestore(self):
        ...
        # Restore geometry
        self.resize(self.MCMCTree_settings.value('size', QSize(1300, 750)))
        self.factory.centerWindow(self)
    ...
    def generate_template(self):
        def generate_ctl(seqfile, treefile):
            ...
            #     'RootAge': 0,
            #     'model': {'JC69': 0, 'K80': 1, 'F81': 2, 'F84': 3, 'HKY': 4, 'T92': 5,
            #               'TN93': 6, 'GTR': 7, 'UNREST': 8, 'REVu': 9, 'UNRESTu': 10,
            #               "cpREV10": 'cpREV10.dat', "cpREV64": 'cpREV64.dat', "dayhoff": 'dayhoff.dat',
            #               "dayhoff-dcmut": 'dayhoff-dcmut.dat',
            #               "g1975a": 'g1975a.dat', "g1975c": 'g1975c.dat', "g1975p": 'g1975p.dat',
            #               "g1975v": 'g1975v.dat', "grantham": 'grantham.dat',
            #               "jones": 'jones.dat', "jones-dcmut": 'jones-dcmut.dat', "lg": 'lg.dat',
            #               "miyata": 'miyata.dat', "mtART": 'mtART.dat',
            #               "mtmam": 'mtmam.dat', "mtREV24": 'mtREV24.dat', "MtZoa": 'MtZoa.dat', "wag": 'wag.dat'},
            #     'alpha': 11,
            #     'ncatG': 3,
            ...
            tipdate = f"TipDate = 1 100 * TipDate (1) & time unit" if self.checkBox_13.isChecked() else ""
            rootage = f"RootAge = >{self.doubleSpinBox.value()}<{self.doubleSpinBox_19.value()}" if \
      ...
      mcmcfile = mcmc.txt
       ...
       usedata = {self.comboBox_3.currentIndex()} *
         clock = {clock_value[self.comboBox_4.currentText()]} *
       {tipdate}
...
'''
    ...
    def prepare_for_run(self, fold_suffix=None, seed=-1):
        ...
        # self.replace_ctl(ctl_file,r"seed\s*=\s*-?\d+",
        #                  f"seed = {seed}")
        self.dict_args["model"] = self.model
        ...
        if flag:
            ...
            if not IQpath:
                reply = QMessageBox.information(
                    self,
                    "Information",
                    "<p style='line-height:25px; height:25px'>Please install IQ-TREE (for model selection and"
    ...
    def usedata_changed(self, text):
        ...
        if (model in ["T92", "TN93", "GTR", "UNREST", "REVu", "UNRESTu", "cpREV10", "cpREV64", "dayhoff",
                      "dayhoff-dcmut", "g1975a", "g1975c", "g1975p", "g1975v",
                      "grantham", "jones", "jones-dcmut", "lg", "miyata", "mtART", "mtmam", "mtREV24", "MtZoa",
                      "wag"]) and \
            ...
            if self.sender() == self.comboBox:
                ...
                QMessageBox.information(
                    ...
                    "</p>")
                self.comboBox.setCurrentText("AUTO")
        if self.sender() == self.comboBox_3:
            if text == "approximate likelihood FAST":
