    ...
    try:
        with open(file_path, 'r', encoding=encoding) as f:
            for index, line in enumerate(f):
                if string in line:
                    if ignore_comment and string.strip().startswith('#'):
                        continue
...
def b_str_in_dir(dir_path, string, console_log=True, include_ext:list[str]=INCLUDE_EXT.copy(), ignore_comment=False):
    ...
    for root, dirs, files in os.walk(dir_path):
        for file in files:
            ...
            indexes = b_str_in_file(file_path, string, False, ignore_comment)
            if indexes:
...
def b_str_finder(path, string, console_log=True, ignore_comment=False):
    ...
    if path.is_file():
        ...
        return b_str_in_dir(path, string, console_log, ignore_comment)
