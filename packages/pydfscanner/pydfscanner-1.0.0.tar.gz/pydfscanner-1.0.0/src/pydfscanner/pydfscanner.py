...
def are_parallel(theta1, theta2):
    '''
    Check whether two lines defined by their Hough angle are parallel.
    '''
    return np.abs(theta1-theta2) < np.pi/8 or\
    np.abs(np.abs(theta1-theta2)-np.pi) < np.pi/8


def check_parallelity(theta):
    '''
    Check parallelity relations between four lines.

    Lines must be parallel in pairs: two parallel lines and two other parallel
    lines.
    '''
    are_par_01 = are_parallel(theta[0], theta[1])
    are_par_02 = are_parallel(theta[0], theta[2])
    are_par_03 = are_parallel(theta[0], theta[3])
    are_par_12 = are_parallel(theta[1], theta[2])
    are_par_13 = are_parallel(theta[1], theta[3])
    are_par_23 = are_parallel(theta[2], theta[3])
...
def line_intersection(rho1, rho2, theta1, theta2):
    '''
    Find intersection between two Hough lines.
    '''
    if np.abs(np.sin(theta1)) < 1e-3:
        x = rho1/np.cos(theta1)
        y = (rho2 - x*np.cos(theta2))/np.sin(theta2)
    elif np.abs(np.sin(theta2)) < 1e-3:
        x = rho2/np.cos(theta2)
        y = (rho1 - x*np.cos(theta1))/np.sin(theta1)
    else:
        x = (rho1/np.sin(theta1) - rho2/np.sin(theta2))/(1/np.tan(theta1) -
            1/np.tan(theta2))
        y = (rho1 - x*np.cos(theta1))/np.sin(theta1)
    return [x, y]
...
def find_doc_edges(edges, doc_corners, doc_edges, max_area):
    ...
    # Check that lines are parallel in pairs.
    if check_parallelity(theta):
       return max_area

    # Find corners.
    corners = np.zeros((4, 2))
    if are_parallel(theta[0], theta[1]) and are_parallel(theta[2], theta[3]):
        corners[0] = line_intersection(rho[0], rho[2], theta[0], theta[2])
        corners[1] = line_intersection(rho[0], rho[3], theta[0], theta[3])
        corners[2] = line_intersection(rho[1], rho[2], theta[1], theta[2])
        corners[3] = line_intersection(rho[1], rho[3], theta[1], theta[3])
    if are_parallel(theta[0], theta[2]) and are_parallel(theta[1], theta[3]):
        corners[0] = line_intersection(rho[0], rho[1], theta[0], theta[1])
        corners[1] = line_intersection(rho[0], rho[3], theta[0], theta[3])
        corners[2] = line_intersection(rho[2], rho[1], theta[2], theta[1])
        corners[3] = line_intersection(rho[2], rho[3], theta[2], theta[3])
    if are_parallel(theta[0], theta[3]) and are_parallel(theta[1], theta[2]):
        corners[0] = line_intersection(rho[0], rho[1], theta[0], theta[1])
        corners[1] = line_intersection(rho[0], rho[2], theta[0], theta[2])
        corners[2] = line_intersection(rho[3], rho[1], theta[3], theta[1])
        corners[3] = line_intersection(rho[3], rho[2], theta[3], theta[2])
...
def corner_detection(im, image_path):
    ...
    # Find the document's corners. We select as corners those defined as the
    # intersection of four lines, which are parallel two by two, and have
    # maximal area.
    max_area = -1.0
    ...
    for comb in combinations(range(np.shape(lines)[0]), 4):
        ...
        max_area = find_doc_edges(lines[comb, :], doc_corners, doc_edges,
                                  max_area)
    ...
    # Mean of vertical (height) and horizontal (widht) lengths.
    mu_hl = (
    ...
    with np.errstate(invalid='ignore'): doc_scale = mu_hl/mu_wl
...
def scanner_main(image_path, dest_name, debug=False):
    ...
    if im is None: 
        print(f'{RED}FATAL{END} Unable to read: {RED}{image_path}{END}')
        exit()
    orig = im.copy()
    
    doc_corners, doc_edges, lines, has_detected_corners, doc_scale =\
    corner_detection(im, image_path)

    # TODO fix scale check hack
    if has_detected_corners and 1 < doc_scale < 3:
        print(f'{GREEN}{image_path}{END}')
        ...
        print((
            f'{YELLOW}WARNING{END} Edge or corner detection failed for: '
            f'{YELLOW}{image_path}{END}\n'
            'Saving resized original as fallback\n'
            f'Use {PURPLE}--debug{END} to inspect detection'
        ))
        # Save a resized copy of the original image so caller always gets a file
        fallback = do_image_thresholding(orig)
        fallback = imutils.resize(fallback, height=1000)
        cv2.imwrite(dest_name, fallback)

    # Debug: write an image of the middle process.
    if debug:
        im_dbg = orig.copy()

        # show edge detection stage
        import matplotlib.pyplot as plt
        im_dbg = imutils.resize(im_dbg, height=500)
        im_dbg = cv2.cvtColor(im_dbg, cv2.COLOR_BGR2GRAY)
        im_dbg = cv2.GaussianBlur(im_dbg, (3, 3), 0) 
        edges = cv2.Canny(im_dbg, 100, 200)
        plt.subplot(121),plt.imshow(im_dbg,cmap = 'gray')
        plt.title('Original Image'), plt.xticks([]), plt.yticks([])
        plt.subplot(122),plt.imshow(edges,cmap = 'gray')
        plt.title('Edge Image'), plt.xticks([]), plt.yticks([])
        plt.savefig(insert_in_namepath(dest_name, '_dbg_Canny'))

        if lines is not None:
            # Overwrite detected document corners.
            circ_radius = int(np.shape(im_dbg)[0]*0.01)
            for corner in doc_corners:
                cv2.circle(im_dbg, (int(corner[0]), int(corner[1])), circ_radius,
                           (0, 0, 255), -1)

            # Overwrite detected lines. If line is vertical return ad hoc values.
            def xy_hough_lines(rho, theta, x):
            ...
            for line in lines:
                rho, theta = line
                rho *= np.shape(im_dbg)[0]/500
                x1 = 0
                y1 = xy_hough_lines(rho, theta, x1)
                x2 = np.shape(im_dbg)[0]
                y2 = xy_hough_lines(rho, theta, x2)
