    ...
    def get_quote(self, req: QuoteRequest) -> APIResult[Quote]:
        ...
        try:
            ...
            return APIResult(
                ok=False,
                error=APIError(
                    code=PMCCErrorCode.SCHEMA_ERROR,
                    message=str(e),
                    detail={"attempts": attempts.get("n", 1), "endpoint": self.endpoint},
                ),
            )
            ...
            self._maybe_block(now)
            msg = str(e)
            detail: dict[str, object] = {"attempts": attempts.get("n", 1), "endpoint": self.endpoint}
            if msg.startswith("http status "):
                from contextlib import suppress

                with suppress(Exception):
                    detail["status_code"] = int(msg.split(" ")[2])
            elif msg.startswith("invalid json"):
                detail["error"] = "invalid_json"
            else:
                name = getattr(e, "__class__", type(e)).__name__.lower()
                if "timeout" in name or "timeout" in msg.lower():
                    detail["error"] = "timeout"
                elif "connection" in name or "connection" in msg.lower():
                    detail["error"] = "connection"
            return APIResult(ok=False, error=APIError(code=PMCCErrorCode.NETWORK_ERROR, message=msg, detail=detail))
