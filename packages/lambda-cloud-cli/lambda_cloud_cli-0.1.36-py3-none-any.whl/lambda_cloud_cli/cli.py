...
def login(force: bool = typer.Option(False, "--force", help="Overwrite existing stored key")):
    ...
    # ✅ Attempt to validate the key before saving
    temp_client = LambdaAPIClient(api_key=api_key)
    try:
        _ = temp_client.list_ssh_keys()
    except Exception:
        typer.secho("❌ Invalid API key! Authentication failed.", fg=typer.colors.RED)
        raise typer.Exit(code=1)
