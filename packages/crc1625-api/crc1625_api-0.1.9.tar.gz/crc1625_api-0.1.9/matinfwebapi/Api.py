...
class MatInfWebApiClient:
    ...
    def download_synthesis_with_edx_and_property_table(self, output_base_dir="synthesis_data"):
        """
        For each Synthesis <- Sample -> EDX CSV chain:
        - Download EDX file
        - Save full Synthesis properties table (all 4 types) as CSV
        - Save both under Sample_<sample_objectid>/
        """
        #print("\n🔍 Finding Synthesis <- Sample -> EDX CSV chains...")
        ...
        for _, r in df.iterrows():
            ...
            edx_id       = int(r["edx_id"])
            edx_name     = str(r["edx_name"]) if r.get("edx_name") is not None else f"EDX_{edx_id}"

            folder = os.path.join(output_base_dir, f"Sample_{sample_id}")
            os.makedirs(folder, exist_ok=True)

            # 1) Download EDX CSV
            edx_file_path = os.path.join(folder, f"{edx_name}.csv")
            #print(f" Downloading EDX CSV → {edx_file_path}")
            self.download(edx_id, file_name=edx_file_path)

            # 2) Download Synthesis properties (table-by-table)
            ...
            if not props_df.empty:
                csv_path = os.path.join(folder, "synthesis_properties_table.csv")
                props_df.to_csv(csv_path, index=False)
                #print(f" Saved synthesis properties table to: {csv_path}")
            else:
                print(" No properties found for this Synthesis.")

        #print(f"\n Done. All folders saved in: {output_base_dir}")
