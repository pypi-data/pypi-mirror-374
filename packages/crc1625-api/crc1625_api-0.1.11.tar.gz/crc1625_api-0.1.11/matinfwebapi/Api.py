...
class MatInfWebApiClient:
    ...
    def execute(self, sql):
        ...
        try:
            ...
            try:
                ...
                #print("⚠️ Warning: Response is not valid JSON. Possibly empty or server error.")
                #print(f"🔍 Raw response content: {response.text[:200]}...")  # Preview first 200 chars
                return None

            if js:
                self.dataframe = pd.DataFrame.from_dict(js)
            else:
                self.dataframe = pd.DataFrame()  # Empty DataFrame
    ...
    def fetch_properties_for_synthesis(self, synthesis_id: int) -> pd.DataFrame:
        """
        Fetch all properties for a Synthesis object (by objectid) from all 4 property tables.
        Returns a single concatenated DataFrame with consistent columns.
        """
        results = []
        queries = [
            ("float",     f"SELECT 'float'     AS [type], [ObjectId] AS objectid, [Row] AS [row], [PropertyName] AS propertyname, CAST([Value] AS NVARCHAR(MAX)) AS [value], [Comment] AS comment, [SourceObjectId] AS sourceobjectid FROM vroPropertyFloat     WHERE [ObjectId] = {synthesis_id}"),
            ("int",       f"SELECT 'int'       AS [type], [ObjectId] AS objectid, [Row] AS [row], [PropertyName] AS propertyname, CAST([Value] AS NVARCHAR(MAX)) AS [value], [Comment] AS comment, [SourceObjectId] AS sourceobjectid FROM vroPropertyInt       WHERE [ObjectId] = {synthesis_id}"),
            ("string",    f"SELECT 'string'    AS [type], [ObjectId] AS objectid, [Row] AS [row], [PropertyName] AS propertyname, [Value]                          AS [value], [Comment] AS comment, [SourceObjectId] AS sourceobjectid FROM vroPropertyString    WHERE [ObjectId] = {synthesis_id}"),
            ("bigstring", f"SELECT 'bigstring' AS [type], [ObjectId] AS objectid, [Row] AS [row], [PropertyName] AS propertyname, [Value]                          AS [value], [Comment] AS comment, [SourceObjectId] AS sourceobjectid FROM vroPropertyBigString WHERE [ObjectId] = {synthesis_id}"),
        ]
        ...
        if not results:
            return pd.DataFrame(columns=["type","objectid","row","propertyname","value","comment","sourceobjectid"])

        df = pd.concat(results, ignore_index=True)
        return df[["type","objectid","row","propertyname","value","comment","sourceobjectid"]]

  

    def _slugify(self, name: str, default: str = "unnamed") -> str:
        if not name:
            return default
        slug = re.sub(r"[^\w\-]+", "-", name, flags=re.UNICODE).strip("-_")
        return slug or default

    def download_synthesis_with_edx_and_property_table(
        self,
        output_base_dir: str = "synthesis_data",
        skip_txt_edx: bool = True,
    ):
        """
        For each Synthesis <- Sample -> EDX CSV chain:
        - Download EDX file (uses real extension; optionally skip .txt)
        - Save Synthesis properties table (all 4 property types) as CSV
        - Files are grouped under Sample_<sample_id>/
        
        Args:
            output_base_dir (str): Output directory to save data.
            skip_txt_edx (bool): If True, skip samples with .txt EDX files entirely.
        """
        os.makedirs(output_base_dir, exist_ok=True)

        query = """
        SELECT 
            syn.objectid       AS synthesis_id,
            syn.objectname     AS synthesis_name,
            smp.objectid       AS sample_id,
            smp.objectname     AS sample_name,
            edx.objectid       AS edx_id,
            edx.objectname     AS edx_name,
            edx.objectfilepath AS edx_filepath
        FROM vroObjectinfo syn
        JOIN vroTypeinfo tsyn ON syn.typeid = tsyn.typeid
        JOIN vroObjectlinkobject l1 ON l1.linkedobjectid = syn.objectid
        JOIN vroObjectinfo smp ON l1.objectid = smp.objectid
        JOIN vroTypeinfo tsmp ON smp.typeid = tsmp.typeid
        JOIN vroObjectlinkobject l2 ON smp.objectid = l2.objectid
        JOIN vroObjectinfo edx ON l2.linkedobjectid = edx.objectid
        JOIN vroTypeinfo tedx ON edx.typeid = tedx.typeid
        WHERE tsyn.typename = 'Synthesis'
        AND tsmp.typename = 'Sample'
        AND tedx.typename = 'EDX CSV'
        """

        rows = self.execute(query)
        if not rows:
            print("❌ No matching synthesis-sample-edx chains found.")
            return

        df = pd.DataFrame(rows)

        for _, r in df.iterrows():
            synthesis_id   = int(r["synthesis_id"])
            synthesis_name = (r.get("synthesis_name") or f"Synthesis_{synthesis_id}").strip()
            sample_id      = int(r["sample_id"])
            edx_id         = int(r["edx_id"])
            edx_name_raw   = r.get("edx_name") or f"EDX_{edx_id}"
            edx_filepath   = (r.get("edx_filepath") or "").strip()

            # --- Decide based on EDX file extension ---
            _, src_ext = os.path.splitext(edx_filepath)
            src_ext = (src_ext or "").lower()

            if skip_txt_edx and src_ext == ".txt":
                print(f"⏭️  Skipping Sample {sample_id} (EDX {edx_id}) because EDX is .txt; "
                    f"not saving Synthesis {synthesis_id} properties or creating a folder.")
                continue  # Skip EDX and synthesis for this sample

            # ✅ Only create folder if not skipped
            folder = os.path.join(output_base_dir, f"Sample_{sample_id}")
            os.makedirs(folder, exist_ok=True)

            # 1) Download EDX file
            dest_ext = src_ext if src_ext in (".csv", ".txt") else ".csv"
            edx_name = self._slugify(str(edx_name_raw), default=f"EDX_{edx_id}")
            edx_file_path = os.path.join(folder, f"{edx_name}_{edx_id}{dest_ext}")
            try:
                self.download(edx_id, file_name=edx_file_path)
            except Exception as e:
                print(f"⚠️ Failed to download EDX {edx_id}: {e}")

            # 2) Download Synthesis properties
            print(f"Fetching full property table for Synthesis ID: {synthesis_id}")
            props_df = self.fetch_properties_for_synthesis(synthesis_id)
            if not props_df.empty:
                syn_slug = self._slugify(synthesis_name, default=f"Synthesis_{synthesis_id}")
                csv_path = os.path.join(folder, f"{syn_slug}_{synthesis_id}_properties.csv")
                try:
                    props_df.to_csv(csv_path, index=False)
                except Exception as e:
                    print(f"⚠️ Failed to save properties for Synthesis {synthesis_id}: {e}")
            else:
                print("No properties found for this Synthesis.")
