...
"""
            ...
            with patch(
                ...
                with redirect_stdout(buf):
                    ...
                    try:
                        ...
                        session_completed = e.code == 0
                    except Exception:
                        session_completed = False

                # Verify session completed successfully
                self.assertTrue(
                    session_completed, "Session should complete successfully"
                )

                # Verify we got expected output
                output = buf.getvalue()
                self.assertIn("shellpomodoro — great work!", output)
    ...
    def test_error_handling_integration(self):
        ...
        with patch("sys.argv", ["shellpomodoro", "--work", "0"]):
            with patch(
                "shellpomodoro.cli.parse_args",
                side_effect=ValueError("Invalid work duration"),
            ):
                stderr_capture = io.StringIO()
    ...
    def test_memory_and_performance_characteristics(self):
        ...
        # Should complete reasonably quickly (relaxed from 1s to 5s for CI environments)
    ...
    def test_concurrent_session_safety(self):
        ...
        def run_session():
            ...
            with patch(
                "sys.argv", ["shellpomodoro", "--work", "1", "--iterations", "1"]
            ):
                stdout_capture = io.StringIO()
                with redirect_stdout(stdout_capture):
                    with fast_session_patches():
...
class TestEndToEnd(FastPatchedTestCase):
    ...
    def test_default_session_workflow(self):
        ...
        with fast_session_patches():
            with patch("sys.argv", ["shellpomodoro"]):
                ...
                # Verify session completed successfully
                self.assertTrue(
                    session_completed, "Default session should complete successfully"
                )
    ...
    def test_custom_configuration_workflow(self):
        ...
        with fast_session_patches():
            with patch(
                "sys.argv",
                ["shellpomodoro", "--work", "30", "--break", "10", "--iterations", "2"],
            ):
                buf = StringIO()
                with redirect_stdout(buf):
                    from src.shellpomodoro import cli
                ...
                self.assertTrue(
                    session_completed,
                    "Custom configuration session should complete successfully",
                )
