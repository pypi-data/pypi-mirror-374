...
class ModelConstellation:  # pylint: disable = too-few-public-methods
    ...
    def _assemble_freq_plan(self) -> None:
        """Assemble stream for frequencz plan"""
        self.fplan_ts = {
    ...
    def _assemble_moc_time_correlations(self) -> None:
        ...
        self.scet_wrt_tdb_withinitial = compute.scet_wrt_tdb_withinitial(
            self.physics_et,
            self.tps_wrt_tdb,
        ...
        describe_streams_dict(
            self.scet_wrt_tdb_withinitial,
            "SCET with respect to TDB at physics rate [SC_%s]",
        )
        ...
        scet_wrt_tdb_telemetry = {
            sc: downsample_physics_to_telem(self.scet_wrt_tdb_withinitial[sc])
            for sc in SatID.names()
        }
        describe_streams_dict(
            scet_wrt_tdb_telemetry, "SCET with respect to TDB at telemetry rate [SC_%s]"
        )

        self.moc_time_correlations = compute.moc_time_correlations(
            self.moc_time_correlation_noises, scet_wrt_tdb_telemetry
        )
        describe_streams_dict(
    ...
    def _assemble_pseudo_ranging_tps(self) -> None:
        """Assemble streams for measured pseudo-ranging on TPS grid"""

        self.tps_mprs = {
            mosa.value: compute.tps_mprs(
                self.scet_wrt_tps_local[mosa.sat.value],
                self.scet_wrt_tps_distant[mosa.value],
                self.ranging_noises[mosa.value],
            )
            for mosa in MosaID
        }
        describe_streams_dict(
    ...
    def _assemble_orbits(self) -> None:
        ...
        self.tps_wrt_tdb = self._sample_func_sats(orbsrc.tps_wrt_tcb, self.physics_t)
