...
from x_model.field import UInt1Field, UInt2Field, UIntField, UInt8Field
...
class CurEx(BaseModel):
    ...
    minimum: int = UIntField(null=True)  # /10^cur.scale
...
class CoinEx(BaseModel):
    ...
    coin_id: int
    ex: ForeignKeyRelation[Ex] = ForeignKeyField("models.Ex", on_update=CASCADE)
    minimum: int = UIntField(null=True)  # /10^self.scale
    scale: int = UInt1Field()
...
class User(TgUser, TsTrait):
    ...
    async def balances(self, ctd: bool = True) -> dict[int, int | float]:  # ctd - convert to decimal
        dbt = {
            ...
            for c, v, s in await Transaction.filter(receiver=self, status=TS.valid)
        ...
        crd = {
            c: v * 10 ** (-s) if ctd else v
            for c, v, s in await Transaction.filter(sender=self, status=TS.valid)
            .prefetch_related("cur")
            .group_by("cur_id", "cur__scale")
    ...
    async def balance(self, cur_id: int) -> int:
        dbt = (
            await Transaction.filter(receiver=self, cur_id=cur_id, status=TS.valid)
            .group_by("cur_id")
            .annotate(debit=Sum("amount"))
            .values_list("debit", flat=True)
        ) or [0]
        crd = (
            await Transaction.filter(sender_id=self.id or None, cur_id=cur_id, status=TS.valid)
            .group_by("cur_id")
            .annotate(credit=Sum("amount"))
...
class Ad(Model, TsTrait):
    ...
    price: int = UIntField()  # /10^cur.scale
    premium: int = SmallIntField(null=True)  # /10_000
    amount: int = UIntField()  # /10^cur.scale
    quantity: int = UInt8Field(null=True)  # /10^coinex.scale
    min_fiat: int = UIntField()  # /10^cur.scale
    max_fiat: int | None = UIntField(null=True)  # /10^cur.scale
...
class Race(Model):
    ...
    road_id: int
    ceil: int = UIntField()  # /10^cur.scale
    target_place: int = UInt1Field(default=1)
...
class Fiat(Model):
    ...
    cred_id: int
    amount: int = UIntField()  # /10^cur.scale
    target: int = UIntField(null=True)  # /10^cur.scale
    min_deposit: int = UIntField(null=True)  # /10^cur.scale
...
class Limit(Model):
    pmcur: ForeignKeyRelation[PmCur] = ForeignKeyField("models.PmCur", on_update=CASCADE)
    pmcur_id: int
    amount: int = UIntField(null=True)  # '$' if unit >= 0 else 'transactions count'
...
class Transfer(Model):
    amount: int = UIntField()  # /10^cur.scale
    order: OneToOneNullableRelation[Order] = OneToOneField("models.Order", "transfer", on_update=CASCADE, null=True)
    order_id: int
    file: OneToOneNullableRelation["File"] = OneToOneField("models.File", "transfer", on_update=CASCADE, null=True)
    file_id: int
    pmid: int = UInt8Field(null=True)
    created_at: datetime | None = DatetimeSecField(auto_now_add=True)
...
class Msg(Model):
    tg_mid: int = UIntField(null=True)
...
class Credit(Model, TsTrait):
    ...
    amount: int = UIntField(default=None)  # 0 - is all remain borrower balance
