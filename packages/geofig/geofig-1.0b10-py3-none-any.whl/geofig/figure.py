...
class Figure:
    ...
    def __init__(
        ...
        *,
        ...
        padding: Padding = Padding(8, 8, 8, 8),
        scale: int | float = 1,
        title: str | None = None,
    ):
    ...
    def line(self, *args: Segment | Point, **kwargs: Any) -> None:
        ...
        The line can be expressed as either a single `Segment` argument, or two `Point`
        arguments.
        """
    ...
    def save(self, path: str | Path) -> None:
        ...
        transformer.scale(self.scale, -self.scale)
        transformer.translate(
            -self._bounds.xmin * self.scale + self.padding.left,
            self._bounds.ymax * self.scale + self.padding.top,
        )
        width = ceil(
            _n(
                (self._bounds.xmax - self._bounds.xmin) * self.scale
                + self.padding.right
                + self.padding.left
            )
        )
        height = ceil(
            _n(
                (self._bounds.ymax - self._bounds.ymin) * self.scale
                + self.padding.top
                + self.padding.bottom
            )
        )
