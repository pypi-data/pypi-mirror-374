...
from .util import create_headers, TOKENS, CLIENTCONFIG
...
class FiuaiSDK(object):
    ...
    def swith_company(self, company: str, tenant: str = "") -> bool:
    ...
    def internal_get(self, doctype, name):
        res = self.client.get(self.url + '/api/v2/internal/doctype/get', data={"doctype": doctype, "name":name}, headers=self.headers)
        return self.post_process(res)
...
def get_client(username: str, password: str="", auth_type: Literal["internal", "password"]="password")-> FiuaiSDK:
    ...
    if auth_type == "password":
        return FiuaiSDK(
            ...
            password=password,
            auth_type=auth_type,
            max_api_retry=CLIENTCONFIG.max_api_retry,
            timeout=CLIENTCONFIG.timeout,
            verify=CLIENTCONFIG.verify,
            )
