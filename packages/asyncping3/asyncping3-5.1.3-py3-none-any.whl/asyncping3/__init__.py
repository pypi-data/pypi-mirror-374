...
def _debug(*args) -> None:
    ...
    def get_logger():
        ...
        try:
            v = importlib.metadata.version("asyncping3")
...
async def send_one_ping(sock: socket.socket, dest_addr: str, icmp_id: int, seq: int, size: int) -> None:
    ...
    try:
        if is_ipv4(sock):
            ...
            sock_addr = (await anyio.getaddrinfo(dest_addr, 0, family=socket.AF_INET6))[0][4]
    except socket.gaierror as err:
        raise errors.HostUnknown(dest_addr=dest_addr) from err
    _debug("Destination IP address:", sock_addr[0])
...
async def receive_one_ping(sock: socket, icmp_id: int, seq: int, timeout: int) -> float:
    ...
    with anyio.fail_after(timeout):
        return await _receive_one_ping(sock, icmp_id, seq)

async def _receive_one_ping(sock: socket, icmp_id: int, seq: int) -> float:
...
@_async_func_logger
async def ping(dest_addr: str, timeout: int = 4, unit: str = "s", src_addr: str = "", ttl=None, seq: int = 0, size: int = 56, interface: str = "", version=None):
    """
