...
class SkimlyClient:
    ...
    def chat(self, req: Optional[Dict[str, Any]] = None, **kwargs: Any) -> Dict[str, Any]:
        """Chat with provider.
        
        Accepts either a single dict (req) or keyword args (provider, model, messages, etc.).
        """
        payload: Dict[str, Any] = {}
        if req is not None:
            if not isinstance(req, dict):
                raise SkimlyError("chat() expects a dict or keyword args")
            payload.update(req)
        if kwargs:
            payload.update(kwargs)
        # Default to uncompressed format for universal compatibility
        if 'response_format' not in payload:
            payload = {'response_format': 'uncompressed', **payload}
        return self._post("/v1/chat", payload)

    def create_message(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Create a message (sync) — alias of chat(params)."""
        return self.chat(params)

    class _MessagesAdapter:
        def __init__(self, outer: "SkimlyClient"):
            self._outer = outer

        def create(self, params: Dict[str, Any]) -> Dict[str, Any]:
            return self._outer.create_message(params)

    @property
    def messages(self) -> "SkimlyClient._MessagesAdapter":
        return SkimlyClient._MessagesAdapter(self)
