...
class BaseCPDLearner(ABC):
    ...
    def __init__(self, node, data, true_model, learnt_bn_structure, num_parameters,
                 ...
                 early_stopping=True, verbose=True, random_state=42, **kwargs):
        self.node = node
        self.data = data
    ...
    def _prepare_data(self):
        ...
        parents = list(self.learnt_bn_structure.predecessors(self.node))
        ...
        if len(parents) == 0:
            ...
            # Node with parents
            self.X = self.data[parents].values
            self.y = self.data[self.node].values
        
        # Store variable cardinalities
        self.node_card = len(self.data[self.node].unique())
        self.parent_cards = [len(self.data[parent].unique()) for parent in parents] if parents else [1]
    ...
    def get_cpd(self):
        ...
        if len(self.parents) == 0:
            ...
            if hasattr(self.model, 'predict_proba'):
                ...
                try:
                    pred = self.model.predict(np.ones((1, 1)), verbose=0)[0]
                except TypeError:
                    # Sklearn model without verbose parameter
                    pred = self.model.predict(np.ones((1, 1)))[0]
        ...
        cpd_values = cpd_values / cpd_values.sum(axis=0, keepdims=True)
