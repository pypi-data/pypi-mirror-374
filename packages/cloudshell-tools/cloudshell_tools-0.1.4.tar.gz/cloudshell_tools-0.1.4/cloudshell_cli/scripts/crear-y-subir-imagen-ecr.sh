...
read -p "👉 ¿Quieres usar el Dockerfile por defecto? (y/n): " USE_DEFAULT
if [ "$USE_DEFAULT" == "y" ]; then
    SCRIPT_DIR="$(dirname "$0")"
    DOCKERFILE_PATH="$SCRIPT_DIR/../dockerfiles"
    DOCKERFILE="Dockerfile"
else
    read -p "👉 Ingresa la ruta de la carpeta donde está el Dockerfile (ej: ./dockerfiles): " DOCKERFILE_PATH
    read -p "👉 Ingresa el nombre del Dockerfile (ej: Dockerfile): " DOCKERFILE
fi
