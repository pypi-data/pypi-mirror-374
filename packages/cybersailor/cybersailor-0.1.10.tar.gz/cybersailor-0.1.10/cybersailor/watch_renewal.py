...
class WatchRenewal:
    """Monitoring task renewal manager"""
    ...
    def _renewal_worker(self):
        """Renewal worker thread"""
        while self.running:
            # Wait for renewal interval
            for _ in range(self.renewal_interval):
                if not self.running:
                    return
                time.sleep(1)  # Check every second if need to stop
                
            if self.running:  # Check if still need to run
                try:
