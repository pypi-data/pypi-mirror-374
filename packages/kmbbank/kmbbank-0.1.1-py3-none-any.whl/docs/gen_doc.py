...
from kmbbank import kmbbank, kmbbankAsync, CapchaOCR, CapchaProcessing
...
def parse_doc(class_doc):
    ...
    doc_md = {}
    doc_md["name"] = "kmbbank." + class_doc.__qualname__
    for i in docs_pr:
...
with open("./docs/api_document/sync_api.md", "w") as f:
    data = from_list_to_md(parse_doc(kmbbank))
    f.write(data)

with open("./docs/api_document/async_api.md", "w") as f:
    data = from_list_to_md(parse_doc(kmbbankAsync))
    f.write(data)
