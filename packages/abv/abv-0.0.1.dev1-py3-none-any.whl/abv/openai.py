"""If you use the OpenAI Python SDK, you can use the ABV drop-in replacement to get full logging by changing only the import.

```diff
- import openai
+ from abv.openai import openai
```
...
See docs for more details: https://abv.dev/docs/integrations/openai
"""
...
from abv._client.span import ABVGeneration
from abv._utils import _get_timestamp
from abv.media import ABVMedia
...
class OpenAiArgsExtractor:
    def __init__(
        ...
        abv_public_key: Optional[str] = None,
        ...
        **kwargs: Any,
        ...
        self.args["name"] = name
        self.args["abv_public_key"] = abv_public_key
        self.args["abv_prompt"] = abv_prompt
        self.args["trace_id"] = trace_id
        self.args["parent_observation_id"] = parent_observation_id

        self.kwargs = kwargs

    def get_abv_args(self) -> Any:
...
def _abv_wrapper(func: Any) -> Any:
    def _with_abv(open_ai_definitions: Any) -> Any:
...
def _extract_chat_prompt(kwargs: Any) -> Any:
    ...
    if prompt:
        # uf user provided functions, we need to send these together with messages to abv
        prompt.update(
            {
                "messages": [
        ...
        # vanilla case, only send messages in openai format to abv
                ...
                processed_content.append(
                    {
                        "type": "input_audio",
                        "input_audio": {
                            "data": ABVMedia(base64_data_uri=base64_data_uri),
                            "format": format,
                        },
                    }
                )
...
def _extract_chat_response(kwargs: Any) -> Any:
    ...
    if kwargs.get("audio") is not None:
        ...
        if "data" in audio and audio["data"] is not None:
            ...
            audio["data"] = ABVMedia(base64_data_uri=base64_data_uri)
...
def _get_abv_data_from_kwargs(resource: OpenAiDefinition, kwargs: Any) -> Any:
    ...
    abv_public_key = kwargs.get("abv_public_key", None)
    if abv_public_key is not None and not isinstance(abv_public_key, str):
        raise TypeError("abv_public_key must be a string")
    ...
    abv_prompt = kwargs.get("abv_prompt", None)

    return {
        "name": name,
        "metadata": metadata,
        "abv_public_key": abv_public_key,
        "trace_id": trace_id,
...
def _get_abv_data_from_default_response(
...
def _wrap(
    ...
    abv_args = arg_extractor.get_abv_args()

    abv_data = _get_abv_data_from_kwargs(open_ai_resource, abv_args)
    abv_client = get_client(public_key=abv_args["abv_public_key"])

    generation = abv_client.start_observation(
        as_type="generation",
        name=abv_data["name"],
        input=abv_data.get("input", None),
        metadata=abv_data.get("metadata", None),
        model_parameters=abv_data.get("model_parameters", None),
        trace_context={
            "trace_id": cast(str, abv_data.get("trace_id", None)),
            "parent_span_id": cast(
                str, abv_data.get("parent_observation_id", None)
            ),
        },
        model=abv_data.get("model", None),
        prompt=abv_data.get("prompt", None),
    )
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            return ABVResponseGeneratorSync(
            ...
            model, completion, usage = _get_abv_data_from_default_response(
                open_ai_resource,
...
async def _wrap_async(
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            return ABVResponseGeneratorAsync(
                resource=open_ai_resource,
                response=openai_response,
                generation=generation,
            )
...
class ABVResponseGeneratorSync:
    def __init__(
        self,
        *,
        resource: Any,
...
class ABVResponseGeneratorAsync:
    def __init__(
        self,
        *,
        resource: Any,
