...
from abv._client.get_client import get_client
from abv._client.attributes import ABVOtelSpanAttributes
from abv._client.span import (
    ABVGeneration,
    ABVSpan,
    ABVAgent,
    ABVChain,
...
from abv._utils import _get_timestamp
from abv.langchain.utils import _extract_model_name
...
try:
    ...
    raise ModuleNotFoundError(
        "Please install langchain to use the ABV langchain integration: 'pip install langchain'"
    )
...
class LangchainCallbackHandler(LangchainBaseCallbackHandler):
    def __init__(
        ...
        Args:
            public_key: Optional ABV public key. If not provided, will use the default client configuration.
            update_trace: Whether to update the ABV trace with the chains input / output / metadata / name. Defaults to False.
        """
    ...
    def on_llm_new_token(
        ...
        **kwargs: Any,
        ...
        if (
            run_id in self.runs
            and isinstance(self.runs[run_id], ABVGeneration)
            and run_id not in self.updated_completion_start_time_memo
        ):
            current_generation = cast(ABVGeneration, self.runs[run_id])
    ...
    def _get_observation_type_from_serialized(
        ...
        """Determine ABV observation type from LangChain component.
        ...
        Returns:
            The appropriate ABV observation type string
        """
    ...
    def on_retriever_error(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            abv_logger.exception(e)

    def _parse_abv_trace_attributes_from_metadata(
        self,
        ...
        if "abv_session_id" in metadata and isinstance(
            metadata["abv_session_id"], str
        ):
            attributes["session_id"] = metadata["abv_session_id"]

        if "abv_user_id" in metadata and isinstance(
            metadata["abv_user_id"], str
        ):
            attributes["user_id"] = metadata["abv_user_id"]

        if "abv_tags" in metadata and isinstance(metadata["abv_tags"], list):
            attributes["tags"] = [str(tag) for tag in metadata["abv_tags"]]
        ...
        try:
            ...
            if parent_run_id is None:
                ...
                span.update_trace(
                    ...
                    **self._parse_abv_trace_attributes_from_metadata(metadata),
                )
    ...
    def _register_abv_prompt(
        ...
        *,
        ...
        """We need to register any passed ABV prompt to the parent_run_id so that we can link following generations with that prompt.
        ...
        abv_prompt = metadata and metadata.get("abv_prompt", None)

        if abv_prompt:
            self.prompt_to_parent_run_map[parent_run_id] = abv_prompt
    ...
    def _deregister_abv_prompt(self, run_id: Optional[UUID]) -> None:
        ...
        try:
            ...
            if hasattr(agent_run, "_otel_span"):
                agent_run._otel_span.set_attribute(
                    ABVOtelSpanAttributes.OBSERVATION_TYPE, "agent"
                )
    ...
    def on_chain_end(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self._deregister_abv_prompt(run_id)
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if parent_run_id is None or parent_run_id not in self.runs:
                ...
                self.runs[run_id] = cast(
                    ABVChain, self.runs[parent_run_id]
                ).start_observation(
                    name=self.get_langchain_run_name(serialized, **kwargs),
                    as_type=observation_type,
                    input=input_str,
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if parent_run_id is None:
                ...
                self.runs[run_id] = cast(
                    ABVRetriever, self.runs[parent_run_id]
                ).start_observation(
                    name=span_name,
                    as_type=observation_type,
                    input=query,
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if registered_prompt:
                self._deregister_abv_prompt(parent_run_id)
            ...
            content = {
                ...
                "metadata": self.__join_tags_and_metadata(
                    ...
                    keep_abv_trace_attributes=True
                    if parent_run_id is None
                    else False,
                ),
                "model": model_name,
            ...
            if parent_run_id is not None and parent_run_id in self.runs:
                self.runs[run_id] = cast(
                    ABVGeneration, self.runs[parent_run_id]
    ...
    def _log_model_parse_warning(self) -> None:
        if not hasattr(self, "_model_parse_warning_logged"):
            abv_logger.warning(
                "ABV was not able to parse the LLM model. The LLM call will be recorded without model name. Please create an issue: https://github.com/abv/abv/issues/new/choose"
            )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if run_id not in self.runs:
                ...
                abv_generation = cast(ABVGeneration, self.runs[run_id])
                abv_generation.update(
    ...
    def __join_tags_and_metadata(
        ...
        tags: Optional[List[str]] = None,
        metadata: Optional[Dict[str, Any]] = None,
        keep_abv_trace_attributes: bool = False,
        ...
        return (
            _strip_abv_keys_from_dict(final_dict, keep_abv_trace_attributes)
            if final_dict != {}
            else None
        )
...
def _parse_usage_model(usage: typing.Union[pydantic.BaseModel, dict]) -> Any:
    ...
    for model_key, abv_key in conversion_list:
        if model_key in usage_model:
            ...
            usage_model[abv_key] = final_count  # Translate key and keep the value
...
def _strip_abv_keys_from_dict(
    metadata: Optional[Dict[str, Any]], keep_abv_trace_attributes: bool
) -> Any:
    if metadata is None or not isinstance(metadata, dict):
        return metadata
    ...
    for key in abv_metadata_keys:
        metadata_copy.pop(key, None)

    if not keep_abv_trace_attributes:
        for key in abv_trace_attribute_keys:
            metadata_copy.pop(key, None)
