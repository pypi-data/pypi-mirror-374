"""OTEL span wrapper for ABV.

This module defines custom span classes that extend OpenTelemetry spans with
ABV-specific functionality. These wrapper classes provide methods for
creating, updating, and scoring various types of spans used in AI application tracing.

Classes:
- ABVObservationWrapper: Abstract base class for all ABV spans
- ABVSpan: Implementation for general-purpose spans
- ABVGeneration: Specialized span implementation for LLM generations

All span classes provide methods for media processing, attribute management,
and scoring integration specific to ABV's observability platform.
"""
...
from abv.model import PromptClient

if TYPE_CHECKING:
    from abv._client.client import ABV

from abv._client.attributes import (
    ABVOtelSpanAttributes,
    create_generation_attributes,
    create_span_attributes,
    create_trace_attributes,
)
from abv._client.constants import (
...
from abv.logger import abv_logger
from abv.types import MapValue, ScoreDataType, SpanLevel
...
_OBSERVATION_CLASS_MAP: Dict[str, Type["ABVObservationWrapper"]] = {}


class ABVObservationWrapper:
    """Abstract base class for all ABV span types.

    This class provides common functionality for all ABV span types, including
    media processing, attribute management, and scoring. It wraps an OpenTelemetry
    span and extends it with ABV-specific features.

    Attributes:
        _otel_span: The underlying OpenTelemetry span
        _abv_client: Reference to the parent ABV client
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new ABV span wrapper.

        Args:
            otel_span: The OpenTelemetry span to wrap
            abv_client: Reference to the parent ABV client
        ...
        self._otel_span.set_attribute(
            ABVOtelSpanAttributes.OBSERVATION_TYPE, as_type
        )
        self._abv_client = abv_client
        self._observation_type = as_type

        self.trace_id = self._abv_client._get_otel_trace_id(otel_span)
        self.id = self._abv_client._get_otel_span_id(otel_span)

        self._environment = environment
        if self._environment is not None:
            self._otel_span.set_attribute(
                ABVOtelSpanAttributes.ENVIRONMENT, self._environment
            )
        ...
        if self._otel_span.is_recording():
            ...
            attributes.pop(ABVOtelSpanAttributes.OBSERVATION_TYPE, None)

            self._otel_span.set_attributes(
                {k: v for k, v in attributes.items() if v is not None}
            )

    def end(self, *, end_time: Optional[int] = None) -> "ABVObservationWrapper":
    ...
    def _mask_attribute(self, *, data: Any) -> Any:
        ...
        """
        if not self._abv_client._mask:
            return data

        try:
            return self._abv_client._mask(data=data)
    ...
    def _process_media_in_attribute(
        ...
        *,
        ...
        """
        if self._abv_client._resources is not None:
            return (
                self._abv_client._resources._media_manager._find_and_process_media(
                    data=data,
                    field=field,
                    trace_id=self.trace_id,
                    observation_id=self.id,
    ...
    def start_observation(
        ...
        *,
        ...
        if as_type == "event":
            timestamp = time_ns()
            event_span = self._abv_client._otel_tracer.start_span(
        ...
        if not observation_class:
            abv_logger.warning(
                f"Unknown observation type: {as_type}, falling back to ABVSpan"
            )
            observation_class = ABVSpan

        with otel_trace_api.use_span(self._otel_span):
            new_otel_span = self._abv_client._otel_tracer.start_span(name=name)
    ...
    def start_as_current_observation(  # type: ignore[misc]
        ...
        *,
        ...
        """
        return self._abv_client._create_span_with_parent_context(
            name=name,
            as_type=as_type,
...
class ABVSpan(ABVObservationWrapper):
    """Standard span implementation for general operations in ABV.

    This class represents a general-purpose span that can be used to trace
    any operation in your application. It extends the base ABVObservationWrapper
    ...
    def start_span(
        ...
        Returns:
            A new ABVSpan that must be ended with .end() when complete

        Example:
            ```python
            parent_span = abv.start_span(name="process-request")
            try:
                # Create a child span
    ...
    def start_as_current_span(
        ...
        *,
        ...
        Returns:
            A context manager that yields a new ABVSpan

        Example:
            ```python
            with abv.start_as_current_span(name="process-request") as parent_span:
    ...
    def start_generation(
        ...
        *,
        ...
        Returns:
            A new ABVGeneration that must be ended with .end() when complete

        Example:
            ```python
            span = abv.start_span(name="process-query")
            try:
    ...
    def start_as_current_generation(
        ...
        *,
        ...
        Returns:
            A context manager that yields a new ABVGeneration
    ...
    def create_event(
        ...
        *,
        ...
        with otel_trace_api.use_span(self._otel_span):
            new_otel_span = self._abv_client._otel_tracer.start_span(
                name=name, start_time=timestamp
            )
...
class ABVGeneration(ABVObservationWrapper):
    """Specialized span implementation for AI model generations in ABV.

    This class represents a generation span specifically designed for tracking
    AI/LLM operations. It extends the base ABVObservationWrapper with specialized
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new ABVGeneration span.
...
class ABVEvent(ABVObservationWrapper):
    """Specialized span implementation for ABV Events."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new ABVEvent span.
...
_OBSERVATION_CLASS_MAP.update(
    {
        ...
        "tool": ABVTool,
        "chain": ABVChain,
        "retriever": ABVRetriever,
        "evaluator": ABVEvaluator,
        "embedding": ABVEmbedding,
        "guardrail": ABVGuardrail,
    }
)
