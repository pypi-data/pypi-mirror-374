"""Tracer implementation for ABV OpenTelemetry integration.

This module provides the ABVTracer class, a thread-safe singleton that manages OpenTelemetry
tracing infrastructure for ABV. It handles tracer initialization, span processors,
API clients, and coordinates background tasks for efficient data processing and media handling.

Key features:
- Thread-safe OpenTelemetry tracer with ABV-specific span processors and sampling
...
from abv._client.constants import ABV_TRACER_NAME
from abv._client.environment_variables import (
    ABV_MEDIA_UPLOAD_ENABLED,
    ABV_MEDIA_UPLOAD_THREAD_COUNT,
    ABV_RELEASE,
    ABV_TRACING_ENVIRONMENT,
)
from abv._client.span_processor import ABVSpanProcessor
from abv._task_manager.media_manager import MediaManager
from abv._task_manager.media_upload_consumer import MediaUploadConsumer
from abv._task_manager.score_ingestion_consumer import ScoreIngestionConsumer
from abv._utils.environment import get_common_release_envs
from abv._utils.prompt_cache import PromptCache
from abv._utils.request import ABVClient
from abv.api.client import AsyncFernABV, FernABV
from abv.logger import abv_logger
from abv.types import MaskFunction

from ..version import __version__ as abv_version


class ABVResourceManager:
    ...
    2. Managing the span processor for exporting spans to the ABV API
    3. Creating and managing ABV API clients (both synchronous and asynchronous)
    ...
    _instances: Dict[str, "ABVResourceManager"] = {}
        ...
        *,
        ...
        with cls._lock:
            if public_key not in cls._instances:
                instance = super(ABVResourceManager, cls).__new__(cls)
    ...
    def _initialize_instance(
        ...
        *,
        ...
        if tracing_enabled:
            ...
            abv_processor = ABVSpanProcessor(
            ...
            tracer_provider.add_span_processor(abv_processor)
        ...
        self.api = FernABV(
            ...
            x_abv_public_key=self.public_key,
            httpx_client=self.httpx_client,
            timeout=timeout,
        )
        self.async_api = AsyncFernABV(
            base_url=host,
            username=self.public_key,
            password=secret_key,
            x_abv_sdk_name="python",
        ...
        score_ingestion_client = ABVClient(
            public_key=self.public_key,
            secret_key=secret_key,
            base_url=host,
            version=abv_version,
        ...
        media_upload_thread_count = media_upload_thread_count or max(
            int(os.getenv(ABV_MEDIA_UPLOAD_THREAD_COUNT, 1)), 1
        )
        ...
        abv_logger.info(
            f"Startup: ABV tracer successfully initialized | "
    ...
    def flush(self) -> None:
        ...
        abv_logger.debug("Successfully flushed OTEL tracer provider")

        self._score_ingestion_queue.join()
        abv_logger.debug("Successfully flushed score ingestion queue")

        self._media_upload_queue.join()
        abv_logger.debug("Successfully flushed media upload queue")
...
def _init_tracer_provider(
    *,
    ...
    environment = environment or os.environ.get(ABV_TRACING_ENVIRONMENT)
    release = release or os.environ.get(ABV_RELEASE) or get_common_release_envs()

    resource_attributes = {
        ABVOtelSpanAttributes.ENVIRONMENT: environment,
