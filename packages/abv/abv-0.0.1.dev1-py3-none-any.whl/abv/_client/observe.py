...
from abv._client.get_client import _set_current_public_key, get_client
...
from abv.types import TraceContext
...
class ABVDecorator:
    """Implementation of the @observe decorator for seamless ABV tracing integration.

    This class provides the core functionality for the @observe decorator, which enables
    automatic tracing of functions and methods in your application with ABV.
    It handles both synchronous and asynchronous functions, maintains proper trace context,
    and intelligently routes to the correct ABV client instance.
    ...
    - Thread-safe client resolution when multiple ABV projects are used
    """

    _log = logging.getLogger("abv")
    ...
    def observe(
        ...
        *,
        ...
        """Wrap a function to create and manage ABV tracing around its execution, supporting both synchronous and asynchronous functions.

        This decorator provides seamless integration of ABV observability into your codebase. It automatically creates
        ...
        Args:
            ...
            as_type (Optional[Literal]): Set the observation type. Supported values:
                    ...
                    Observation types are highlighted in the ABV UI for filtering and visualization.
                    The types "generation" and "embedding" create a span on which additional attributes such as model metrics
                    can be set.

        Returns:
            Callable: A wrapped version of the original function that automatically creates and manages ABV spans.
        ...
        Notes:
            ...
            - Special keyword arguments can be passed to control tracing:
              - abv_trace_id: Explicitly set the trace ID for this function call
              - abv_parent_observation_id: Explicitly set the parent span ID
              - abv_public_key: Use a specific ABV project (when multiple clients exist)
        ...
        function_io_capture_enabled = os.environ.get(
            ABV_OBSERVE_DECORATOR_IO_CAPTURE_ENABLED, "True"
    ...
    def _async_observe(
        ...
        *,
        ...
        async def async_wrapper(*args: Tuple[Any], **kwargs: Dict[str, Any]) -> Any:
            trace_id = cast(str, kwargs.pop("abv_trace_id", None))
            parent_observation_id = cast(
                str, kwargs.pop("abv_parent_observation_id", None)
            )
            ...
            with _set_current_public_key(public_key):
                ...
                context_manager: Optional[
                    ...
                    abv_client.start_as_current_observation(
                        name=final_name,
                ...
                with context_manager as abv_span_or_generation:
                    ...
                    try:
                        ...
                        if capture_output is True:
                            ...
                            abv_span_or_generation.update(output=result)

                        return result
                    except Exception as e:
                        abv_span_or_generation.update(
    ...
    def _sync_observe(
        ...
        *,
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            trace_id = kwargs.pop("abv_trace_id", None)
            parent_observation_id = kwargs.pop("abv_parent_observation_id", None)
            trace_context: Optional[TraceContext] = (
                {
                    "trace_id": trace_id,
                    "parent_span_id": parent_observation_id,
            ...
            public_key = kwargs.pop("abv_public_key", None)
    ...
    def _wrap_sync_generator_result(
        ...
        try:
            ...
            abv_span_or_generation.update(output=output)
            abv_span_or_generation.end()
