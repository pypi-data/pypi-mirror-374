...
# Context variable to track the current abv_public_key in execution context
_current_public_key: ContextVar[Optional[str]] = ContextVar(
    "abv_public_key", default=None
)
...
def get_client(*, public_key: Optional[str] = None) -> ABV:
    ...
    providing a public_key is required. Multi-project support is experimental - see ABV docs.
    ...
    with ABVResourceManager._lock:
        ...
        if not public_key:
            ...
            if len(active_instances) == 1:
                ...
                abv_logger.warning(
                    "No 'abv_public_key' passed to decorated function, but multiple abv clients are instantiated in current process. Skipping tracing for this function to avoid cross-project leakage."
                )
                return ABV(
            ...
            if target_instance is None:
                ...
                abv_logger.warning(
                    f"No ABV client with public key {public_key} has been initialized. Skipping tracing for decorated function."
                )
                return ABV(
                    tracing_enabled=False, public_key="fake", secret_key="fake"
                )
