...
from abv._client.datasets import DatasetClient, DatasetItemClient
...
from abv._client.resource_manager import ABVResourceManager
...
from abv._utils.parse_error import handle_fern_exception
from abv._utils.prompt_cache import PromptCache
from abv.api.resources.commons.errors.error import Error
from abv.api.resources.ingestion.types.score_body import ScoreBody
from abv.api.resources.prompts.types import (
...
from abv.types import MaskFunction, ScoreDataType, SpanLevel, TraceContext
...
class ABV:
    ...
    Parameters:
        public_key (Optional[str]): Your ABV public API key. Can also be set via ABV_PUBLIC_KEY environment variable.
        secret_key (Optional[str]): Your ABV secret API key. Can also be set via ABV_SECRET_KEY environment variable.
        host (Optional[str]): The ABV API host URL. Defaults to "https://app.abv.dev". Can also be set via ABV_HOST environment variable.
    ...
    def __init__(
        ...
        *,
        ...
        self._host = host or cast(
            str, os.environ.get(ABV_HOST, "https://app.abv.dev")
        )
        ...
        public_key = public_key or os.environ.get(ABV_PUBLIC_KEY)
        if public_key is None:
            abv_logger.warning(
                "Authentication error: ABV client initialized without public_key. Client will be disabled. "
                "Provide a public_key parameter or set ABV_PUBLIC_KEY environment variable. "
            )
            self._otel_tracer = otel_trace_api.NoOpTracer()
            return

        secret_key = secret_key or os.environ.get(ABV_SECRET_KEY)
        if secret_key is None:
            abv_logger.warning(
                "Authentication error: ABV client initialized without secret_key. Client will be disabled. "
                "Provide a secret_key parameter or set ABV_SECRET_KEY environment variable. "
            )
            self._otel_tracer = otel_trace_api.NoOpTracer()
            return
