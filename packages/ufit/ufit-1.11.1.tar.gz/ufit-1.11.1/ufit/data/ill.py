...
def read_data(filename, fp):
    ...
    while line.strip() != 'DATA_:':
        ...
        elif line.startswith('DATE_:'):
            meta['date'] = line[6:].strip()
        elif line.startswith('USER_:'):
            meta['users'] = line[6:].strip()
    ...
    while True:
        if len(line) == 81 and len(set(line)) == 2:
            ...
            if section == 1:
                ...
                # experiment, user, date
                fp.readline()
                line = fp.readline()
                parts = line.strip('\x00').split(None, 3)
                meta['instrument'] = parts[0]
                if len(parts) > 1:
                    meta['users'] = parts[1]
                meta['date'] = line.strip('\x00')[14:].rstrip()
            elif section == 3:
                # experiment title (short) and "scan type" (can be empty)
                fp.readline()
                line = fp.readline()
                meta['title'] = line[:72].strip()
                meta['experiment'] = meta['title']
                if len(line) >= 72:
