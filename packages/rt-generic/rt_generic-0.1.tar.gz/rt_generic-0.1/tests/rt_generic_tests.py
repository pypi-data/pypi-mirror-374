...
class E_Cls1(RTGeneric, Generic[T, U, V]):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        utype = cls.tv2type(E_Cls1, U)  # type: ignore[misc] # mypy can't imagine any use for passing in a TypeVar
        if utype == TypeErrorT:
            # print(f"\tdefering until we have a subclass without remaining generics")
            return
        if cls.generic_true(utype):
...
class T_Cls(unittest.TestCase):
    ...
    def test_10_background_RT(self):
        ...
        self.assertEqual(c1.generic_true(TrueT), True)
        self.assertEqual(c1.generic_true(Literal["True"]), True)
        self.assertEqual(c1.generic_true(Literal["Yes"]), True)
        self.assertEqual(c1.generic_true(Literal["true"]), True)
        self.assertEqual(c1.generic_true(Literal["yes"]), True)
        self.assertEqual(c1.generic_true(Literal["T"]), True)
        self.assertEqual(c1.generic_true(Literal["Y"]), True)
        self.assertEqual(c1.generic_true(Literal["t"]), True)
        self.assertEqual(c1.generic_true(Literal["y"]), True)
        self.assertEqual(c1.generic_true(Literal[1]), True)
        self.assertEqual(c1.generic_false(TrueT), False)
        self.assertEqual(c1.generic_false(Literal["True"]), False)
        self.assertEqual(c1.generic_false(Literal["Yes"]), False)
        self.assertEqual(c1.generic_false(Literal["true"]), False)
        self.assertEqual(c1.generic_false(Literal["yes"]), False)
        self.assertEqual(c1.generic_false(Literal["T"]), False)
        self.assertEqual(c1.generic_false(Literal["Y"]), False)
        self.assertEqual(c1.generic_false(Literal["t"]), False)
        self.assertEqual(c1.generic_false(Literal["y"]), False)
        self.assertEqual(c1.generic_false(Literal[1]), False)

        self.assertEqual(c1.generic_true(FalseT), False)
        self.assertEqual(c1.generic_true(Literal["False"]), False)
        self.assertEqual(c1.generic_true(Literal["No"]), False)
        self.assertEqual(c1.generic_true(Literal["false"]), False)
        self.assertEqual(c1.generic_true(Literal["no"]), False)
        self.assertEqual(c1.generic_true(Literal["F"]), False)
        self.assertEqual(c1.generic_true(Literal["N"]), False)
        self.assertEqual(c1.generic_true(Literal["f"]), False)
        self.assertEqual(c1.generic_true(Literal["n"]), False)
        self.assertEqual(c1.generic_true(Literal[0]), False)
        self.assertEqual(c1.generic_false(FalseT), True)
        self.assertEqual(c1.generic_false(Literal["False"]), True)
        self.assertEqual(c1.generic_false(Literal["No"]), True)
        self.assertEqual(c1.generic_false(Literal["false"]), True)
        self.assertEqual(c1.generic_false(Literal["no"]), True)
        self.assertEqual(c1.generic_false(Literal["F"]), True)
        self.assertEqual(c1.generic_false(Literal["N"]), True)
        self.assertEqual(c1.generic_false(Literal["f"]), True)
        self.assertEqual(c1.generic_false(Literal["n"]), True)
        self.assertEqual(c1.generic_false(Literal[0]), True)

        self.assertEqual(c1.generic_lit_values(Literal[5, "abc"]), (5, "abc"))

    def test_11_Base_Class(self):
        c1 = E_Cls1(None, None, None)
        self.assertEqual(c1.fn0(), ("", None, None, None))
        self.assertEqual(
            c1.fn1(),
            (
                TypeErrorT,
        ...
        self.assertEqual(
            "'E_Cls1' object has no attribute 'fn2'", str(context.exception)
        )
    ...
    def test_12_E_Cls3(self):
        ...
        self.assertEqual(
            "'E_Cls3' object has no attribute 'fn2'", str(context.exception)
        )
    ...
    def test_13_E_Cls4(self):
        ...
        self.assertEqual(
            "'E_Cls4' object has no attribute 'fn2'", str(context.exception)
        )
