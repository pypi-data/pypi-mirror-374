...
from .api_service_utils import reset_agent_client_services, reset_agent_agent_clients
...
@agent_router.post("/for_agent/{agent_id}/reset_client_services_file", response_model=APIResponse)
@handle_exceptions
async def agent_reset_client_services_file(agent_id: str):
    """Agent 级别重置 client_services 文件"""
    validate_agent_id(agent_id)
    store = get_store()
    return await reset_agent_client_services(store, agent_id)

@agent_router.post("/for_agent/{agent_id}/reset_agent_clients_file", response_model=APIResponse)
@handle_exceptions
async def agent_reset_agent_clients_file(agent_id: str):
    """Agent 级别重置 agent_clients 文件"""
    validate_agent_id(agent_id)
    store = get_store()
    return await reset_agent_agent_clients(store, agent_id)
