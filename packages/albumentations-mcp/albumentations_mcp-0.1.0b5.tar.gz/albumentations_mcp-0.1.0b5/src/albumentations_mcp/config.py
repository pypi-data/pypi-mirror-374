...
def validate_environment_variables() -> Dict[str, Any]:
    ...
    if strict_mode_str in ("true", "1", "yes", "on"):
        ...
        errors.append(
            f"STRICT_MODE must be true/false, got: {strict_mode_str}"
        )
    ...
    try:
        ...
        if max_bytes < 1024:  # 1KB minimum
            ...
            errors.append(
                "MAX_BYTES_IN must be at most 1GB (1073741824 bytes)"
            )
        else:
    ...
    if log_level not in valid_levels:
        errors.append(
            f"MCP_LOG_LEVEL must be one of {valid_levels}, got: {log_level}"
        )
    else:
    ...
    if default_seed_str is not None:
        try:
            ...
            errors.append(
                f"DEFAULT_SEED must be an integer, got: {default_seed_str}"
            )
    ...
    try:
        max_security_length = int(
            os.getenv("MAX_SECURITY_CHECK_LENGTH", "10000000")
        )
        if max_security_length < 1000:
            errors.append(
                "MAX_SECURITY_CHECK_LENGTH must be at least 1000 characters"
            )
