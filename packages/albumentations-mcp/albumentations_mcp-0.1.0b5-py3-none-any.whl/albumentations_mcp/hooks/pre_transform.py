...
class PreTransformHook(BaseHook):
        ...
        try:
            ...
            if needs_resize:
                ...
                if config["STRICT_MODE"]:
                    ...
                    resized_image, resize_info = self._resize_image(
                        image, context
                    )
                    if resized_image:
            ...
            size_warnings = check_image_size_warnings(
    ...
    def _analyze_transform(
        ...
        if transform_name in ["Blur", "GaussianBlur", "MotionBlur"]:
            ...
            if var_limit:
                if (
                    isinstance(var_limit, (list, tuple))
                    and len(var_limit) >= 2
                ):
    ...
    def _check_resize_needed(
        ...
        if pixels > config["MAX_PIXELS_IN"]:
            reasons.append(
                f"total pixels {pixels:,} > {config['MAX_PIXELS_IN']:,}"
            )
