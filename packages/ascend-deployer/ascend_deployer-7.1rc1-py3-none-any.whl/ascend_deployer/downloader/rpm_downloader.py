...
class Yum(object):
    ...
    def __init__(self, source_file, arch):
        ...
        self.db_tmps = os.path.join(os.path.realpath(self.config_dir), "db_tmps")
        self.installed_file = os.path.join(os.path.realpath(self.config_dir), 'installed.txt')
    ...
    @staticmethod
    def download_file(url, dst_file, sha256=""):
        """
        download_file
        ...
        try:
            ...
            if DOWNLOAD_INST.download(url, dst_file, sha256=sha256):
                return True
            return False
    ...
    def make_cache(self):
        ...
        for name, url in self.sources.items():
            ...
            print(f"analysis results: source name: {name}")
    ...
    def download_with_dep(self, name, dst_dir, ver=None, rel=None):
        ...
        for name, pkg in self.to_be_download.items():
            ...
            if not self.need_download_again(pkg.checksum, dst_file):
                ...
                continue
            print(f"analysis results: filename: {file_name}")
            LOG.info(f'{file_name} download from {url}')
            res.append(self.download_file(url, dst_file))
            self.downloaded.append(name)
    ...
    def download_without_dep(self, name, dst_dir, ver=None, release=None):
        ...
        print(f"analysis results: filename: {file_name}")
        LOG.info(f'{file_name} download from {url}')
        if self.download_file(url, dst_file, sha256=pkg.checksum):
            print(file_name.ljust(60), 'download success')
            return True
        return False
    ...
    def download(self, pkg, dst_dir):
        ...
        if pkg.get('url'):
            ...
            if checksum and not self.need_download_again(checksum, dst_file):
                print(file_name.ljust(60), 'exists')
                return True
            if self.download_file(pkg['url'], dst_file, checksum):
                print(file_name.ljust(60), 'download success')
                return True
            print(file_name.ljust(60), 'download failed')
            raise RuntimeError

        if pkg.get('autodependency', '') == 'true':
