...
def generate_openapi(entrypoint: str = os.environ.get(ENTRYPOINT_ENV, DEFAULT_ENTRYPOINT),
    ...
    parameter_schemas, parameter_schema_definitions, required_parameters = generate_parameter_schema(entrypoint_signature)
    ...
    openapi = populate_openapi_template(openapi_template, parameter_schemas, return_schema, schema_definitions, required_parameters, title, version)
...
def populate_openapi_template(openapi_template: Dict[str, Any],
    ...
    if input_schema:
        ...
        if required_parameters:
            openapi["paths"]["/"]["post"]["requestBody"]["content"]["application/json"]["schema"]["required"] = required_parameters
    else:
