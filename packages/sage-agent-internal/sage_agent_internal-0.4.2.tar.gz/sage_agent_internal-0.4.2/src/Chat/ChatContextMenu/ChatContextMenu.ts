...
export class ChatContextMenu {
  ...
  private async refreshContextItems(categoryId: string): Promise<void> {
    // Show loading state while refreshing category
    if (this.currentView === 'items' && this.selectedCategory === categoryId) {
      this.showLoadingState(`Loading ${categoryId}...`);
    }

    try {
      switch (categoryId) {
        case 'data':
          const dataItems = await this.contextLoaders.loadDatasets(this.currentDataPath);
  ...
  private async renderCategoryItems(
    ...
    // Load fresh data for the category if needed
    if (categoryId === 'data') {
      await this.refreshContextItems('data');
    }
    ...
    // For data category, separate directories and files, and show directories first
    if (categoryId === 'data') {
      ...
      displayDirectories.forEach(item => {
        const itemElement = createDropdownItemElement(
          item,
