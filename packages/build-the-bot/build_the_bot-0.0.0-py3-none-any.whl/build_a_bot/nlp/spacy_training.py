...
def load_examples_from_json_file(json_file_path):
    """
...
def convert_user_assistant_examples_to_docs(nlp, examples, entity_names):
    ...
    [{"role":"user", "content": "user message with entities"}, {"role":"assistant":"{"entity_1": "entity_1_value", "entity_2": "entity_2_value"}"]
    :param [str] entity_names: list of entity names for the model to recognize. Names must match the entities defined in the json provided.
        ...
        for entity_name in entity_names:
            if entity_name in entity_dict:
                ...
                if entity_span is None:
                    print(f"Skipping {entity_name} span: {user_message}")
                else:
