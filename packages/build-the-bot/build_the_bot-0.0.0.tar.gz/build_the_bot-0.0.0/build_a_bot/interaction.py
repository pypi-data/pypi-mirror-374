...
class Interaction(ABC):
    ...
    async def send_message(self, message, channel_id, thread_id=None):
        ...
        :param str thread_id: uuid of the thread the user is interacting from. None, if messaging without a thread (dms).
        """
...
class SlackInteraction(Interaction):
    ...
    async def send_message(self, message, channel_id, thread_id=None):
        ...
        :param str thread_id: Slack uuid of the thread the user is interacting from. None, if messaging without a thread (dms).
        """
        ...
        if messages_to_send > 0:
            for i in range(messages_to_send):
                await self.client.chat_postMessage(
                    text="```" + message[init_value : (((i + 1) * max_char) - 1)] + "```", channel=channel_id, thread_ts=thread_id
                )
    ...
    async def send_buttons(self, title, buttons: dict, channel_id, thread_id=None, emoji=False):
        ...
        await self.client.chat_postMessage(
            ...
            blocks=[
                SectionBlock(
                    text=PlainTextObject(text=title, emoji=emoji)
                ),
                ActionsBlock(
