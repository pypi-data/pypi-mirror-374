...
class Form:
    """
    Defines the inputs a user must provide, their values once the user has provided them, if they have provided them all and which one the bot has asked for.
    """
...
class AppState:
    ...
    def new_event(self, message_id):
        """
        Check if the event is a new one because some messaging platforms resend them if the bot hasn't messaged in 3 seconds.
    ...
    async def clear_state(self, context: Context):
        ...
        :param Context context: Context object holding information about the current user interacting with the appe
        """
    ...
    async def handle_form(self, context: Context, form_key):
        """
        Save the user's message to the provided form and the requested input. Call validation if it exists and ask for the next input if the form is not filled.
        :param Context context: Context object holding information about the current user interacting with the app
        ...
        if context.user_message == "q":
            ...
            await self.interaction.send_message(message="Quitting.", channel_id=context.channel_id, thread_id=context.thread_id)
            return
        if form.requested_input:
            requested_input = form.requested_input
            if validate_input_map.get(requested_input):
                if not await validate_input_map[requested_input](
                    self, context, form_key, context.user_message
                ):
                    return
            else:
    ...
    async def handle_intent(self, intents: Dict[str, str], intent, context):
        ...
        :param Dict[str, str] intents: a dictionary mapping the intent name to the function name to trigger in business.py
    ...
    def set_input(self, context: Context, form_key, input_name, input_value):
        ...
        """
        self.users[context.user_id].get_form(form_key, context.channel_id, context.thread_id).inputs[input_name] = input_value
