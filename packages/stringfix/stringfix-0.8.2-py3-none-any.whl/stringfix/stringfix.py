...
def get_args():
    ...
    optional.add_argument('-xsa', action = 'store_true',
                        help='Set this option to exclude secondary alignment.')
...
def main():
    ...
    if (args.input is None): # | (args.gtf is None) | (args.gnm is None):
        ...
        sf.StringFix_assemble( Input = sam_file, genome_fa = genome_fa, annot_gtf = gtf_file, out_dir = args.dir, \
                        n_cores = n_cores, out_custom_genome = args.g, suffix = args.sfx, \
                        mcv = args.mcv, mcd = args.mcd, mtl = args.mtl, mpl = args.mpl, \
                        mdi = args.mdi, mdd = args.mdd, mdp = args.mdp, \
                        mdf = args.mdf, n_p = args.np, xsa = args.xsa, jump_to = jump_to )
