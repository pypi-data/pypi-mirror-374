...
class ActionInstance:
    ...
    def build_docker_commands(self, images):
        """
        Build docker run commands for given image definitions.
        Returns a single shell command string.
        """
        commands = []

        # Always create a network for inter-container communication
        ...
        for img in images:
            ...
            cmd = f"docker run -d --name {name} --network appnet"

            # Database gets volume
            if type_ == "database":
                cmd += " -v db_data:/data/db"

            # Backend exposes a port and links to database
            if type_ == "backend":
                cmd += " -p 8080:8080"
                cmd += " -e DB_HOST=database-deploy -e DB_PORT=27017"

            cmd += f" {image}"
            commands.append(cmd)

        # Join all into a single command (executed sequentially)
        return " && \\\n".join(commands)
...
def facial_recognition_docker_compose(self: ActionInstance):
    ...
    cmd = self.build_docker_commands(images=images)
    logging.info("cmd is: %s", cmd)
    self.start(cmd, "facial_recognition_docker_componse")
