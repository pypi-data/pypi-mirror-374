"""
Underground Pipeline Defect Monitoring Use Case for Post-Processing

This module provides Underground Pipeline Defect monitoring functionality with defect detection,
...
class UndergroundPipelineDefectConfig(BaseConfig):
    ...
    index_to_category: Optional[Dict[int, str]] = field(
        default_factory=lambda: {0 : "hole" , 1 : "crack", 2 : "obstacle"}
    )
...
class UndergroundPipelineDefectUseCase(BaseProcessor):
        ...
        if config.enable_smoothing:
            if self.smoothing_tracker is None:
                smoothing_config = BBoxSmoothingConfig(
                    ...
                    confidence_threshold=config.confidence_threshold,  # Use defect threshold as default
    ...
    def _generate_tracking_stats(
        ...
        for cat, count in per_category_count.items():
            human_text_lines.append(f"\t{cat}: {count}[s] detected")
    ...
    def _generate_business_analytics(self, counting_summary: Dict, alerts: Any, config: UndergroundPipelineDefectConfig, stream_info: Optional[Dict[str, Any]] = None, is_empty=False) -> List[Dict]:
