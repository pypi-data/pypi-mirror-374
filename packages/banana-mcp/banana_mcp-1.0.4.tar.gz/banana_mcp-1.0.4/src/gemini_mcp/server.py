            ...
            async with client.stream('POST', API_BASE_URL, headers=headers, json=payload) as response:
                ...
                try:
                    async for chunk in response.aiter_lines():
                        ...
                        # 添加保护措施：如果读取了很多块但没有收到[DONE]，检查是否已经包含完整内容
                        if chunk_count > 500 and "![" in result and "](http" in result:
                            logger.info(f"Found complete image response after {chunk_count} chunks, stopping early")
                            break
                        
                        # 防止无限循环
                        if chunk_count > 1000:
                            logger.warning(f"Stopping after {chunk_count} chunks to prevent infinite loop")
                            break
                            
                except Exception as e:
                    logger.error(f"Error while reading stream: {e}")
                    # 即使流读取出现错误，也尝试处理已接收到的内容
...
async def extract_and_save_image(api_response: str, prompt: str) -> Optional[str]:
    ...
    logger.info(f"Response preview: {api_response[-200:] if len(api_response) > 200 else api_response}")
    ...
    url_patterns = [
        # 匹配Markdown图片格式: ![alt](url) - 最优先
        r'!\[[^\]]*\]\((https?://[^)]+)\)',
        # 匹配这个API常用的CDN格式
        r'https?://s3\.ffire\.cc/[^\s"\'<>\[\]{}()]+',
        r'https?://[^\s"\'<>\[\]{}()]*ffire[^\s"\'<>\[\]{}()]*',
        # 匹配其他图片服务
        r'https?://[^\s"\'<>\[\]{}()]*(?:cdn|img|images?|photo|picture)[^\s"\'<>\[\]{}()]*',
        # 匹配以常见图片扩展名结尾的URL
        r'https?://[^\s"\'<>\[\]{}()]*\.(?:jpg|jpeg|png|gif|webp|bmp)(?:\?[^\s"\'<>\[\]{}()]*)?',
        # 匹配JSON中的URL字段
        r'"(?:url|image|image_url|data_url)"\s*:\s*"(https?://[^"]+)"',
        # 匹配任何HTTP URL (作为最后的fallback)
    ...
    # 特殊处理：如果响应看起来被截断了，尝试重新发起请求或提示用户
    if not image_url and not base64_data:
        logger.warning("No image URL or base64 data found in response")
        logger.debug(f"Full response: {api_response}")
        
        # 检查是否是被截断的响应
        if api_response.endswith('`...') or (len(api_response) < 500 and '```json' in api_response):
            logger.warning("Response appears to be truncated - image URL may be missing")
