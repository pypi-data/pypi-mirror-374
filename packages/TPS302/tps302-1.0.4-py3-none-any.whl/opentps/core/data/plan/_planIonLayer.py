...
class PlanIonLayer:
    ...
    def __init__(self, nominalEnergy: float = 0.0):
        ...
        self._ScanningDuration = np.array([])
    ...
    def appendSpot_XYZ(self, x: Union[float, Sequence[float]], y: Union[float, Sequence[float]],
                   mu: Union[float, Sequence[float]], posX: Union[float, Sequence[float]],
                   posY: Union[float, Sequence[float]], posZ: Union[float, Sequence[float]],
                   startTime: Optional[Union[float, Sequence[float]]] = None,
                   irradiationDuration: Optional[Union[float, Sequence[float]]] = None):
        if not isinstance(x, Iterable): x = [x]
        if not isinstance(y, Iterable): y = [y]
        if not isinstance(mu, Iterable): mu = [mu]
        if not isinstance(posX, Iterable): posX = [posX]
        if not isinstance(posY, Iterable): posY = [posY]
        if not isinstance(posZ, Iterable): posZ = [posZ]
        if startTime is not None and not isinstance(startTime, Iterable): startTime = [startTime]
        if irradiationDuration is not None and not isinstance(irradiationDuration, Iterable): irradiationDuration = [
            irradiationDuration]
        for i, xElem in enumerate(x):
            t = startTime[i] if startTime is not None else None
            d = irradiationDuration[i] if irradiationDuration is not None else None
            self._appendSingleSpot_XYZ(xElem, y[i], mu[i], posX[i], posY[i], posZ[i], t, d)

    def _appendSingleSpot_XYZ(self, x: float, y: float, mu: float, posX: float, posY: float, posZ: float,
                          startTime: Optional[float] = None, irradiationDuration: Optional[float] = None):
        alreadyExists, where = self.spotDefinedInXY(x, y)
        if alreadyExists:
        ...
        self._posX = np.append(self._posX, posX)
        self._posY = np.append(self._posY, posY)
        self._posZ = np.append(self._posZ, posZ)
    ...
    def _appendSingleSpot(self, x: float, y: float, mu: float,
                        startTime: Optional[float] = None, irradiationDuration: Optional[float] = None):
        alreadyExists, where = self.spotDefinedInXY(x, y)
        if alreadyExists:
