...
for a in range(5):
    ...
    # 更新最大覆盖率和对应的plan_selected
    if coverage_percentage > max_coverage_percentage:
...
plt.plot(ELStime_array,"*")
plt.xlabel("index")
plt.ylabel("Energy layer switching time(s)")
plt.show()
...
dcm_file = os.path.join(output_path, "Arcplan_ELSA_90GA_Opti.dcm")
tps_file = os.path.join(output_path, "Arcplan_ELSA_90GA_Opti.tps")
