...
DLLEXPORT_TAG void transport_spots_inside_target(float *SPR,float *spot_coordinates, bool *Target_mask, float *Offset, float *PixelSpacing, int *GridSize, float *positions, float *WETs, float *Layers, float *direction, int NumSpots, int max_number_layers, float minWET, float LayerSpacing){
    ...
    float Layer_WET = minWET + NumLayer * LayerSpacing;
      ...
      if(WETs[s] >= Layer_WET){
        ...
        if(Target_mask[id_vox]){
          ...
          // 存储 spot 坐标信息到数组
          spot_coordinates[id_layer*3] = x;
          spot_coordinates[id_layer*3+1] = y;
          spot_coordinates[id_layer*3+2] = z;
        }
        NumLayer++;
