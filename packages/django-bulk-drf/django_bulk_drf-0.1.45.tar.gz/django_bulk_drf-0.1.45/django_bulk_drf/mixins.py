                ...
                if serializer.is_valid():
                    ...
                    for field_name, value in validated_data.items():
                        ...
                        if hasattr(model_class, field_name):
                            ...
                            if hasattr(field, "related_model") and field.related_model:
                                ...
                                # SlugRelatedField typically uses string primary keys
                                if hasattr(value, "_meta") and hasattr(
                                    value._meta, "pk"
                                ):
                                    # Value is a model instance - check its primary key type
                                    pk_field = value._meta.pk
                                    # Check if the primary key is a string field (CharField, SlugField, etc.)
                                    if (
                                        hasattr(pk_field, "max_length")
                                        and pk_field.max_length is not None
                                        and pk_field.max_length > 0
                                    ) or isinstance(pk_field, (str, type(None))):
                                        # It's a string primary key - don't use _id suffix
                                    ...
                                    if related_model and hasattr(
                                        ...
                                        if (
                                            ...
                                            related_pk_field, (str, type(None))
                                        ):
                                            # Related model uses string PK - don't use _id suffix
                                            model_data[field_name] = str(actual_value)
                                            print(
                                                f"BulkOperationsMixin._handle_array_upsert_direct() - Related model uses string PK for field '{field_name}': {actual_value}",
                                                file=sys.stderr,
                                            )
                                        else:
                                            ...
                                            print(
                                                f"BulkOperationsMixin._handle_array_upsert_direct() - Related model uses integer PK for field '{field_name}': {actual_value}",
                                                file=sys.stderr,
                                            )
                                    else:
                                ...
                                field_suffix = (
                                    "_id" if f"{field_name}_id" in model_data else ""
                                )
                                print(
                                    f"BulkOperationsMixin._handle_array_upsert_direct() - Converting foreign key field '{field_name}' to '{field_name}{field_suffix}' with value {value} -> {actual_value}",
                                    file=sys.stderr,
                                )
                            else:
                    ...
                    for field_name, value in validated_data.items():
                        if hasattr(model_class, field_name):
                            field = model_class._meta.get_field(field_name)
                            if hasattr(field, "related_model") and field.related_model:
                                # Clear the original field value to avoid conflicts
