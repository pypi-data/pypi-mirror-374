                    ...
                    for field_name, value in validated_data.items():
                        ...
                        if hasattr(model_class, field_name):
                            ...
                            if hasattr(field, "related_model") and field.related_model:
                                ...
                                # Check if this is a SlugRelatedField (uses string primary keys)
                                # SlugRelatedField typically uses string primary keys, not integer IDs
                                related_model = field.related_model
                                print(
                                    f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Field '{field_name}' related_model: {related_model}",
                                    file=sys.stderr,
                                )
                                ...
                                if related_model and hasattr(related_model._meta, "pk"):
                                    ...
                                    print(
                                        f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Field '{field_name}' related_pk_field: {related_pk_field} (type: {type(related_pk_field).__name__})",
                                        file=sys.stderr,
                                    )
                                    print(
                                        f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Field '{field_name}' related_pk_field has max_length: {hasattr(related_pk_field, 'max_length')}",
                                        file=sys.stderr,
                                    )
                                    if hasattr(related_pk_field, "max_length"):
                                        print(
                                            f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Field '{field_name}' related_pk_field max_length: {related_pk_field.max_length}",
                                            file=sys.stderr,
                                        )
                                    ...
                                    is_string_pk = (
                                        ...
                                        and related_pk_field.max_length is not None
                                        and related_pk_field.max_length > 0
                                    ) or isinstance(related_pk_field, (str, type(None)))

                                    print(
                                        f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Field '{field_name}' is_string_pk: {is_string_pk}",
                                        file=sys.stderr,
                                    )
                                    ...
                                    if is_string_pk:
                                        ...
                                        print(
                                            f"BulkOperationsMixin._handle_array_upsert_direct() - Using string PK for field '{field_name}': {actual_value}",
                                            file=sys.stderr,
                                        )
                                    else:
                                        ...
                                        print(
                                            f"BulkOperationsMixin._handle_array_upsert_direct() - Using integer PK for field '{field_name}': {actual_value}",
                                            file=sys.stderr,
                                        )
                                else:
                                    ...
                                    model_data[f"{field_name}_id"] = actual_value
                                    print(
                                        f"BulkOperationsMixin._handle_array_upsert_direct() - Default _id suffix for field '{field_name}': {actual_value}",
                                        file=sys.stderr,
                                    )
                            else:
