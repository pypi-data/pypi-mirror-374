                    ...
                    for field_name, value in validated_data.items():
                        ...
                        if hasattr(model_class, field_name):
                            ...
                            if hasattr(field, "related_model") and field.related_model:
                                ...
                                # For Django foreign keys, always use the _id suffix for the database field
                                # This is the standard Django convention
                                model_data[f"{field_name}_id"] = actual_value
                                print(
                                    f"BulkOperationsMixin._handle_array_upsert_direct() - Converting foreign key field '{field_name}' to '{field_name}_id' with value {value} -> {actual_value}",
                                    file=sys.stderr,
                                )
                            else:
