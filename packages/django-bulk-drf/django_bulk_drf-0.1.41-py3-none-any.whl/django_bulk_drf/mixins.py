...
class BulkOperationsMixin:
    ...
    def _handle_array_upsert_direct(
        ...
        for index, item_data in enumerate(data_list):
            ...
            try:
                                ...
                                def bypass_uniqueness_validation(data):
                                    try:
                                        ...
                                        if 'unique' in error_str and 'already exists' in error_str:
                                            print(
                                                f"BulkOperationsMixin._handle_array_upsert_direct() - Suppressing uniqueness error for field '{field_name}': {e}",
                                                file=sys.stderr,
                                            )
                ...
                if serializer.is_valid():
                    ...
                    if index == 0:
                        print(
                            f"BulkOperationsMixin._handle_array_upsert_direct() - Checking model meta constraints...",
                            file=sys.stderr,
                        )
        ...
        print(
            f"BulkOperationsMixin._handle_array_upsert_direct() - PHASE 2: TRUE bulk upsert",
            file=sys.stderr,
        )
        print(
        ...
        if instances_to_upsert:
            try:
                ...
                print(
                    f"BulkOperationsMixin._handle_array_upsert_direct() - Executing bulk_create with update_conflicts",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Instances being passed to bulk_create:",
                    file=sys.stderr,
                )
                ...
                with transaction.atomic():
                    ...
                    print(
                        f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Calling bulk_create with:",
                        file=sys.stderr,
                    )
                    print(
                    ...
                    print(
                        f"BulkOperationsMixin._handle_array_upsert_direct() -   update_conflicts: True",
                        file=sys.stderr,
                    )
                    print(
                        f"BulkOperationsMixin._handle_array_upsert_direct() -   update_fields: {fields_to_update}",
                    ...
                    print(
                        f"BulkOperationsMixin._handle_array_upsert_direct() -   batch_size: 1000",
                        file=sys.stderr,
                    )
