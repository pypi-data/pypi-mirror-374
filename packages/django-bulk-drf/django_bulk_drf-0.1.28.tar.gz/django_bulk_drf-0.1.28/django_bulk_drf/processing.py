            ...
            try:
                with transaction.atomic():
                    ...
                    if all_instances:
                        ...
                        if fields_to_update:
                            # Check if unique_fields match any database constraint
                            constraint_matches = []
                            for constraint in model_class._meta.constraints:
                                if hasattr(constraint, 'fields'):
                                    constraint_fields = list(constraint.fields)
                                    constraint_matches.append(constraint_fields)

                            # Also check unique_together (legacy way)
                            if hasattr(model_class._meta, 'unique_together'):
                                for unique_set in model_class._meta.unique_together:
                                    constraint_matches.append(list(unique_set))

                            # Check if any constraint matches our unique_fields
                            matching_constraint = None
                            for constraint_fields in constraint_matches:
                                if set(unique_fields) == set(constraint_fields):
                                    matching_constraint = constraint_fields
                                    break
                            ...
                            if matching_constraint:
                                ...
                                print("async_upsert_task - Found matching constraint: " + str(matching_constraint), file=sys.stderr)
                                ...
                                print("async_upsert_task - No matching constraint found for unique_fields: " + str(unique_fields), file=sys.stderr)
                                print("async_upsert_task - Available constraints: " + str(constraint_matches), file=sys.stderr)
                                print("async_upsert_task - Falling back to separate create/update operations", file=sys.stderr)

                                # Fall back to separate operations when no constraint matches
                                try:
                                    ...
                                    created_instances = []  # Reset so we don't process twice below
