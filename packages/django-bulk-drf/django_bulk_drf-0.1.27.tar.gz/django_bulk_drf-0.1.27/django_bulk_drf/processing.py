...
def async_upsert_task(
    ...
    Args:
        ...
        user_id: Optional user ID for audit purposes
    """
    print(f"async_upsert_task - STARTED: Processing {len(data_list)} items", file=sys.stderr)
    print(f"async_upsert_task - Serializer: {serializer_class_path}", file=sys.stderr)
    print(f"async_upsert_task - Unique fields: {unique_fields}", file=sys.stderr)
    print(f"async_upsert_task - Update fields: {update_fields}", file=sys.stderr)
    ...
    try:
        print(f"async_upsert_task - Importing serializer: {serializer_class_path}", file=sys.stderr)
        serializer_class = import_string(serializer_class_path)
        model_class = serializer_class.Meta.model
        print(f"async_upsert_task - Using model: {model_class.__name__}", file=sys.stderr)
        instances_to_create = []
        instances_to_update = []

        # Validate all items first
        print(f"async_upsert_task - Starting validation of {len(data_list)} items", file=sys.stderr)
        OperationCache.set_task_progress(task_id, 0, len(data_list), "Validating data...")
        ...
        if instances_to_create or instances_to_update:
            print(f"async_upsert_task - Preparing upsert: create={len(instances_to_create)}, update={len(instances_to_update)}", file=sys.stderr)
            OperationCache.set_task_progress(
            ...
            try:
                with transaction.atomic():
                    ...
                    all_instances = []
                    print(f"async_upsert_task - Preparing {len(instances_to_create) + len(instances_to_update)} instances for bulk operation", file=sys.stderr)
                    ...
                    if all_instances:
                        ...
                        if update_fields:
                            ...
                            print(f"async_upsert_task - Using provided update_fields: {fields_to_update}", file=sys.stderr)
                        else:
                        ...
                        if fields_to_update:
                            ...
                            try:
                                ...
                                if created_instances:
                                    print(f"async_upsert_task - bulk_create completed, returned {len(created_instances)} instances", file=sys.stderr)
                                ...
                                result.add_error(0, f"Bulk upsert failed: {str(bulk_error)}")
                                created_instances = []
                        else:
        ...
        print(f"async_upsert_task - FATAL ERROR: {str(e)}", file=sys.stderr)
