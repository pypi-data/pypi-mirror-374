        ...
        for index, item_data in enumerate(data_list):
            ...
            try:
                ...
                if serializer.is_valid():
                    ...
                    # Build unique filter for lookup
                    unique_filter = {}
                    for field in unique_fields:
                        if field in validated_data:
                            unique_filter[field] = validated_data[field]
                        else:
                            error_msg = f"Missing required unique field: {field}"
                    ...
                    if unique_filter:
                        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Looking for existing instance with filter: {unique_filter}", file=sys.stderr)
                        # Try to find existing instance
                        existing_instance = model_class.objects.filter(**unique_filter).first()
                        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Existing instance found: {existing_instance is not None}", file=sys.stderr)
                        ...
                        if existing_instance:
                            ...
                            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Updating instance {existing_instance.id} with data: {update_data}", file=sys.stderr)
                            ...
                            success_count += 1
                            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Successfully updated instance {existing_instance.id}", file=sys.stderr)
                        else:
                            # Create new instance
                            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Creating new instance with data: {validated_data}", file=sys.stderr)
                            new_instance = serializer.save()
                            created_instances.append(new_instance)
                            success_count += 1
                            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Successfully created instance {new_instance.id}", file=sys.stderr)
                    else:
                        error_msg = "No valid unique fields found"
                        print(f"BulkOperationsMixin._handle_array_upsert_direct() - ERROR at index {index}: {error_msg}", file=sys.stderr)
                        errors.append({
                            "index": index,
                            "error": error_msg,
                    ...
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - ERROR at index {index}: {error_msg}", file=sys.stderr)
                    import traceback
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Validation error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
