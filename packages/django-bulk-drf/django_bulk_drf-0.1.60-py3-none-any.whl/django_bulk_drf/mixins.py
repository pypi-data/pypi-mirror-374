...
class BulkOperationsMixin:
    ...
    def _handle_array_upsert_direct(
        ...
        for index, item_data in enumerate(data_list):
            try:
                # Create serializer with the original data
                # Pass upsert context to skip uniqueness validation
                serializer = serializer_class(
                    data=item_data, context={"upsert_mode": True}
                )

                # For upsert operations, we need to bypass DRF's uniqueness validation
                # by using to_internal_value() instead of is_valid()
                try:
                    # Use to_internal_value() to get converted field values
                    # This bypasses DRF's uniqueness validation while still doing field conversion
                    validated_data = serializer.to_internal_value(item_data)
                    ...
                    # If to_internal_value fails, fall back to regular validation
                    # but skip uniqueness errors in upsert mode
                    if serializer.is_valid():
                        validated_data = serializer.validated_data
                        new_instance = model_class(**validated_data)
                        instances_to_upsert.append(new_instance)
                    else:
                        # Check if the only error is uniqueness validation
                        errors_dict = serializer.errors
                        is_uniqueness_error = False

                        # Check non_field_errors for uniqueness
                        if "non_field_errors" in errors_dict:
                            non_field_errors = errors_dict["non_field_errors"]
                            if any(
                                "unique" in str(error) for error in non_field_errors
                            ):
                                is_uniqueness_error = True

                        # Check individual field errors for uniqueness
                        if not is_uniqueness_error:
                            for field_name, field_errors in errors_dict.items():
                                if field_name != "non_field_errors":
                                    if any(
                                        "unique" in str(error) for error in field_errors
                                    ):
                                        is_uniqueness_error = True
                                        break
                        ...
                        if is_uniqueness_error:
                            print(
                                f"BulkOperationsMixin._handle_array_upsert_direct() - Skipping uniqueness validation for item {index} in upsert mode",
                                file=sys.stderr,
                            )
                            # Manually convert field values without going through serializer validation
                            ...
                            for field_name, field_value in item_data.items():
                                if (
                                    ...
                                    try:
                                        # Use the field's to_internal_value method directly
                        ...
                        print(
                            f"BulkOperationsMixin._handle_array_upsert_direct() - Validation failed for item {index}: {serializer.errors}",
                            file=sys.stderr,
                        )
                        errors.append(
                            {
        ...
        if instances_to_upsert:
            try:
                ...
                return Response(
                    {"error": "Bulk upsert failed", "details": str(e)},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            print(
