...
def async_create_task(self, serializer_class_path: str, data_list: list[dict], user_id: int | None = None):
    ...
    """
    print(f"async_create_task - STARTED: Processing {len(data_list)} items for bulk create", file=sys.stderr)
    print(f"async_create_task - Serializer: {serializer_class_path}", file=sys.stderr)
    ...
    try:
        print(f"async_create_task - Importing serializer: {serializer_class_path}", file=sys.stderr)
        serializer_class = import_string(serializer_class_path)
        model_class = serializer_class.Meta.model
        print(f"async_create_task - Using model: {model_class.__name__}", file=sys.stderr)
        instances_to_create = []

        # Validate all items first
        print(f"async_create_task - Starting validation of {len(data_list)} items", file=sys.stderr)
        ...
        for index, item_data in enumerate(data_list):
            try:
                ...
                if serializer.is_valid():
                    ...
                    print(f"async_create_task - {error_msg}", file=sys.stderr)
                    print(f"async_create_task - Validation error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                    logger.exception("Serializer validation failed in async create task %s at index %s", task_id, index)
                    result.add_error(index, error_msg, item_data)
            except (ValidationError, ValueError) as e:
                # Log full stack trace for debugging
                error_msg = f"Validation error at index {index}: {str(e)}"
                print(f"async_create_task - {error_msg}", file=sys.stderr)
                print(f"async_create_task - Stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                logger.exception("Validation error in async create task %s at index %s", task_id, index)
                result.add_error(index, error_msg, item_data)
            
            # Update progress every 10 items or at the end
            if (index + 1) % 10 == 0 or index == len(data_list) - 1:
                OperationCache.set_task_progress(
                    task_id, index + 1, len(data_list), "Validated " + str(index + 1) + "/" + str(len(data_list)) + " items",
                )
            ...
            with transaction.atomic():
                ...
                for instance in created_instances:
                    ...
                    if instance is not None:
                        ...
                        print("async_create_task - WARNING: bulk_create returned None instance", file=sys.stderr)

        # Store final result in cache
        OperationCache.set_task_result(task_id, result.to_dict())

        print(f"async_create_task - COMPLETED: success={result.success_count}, errors={result.error_count}", file=sys.stderr)
        print(f"async_create_task - Final result: {result.to_dict()}", file=sys.stderr)
        ...
        print(f"async_create_task - FATAL ERROR: {str(e)}", file=sys.stderr)
        print(f"async_create_task - Fatal error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
...
def async_update_task(self, serializer_class_path: str, updates_list: list[dict], user_id: int | None = None):
    ...
    try:
        ...
        for index, update_data in enumerate(updates_list):
            try:
                ...
                if serializer.is_valid():
                    ...
                    # Log validation errors with stack trace for debugging
                    error_msg = f"Serializer validation failed at index {index}: {str(serializer.errors)}"
                    print(f"async_update_task - {error_msg}", file=sys.stderr)
                    print(f"async_update_task - Validation error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                    logger.exception("Serializer validation failed in async update task %s at index %s", task_id, index)
                ...
                error_msg = f"Validation error at index {index}: {str(e)}"
                print(f"async_update_task - {error_msg}", file=sys.stderr)
                print(f"async_update_task - Stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                logger.exception("Validation error in async update task %s at index %s", task_id, index)
                result.add_error(index, error_msg, update_data)
        ...
        print(f"async_update_task - Fatal error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
...
def async_replace_task(self, serializer_class_path: str, replacements_list: list[dict], user_id: int | None = None):
    ...
    try:
        ...
        for index, replacement_data in enumerate(replacements_list):
            try:
                ...
                if serializer.is_valid():
                    ...
                    # Log validation errors with stack trace for debugging
                    error_msg = f"Serializer validation failed at index {index}: {str(serializer.errors)}"
                    print(f"async_replace_task - {error_msg}", file=sys.stderr)
                    print(f"async_replace_task - Validation error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                    logger.exception("Serializer validation failed in async replace task %s at index %s", task_id, index)
                ...
                error_msg = f"Validation error at index {index}: {str(e)}"
                print(f"async_replace_task - {error_msg}", file=sys.stderr)
                print(f"async_replace_task - Stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                logger.exception("Validation error in async replace task %s at index %s", task_id, index)
                result.add_error(index, error_msg, replacement_data)
        ...
        print(f"async_replace_task - Fatal error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
...
def async_delete_task(self, model_class_path: str, ids_list: list[int], user_id: int | None = None):
    ...
    try:
        ...
        print(f"async_delete_task - Fatal error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
...
def async_get_task(
    ...
    try:
        ...
        print(f"async_get_task - Fatal error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
...
def async_upsert_task(
    ...
    Args:
        ...
        user_id: Optional user ID for audit purposes
    """
    print("async_upsert_task - STARTED: Processing " + str(len(data_list)) + " items", file=sys.stderr)
    print("async_upsert_task - Serializer: " + serializer_class_path, file=sys.stderr)
    print("async_upsert_task - Unique fields: " + str(unique_fields), file=sys.stderr)
    print("async_upsert_task - Update fields: " + str(update_fields), file=sys.stderr)
    print("async_upsert_task - Upsert context: " + str(upsert_context), file=sys.stderr)
    ...
    try:
        print("async_upsert_task - Importing serializer: " + serializer_class_path, file=sys.stderr)
        serializer_class = import_string(serializer_class_path)
        model_class = serializer_class.Meta.model
        print("async_upsert_task - Using model: " + model_class.__name__, file=sys.stderr)

        # Handle upsert context for skipping uniqueness validation
        skip_uniqueness = upsert_context and upsert_context.get('skip_uniqueness_validation', False)
        if skip_uniqueness:
            print("async_upsert_task - UPSERT MODE: Skipping uniqueness validation for upsert operation", file=sys.stderr)
        ...
        # Validate all items first
        print("async_upsert_task - Starting validation of " + str(len(data_list)) + " items", file=sys.stderr)
        OperationCache.set_task_progress(task_id, 0, len(data_list), "Validating data...")
        ...
        for index, item_data in enumerate(data_list):
            try:
                ...
                # Skip uniqueness validation for upsert operations
                if skip_uniqueness:
                    print(f"async_upsert_task - Filtering uniqueness validators for item {index}", file=sys.stderr)
                    # Remove UniqueTogetherValidator and UniqueValidator from serializer validators
                    original_validators = serializer.validators[:]
                    serializer.validators = [v for v in serializer.validators
                                           if not (hasattr(v, 'fields') and
                                                  hasattr(v, 'queryset') and
                                                  any(field in str(unique_fields) for field in v.fields))]
                    print(f"async_upsert_task - Filtered validators: {len(original_validators)} -> {len(serializer.validators)}", file=sys.stderr)
                ...
                if serializer.is_valid():
                    ...
                    # Log validation errors with stack trace for debugging
                    error_msg = f"Serializer validation failed at index {index}: {str(serializer.errors)}"
                    print(f"async_upsert_task - {error_msg}", file=sys.stderr)
                    print(f"async_upsert_task - Validation error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                    logger.exception("Serializer validation failed in async upsert task %s at index %s", task_id, index)
                    result.add_error(index, error_msg, item_data)
            except (ValidationError, ValueError) as e:
                # Log full stack trace for debugging
                error_msg = f"Validation error at index {index}: {str(e)}"
                print(f"async_upsert_task - {error_msg}", file=sys.stderr)
                print(f"async_upsert_task - Stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                logger.exception("Validation error in async upsert task %s at index %s", task_id, index)
                result.add_error(index, error_msg, item_data)
        ...
        if instances_to_create or instances_to_update:
            print("async_upsert_task - Preparing upsert: create=" + str(len(instances_to_create)) + ", update=" + str(len(instances_to_update)), file=sys.stderr)
            OperationCache.set_task_progress(
                task_id, len(data_list), len(data_list), "Performing upsert...",
            )
            ...
            try:
                with transaction.atomic():
                    ...
                    all_instances = []
                    print("async_upsert_task - Preparing " + str(len(instances_to_create) + len(instances_to_update)) + " instances for bulk operation", file=sys.stderr)
                    ...
                    if all_instances:
                        ...
                        if update_fields:
                            ...
                            print("async_upsert_task - Using provided update_fields: " + str(fields_to_update), file=sys.stderr)
                        else:
                            ...
                            print("async_upsert_task - Auto-determined update_fields: " + str(fields_to_update), file=sys.stderr)

                        # Perform true upsert with unique constraint
                        if fields_to_update:
                            print("async_upsert_task - Executing bulk_create with update_conflicts", file=sys.stderr)
                            print("async_upsert_task - unique_fields: " + str(unique_fields), file=sys.stderr)
                            print("async_upsert_task - update_fields: " + str(fields_to_update), file=sys.stderr)
                            ...
                            try:
                                ...
                                if created_instances:
                                    print("async_upsert_task - bulk_create completed, returned " + str(len(created_instances)) + " instances", file=sys.stderr)
                                    ...
                                    for instance in created_instances:
                                        ...
                                        if instance is not None:
                                            ...
                                            print("async_upsert_task - WARNING: bulk_create returned None instance", file=sys.stderr)
                                else:
                                    print("async_upsert_task - bulk_create returned no instances", file=sys.stderr)
                                    result.add_error(0, "Bulk create operation returned no instances")
                            except Exception as bulk_error:
                                print("async_upsert_task - bulk_create failed: " + str(bulk_error), file=sys.stderr)
                                print(f"async_upsert_task - Bulk create error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                                logger.exception("Bulk create error in async upsert task %s", task_id)
                                result.add_error(0, "Bulk upsert failed: " + str(bulk_error))
                                created_instances = []
                        else:
                            print("async_upsert_task - No update fields, using separate operations", file=sys.stderr)
                            ...
                            try:
                                ...
                                if created_instances:
                                    for instance in created_instances:
                                        ...
                                        if instance is not None:
                                            ...
                                            print("async_upsert_task - WARNING: bulk_create (fallback) returned None instance", file=sys.stderr)
                                ...
                                print("async_upsert_task - Fallback operations failed: " + str(fallback_error), file=sys.stderr)
                                result.add_error(0, "Fallback upsert operations failed: " + str(fallback_error))
                                created_instances = []

            except Exception as e:
                print("async_upsert_task - ERROR during bulk operation: " + str(e), file=sys.stderr)
                print(f"async_upsert_task - Bulk operation error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
                logger.exception("Bulk operation error in async upsert task %s", task_id)
                # Handle unique constraint errors specifically
                error_msg = str(e)
                if "unique constraint" in error_msg.lower() or "unique_fields" in error_msg.lower():
                    result.add_error(0, "Unique constraint error: " + error_msg + ". "
                                      "Ensure fields " + str(unique_fields) + " form a unique constraint in your database.")
                else:
        ...
        print("async_upsert_task - COMPLETED: success=" + str(result.success_count) + ", errors=" + str(result.error_count), file=sys.stderr)
        print("async_upsert_task - Final result: " + str(result.to_dict()), file=sys.stderr)
        ...
        print("async_upsert_task - FATAL ERROR: " + str(e), file=sys.stderr)
        print(f"async_upsert_task - Fatal error stack trace:\n{traceback.format_exc()}", file=sys.stderr)
