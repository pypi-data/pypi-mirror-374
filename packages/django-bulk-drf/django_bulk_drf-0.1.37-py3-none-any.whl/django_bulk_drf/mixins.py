...
class BulkOperationsMixin:
    ...
    def get_serializer(self, *args, **kwargs):
        ...
        try:
            ...
            # If this is an upsert operation, modify serializer to skip uniqueness validation
            if hasattr(self, 'request') and self.request:
                unique_fields_param = self.request.query_params.get("unique_fields")
                if unique_fields_param and isinstance(data, list):
                    print(f"BulkOperationsMixin.get_serializer() - UPSERT MODE: Disabling uniqueness validation for fields: {unique_fields_param}", file=sys.stderr)
                    # Remove UniqueTogetherValidator and UniqueValidator from serializer validators
                    serializer.validators = [v for v in serializer.validators
                                           if not (hasattr(v, 'fields') and
                                                  hasattr(v, 'queryset') and
                                                  any(field in unique_fields_param for field in v.fields))]
    ...
    def _handle_array_upsert(self, request, unique_fields, update_fields, data_list):
        ...
        if DEBUG_BYPASS_CELERY:
            print("BulkOperationsMixin._handle_array_upsert() - DEBUG MODE: Bypassing Celery for direct execution", file=sys.stderr)
            return self._handle_array_upsert_direct(request, unique_fields, update_fields, data_list)
        ...
        task = async_upsert_task.delay(
            serializer_class_path, data_list, unique_fields, update_fields, user_id, upsert_context
        )
    ...
    def _handle_array_upsert_direct(self, request, unique_fields, update_fields, data_list):
        """Direct upsert implementation for debugging - bypasses Celery."""
        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Starting direct upsert with {len(data_list)} items", file=sys.stderr)
        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Unique fields: {unique_fields}", file=sys.stderr)
        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Update fields: {update_fields}", file=sys.stderr)

        if not unique_fields:
            print("BulkOperationsMixin._handle_array_upsert_direct() - ERROR: No unique_fields provided", file=sys.stderr)
            return Response(
                {"error": "unique_fields parameter is required for upsert operations"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not update_fields:
            update_fields = self._infer_update_fields(data_list, unique_fields)
            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Auto-inferred update_fields: {update_fields}", file=sys.stderr)

        # Get serializer and model classes
        serializer_class = self.get_serializer_class()
        model_class = serializer_class.Meta.model
        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Using model: {model_class.__name__}", file=sys.stderr)
        ...
        print(f"BulkOperationsMixin._handle_array_upsert_direct() - PHASE 1: Validating {len(data_list)} items (no DB queries)", file=sys.stderr)

        for index, item_data in enumerate(data_list):
            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Processing item {index}: {item_data}", file=sys.stderr)

            try:
                # Create serializer and validate
                serializer = serializer_class(data=item_data)
                print(f"BulkOperationsMixin._handle_array_upsert_direct() - Created serializer for item {index}", file=sys.stderr)

                if serializer.is_valid():
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Serializer validation passed for item {index}", file=sys.stderr)
                    ...
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Queued for bulk upsert: {validated_data}", file=sys.stderr)

                    # Check database constraints (only once)
                    if index == 0:
                        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Checking model meta constraints...", file=sys.stderr)
                        if hasattr(model_class._meta, 'constraints'):
                            for constraint in model_class._meta.constraints:
                                print(f"BulkOperationsMixin._handle_array_upsert_direct() - Model constraint: {constraint}", file=sys.stderr)

                        # Check unique_together
                        if hasattr(model_class._meta, 'unique_together') and model_class._meta.unique_together:
                            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Model unique_together: {model_class._meta.unique_together}", file=sys.stderr)

                else:
                    # Enhanced error logging with detailed debugging info
                    error_msg = f"Serializer validation failed: {str(serializer.errors)}"
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - ERROR at index {index}: {error_msg}", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Detailed errors: {serializer.errors}", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Input data: {item_data}", file=sys.stderr)

                    # Try to understand what validation failed
                    if 'non_field_errors' in serializer.errors:
                        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Non-field errors: {serializer.errors['non_field_errors']}", file=sys.stderr)
                        for error in serializer.errors['non_field_errors']:
                            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Error code: {error.code}, message: {error}", file=sys.stderr)

                    # Check field-specific errors
                    if 'financial_provider' in serializer.errors:
                        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Financial provider field error: {serializer.errors['financial_provider']}", file=sys.stderr)
                        print(f"BulkOperationsMixin._handle_array_upsert_direct() - This might be SlugRelatedField validation failing", file=sys.stderr)

                    if 'account_number' in serializer.errors:
                        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Account number field error: {serializer.errors['account_number']}", file=sys.stderr)
                ...
                print(f"BulkOperationsMixin._handle_array_upsert_direct() - EXCEPTION at index {index}: {error_msg}", file=sys.stderr)
                import traceback
                print(f"BulkOperationsMixin._handle_array_upsert_direct() - Exception stack trace:\n{traceback.format_exc()}", file=sys.stderr)
        ...
        print(f"BulkOperationsMixin._handle_array_upsert_direct() - PHASE 2: TRUE bulk upsert", file=sys.stderr)
        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Instances to upsert: {len(instances_to_upsert)}", file=sys.stderr)
        ...
        if instances_to_upsert:
            try:
                ...
                if update_fields:
                    ...
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Using specified update_fields: {fields_to_update}", file=sys.stderr)
                else:
                    # Auto-determine update fields (all non-unique fields)
                    if instances_to_upsert:
                        sample_instance = instances_to_upsert[0]
                        fields_to_update = [field.name for field in sample_instance._meta.fields
                                          if field.name not in unique_fields and not field.primary_key]
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Auto-determined update_fields: {fields_to_update}", file=sys.stderr)

                print(f"BulkOperationsMixin._handle_array_upsert_direct() - Executing bulk_create with update_conflicts", file=sys.stderr)
                print(f"BulkOperationsMixin._handle_array_upsert_direct() - unique_fields: {unique_fields}", file=sys.stderr)
                print(f"BulkOperationsMixin._handle_array_upsert_direct() - update_fields: {fields_to_update}", file=sys.stderr)

                # DEBUG: Show what instances are being passed to bulk_create
                print(f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Instances being passed to bulk_create:", file=sys.stderr)
                for i, instance in enumerate(instances_to_upsert):
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Instance {i}: {instance.__dict__}", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - Instance {i} model: {instance._meta.label}", file=sys.stderr)

                with transaction.atomic():
                    # TRUE BULK UPSERT: Single database operation handles both create and update
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - DEBUG: Calling bulk_create with:", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() -   instances: {len(instances_to_upsert)} items", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() -   update_conflicts: True", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() -   update_fields: {fields_to_update}", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() -   unique_fields: {unique_fields}", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() -   batch_size: 1000", file=sys.stderr)
                    ...
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - BULK UPSERT COMPLETED: {len(upserted_instances)} instances processed", file=sys.stderr)
                ...
                print(f"BulkOperationsMixin._handle_array_upsert_direct() - BULK UPSERT FAILED: {bulk_error}", file=sys.stderr)
                import traceback
                print(f"BulkOperationsMixin._handle_array_upsert_direct() - Bulk upsert stack trace:\n{traceback.format_exc()}", file=sys.stderr)

                # Check if it's a unique constraint error
                error_str = str(bulk_error).lower()
                if 'unique' in error_str or 'constraint' in error_str:
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - UNIQUE CONSTRAINT ERROR detected", file=sys.stderr)
                    print(f"BulkOperationsMixin._handle_array_upsert_direct() - This usually means the unique constraint isn't properly configured", file=sys.stderr)
        ...
        # Prepare response
        print(f"BulkOperationsMixin._handle_array_upsert_direct() - Completed: success={success_count}, errors={error_count}", file=sys.stderr)

        if error_count > 0:
            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Returning errors: {errors}", file=sys.stderr)
            return Response(
        ...
        if all_instances:
            print(f"BulkOperationsMixin._handle_array_upsert_direct() - Returning {len(all_instances)} instances", file=sys.stderr)
            serializer = serializer_class(all_instances, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        else:
            print("BulkOperationsMixin._handle_array_upsert_direct() - No instances processed", file=sys.stderr)
            return Response([], status=status.HTTP_200_OK)
