...
if TYPE_CHECKING:
    ...
    from kp_ssf_tools.models.types import DisplayableValue


class DefaultColumnWidthAdjuster(ColumnWidthAdjusterProtocol):
    """Default implementation for auto-adjusting column widths."""

    def __init__(
        self,
        sheet_name_sanitizer: DefaultSheetNameSanitizer,
    ) -> None:
        """
        Initialize the DefaultColumnWidthAdjuster.

        Args:
            sheet_name_sanitizer: Optional sanitizer for sheet names. If not provided,
                a default sanitizer will be used.

        """
        self.sheet_name_sanitizer: DefaultSheetNameSanitizer = sheet_name_sanitizer
    ...
    def auto_adjust_column_widths(
        ...
        for col_num, column_name in enumerate(data_frame.columns, 1):
            column_letter: str = self.sheet_name_sanitizer.get_column_letter(
                col_num,
            )
...
class DefaultDateFormatter(DateFormatterProtocol):
...
class DefaultRowHeightAdjuster(RowHeightAdjusterProtocol):
...
class DefaultExcelFormatter(ExcelFormatterProtocol):
    ...
    def __init__(
        ...
        self._column_width_adjuster: (
            ColumnWidthAdjusterProtocol | DefaultColumnWidthAdjuster
        ) = column_width_adjuster
...
class DefaultTableStyler(TableStylerProtocol):
...
class DefaultTitleFormatter(TitleFormatterProtocol):
