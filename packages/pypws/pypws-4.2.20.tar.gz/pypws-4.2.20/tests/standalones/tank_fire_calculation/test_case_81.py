...
def test_case_81():
    ...
    material_name = "N-OCTANE"
    diameter = 2.0
    height = 5.0
    wind_speed = 5.0
    fire_type = PoolFireType.LATE
    weather_stability = AtmosphericStabilityClass.STABILITY_D

    # Define the initial state of the tank.
    state = State(temperature = 280.0, pressure = 1.0E+06, liquid_fraction = 0.0)
    ...
    if result_code == ResultCode.SUCCESS:
        if (abs((tank_fire_calculation.pool_fire_flame_result.surface_emissive_power - 114386.2734375) / 114386.2734375) > 1e-3):
        ...
        if (abs((tank_fire_calculation.pool_fire_flame_result.flame_length - 5.776425838470459) / 5.776425838470459) > 1e-3):
