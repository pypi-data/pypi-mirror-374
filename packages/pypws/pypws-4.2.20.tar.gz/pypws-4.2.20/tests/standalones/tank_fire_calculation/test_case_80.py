...
def test_case_80():
    ...
    weather_stability = AtmosphericStabilityClass.STABILITY_E

    # Define the initial state of the tank.
    state = State(temperature = 260.0, pressure = 101325, liquid_fraction = 0.0)
    ...
    if result_code == ResultCode.SUCCESS:
        if (abs((tank_fire_calculation.pool_fire_flame_result.surface_emissive_power - 133009.03125) / 133009.03125) > 1e-3):
        ...
        if (abs((tank_fire_calculation.pool_fire_flame_result.flame_length - 2.8789072036743164) / 2.8789072036743164) > 1e-3):
