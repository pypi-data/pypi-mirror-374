...
def test_case_166():
	...
	if resultCode == ResultCode.SUCCESS:
		if (abs((load_mass_inventory_vessel_for_relief_valve_scenario_calculation.vessel.diameter-4.665573040127879)/4.665573040127879)>1e-3):
		...
		if (abs((load_mass_inventory_vessel_for_relief_valve_scenario_calculation.vessel.location.z-(-1.3327865200639395))/(-1.3327865200639395))>1e-3):
	...
	if resultCode == ResultCode.SUCCESS:
		if (abs(vessel_relief_valve_linked_run_calculation.discharge_record.mass_flow - 52.423581478093844)/52.423581478093844> 1e-3):
		...
		if (abs(vessel_relief_valve_linked_run_calculation.jet_fire_flame_result.flame_length - 104.0295077327639)/104.0295077327639 > 1e-3):
		...
		if (len(vessel_relief_valve_linked_run_calculation.flam_conc_contour_points) != 525):
		...
		if (len(vessel_relief_valve_linked_run_calculation.toxic_conc_contour_points) != 533):
		...
		if (abs(vessel_relief_valve_linked_run_calculation.area_footprint_flam_conc[0]-2.3264037495948804)/2.3264037495948804 > 1e-3):
		...
		if (abs(vessel_relief_valve_linked_run_calculation.area_footprint_toxic_conc[0] -28984.72248507072)/28984.72248507072 > 1e-3):
		...
		if (abs(vessel_relief_valve_linked_run_calculation.area_contour_jet[0]-  21777.94192310462)/ 21777.94192310462 > 1e-3):
		...
		if (abs(vessel_relief_valve_linked_run_calculation.explosion_overpressure_results[0].exploded_mass-93.15833898482984)/93.15833898482984 > 1e-3):	
		...
		if (abs(vessel_relief_valve_linked_run_calculation.explosion_overpressure_results[0].maximum_distance - 278.44898736729385)/278.44898736729385 > 1e-3):
