...
from pypws.entities import Material, MaterialComponent, Weather, Substrate, Bund, DispersionParameters, DispersionOutputConfig, FlammableParameters, FlammableOutputConfig, ExplosionParameters, ExplosionOutputConfig, ExplosionConfinedVolume, DischargeParameters
...
def test_case_164():
    ...
    if resultCode == ResultCode.SUCCESS:
        ...
        if (abs((load_mass_inventory_vessel_for_line_rupture_scenario_calculation.vessel.location.z-(-1.7170445294165657))/(-1.7170445294165657))>1e-3):
    ...
    if resultCode == ResultCode.SUCCESS:
        if (abs(vessel_line_rupture_linked_run_calculation.discharge_record.mass_flow - 85.71385795899046)/85.71385795899046> 1e-3):
        ...
        if (abs(vessel_line_rupture_linked_run_calculation.jet_fire_flame_result.flame_length - 41.515902270698355)/41.515902270698355 > 1e-3):
        ...
        if (abs(vessel_line_rupture_linked_run_calculation.pool_fire_flame_result.flame_diameter - 58.542781829833984)/58.542781829833984 > 1e-3):
        ...
        if (len(vessel_line_rupture_linked_run_calculation.flam_conc_contour_points) != 1456):
        ...
        if (len(vessel_line_rupture_linked_run_calculation.toxic_conc_contour_points) != 629):
        ...
        if (abs(vessel_line_rupture_linked_run_calculation.area_footprint_flam_conc[0]-  893.3122239763907)/  893.3122239763907 > 1e-3):
        ...
        if (abs(vessel_line_rupture_linked_run_calculation.area_footprint_toxic_conc[0] -266309.7656022776)/266309.7656022776 > 1e-3):
        ...
        if (abs(vessel_line_rupture_linked_run_calculation.area_contour_jet[0]- 17220.13186859785)/17220.13186859785 > 1e-3):
        ...
        if (abs(vessel_line_rupture_linked_run_calculation.area_contour_pool[0] - 20853.779839337778)/20853.779839337778 > 1e-3):
        ...
        if (abs(vessel_line_rupture_linked_run_calculation.explosion_overpressure_results[0].exploded_mass-5.945743734375853)/5.945743734375853 > 1e-3):	
        ...
        if (abs(vessel_line_rupture_linked_run_calculation.explosion_overpressure_results[0].maximum_distance - 148.38430628846515)/148.38430628846515 > 1e-3):
