...
def test_vlr_160():
    ...
    material = Material("CHLORINE", [MaterialComponent("CHLORINE", 1.0)], component_count = 1)
    ...
    dispersion_parameters = [DispersionParameters(averaging_time = 600.0), DispersionParameters(averaging_time = 18.75)]
    ...
    if resultCode == ResultCode.SUCCESS:
        if (abs(vessel_leak_linked_run_calculation.discharge_record.mass_flow - 121.95136629549849)/121.95136629549849 > 1e-3):
        ...
        if (vessel_leak_linked_run_calculation.jet_fire_flame_result.flame_length !=0.0):
        ...
        if (vessel_leak_linked_run_calculation.pool_fire_flame_result.flame_diameter != 0.0):
        ...
        if (len(vessel_leak_linked_run_calculation.toxic_conc_contour_points) != 665):
        ...
        if (abs(vessel_leak_linked_run_calculation.area_footprint_toxic_conc[0] - 688622.679542524)/688622.679542524 > 1e-3):
        ...
        if (vessel_leak_linked_run_calculation.area_contour_pool[0] != 0.0):
