...
def test_vlr_159():
    ...
    material = Material("H2S+N-BUTANE", [MaterialComponent("HYDROGEN SULFIDE", 0.02), MaterialComponent("N-BUTANE", 0.98)], component_count = 2)
    ...
    if resultCode == ResultCode.SUCCESS:
        if (abs(vessel_leak_linked_run_calculation.discharge_record.mass_flow - 78.2703129718767)/78.2703129718767 > 1e-3):
        ...
        if (abs(vessel_leak_linked_run_calculation.jet_fire_flame_result.flame_length -  79.83450740611565)/ 79.83450740611565 > 1e-3):
        ...
        if (abs(vessel_leak_linked_run_calculation.pool_fire_flame_result.flame_diameter - 40.45197677612305)/40.45197677612305 > 1e-3):
        ...
        if (len(vessel_leak_linked_run_calculation.flam_conc_contour_points) != 593):
        ...
        if (len(vessel_leak_linked_run_calculation.toxic_conc_contour_points) != 695):
        ...
        if (abs(vessel_leak_linked_run_calculation.area_footprint_flam_conc[0] - 11956.182247202978)/11956.182247202978 > 1e-3):
        ...
        if (abs(vessel_leak_linked_run_calculation.area_footprint_toxic_conc[0] - 508942.11044945125)/508942.11044945125 > 1e-3):
        ...
        if (abs(vessel_leak_linked_run_calculation.area_contour_jet[0] - 59421.0918161459)/59421.0918161459 > 1e-3):
        ...
        if (abs(vessel_leak_linked_run_calculation.area_contour_pool[0] - 89742.0801748534)/89742.0801748534 > 1e-3):
        ...
        if (abs(vessel_leak_linked_run_calculation.explosion_overpressure_results[0].exploded_mass -  599.1546378493579)/ 599.1546378493579 > 1e-3):
        ...
        if (abs(vessel_leak_linked_run_calculation.explosion_overpressure_results[0].maximum_distance - 536.4452979306577)/536.4452979306577 > 1e-3):
