...
def test_case_73():
    ...
    flammable_output_configs = [FlammableOutputConfig(position = LocalPosition(5.0, 0.0, 0.0), transect = Transect(transect_start_point = LocalPosition(), transect_end_point = LocalPosition()), radiation_resolution= radiation_resolution), 
                            FlammableOutputConfig(position = LocalPosition(20.0, 0.0, 0.0), transect = Transect(transect_start_point = LocalPosition(), transect_end_point = LocalPosition()), radiation_resolution= radiation_resolution), 
                            FlammableOutputConfig(position = LocalPosition(20.0, 0.0, 1.0), transect = Transect(transect_start_point = LocalPosition(), transect_end_point = LocalPosition()), radiation_resolution= radiation_resolution),
                            FlammableOutputConfig(position = LocalPosition(20.0, 5.0, 1.0), transect = Transect(transect_start_point = LocalPosition(), transect_end_point = LocalPosition()), radiation_resolution= radiation_resolution)]
    ...
    if result_code == ResultCode.SUCCESS:
        if (abs((radiation_at_points_calculation.radiation[0]- 350000.0)/ 350000.0)>1e-3):
            assert False,f'Regression failed with radiation_at_points.radiation[0] = {radiation_at_points_calculation.radiation[0]}'
        if (abs((radiation_at_points_calculation.flame_result.flame_length-140.8483651650435)/140.8483651650435)>1e-3):
