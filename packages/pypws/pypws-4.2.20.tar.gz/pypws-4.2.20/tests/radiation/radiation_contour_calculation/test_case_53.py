...
def test_case_53():
    ...
    flammable_output_config = FlammableOutputConfig(position = LocalPosition(0.0, 0.0, 2.0), transect = Transect(transect_start_point = LocalPosition(), transect_end_point = LocalPosition()), radiation_resolution= radiation_resolution, contour_type=ContourType.SIDEVIEW)
    ...
    if result_code == ResultCode.SUCCESS:
        if (len(radiation_contour_calculation.contour_points) != 77):
        ...
        if (abs((radiation_contour_calculation.contour_points[len(radiation_contour_calculation.contour_points)-2].x-(-4.867104864552701))/(-4.867104864552701))>1e-3):
