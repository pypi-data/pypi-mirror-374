...
def test_vrv_methane():

    # Define the initial state of the vessel.
    state = State(temperature=240.0, pressure=float(5e6), liquid_fraction=1.0)
    ...
    # Create a vessel to use in the relief valve calculation using the previously defined entities. The vessel is a sphere with a diameter of 3m.
    ...
    # Create a relief valve to use in the relief valve calculation. Pipe diameter is 0.1m, pipe length is 5m, and pipe height fraction is 1.
    relief_valve = ReliefValve(pipe_diameter=0.1, pipe_length=5.0, pipe_height_fraction=1, relief_valve_constriction_diameter=0.01)
    ...
    if resultCode == ResultCode.SUCCESS:
        if (abs((vessel_relief_valve_calculation.discharge_result.release_mass -  701.8787816018365)/ 701.8787816018365) > 1e-3):
        ...
        if (abs((vessel_relief_valve_calculation.discharge_records[1].time - 798.3770006929267) / 798.3770006929267) > 1e-3):
        ...
        if (abs((vessel_relief_valve_calculation.discharge_records[0].mass_flow -  0.8791320153169022) /  0.8791320153169022) > 1e-3):
        ...
        if (abs((vessel_relief_valve_calculation.discharge_records[0].orifice_state.pressure -  101325.0)/   101325.0) > 1e-3):
        ...
        if (abs((vessel_relief_valve_calculation.discharge_records[0].final_velocity -  112.52375086203219)/  112.52375086203219) > 1e-3):
