...
def test_vlr_oxygen():
    ...
    state = State(temperature=300.0, pressure=float(1e7), liquid_fraction=1.0)

    # Define the material contained by the vessel.
    material = Material("OXYGEN", [MaterialComponent("OXYGEN", 1.0)])
    ...
    # Create a vessel to use in the line rupture calculation using the previously defined entities. The vessel is a sphere with a diameter of 5m.
    ...
    # Create a line rupture to use in the vessel line rupture calculation. Pipe diameter is 0.5m, pipe length is 10m, and pipe height fraction is 1.
    line_rupture = LineRupture(pipe_diameter=0.5, pipe_length=10.0, pipe_height_fraction=1)
    ...
    if resultCode == ResultCode.SUCCESS:
        ...
        if (abs((vessel_line_rupture_calculation.discharge_records[0].mass_flow - 4351.317585307134) / 4351.317585307134) > 1e-3):
        ...
        if (abs((vessel_line_rupture_calculation.discharge_records[0].final_state.temperature - 147.19547540940934) / 147.19547540940934) > 1e-3):
        ...
        if (abs((vessel_line_rupture_calculation.discharge_records[0].final_velocity - 510.5081623160129) / 510.5081623160129) > 1e-3):
        ...
        if (abs((vessel_line_rupture_calculation.discharge_result.release_mass - 8699.938240300542) / 8699.938240300542) > 1e-3):
