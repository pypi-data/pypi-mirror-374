...
def test_nitrogen_pentane_vessel_state_calculation():
    # Define the initial state of the vessel.
    state = State(temperature=270.0, pressure=4.0e5, liquid_fraction=0.3)
    # Define the material contained by the vessel.
    material = Material("NITROGEN_PENTANE", [MaterialComponent("NITROGEN", 0.5), MaterialComponent("N-PENTANE", 0.5)], component_count=2)
    ...
    if resultCode == resultCode.SUCCESS:
        if (vessel_state_calculation.vessel_conditions != 1):
