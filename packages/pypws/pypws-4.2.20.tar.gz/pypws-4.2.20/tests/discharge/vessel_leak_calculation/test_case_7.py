...
def test_case_7():
    ...
    if resultCode == ResultCode.SUCCESS:
        if (abs((vessel_leak_calculation.discharge_records[0].mass_flow- 12.605655881613954)/ 12.605655881613954)>1e-3):
        ...
        if (len(vessel_leak_calculation.discharge_records) != 900):
        ...
        if (abs((vessel_leak_calculation.discharge_records[0].final_velocity - 12.154668210488419)/12.154668210488419)>1e-3):
        ...
        if (abs((vessel_leak_calculation.discharge_records[0].droplet_diameter - 0.002048354602442208)/0.002048354602442208)>1e-3):
