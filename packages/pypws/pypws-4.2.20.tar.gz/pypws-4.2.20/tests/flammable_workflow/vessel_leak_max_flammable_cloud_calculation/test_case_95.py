...
def test_case_95():
    ...
    # Define the weather
    weather = Weather(wind_speed = 1.0, stability_class = AtmosphericStabilityClass.STABILITY_AB, wind_profile_flag = WindProfileFlag.LOGARITHMIC_PROFILE)
    ...
    dispersion_output_config = DispersionOutputConfig(special_concentration = SpecialConcentration.UFL, resolution = Resolution.LOW, elevation = 0.0)
    ...
    if resultCode == ResultCode.SUCCESS:
        ...
        if (abs((vessel_leak_max_flammable_cloud_calculation.vessel_leak_max_flammable_cloud_results.lfl_extent - 14.438316607373737)/ 14.438316607373737)>1e-3):
        ...
        if (abs((vessel_leak_max_flammable_cloud_calculation.vessel_leak_max_flammable_cloud_results.lfl_area - 13.222080129736838)/ 13.222080129736838)>1e-3):
        ...
        if (abs((vessel_leak_max_flammable_cloud_calculation.vessel_leak_max_flammable_cloud_results.lfl_height - 0.9839692919828957)/  0.9839692919828957)>1e-3):
