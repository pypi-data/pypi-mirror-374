...
def test_case_110():
    ...
    elevation = 3.0
    resolution = Resolution.VERY_HIGH
    special_concentration = SpecialConcentration.UFL
    lfl_fraction = 0.1
    ...
    if resultCode == resultCode.SUCCESS:
        if (len(max_conc_distance_calculation.concentration_records) >0):
            if (abs((max_conc_distance_calculation.concentration_records[len(max_conc_distance_calculation.concentration_records)-1].concentration-0.11220565556406681)/0.11220565556406681) > 1e-3):
                assert False,f'Regression failed with max_conc_distance_calculation.concentration_records[len(max_conc_distance_calculation.concentration_records)-1].concentration = {max_conc_distance_calculation.concentration_records[len(max_conc_distance_calculation.concentration_records)-1].concentration}'
        if (len(max_conc_distance_calculation.concentration_records) != 418):
        ...
        if (len(max_conc_distance_calculation.concentration_records) >0):    
            if (abs((max_conc_distance_calculation.concentration_records[len(max_conc_distance_calculation.concentration_records)-1].position.x- 3.5835448524927003)/ 3.5835448524927003) > 1e-3):
                assert False,f'Regression failed with max_conc_distance_calculation.concentration_records[len(max_conc_distance_calculation.concentration_records)-1].concentration = {max_conc_distance_calculation.concentration_records[len(max_conc_distance_calculation.concentration_records)-1].position.x}'
