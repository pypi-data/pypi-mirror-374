...
def test_case_107():  # Disabled temporarily for debugging
    ...
    lfl_fraction = 0.5
    component_to_track_name = 'N-BUTANE'


    # Define the material
    material = Material(name = material_name, components = [MaterialComponent(name = 'ETHANE', mole_fraction = 0.5),
                                                            MaterialComponent(name = 'N-BUTANE', mole_fraction = 0.3), 
    ...
    if resultCode == resultCode.SUCCESS:
        if (len(max_conc_distance_calculation.concentration_records) >0):
            if (abs((max_conc_distance_calculation.concentration_records[len(max_conc_distance_calculation.concentration_records)-1].concentration-0.02941206246379815)/0.02941206246379815) > 1e-3):
        ...
        if (len(max_conc_distance_calculation.concentration_records) != 168):
        ...
        if (len(max_conc_distance_calculation.concentration_records) >0):
            if (abs((max_conc_distance_calculation.concentration_records[len(max_conc_distance_calculation.concentration_records)-1].position.x-4.048969882876413)/4.048969882876413) > 1e-3):
