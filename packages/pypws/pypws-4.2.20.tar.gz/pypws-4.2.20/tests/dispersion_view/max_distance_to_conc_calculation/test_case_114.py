...
from pypws.enums import AtmosphericStabilityClass, ResultCode, SurfaceType, SpecialConcentration, WindProfileFlag


def test_case_114():

    """
        Maximum distance to concentration calculation test case with the following properties:
    ...
    dispersion_output_config = DispersionOutputConfig(concentration = end_point_concentration, elevation = 1.0, special_concentration = SpecialConcentration.LFL)
    ...
    if resultCode == resultCode.SUCCESS:
        if (abs((max_distance_to_conc_calculation.distance - 42.28674862332736)/ 42.28674862332736) > 1e-3):
