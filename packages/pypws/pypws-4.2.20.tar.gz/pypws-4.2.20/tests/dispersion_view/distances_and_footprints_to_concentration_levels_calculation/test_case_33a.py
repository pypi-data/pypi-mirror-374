...
def test_case_33a():
    ...
    dispersion_output_config = DispersionOutputConfig(concentration = end_point_concentration, elevation=2.5)
    ...
    if resultCode == resultCode.SUCCESS:
        if (len(distances_and_footprints_to_concentration_levels_calculation.contour_points) != 553):
        ...
        if (len(distances_and_footprints_to_concentration_levels_calculation.contour_points)>0):
            if (abs((distances_and_footprints_to_concentration_levels_calculation.contour_points[len(distances_and_footprints_to_concentration_levels_calculation.contour_points)-3].x -(-0.09459660419095317))/(-0.09459660419095317)) > 1e-3):
                assert False, f'Regression failed with last element of contour points = {distances_and_footprints_to_concentration_levels_calculation.contour_points[len(distances_and_footprints_to_concentration_levels_calculation.contour_points)-3].x}'
        if (abs((distances_and_footprints_to_concentration_levels_calculation.distances_concentration[0] - 10.10796681882425)/ 10.10796681882425) > 1e-3):
            assert False, f'Regression failed with distances_and_footprints_to_concentration_levels_calculation.distances_concentration[0] = {distances_and_footprints_to_concentration_levels_calculation.distances_concentration[0]}'
