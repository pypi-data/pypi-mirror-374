WITH web_v1 AS (
  ...
  WHERE
    ...
    AND NOT (ws_item_sk IS NULL) /* `NOT` operator requires parenthesis https://github.com/lakehq/sail/issues/729 */
  GROUP BY
    ws_item_sk,
    d_date
), store_v1 AS (
  ...
  WHERE
    d_month_seq BETWEEN 1183 AND 1183 + 11
    AND NOT (ss_item_sk IS NULL) /* `NOT` operator requires parenthesis https://github.com/lakehq/sail/issues/729 */
  GROUP BY
    ss_item_sk,
    d_date
)
...
FROM (
  ...
  FROM (
    SELECT
      CASE WHEN NOT (web.item_sk IS NULL) THEN web.item_sk ELSE store.item_sk END AS item_sk, /* `NOT` operator requires parenthesis https://github.com/lakehq/sail/issues/729 */
      CASE WHEN NOT (web.d_date IS NULL) THEN web.d_date ELSE store.d_date END AS d_date, /* `NOT` operator requires parenthesis https://github.com/lakehq/sail/issues/729 */
