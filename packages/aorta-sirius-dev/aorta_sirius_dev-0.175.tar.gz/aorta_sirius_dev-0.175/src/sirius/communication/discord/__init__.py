...
class DiscordHTTPSession(AsyncHTTPSession):
    ...
    async def post(self, url: str, data: Dict[str, Any] | None = None, headers: Dict[str, Any] | None = None, is_form_url_encoded: bool = False, file: RequestFiles | None = None) -> HTTPResponse:
        try:
...
DiscordHTTPSession(constants.URL, {"Authorization": f"Bot {common.get_environmental_secret(EnvironmentSecret.DISCORD_BOT_TOKEN)}"})
...
class Message(DataClass):
    ...
    def get(message: discord.message.Message) -> "Message":
        return Message(
            ...
            reference=Message.get(message.reference.resolved) if message.reference is not None else None  # type: ignore[arg-type]
        )
