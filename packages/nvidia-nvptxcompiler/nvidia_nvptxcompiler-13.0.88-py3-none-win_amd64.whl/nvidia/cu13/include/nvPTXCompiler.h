...
/** \ingroup versioning
 *
 * \brief            Sets the output paramater \p numVersions with the number of PTX ISA versions supported by
 *                   PTX Compiler Library.
 *                   This can be used to pass a 2D Array to nvPTXCompilerGetSupportedVersions. Number of rows in 2D Array
 *                   will be equal to value populated in \p numVersions.
 *
 * \param            [out] numVersions   Number of supported PTX ISA versions
 *
 * \return
 *   - \link #nvPTXCompileResult NVPTXCOMPILE_SUCCESS \endlink
 *   - \link #nvPTXCompileResult NVPTXCOMPILE_ERROR_INVALID_INPUT \endlink
 */
nvPTXCompileResult nvPTXCompilerGetNumSupportedVersions(int *numVersions);

/** \ingroup versioning
 *
 * \brief            Populates the 2D Array passed via output paramater \p versions with PTX ISA Versions major, minor pair supported by
 *                   PTX Compiler Library. The array is sorted in the ascending order.
 *                   the row size of the 2D array can be determined using nvPTXCompilerGetNumSupportedVersions.
 *
 * \param            [out] versions   Sorted 2D array with column size 2, having pair of major and minor PTX ISA version supported.
 *
 * \return
 *   - \link #nvPTXCompileResult NVPTXCOMPILE_SUCCESS \endlink
 *   - \link #nvPTXCompileResult NVPTXCOMPILE_ERROR_INVALID_INPUT \endlink
 */
nvPTXCompileResult nvPTXCompilerGetSupportedVersions(int (*versions)[2]);
