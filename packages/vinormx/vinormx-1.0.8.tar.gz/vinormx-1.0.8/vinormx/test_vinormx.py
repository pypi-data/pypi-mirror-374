...
def test_covid_example():
    ...
    expected = "hàm này được phát triển từ tháng tám năm hai nghìn không trăm mười chín . có phải tháng mười hai năm hai nghìn không trăm hai mươi đã có vaccine phòng ngừa covid mười chín ích mờ giét ."
...
def test_time():
    """Test chuẩn hóa thời gian"""
    print("\n[TEST] Time Normalization")
    print("-" * 40)
    
    test_cases = [
        # Basic time formats
        ("8h30", "tám giờ ba mươi phút"),  # Vietnamese format
        ("8:30", "tám giờ ba mươi phút"),  # 24-hour format
        ("8:30AM", "tám giờ ba mươi phút sáng"),  # 12-hour format with AM
        ("8h30p", "tám giờ ba mươi phút"),  # Vietnamese with 'p' suffix
        ("14:30", "mười bốn giờ ba mươi phút"),  # 24-hour afternoon
        ("2:30PM", "mười bốn giờ ba mươi phút chiều"),  # 12-hour afternoon
        
        # Edge cases
        ("00:00", "không giờ không phút"),  # Midnight
        ("12:00", "mười hai giờ không phút"),  # Noon
        ("23:59", "hai mươi ba giờ năm mươi chín phút"),  # Late night
        ("1:00", "một giờ không phút"),  # Single digit hour
        ("9:05", "chín giờ năm phút"),  # Single digit minute
        
        # With seconds
        ("8:30:45", "tám giờ ba mươi phút bốn mươi lăm giây"),  # With seconds
        ("12:00:00", "mười hai giờ không phút không giây"),  # Noon with seconds
        
        # Different separators and formats
        ("8h", "tám giờ"),  # Just hour
        ("8 giờ", "tám giờ"),  # Vietnamese hour format
        ("8h30phút", "tám giờ ba mươi phút"),  # Full Vietnamese format
        
        # Mixed content
        ("Họp lúc 8h30 sáng mai", "họp lúc tám giờ ba mươi phút sáng mai"),  # Time in sentence
        ("Cuộc họp từ 9:00 đến 17:00", "cuộc họp từ chín giờ không phút đến mười bảy giờ không phút"),  # Time range
    ]
    ...
    for input_time, expected in test_cases:
        try:
            result = TTSnorm(input_time)
            status = "[OK]" if result == expected else "[WARN]"
            print(f"{status} {input_time:>15} -> {result}")
            if result == expected:
                passed += 1
            else:
                print(f"      Expected: {expected}")
        except Exception as e:
            print(f"[ERROR] {input_time:>15} -> ERROR: {e}")
    
    print(f"\nTime test results: {passed}/{total} patterns processed correctly")
    return passed >= total * 0.8  # 80% success rate


def test_time_context_detection():
    """Test context detection for time vs ratio patterns"""
    print("\n[TEST] Time Context Detection")
    print("-" * 40)
    
    test_cases = [
        # Time context cases
        ("lúc 8:10 sáng", "lúc tám giờ mười phút sáng"),
        ("hồi 8:10", "hồi tám giờ mười phút"),
        ("thời gian 8:10", "thời gian tám giờ mười phút"),
        ("giờ 8:10", "giờ tám giờ mười phút"),
        ("8:10 sáng mai", "tám giờ mười phút sáng mai"),
        ("cuộc họp lúc 14:30", "cuộc họp lúc mười bốn giờ ba mươi phút"),
        ("đến 12:00", "đến mười hai giờ không phút"),
        
        # Ratio context cases
        ("tỉ số 8:10", "tỉ số tám mười"),
        ("tỷ lệ 8:10", "tỷ lệ tám mười"),
        ("so với 8:10", "so với tám mười"),
        ("chia 8:10", "chia tám mười"),
        ("kết quả 8:10", "kết quả tám mười"),
        ("điểm số 8:10", "điểm số tám mười"),
        ("bài thi 8:10", "bài thi tám mười"),
        
        # Edge cases - should default to time context
        ("8:10", "tám giờ mười phút"),  # Ambiguous, default to time
        ("15:30", "mười lăm giờ ba mươi phút"),  # Valid time
        ("25:10", "hai mươi lăm mười"),  # Invalid time, likely ratio
        ("8:70", "tám bảy mươi"),  # Invalid time, likely ratio
    ]
    ...
    for input_text, expected in test_cases:
        try:
            ...
            print(f"[ERROR] {input_text:>20} -> ERROR: {e}")
    
    print(f"\nContext detection results: {passed}/{total} patterns processed correctly")
    return passed >= total * 0.8  # 80% success rate
...
def main():
    ...
    tests = [
        ...
        ("Time Normalization", test_time),
        ("Time Context Detection", test_time_context_detection),
