...
class PunctuationMapper:
    ...
    def normalize_punctuation(self, text: str, keep_punct: bool = False) -> str:
        ...
        # Protect time patterns from punctuation normalization
        time_patterns = [
            r'\b\d{1,2}h\d{1,2}(?:p|phút)?\b',  # 8h30, 8h30p, 8h30phút
            r'\b\d{1,2}:\d{1,2}(?::\d{1,2})?\b',  # 8:30, 8:30:45
            r'\b\d{1,2}:\d{1,2}\s*(AM|PM|am|pm|A\.M\.|P\.M\.|a\.m\.|p\.m\.)\b',  # 8:30AM, 8:30 PM
            r'\b\d{1,2}h\d{1,2}\s*(AM|PM|am|pm|A\.M\.|P\.M\.|a\.m\.|p\.m\.)\b',  # 8h30AM, 8h30 PM
            r'\b\d{1,2}h\b',  # 8h
            r'\b\d{1,2}:00(?::00)?\b',  # 8:00, 8:00:00
        ]
        
        # Protect time patterns
        protected_text = text
        replacements = {}
        
        for pattern in time_patterns:
            time_matches = list(re.finditer(pattern, protected_text, re.IGNORECASE))
            for i, match in enumerate(time_matches):
                placeholder = f"__TIME_PUNCT_PLACEHOLDER_{i}__"
        ...
        protected_text = re.sub(r'[()[\]{}]+', '', protected_text)
        protected_text = re.sub(r'["\'""`''""]+', '', protected_text)
        protected_text = re.sub(r'[-–—]+', ' ', protected_text)  # Remove dashes
...
class MappingManager:
    ...
    def _convert_numbers_in_text(self, text: str) -> str:
        ...
        # Find all date and time patterns and protect them
        protected_text = text
        replacements = {}
        
        # Protect date patterns
        ...
        for pattern in time_patterns:
            time_matches = list(re.finditer(pattern, protected_text, re.IGNORECASE))
            for i, match in enumerate(time_matches):
                placeholder = f"__TIME_PLACEHOLDER_{i}__"
                replacements[placeholder] = match.group(0)
                protected_text = protected_text.replace(match.group(0), placeholder, 1)
        ...
        # Restore date and time patterns
        for placeholder, original in replacements.items():
            protected_text = protected_text.replace(placeholder, original)
