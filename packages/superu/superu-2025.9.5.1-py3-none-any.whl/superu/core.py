...
class PlutoWrapper:
    def __init__(self, api_key, assistants):
        ...
        self.pluto_1_1_url = "pluto_1_1_ws.superu.ai:5000"

    def register_call(self, call_uuid):
        response = requests.post(f'http://{self.pluto_1_1_url}/register_call', json={'api_key': self.api_key, "call_uuid": call_uuid})
        if response.status_code != 200:
    ...
    def create_call(self , voice_id , call_uuid=None , model=None , assistant_id=None , system_prompt=None , first_message=None , language_clue="en-US" , background_audio=False, prewarm_agent=False , localhost=None , Telephony=False):
        if not call_uuid:
        ...
        if assistant_id:
            ...
            if assistant:
                ...
                if prewarm_agent:
                    self.join_agent_advance(self.pluto_url , call_id , json_for_url)
    ...
    def create_call_plivo(self , voice_id , from_ , to_ , model=None , assistant_id=None , system_prompt=None , first_message=None , language_clue="en-US" , background_audio=False, prewarm_agent=False , localhost=None ,max_duration_seconds=120 , **kwargs):
        data_json = {
            'from_': from_,
            'to_': to_,
            'max_duration_seconds': max_duration_seconds,
        ...
        self.create_call(voice_id , model=model , call_uuid=plivo_call_uuid , assistant_id=assistant_id , system_prompt=system_prompt , first_message=first_message , language_clue=language_clue , background_audio=background_audio , prewarm_agent=prewarm_agent , localhost=localhost , Telephony=True)
