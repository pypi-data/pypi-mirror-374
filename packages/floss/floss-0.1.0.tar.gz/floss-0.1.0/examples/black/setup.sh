...
case "$BUG_NUMBER" in
  [1-9]|1[0-9]|2[0-3])
    echo "==> Setting up black bug $BUG_NUMBER"
    ;;
  *)
    echo "Error: Invalid bug number '$BUG_NUMBER'. Available bugs: 1 to 23" >&2
    exit 1
    ;;
esac
...
VENV_NAME="black-bug${BUG_NUMBER}"
REQUIRED_PY="3.8"
...
echo "==> Checking out black buggy version (issue $BUG_NUMBER)"
bugsinpy-checkout -p black -v 0 -i "$BUG_NUMBER" -w "$SCRIPT_DIR"
