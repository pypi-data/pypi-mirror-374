...
class RouteCreator(AttributeInitialiserMixin):
    ...
    def _prepare_relation_route_data(self, relation_data: dict[str, Any], session: Any) -> dict[str, Any]:
        ...
        # Determine relation URL style
        relation_key = relation_data["relationship"]
        relation_style = get_config_or_model_meta("API_RELATION_URL_STYLE", model=child_model, default="relation-key")
        # Use relation key as the final path segment when configured
        final_segment = relation_key if relation_style == "relation-key" else child_endpoint
        ...
        return {
            ...
            "url": relation_url,
            # Ensure internal route name uniqueness by including relation key
            "name": f"{child_model.__name__.lower()}_join_to_{parent_model.__name__.lower()}_{relation_key}",
    ...
    def _create_unique_route_function(
        ...
        *,
        ...
        suffix = f"_{relation_name}" if relation_name else ""
        unique_function_name = (
            f"route_wrapper_{http_method}_collection_{url.replace('/', '_')}{suffix}" if is_many else f"route_wrapper_{http_method}_single_{url.replace('/', '_')}{suffix}"
        )
    ...
    def _add_relation_url_function_to_model(self, id_key: str, child: Callable, parent: Callable, relation_name: str | None = None):
        ...
        relation_style = get_config_or_model_meta("API_RELATION_URL_STYLE", model=child, default="relation-key")
        final_segment = relation_name if (relation_style == "relation-key" and relation_name) else child_endpoint
