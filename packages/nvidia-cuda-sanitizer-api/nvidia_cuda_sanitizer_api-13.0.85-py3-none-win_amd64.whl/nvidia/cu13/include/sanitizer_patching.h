 ...
 * If a patch returns SANITIZER_PATCH_ERROR, the full warp which the thread
 * belongs to will be exited.
 */
typedef enum {
    /**
     * No error.
 ...
 * \param kind Barrier action type. See \ref Sanitizer_CudaBarrierInstructionKind.
 * \param data Barrier data. This is specific to each action type, refer to \ref Sanitizer_CudaBarrierInstructionKind.
 * \param result In case of a barrier wait, this indicates whether the wait was successful or not.
 */
typedef SanitizerPatchResult (SANITIZERAPI *SanitizerCallbackCudaBarrierAttempt)(void* userdata, uint64_t pc, void* barrier, uint32_t kind, uint32_t data, uint32_t result);
 ...
 */
typedef SanitizerPatchResult (SANITIZERAPI *SanitizerCallbackClusterBarrierWait)(void* userdata, uint64_t pc);
...
/**
 * \brief Function type for a async bulk copy from shared to global memory.
 *
 * This can be generated by a cp.async.bulk.global.shared::cta instruction.
 *
 * \param userdata Pointer to user data. See \ref sanitizerPatchModule.
 * \param pc Program counter of the patched instruction.
 * \param dst Destination global memory address.
 * \param src Source DSMEM address.
 * \param numBlocks Contains the number of 16 bytes blocks requested to be copied.
 *
 * All the active threads in a warp have the same parameter values.
 */
typedef SanitizerPatchResult (SANITIZERAPI *SanitizerCallbackBulkCopySharedToGlobal)(void* userdata, uint64_t pc, uint64_t dst, uint32_t src, uint32_t numBlocks);

/**
 * \brief Function type for a async bulk copy from shared to shered memory.
 *
 ...
 * All the active threads in a warp have the same parameter values.
 */
typedef SanitizerPatchResult (SANITIZERAPI *SanitizerCallbackBulkCopySharedToShared)(void* userdata, uint64_t pc, uint32_t src, uint32_t dst, uint32_t barrier, uint32_t numBlocks);

/**
 * \brief Function type for a async bulk reduction from shared to global memory.
 *
 ...
 * All the active threads in a warp have the same parameter values.
 */
typedef SanitizerPatchResult (SANITIZERAPI *SanitizerCallbackBulkReductionSharedToGlobal)(void* userdata, uint64_t pc, uint64_t dst, uint32_t src, uint32_t numBlocks);

/**
 * \brief Function type for a async bulk reduction from shared to shered memory.
 *
 ...
 * All the active threads in a warp have the same parameter values.
 */
typedef SanitizerPatchResult (SANITIZERAPI *SanitizerCallbackBulkReductionSharedToShared)(void* userdata, uint64_t pc, uint32_t src, uint32_t dst, uint32_t barrier, uint32_t numBlocks);
...
typedef enum {
     ...
     */
    SANITIZER_INSTRUCTION_BULK_COPY_SHARED_TO_GLOBAL  = 35,
     ...
     */
    SANITIZER_INSTRUCTION_BULK_COPY_SHARED_TO_SHARED  = 36,
     ...
     */
    SANITIZER_INSTRUCTION_BULK_REDUCTION_SHARED_TO_GLOBAL  = 37,
     ...
     */
    SANITIZER_INSTRUCTION_BULK_REDUCTION_SHARED_TO_SHARED  = 38,

    /**
     * CUDA Barrier operation (potentially unsuccessful). The type of the callback must be
     * SanitizerCallbackCudaBarrierAttempt.
     */
    SANITIZER_INSTRUCTION_CUDA_BARRIER_ATTEMPT        = 39,

    /**
     * Entering cluster barrier wait instruction. The type of the callback must
     * be SanitizerCallbackClusterBarrierWait.
     */
    SANITIZER_INSTRUCTION_CLUSTER_BARRIER_WAIT_ENTRY  = 40,
