...
class GenomeAnnotationWorkflow:
    ...
    def __init__(
            ...
            debug=False,
            flags= "genome_fasta:\n    class: File\n    location: {}") -> None: # run_interproscan: True\ninterproscan_applications: Pfam\n
    ...
    def process_batch(self, fasta_paths: str | list[str] | set[str], threads: int = 1) -> list[str]:
        ...
        if isinstance(fasta_paths, (str, Path)):
            ...
            if not is_url(fasta_paths) and path_obj.is_file() and path_obj.suffix not in [".fasta", ".fa", ".fna", "fasta.gz", "fna.gz"]:
        ...
        for fasta in fasta_paths: # tqdm(fasta_paths, desc="Annotating genomes"):
            logger.debug(f"fasta: {fasta}")
            genome_identifier = re.findall( r"\b([A-Z]{3}_\d{9})\b", fasta)[0]

            specific_output_dir = Path(self.output_dir) / "genomes" / genome_identifier[:7]  / genome_identifier[:10] / f"{genome_identifier}_annotation"
            logger.debug(f"specific_output_dir: {specific_output_dir}")
            logger.debug(os.path.join(specific_output_dir, '*.SAPP.hdt.gz'))
            logger.debug(glob.glob(os.path.join(specific_output_dir, '*.SAPP.hdt.gz')))
            os.makedirs(specific_output_dir, exist_ok=True)

            if not glob.glob(os.path.join(specific_output_dir, "*.SAPP.hdt.gz")):
