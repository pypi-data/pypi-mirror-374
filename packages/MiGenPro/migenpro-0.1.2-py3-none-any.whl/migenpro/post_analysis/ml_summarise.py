...
class MachineLearningData:
    ...
    def get_results_for_method(self, method: str, test:  str="test") -> dict:
        """
        ...
        """
        # Support both historical naming ("*-{test}-output.tsv") and simpler ("*-{test}.tsv")
...
class SummaryGraphs:
    def __init__(self, machine_learning_output_data: MachineLearningData, output_dir: str, test=True, debug=False):
    ...
    def analyse_classifiers(self, scenario: str="test"):
        """Main analysis entry point"""
        for classifier in self.classifiers:
            self._process_classifier(classifier, scenario)
    ...
    def _process_classifier(self, classifier: str, scenario: str="test"):
        """Process individual classifier results"""
        results = self.machine_learning_output_data.get_results_for_method(classifier, scenario)
    ...
    @staticmethod
    def _get_true_probability(results):
        """
        Calculates the true class probabilities from the provided probability classes.

        Args:
            results (dict): A dictionary containing 'probability_classes' and
                'observed_values'. 'probability_classes' is expected to be a list of
                dictionaries, where each dictionary represents the probability
                distribution for a specific instance. 'observed_values' is an array-like
                object representing the true class labels for the instances.

        Returns:
            list: A list of probabilities corresponding to the true class labels for
            each instance in the provided data.
        """
        probability_classes = pd.DataFrame(results["probability_classes"])
    ...
    def _calculate_basic_metrics(self, results):
        ...
        return {
            ...
            'auc': roc_auc_score(results["observed_values"], probability_true_class)
        }
