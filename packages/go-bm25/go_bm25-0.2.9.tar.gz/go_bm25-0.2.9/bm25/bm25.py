...
if _lib:
    ...
    _lib.SetBM25Parameters.argtypes = [ctypes.c_longlong, ctypes.c_double, ctypes.c_double, ctypes.c_double]
    _lib.SetBM25Parameters.restype = ctypes.c_int
...
class BM25Index:
    ...
    def set_parameters(self, k1: float, b: float, epsilon: float) -> bool:
        """Set BM25 parameters for this index."""
        if not _lib:
            raise RuntimeError("BM25 library not loaded")
        
        result = _lib.SetBM25Parameters(self.handle, ctypes.c_double(k1), ctypes.c_double(b), ctypes.c_double(epsilon))
        return bool(result)
