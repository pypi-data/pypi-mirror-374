...
class BM25:
    ...
    def __init__(self, documents: Optional[List[List[str]]] = None, 
        ...
        # Set BM25 parameters in the Go implementation
        self._index.set_parameters(k1, b, epsilon)
    ...
    def get_term_idf(self, term: str) -> float:
        ...
        Returns:
            IDF value for the term, or 0.0 if term does not exist
        
        Example:
            >>> bm25_index = BM25([["hello", "world"], ["hello", "python"]])
            >>> idf = bm25_index.get_term_idf("hello")
            >>> print(f"IDF for hello: {idf}")
        """
        return self._index.get_term_idf(term)
    ...
    def get_all_term_idfs(self) -> List[Dict[str, Any]]:
        ...
        Example:
            >>> bm25_index = BM25([["hello", "world"], ["hello", "python"]])
            >>> all_idfs = bm25_index.get_all_term_idfs()
            >>> for term_info in all_idfs:
            ...     print(f"Term: {term_info["Term"]}, IDF: {term_info["IDF"]:.3f}")
        """
        return self._index.get_all_term_idfs()
    ...
    def get_term_idfs_sorted(self, sort_by: str = "idf") -> List[Dict[str, Any]]:
        ...
        Example:
            >>> bm25_index = BM25([["hello", "world"], ["hello", "python"]])
            >>> sorted_idfs = bm25_index.get_term_idfs_sorted("idf")
            >>> for term_info in sorted_idfs:
            ...     print(f"Term: {term_info["Term"]}, IDF: {term_info["IDF"]:.3f}")
        """
        return self._index.get_term_idfs_sorted(sort_by)
    ...
    def get_term_stats(self, term: str) -> Dict[str, int]:
        ...
        Example:
            >>> bm25_index = BM25([["hello", "world"], ["hello", "python"]])
            >>> stats = bm25_index.get_term_stats("hello")
            >>> print(f"Document count: {stats["docCount"]}, Total frequency: {stats["totalFreq"]}")
        """
        return self._index.get_term_stats(term)
