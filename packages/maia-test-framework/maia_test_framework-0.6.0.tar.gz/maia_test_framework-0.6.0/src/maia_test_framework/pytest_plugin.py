...
def pytest_runtest_teardown(item):
    ...
    # Validators were already run in pytest_runtest_call
    # Just check if any failed to set the final status
    if any(v.status == "failed" for v in test_instance.validator_results):
        final_pytest_status = "failed"
