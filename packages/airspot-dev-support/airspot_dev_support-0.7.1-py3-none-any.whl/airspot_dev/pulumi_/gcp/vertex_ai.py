...
class _AgentEngineProvider(ResourceProvider):
    ...
    def create(self, inputs):
        ...
        try:
            ...
            engine = next(iter(agent_engines.list(filter=f"display_name='{display_name}'")))
        ...
        return CreateResult(id_=agent_engine_id, outs={'agent_engine_id': agent_engine_id})
    ...
    def delete(self, id, props):
        ...
        try:
            ...
            pulumi.log.info(f"Successfully initiated deletion for Agent Engine: {resource_name}")
        except Exception as e:
            # Log errors but don't fail the destroy operation
            pulumi.log.warn(f"Could not delete Agent Engine {resource_name}: {str(e)}")
...
class _AgentEngine(Resource):
    ...
    """
    agent_engine_id: pulumi.Output[str]
