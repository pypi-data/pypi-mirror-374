...
def generate_map(df: pd.DataFrame, map_file: str) -> None:
    ...
    df['chr'] = df['chr'].replace({'M': '26', 'MT': '26'})statu
...
def read_pyarrow(
    ...
    logging.info("[read_pyarrow] read %s, df shape: %s.", path, df.shape)
...
def prepare_pylmm_inputs(
    ...
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    logging.debug("ensuring directory exists [complete]")
    ...
    for measure_id in measure_ids:
        for sex in ("f", "m"):
            ...
            logging.debug("Creating PLINK for: %s.%s", measure_id, sex)

            if not os.path.exists(measure_ped_file):
                logging.debug(f"Data not available for sex: {sex}")
                break
            ...
            logging.debug("bed bim fam [complete]")
            ...
            try:
                ...
                logging.debug("kinship [complete]")
    ...
    logging.debug("Directory contents: %s", os.listdir(outdir))
