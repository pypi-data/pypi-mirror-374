...
from msgflux.exceptions import KeyExhaustedError, ParserNotFoundError
...
class OpenAIChatCompletion(_BaseOpenAI, ChatCompletionModel):
    ...
    def __init__(
        ...
        *,
        ...
        if stop:
            self.sampling_run_params["stop"] = stop
        if verbosity:
            self.sampling_run_params["verbosity"] = verbosity
        if modalities:
    ...
    def _generate(self, **kwargs: Mapping[str, Any]) -> ModelResponse:
        ...
        if generation_schema is not None and typed_parser is None:
            json_schema = msgspec.json.schema(generation_schema)
            response_format = response_format_from_json_schema(json_schema)
        ...
        if choice.message.tool_calls:
            ...
            if typed_parser is not None:
                ...
                response_content = parser.parse(choice.message.content)
                # Type validation
