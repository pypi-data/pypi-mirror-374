...
try:
    ...
    raise ImportError(
        "`rank_bm25` client is not detected, please install"
        "using `pip install rank_bm25`"
    ) from e
...
class RankBM25LexicalRetriever(BaseRetriever, LexicalRetriever):
    ...
    def _search(self, queries, top_k, threshold, return_score):
        ...
        results = F.map_gather(process_query, kwargs_list=queries)
        return results
    ...
    def __call__(
        ...
        *,
        ...
        # TODO adaptar o formato de resposta para a nova convenção
