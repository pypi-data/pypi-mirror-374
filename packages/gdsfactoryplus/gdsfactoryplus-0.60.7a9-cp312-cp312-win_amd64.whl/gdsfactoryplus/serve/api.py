...
from gdsfactoryplus.core.bbox import bbox as _bbox
from gdsfactoryplus.core.build import build_by_names
from gdsfactoryplus.core.export_spice import SpiceFlavor
from gdsfactoryplus.core.export_spice import export_spice as _export_spice
from gdsfactoryplus.core.parse_spice import parse_oc_spice
...
def simulate(
    ...
    try:
        sdict = sax.sdict(
            _simulate(sim.name, layout=sim.layout, model=sim.model, how=how)
        )
...
def parse_spice_api(request: ParseSpiceRequest) -> dict[str, str]:
    ...
    try:
        ...
        recnet = parse_oc_spice(file_path)
...
def export_spice_api(request: ExportSpiceRequest) -> dict[str, str]:
    ...
    try:
        ...
        # Export to SPICE
        spice_content = _export_spice(file_path, cast(SpiceFlavor, flavor))
...
@app.post("/api/bbox")
def bbox_api(request: BboxRequest) -> dict[str, str]:
