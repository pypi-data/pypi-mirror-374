...
from gdsfactoryplus.models import DoItForMe
...
@app.post("/doitforme")
async def doitforme_post(data: DoItForMe):  # noqa: ANN201
    """Request a DoItForMe by posting an initial circuit."""
    logger.warning(f"Received DoItForMe request: {data}")
...
async def doitforme_get(  # noqa: ANN201
    ...
    """Request a DoItForMe."""
    logger.warning(f"Received DoItForMe request: {prompt}")
    return await websocket_client(
        prompt=prompt,
...
def _get_doitforme_https() -> str:
    return os.environ.get("GFP_GPT_HOST", "https://doitforme.gdsfactory.com")
