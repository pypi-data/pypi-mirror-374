#!/usr/bin/env python3
"""
Runner script for xSystem security tests.
"""
...
def run_tests(verbose=True, coverage=False):
    """Run the xSystem security tests."""
    cmd = [sys.executable, "-m", "pytest", str(Path(__file__).parent)]
    if verbose:
        cmd.append("-v")
    if coverage:
        cmd.extend(["--cov=exonware.xsystem.security", "--cov-report=term-missing"])
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run xSystem security tests")
