    ...
    for imode in range(0, nmodes):
        ...
        for ivel in range(0, Uv.size):
            ...
            while cond == 1:
                ...
                bigdet = linalg.det(
                    (A * U**2 / bchar**2 - dynpress * Q_2h) * p**2
                    + (C * U / bchar - dynpress * Q_1h) * p
                    + (E - dynpress * Q_0h)
                )
                ...
                preal = preal + np.take(
                    solvec[0], 0
                )  # Converting elements of arrays to scalars is a pain!
                # Update k
        ...
        for ivel in range(0, Uv.size):
            ...
            while cond == 1:
                ...
                line1 = np.concatenate(
                    (np.zeros((nmodes, nmodes)), np.eye(nmodes)), axis=1
                )
                line2 = np.concatenate(
                    (-linalg.solve(A, E - Qr), -linalg.solve(A, C - Qi)), axis=1
                )
...
def flutsolve_flex(
    ...
    Q0 = (
        allbodies["Phi_xall"][0].T @ allbodies["Fx0"][0]
        + allbodies["Phi_yall"][0].T @ allbodies["Fy0"][0]
    ...
    print(
        "Calculating generalized aerodynamic force matrix at "
        + str(len(kvec))
    ...
    for ik in range(0, kvec.size):
        ...
        Fx1, Fy1, Fz1, _, _, _ = SDPMcalcs.aeroforces(
            cp1,
            allbodies["nx0all"][0],
            allbodies["ny0all"][0],
            allbodies["nz0all"][0],
        ...
        Q1[ik, :, :] = (
            allbodies["Phi_xall"][0].T @ Fx1
            + allbodies["Phi_yall"][0].T @ Fy1
            + allbodies["Phi_zall"][0].T @ Fz1
        )
    ...
    if ijflut.size != 0:
        ...
        Uflut, freqflut = flutfind(
            A, C, E, Q_0, Q_1, Q_2, kvec, Uini, c0 / 2.0, rho, wini
        )
...
def flutsolve_pitchplunge(
    ...
    Fx0, Fy0, Fz0, Mx0, My0, Mz0 = SDPMcalcs.aeroforces(
        allbodies["cp0"][0],
        allbodies["nx0all"][0],
        allbodies["ny0all"][0],
        allbodies["nz0all"][0],
    ...
    for ik in range(0, kvec.size):
        ...
        # Calculate the unsteady pressure coefficients
        cpalpha, cphdot, cpalphadot, cph2dot, cpalpha2dot = (
            SDPMcalcs.unsteadysolve_pitchplunge(
                body,
                allbodies,
                Aphi,
                Bphi,
        ...
        Fxalpha, Fyalpha, Fzalpha, Mxalpha, Myalpha, Mzalpha = SDPMcalcs.aeroforces(
            cpalpha,
            allbodies["nx0all"][0],
            allbodies["ny0all"][0],
            allbodies["nz0all"][0],
        ...
        Fxhdot, Fyhdot, Fzhdot, Mxhdot, Myhdot, Mzhdot = SDPMcalcs.aeroforces(
            cphdot,
            allbodies["nx0all"][0],
            allbodies["ny0all"][0],
            allbodies["nz0all"][0],
        ...
        Fxalphadot, Fyalphadot, Fzalphadot, Mxalphadot, Myalphadot, Mzalphadot = (
            SDPMcalcs.aeroforces(
                cpalphadot,
                allbodies["nx0all"][0],
                allbodies["ny0all"][0],
                allbodies["nz0all"][0],
        ...
        Fxh2dot, Fyh2dot, Fzh2dot, Mxh2dot, Myh2dot, Mzh2dot = SDPMcalcs.aeroforces(
            cph2dot,
            allbodies["nx0all"][0],
            allbodies["ny0all"][0],
            allbodies["nz0all"][0],
        ...
        Fxalpha2dot, Fyalpha2dot, Fzalpha2dot, Mxalpha2dot, Myalpha2dot, Mzalpha2dot = (
            SDPMcalcs.aeroforces(
                cpalpha2dot,
                allbodies["nx0all"][0],
                allbodies["ny0all"][0],
        ...
        Q_1[ik, :, :] = np.array(
            [
                [-np.sum(Fzhdot), -np.sum(Fzalphadot)],
                [np.sum(Myhdot), np.sum(Myalphadot)],
            ]
        )
        # Calculate unsteady generalized aerodynamic mass matrix
        Q_2[ik, :, :] = np.array(
            [
                [-np.sum(Fzh2dot), -np.sum(Fzalpha2dot)],
                [np.sum(Myh2dot), np.sum(Myalpha2dot)],
            ]
        )
        # Calculate total unsteady generalized aerodynamic load matrix
        Q1[ik, :, :] = (
            Q_0[ik, :, :] + 1j * k * Q_1[ik, :, :] + (1j * k) ** 2.0 * Q_2[ik, :, :]
        )
    # End for
