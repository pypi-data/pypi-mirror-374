"""
Script to archive the current version of the src/ folder.
Copies the src/ directory to ext/archive/versions/ based on the current version.
"""
...
def archive_current_version():
    """Archive the current src/ folder based on version."""
    ...
    for root, dirs, files in os.walk(src_path):
        dirs[:] = [
            d for d in dirs if not should_exclude_file(Path(root) / d, exclude_patterns)
        ]
        for file in files:
    ...
    with Progress(
        ...
        with zipfile.ZipFile(dest_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            # Walk through the src directory and add all files
            for root, dirs, files in os.walk(src_path):
                ...
                for file in files:
                    ...
                    file_count += 1
                    progress.update(
                        archive_task, advance=1, description=f"Adding: {relative_path}"
                    )
