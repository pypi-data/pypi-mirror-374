...
def start_servers():
    for i in range(ranges[0], ranges[1]):
        p = subprocess.Popen(['python', '-u', path, str(i), arch[0]], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)#, creationflags=subprocess.CREATE_NEW_CONSOLE)#
    ...
    def on_transform_work(self, instance, value):
        ...
        if f != compute64:
            ...
            if not do_sync[i]:
                ...
                p = subprocess.Popen(['python', '-u', path, str(curr_max_n[0]), arch[0]], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)#, creationflags=subprocess.CREATE_NEW_CONSOLE)#
                subprocesses.append(p)
