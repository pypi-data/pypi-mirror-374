...
def is_korean(char: str) -> bool:
    """
    a function that checks if it's korean
...
def group(group_name: Literal['Hangul Jamo', 'Hangul Compatibility Jamo', 'Hangul Jamo Extended-A', 'Hangul Jamo Extended-B', 'Hangul Syllables']) -> range:
    """
    a function that gives you a matching range object by hangul unicode block names

    input      | dtype          | default
    group_name | Literal value  | -
...
def combine(s: Union[str, bytes], *, ensure_korean: bool = True, only_syl: bool = False) -> Union[str, bytes]:
    """
    a function that is similar with jamo.jamo_to_hangul(), but more flexible

    input         | dtype                    | default
    s             | Union value (str, bytes) | -
    ensure_korean | bool                     | True
    only_syl      | bool                     | False

    return: Union value (str, bytes)
    """
    savetype = type(s)
...
def extend(s: Union[str, bytes], *, ensure_korean: bool = True, only_syl: bool = False) -> Union[str, bytes]:
    """
    a function that is similar with jamo.hangul_to_jamo(), but more flexible
...
def get_sound(s: Union[str, bytes], *, only_korean: bool = True) -> Union[str, bytes]:
    """
    a function that returns the sound (romanize) of the korean text

    input       | dtype                    | default
    s           | Union value (str, bytes) | -
    only_korean | bool                     | True
...
def from_sound(s: Union[str, bytes]) -> Union[str, bytes]:
    """
    a function that changes the sound (romanized text) to korean
...
def words(*, source: str = None, filter: bool = True) -> list:
    """
    a function that gets all the words in korean
...
def to_korean(text: str, *, start: str = None) -> str:
    """
    a function that translates text to korean
...
def from_korean(text: str, *, destination: str) -> str:
    """
    a function that translates korean to text
...
def to_int(korean_text: str, *, needs_place_values: bool = True) -> int:
    """
    a function that changes korean numeral to integer

    input              | dtype | default
    korean_text        | str   | -
    needs_place_values | bool  | True
...
def from_int(num: int, *, include_place_values: bool=True) -> str:
    """
    a function that changes integer to korean numeral

    input                | dtype | default
    num                  | int   | -
    include_place_values | bool  | True
...
def korean_ratio(text: str, *, style: str = None) -> Union[float, str]:
    """
    a function that checks the korean ratio of the text

    input | dtype | default
    text  | str   | -
    style | str   | None

    return: Union value (float, str)
    """
    pct_count_int = 0
    for char in text:
...
def fully_korean(text: str) -> bool:
    """
    a function that checks if the text is fully korean
...
def from_datetime(dt: datetime, *, place_values: bool = True, has_seperators: bool = True) -> str:
    """
    a function that converts date & time to korean text

    input          | dtype             | default
    dt             | datetime.datetime | -
    place_values   | bool              | True
    has_seperators | bool              | True
...
def to_datetime(text: str, *, needs_place_values: bool = True, needs_seperators: bool = True) -> datetime:
    """
    a function that takes a korean text and convert it to date & time

    input              | dtype | default
    text               | str   | -
    needs_place_values | bool  | True
    needs_seperators   | bool  | True
...
def similarity(word1: str, word2: str, *, allow_sentence: bool = False, style: str = None) -> Union[str, float]:
    ...
    return: Union value (str, float)
    """
    if (len(word1.split(" ")) > 1 or len(word2.split(" ")) > 1) and not allow_sentence:
        raise ValueError("sentence found")
    group_syl = list(group('Hangul Syllables'))
    length = len(list(zip(word1, word2)))
    point = 0.0
    for char1, char2 in zip(word1, word2):
        unicode1 = ord(char1)
        unicode2 = ord(char2)
        if unicode1 in group_syl and unicode2 in group_syl:
            calculated = (1-abs(unicode1-unicode2)/len(group_syl))/length
            point += calculated
        else:
            point += 0.0
    if style == None:
...
def accuracy_per_word(word: str, *, sort: bool = False, allow_sentence: bool = False, style: str = None, source: str = None, filter: bool = True) -> dict:
    """
    a function that gives you accuracy word-by-word
    ...
    return: dict
    """
    all_words = words(source=source, filter=filter)
    temporary = []
    for word_ in all_words:
        temporary.append((similarity(word, word_, allow_sentence=allow_sentence), word_))
    if sort:
        temporary = sorted(temporary)
    return_dict = {}
    for s, w in temporary:
        if style == None:
            return_dict.update({w: s})
        else:
            return_dict.update({w: style % (s*100)})
    return return_dict

def fix_spelling(word: str, *, allow_sentence: bool=True) -> str:
    """
    a function that fixes korean word's spelling
    ...
    return: str
    """
    return accuracy_per_word(word, sort=True, allow_sentence=allow_sentence)[0]