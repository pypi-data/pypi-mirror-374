...
class Plotter:
    ...
    def Plot(config: dict, graphs:list[dict], objects: list[dict] | None = None):
        ...
        if Basics.dictMultiGet(plot_conf, ('show', 'print'), True):
            plt.show()
        return plt

    def QuickPlotArray(array: np.ndarray, x: int = 0, y: tuple[int]|list[int]|int = "*", range_x = None, range_y = None, save: str = None):
        dim = array.ndim
        if dim != 2:
