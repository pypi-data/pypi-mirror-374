...
class GitlabClient:
    ...
    def get_commits(
        ...
        # We use pagination here instead of using the iterator kwarg. The generator bypasses our retry_for_status check.
        # We can get ChunkedEncodingErrors, which is handled by retry_for_status.
        ...
        try:
            while True:
                commits = retry_for_status(project.commits.list, statuses_to_retry=GITLAB_STATUSES_TO_RETRY, **kwargs_dict)  # type: ignore
                if not commits:
                    break
