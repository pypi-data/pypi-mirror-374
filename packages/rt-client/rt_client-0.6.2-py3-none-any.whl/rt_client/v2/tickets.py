...
class TicketManager(RecordManager):
    ...
    @staticmethod
    def _prep_attachments(attachments):
        files = []
        for attachment in attachments:
            # Note that we read the content of the attachment early here,
            # avoid passing a BufferIO object to requests that can only be read
            # once.  This allows us to retry creating a ticket with attachments
            # if we have to re-authenticate.
            with open(attachment, "rb") as att_in:
                content = att_in.read()
            files.append(
                (
                    "Attachments",
