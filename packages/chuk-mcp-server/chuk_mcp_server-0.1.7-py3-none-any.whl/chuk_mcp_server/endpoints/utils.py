...
def error_response_fast(code: int, message: Optional[str] = None) -> Response:
    """
    ...
    if code in _ERROR_RESPONSES and message is None:
        ...
        return Response(
            orjson.dumps(error_data),
            status_code=code,
            media_type="application/json",
            headers=_CORS_NOCACHE
        )

def success_response_fast(
    data: Any = None,
    message: str = "success",
    cache_level: str = "none"
) -> Response:
    ...
    if data is None:
        ...
        response_data = {"status": message, "data": data}
...
def method_not_allowed_response(allowed_methods: Optional[list] = None) -> Response:
...
def validate_json_request_fast(request_body: bytes) -> tuple[bool, Union[dict, str]]:
    """
...
def create_cors_preflight_response_fast(
...
def pooled_json_response(data: Union[dict, list], status_code: int = 200) -> Response:
    """
