...
请先执行pycmakebuild --init初始化或指定正确的json文件。"""
    ...
    def _build_and_install(
        ...
        install_full_path = install_full_path.joinpath(name)
        ...
        build_full_dir = (
            f"{self.__build_dir}/{name}/build_{self.__arch}_{build_type.value}"
        )
    ...
    def _execute_cmd(cls, cmd: str, cwd: str = None):
        """执行命令
