...
class DuelingBuffaloEnv(gym.Env):
    """
    ...
    def step(self, action: np.ndarray) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        if isinstance(action, np.ndarray) is False:
            try:
                ...
                raise AssertionError("Tried to coerce action to np.ndarray with wrong type")

        if action.shape != (2,):
            raise ValueError("Action shape must be (2,)")
        if action[0] == action[1]:
            raise ValueError("Arms cannot be the same")

        reward = 1 * (self.rng.normal(self.offsets[0][action[0]], self.stds[action[0]], 1)[0] >
                      self.rng.normal(self.offsets[0][action[1]], self.stds[action[1]], 1)[0])
