...
def video_to_gif(
    ...
    try:
        ...
        try:
            # 第一遍：生成调色板
            in1 = ffmpeg.input(video_path, ss=ss_arg, t=t_arg) if (
                        ss_arg is not None or t_arg is not None) else ffmpeg.input(video_path)
            ...
            # 第二遍：应用调色板生成 GIF
            in2 = ffmpeg.input(video_path, ss=ss_arg, t=t_arg) if (
                        ss_arg is not None or t_arg is not None) else ffmpeg.input(video_path)
