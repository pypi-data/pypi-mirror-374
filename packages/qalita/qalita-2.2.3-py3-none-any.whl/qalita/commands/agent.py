...
def pull_pack(config, pack_id, pack_version=None):
    ...
    parts = pack_url.split("/")
    file_name = parts[-1]
    bucket_name = parts[3] if len(parts) > 3 else "assets"
    s3_rel_parts = parts[4:-1] if len(parts) > 5 else []
    local_dir = os.path.join(agent_run_temp_path, bucket_name, *s3_rel_parts)
...
def job_run(
    ...
    # Derive pack folder name robustly across OS and extensions (.tar.gz/.tgz)
    base_name = os.path.basename(pack_file_path)
    if base_name.endswith(".tar.gz"):
        name_root = base_name[:-7]
    elif base_name.endswith(".tgz"):
        name_root = base_name[:-4]
    else:
        name_root = os.path.splitext(base_name)[0]
    # Remove trailing .tar if present
    if name_root.endswith(".tar"):
        name_root = name_root[:-4]
    pack_folder = f"{name_root}_pack"
    ...
    agent_run_temp_path = config.get_agent_run_path()
    temp_folder_name = os.path.join(agent_run_temp_path, f"{datetime_string}_{random_seed}")
