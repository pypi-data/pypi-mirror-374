...
class TemperatureScheduleTargetType(str, Enum):
    HVAC = "hvac"


class TemperatureScheduleHourMinuteTypedDict(TypedDict):
...
class TemperatureScheduleHourMinute(BaseModel):
    r"""An interval composed of two clock times during which this rule applies each day. `to` always resolves to a timestamp after `from`, and thus may span across midnight and fall on the next day."""
...
class TemperatureScheduleWeekday(int, Enum):
    ZERO = 0
    ONE = 1
    TWO = 2
    THREE = 3
...
class TemperatureScheduleRuleTypedDict(TypedDict):
    target_state: TemperatureTargetStateTypedDict
    hour_minute: NotRequired[TemperatureScheduleHourMinuteTypedDict]
    r"""An interval composed of two clock times during which this rule applies each day. `to` always resolves to a timestamp after `from`, and thus may span across midnight and fall on the next day."""
...
class TemperatureScheduleRule(BaseModel):
    ...
    weekdays: Optional[List[TemperatureScheduleWeekday]] = None
    r"""An array of weekdays to which this rule should apply. A weekday starts with 0 for Monday and ends with 6 for Sunday."""


class TemperatureScheduleTypedDict(TypedDict):
    target_id: str
    r"""ID of the HVAC asset to which this schedule applies."""
    target_type: TemperatureScheduleTargetType
    default_target_state: TemperatureTargetStateTypedDict
    rules: List[TemperatureScheduleRuleTypedDict]
...
class TemperatureSchedule(BaseModel):
    ...
    target_type: Annotated[
        TemperatureScheduleTargetType, pydantic.Field(alias="targetType")
    ]
    ...
    rules: List[TemperatureScheduleRule]
    r"""Each rule sets a value for `holdType` and associated settings. All other properties of the rule are optional filters that limit the times to which this rule applies."""
