...
from enode.httpclient import HttpClient
import base64
from enode.sdkconfiguration import SDKConfiguration
...
class ClientCredentialsHook(SDKInitHook, BeforeRequestHook, AfterErrorHook):
    ...
    def get_credentials_global(self, security: Any) -> Optional[Credentials]:
        if (
            ...
            or security.client_secret is None
        ):
            return None
    ...
    def do_token_request(
        ...
        response = self.client.send(
            self.client.build_request(
                method="POST", url=token_url, headers=headers, data=payload
            )
        )
