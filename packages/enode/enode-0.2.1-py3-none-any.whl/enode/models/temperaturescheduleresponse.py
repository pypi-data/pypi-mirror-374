...
class TemperatureScheduleResponseTargetType(str, Enum):
    HVAC = "hvac"


class TemperatureScheduleResponseHourMinuteTypedDict(TypedDict):
...
class TemperatureScheduleResponseHourMinute(BaseModel):
    r"""An interval composed of two clock times during which this rule applies each day. `to` always resolves to a timestamp after `from`, and thus may span across midnight and fall on the next day."""
...
class TemperatureScheduleResponseWeekday(int, Enum):
    ZERO = 0
    ONE = 1
    TWO = 2
    THREE = 3
...
class TemperatureScheduleResponseRuleTypedDict(TypedDict):
    target_state: TemperatureTargetStateTypedDict
    hour_minute: NotRequired[TemperatureScheduleResponseHourMinuteTypedDict]
    r"""An interval composed of two clock times during which this rule applies each day. `to` always resolves to a timestamp after `from`, and thus may span across midnight and fall on the next day."""
...
class TemperatureScheduleResponseRule(BaseModel):
    ...
    weekdays: Optional[List[TemperatureScheduleResponseWeekday]] = None
    r"""An array of weekdays to which this rule should apply. A weekday starts with 0 for Monday and ends with 6 for Sunday."""


class TemperatureScheduleResponseTypedDict(TypedDict):
    target_id: str
    r"""ID of the HVAC asset to which this schedule applies."""
    target_type: TemperatureScheduleResponseTargetType
    default_target_state: TemperatureTargetStateTypedDict
    rules: List[TemperatureScheduleResponseRuleTypedDict]
...
class TemperatureScheduleResponse(BaseModel):
    target_id: Annotated[str, pydantic.Field(alias="targetId")]
    r"""ID of the HVAC asset to which this schedule applies."""

    target_type: Annotated[
        TemperatureScheduleResponseTargetType, pydantic.Field(alias="targetType")
    ]

    default_target_state: Annotated[
        TemperatureTargetState, pydantic.Field(alias="defaultTargetState")
    ]

    rules: List[TemperatureScheduleResponseRule]
    r"""Each rule sets a value for `holdType` and associated settings. All other properties of the rule are optional filters that limit the times to which this rule applies."""
