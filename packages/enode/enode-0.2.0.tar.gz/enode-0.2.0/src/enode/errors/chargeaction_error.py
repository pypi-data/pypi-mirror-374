...
from enode.models import (
    ...
    chargeaction as models_chargeaction,
    chargingaction as models_chargingaction,
)
...
class ChargeActionErrorData(BaseModel):
    id: str
    ...
    * `CANCELLED`: A required precondition was not met during the action's timeout window or another action has been sent to the target, overriding this one.
    ...
    kind: models_chargingaction.ChargingAction

    failure_reason: Annotated[
        Nullable[models_chargeaction.ChargeActionFailureReason],
        pydantic.Field(alias="failureReason"),
    ]


class ChargeActionError(EnodeError):
    data: ChargeActionErrorData
