...
@mcp.tool()
async def continue_email_generation(file_path: str, row_number: int):
    row_number += 1
    result = await generate_email(file_path, row_number)
    return result

@mcp.tool()
async def start_email_generation(res: FileResource):
    file_path = os.path.join(os.path.dirname(__file__), res.path)
    result = await generate_email(file_path, 1)
    return result

@mcp.tool()
async def generate_email(file_path: str, row_number: int):
    email = "Email"
    try:
        with open(file_path, mode="r", encoding="utf-8") as file:
            ...
            for i, row in enumerate(csv_reader, start=1):
                try:
                    ...
                    result = f"{await (gtm_agent.generate_mail(row, i))}"
