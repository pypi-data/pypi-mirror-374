...
class BaseTrait(BaseModel):
    ...
    async def around_execute(self, task: "Task", worker: "Worker", execute_func: Callable[[], Any]) -> Any:
        """
        Hook to wrap the task's action execution.
        Allows for setup/teardown or modifying the execution context (e.g., adding a timeout).
        """
...
class RetryableTrait(BaseTrait):
    ...
    def on_failure(
        ...
        if self.tries < self.max_tries:
            ...
            # The trait is now responsible for scheduling the retry.
            backoff_delay = float(self.backoff())
            ready_at = worker.get_time() + backoff_delay
            heapq.heappush(worker.retries, (ready_at, worker.retry_counter, task))
            worker.retry_counter += 1
            if worker._retry_scheduled_event:
                worker._retry_scheduled_event.set()

            worker.log(
                f"Task {task.id} failed, scheduling retry {self.tries}/{self.max_tries} in {backoff_delay:.2f}s.",
                logging.INFO,
            )
        else:
            task.status = TaskState.fail
            worker.log(
                f"Task {task.id} failed after exhausting all {self.max_tries} retries.",
                logging.WARNING,
            )
...
class RequiresPropsTrait(BaseTrait):
    ...
    def get_filtered_props(self, props: Dict[str, "Prop"]) -> Dict[str, Any]:
        """Returns the props required by this task."""
...
class TimeoutTrait(BaseTrait):
    """Trait that adds a timeout to a task's execution."""

    trait_name: Literal["timeoutable"] = "timeoutable"
    timeout: float = Field(
        gt=0.0, description="Maximum execution time in seconds for the task."
    )

    async def around_execute(self, task: "Task", worker: "Worker", execute_func: Callable[[], Any]) -> Any:
        """Wraps the execution with an asyncio timeout."""
        import asyncio
        return await asyncio.wait_for(execute_func(), timeout=self.timeout)


class DeduplicationTrait(BaseTrait):
    """Trait that prevents duplicate tasks from being executed within a time window."""

    trait_name: Literal["deduplicatable"] = "deduplicatable"
    window: timedelta = Field(
        default=timedelta(seconds=60),
        description="The time window within which duplicate tasks will be skipped.",
    )

    def _get_task_key(self, task: "Task") -> str:
        """Creates a stable, unique key based on the task's action and arguments."""
        # Use local imports to prevent circular dependencies.
        import msgpack

        from wombat.multiprocessing.queues import default_encoder
        ...
        # Hash the result to keep the key size manageable and consistent.
        return f"{task.action}:{hash(args_bytes)}"

    def on_before_execute(self, task: "Task", worker: "Worker") -> bool:
        key = self._get_task_key(task)
        now = worker.get_time()

        if key in worker.deduplication_cache:
            last_seen = worker.deduplication_cache[key]
            if now - last_seen <= self.window.total_seconds():
                worker.log(
                    f"Task {task.id} (key: {key}) is a duplicate within the {self.window} window. Skipping.",
                    logging.INFO,
                )
                task.status = TaskState.skipped
                return False

        worker.deduplication_cache[key] = now
        return True
...
class CircuitBreakerTrait(BaseTrait):
    """
    Trait that implements the Circuit Breaker pattern. Requires 'circuit_breaker_lock'
    and 'circuit_breaker_states' Props, which are auto-injected by the Orchestrator.
    """

    trait_name: Literal["circuit_breakable"] = "circuit_breakable"
    ...
    def _get_circuit_key(self, task: "Task") -> str:
        """The key is based on the action, as the circuit protects a resource or service."""
        return task.action

    def on_before_execute(self, task: "Task", worker: "Worker") -> bool:
        lock = worker.props["circuit_breaker_lock"].instance
        states_dict = worker.props["circuit_breaker_states"].instance
        key = self._get_circuit_key(task)
        now = worker.get_time()

        if key not in states_dict:
            return True  # Circuit not configured for this action

        shared_state = states_dict[key]
        with lock:
            current_state = CircuitBreakerState(shared_state["state"].value)
            if current_state == CircuitBreakerState.OPEN:
                if now - shared_state["opened_at"].value > self.recovery_timeout:
                    shared_state["state"].value = CircuitBreakerState.HALF_OPEN.value
                    worker.log(
                        f"Circuit breaker for {key} is now HALF_OPEN.", logging.INFO
                    )
                    return True  # Allow one test task
                else:
                    task.status = TaskState.fail
    ...
    def on_success(self, task: "Task", worker: "Worker", result: Any):
        ...
        shared_state = states_dict[key]
        with lock:
            if CircuitBreakerState(shared_state["state"].value) == CircuitBreakerState.HALF_OPEN:
                worker.log(f"Circuit breaker for {key} has CLOSED.", logging.INFO)
                shared_state["state"].value = CircuitBreakerState.CLOSED.value
                shared_state["failures"].value = 0
    ...
    def on_failure(
        ...
        with lock:
            ...
            if current_state == CircuitBreakerState.HALF_OPEN:
                shared_state["state"].value = CircuitBreakerState.OPEN.value
                shared_state["opened_at"].value = now
                worker.log(
                ...
                shared_state["failures"].value += 1
                if shared_state["failures"].value >= self.failure_threshold:
                    shared_state["state"].value = CircuitBreakerState.OPEN.value
                    shared_state["opened_at"].value = now
                    worker.log(
                        f"Circuit for {key} has OPENED due to {shared_state['failures'].value} failures.",
                        logging.WARNING,
                    )
