...
from wombat.multiprocessing.models import CircuitBreakerState, Prop, Task, TaskState
...
@task()
@timeoutable()
def sleep_then_finish_timeoutable(_worker: Worker, duration: float):
    """An action that sleeps for a given duration, with timeout capability."""
    time.sleep(duration)


@task()
@requires_props(requires_props=["side_effect_tracker", "lock"])
@deduplicatable()
def dedupe_side_effect_action(
    _worker: Worker, task_key: str, props: dict[str, Prop]
):
    """An action that records its execution in a shared dict."""
    tracker = props["side_effect_tracker"].instance
    lock = props["lock"].instance
    with lock:
        current_count = tracker.get(task_key, 0)
        tracker[task_key] = current_count + 1


@task()
@requires_props(
    requires_props=["attempt_tracker", "lock", "circuit_breaker_lock", "circuit_breaker_states"]
)
@circuit_breakable()
def circuit_breaker_action(
    ...
    """
    Action that fails until a certain attempt number is reached, for testing
    the circuit breaker.
    """
...
@pytest.mark.asyncio
async def test_task_timeout(logging_config):
    """Verify that the TimeoutTrait correctly terminates a long-running task."""
    test_actions = {
        sleep_then_finish_timeoutable.action_name: sleep_then_finish_timeoutable
    }
    ...
    async with Orchestrator(
        ...
        # Task that should time out
        timeout_task = sleep_then_finish_timeoutable(
            duration=5,
            timeout=0.2,
        )

        # Task that should complete successfully
        success_task = sleep_then_finish_timeoutable(
            duration=0.1,
            timeout=5,
        )

        await orchestrator.add_tasks([timeout_task, success_task])
        results = await orchestrator.stop_workers()

    check.equal(len(results), 2)

    timeout_result = next((r for r in results if r.task.id == timeout_task.id), None)
    success_result = next((r for r in results if r.task.id == success_task.id), None)

    check.is_not_none(timeout_result, "Timeout result not found")
    check.is_not_none(success_result, "Success result not found")

    check.equal(timeout_result.task.status, TaskState.fail)
    check.is_instance(timeout_result.result, list)
    # The traceback string will have the specific exception name
    check.is_in("TimeoutError", timeout_result.result[0])

    check.equal(success_result.task.status, TaskState.success)


@pytest.mark.asyncio
async def test_deduplication_trait(logging_config, manager):
    """Verify that the DeduplicationTrait prevents duplicate tasks from running."""
    side_effect_tracker = manager.dict()
    lock = manager.Lock()
    test_props = {
        "side_effect_tracker": Prop(
            initializer=side_effect_tracker, use_context_manager=False
        ),
        "lock": Prop(initializer=lock, use_context_manager=False),
    }

    test_actions = {dedupe_side_effect_action.action_name: dedupe_side_effect_action}
    ...
    async with Orchestrator(
        ...
        # Create five identical tasks. The key for deduplication is based on the
        # action name, args, and kwargs, so these will all be duplicates.
        tasks = [
            dedupe_side_effect_action("task1", window=timedelta(seconds=5))
            for _ in range(5)
        ]

        # Add another unique task that should not be deduplicated.
        unique_task = dedupe_side_effect_action("task2", window=timedelta(seconds=5))
        tasks.append(unique_task)

        await orchestrator.add_tasks(tasks)
        results = await orchestrator.stop_workers()

    check.equal(len(results), 6)

    success_results = [r for r in results if r.task.status == TaskState.success]
    skipped_results = [r for r in results if r.task.status == TaskState.skipped]

    # Only one of the first 5 tasks should succeed, and the unique task should also succeed.
    check.equal(len(success_results), 2, "Expected 2 tasks to succeed.")
    # The other 4 identical tasks should have been skipped by the deduplication logic.
    check.equal(len(skipped_results), 4, "Expected 4 tasks to be skipped.")

    # Verify the side effect: the tracker should show that task1 ran once and task2 ran once.
    check.equal(side_effect_tracker.get("task1"), 1)
    check.equal(side_effect_tracker.get("task2"), 1)

    for r in skipped_results:
        check.is_in("skipped by pre-flight hook", r.result)


@pytest.mark.asyncio
async def test_circuit_breaker_trait(logging_config, manager):
    """Verify that the CircuitBreakerTrait correctly opens, recovers, and closes."""
    # The 'manager' fixture provides shared objects. The test must provide
    # circuit breaker props since the Orchestrator no longer auto-injects them.
    attempt_tracker = manager.dict()
    lock = manager.Lock()

    # Create shared state for the circuit breaker
    context = get_context("spawn")
    circuit_breaker_lock = context.Lock()
    circuit_states = {
        circuit_breaker_action.action_name: {
            "state": context.Value("i", CircuitBreakerState.CLOSED.value),
            "failures": context.Value("i", 0),
            "opened_at": context.Value("d", 0.0),
        }
    }
    ...
    test_props = {
        ...
        "lock": Prop(initializer=lock, use_context_manager=False),
        "circuit_breaker_lock": Prop(
            initializer=circuit_breaker_lock, use_context_manager=False
        ),
        "circuit_breaker_states": Prop(
            initializer=circuit_states, use_context_manager=False
        ),
    }
    test_actions = {circuit_breaker_action.action_name: circuit_breaker_action}
    ...
    async with Orchestrator(
        ...
        tasks_to_fail = [
            circuit_breaker_action(
                "trip_circuit",
                succeed_on_try=100,  # Guaranteed to fail
                failure_threshold=fail_threshold,
                recovery_timeout=recovery_timeout,
            )
            for _ in range(fail_threshold)
        ]
        await orchestrator.add_tasks(tasks_to_fail)
        await orchestrator.finish_tasks()
        # Clear the results buffer from the initial failing tasks.
        _ = list(orchestrator.get_results())

        # Check that the circuit is now open
        action_key = circuit_breaker_action.action_name
        check.is_true(action_key in circuit_states)
        check.equal(circuit_states[action_key]["state"].value, 1)  # 1 == OPEN

        # --- 2. Verify the circuit is blocking new tasks ---
        blocked_task = circuit_breaker_action(
            "blocked_task",
            succeed_on_try=1,
            failure_threshold=fail_threshold,
            recovery_timeout=recovery_timeout,
        )
        await orchestrator.add_task(blocked_task)
        await orchestrator.finish_tasks()
        blocked_result = list(orchestrator.get_results())[0]

        check.equal(blocked_result.task.status, TaskState.fail)
        check.is_in("skipped by pre-flight hook", blocked_result.result)

        # --- 3. Wait for recovery and test half-open state ---
        await asyncio.sleep(recovery_timeout + 0.1)

        # This task should be allowed through and succeed, closing the circuit
        recovery_task = circuit_breaker_action(
            "recovery_task",
            succeed_on_try=1,
            failure_threshold=fail_threshold,
            recovery_timeout=recovery_timeout,
        )
        await orchestrator.add_task(recovery_task)
        await orchestrator.finish_tasks()
        recovery_result = list(orchestrator.get_results())[0]

        check.equal(recovery_result.task.status, TaskState.success)
        check.equal(recovery_result.result, "Success")

        # --- 4. Verify the circuit is closed again ---
        check.equal(circuit_states[action_key]["state"].value, 0)  # 0 == CLOSED
        check.equal(circuit_states[action_key]["failures"].value, 0)
