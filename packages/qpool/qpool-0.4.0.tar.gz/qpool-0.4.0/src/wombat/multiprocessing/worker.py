    ...
    async def execute_task(
        ...
        try:
            if hasattr(task, "traits"):
                ...
                if retry_trait:
                    self.log(
                        f"DEBUG_TRIES: execute_task for task {getattr(task, 'id', 'N/A')}: tries={retry_trait.tries}",
                        logging.DEBUG,
                    )
            ...
            # A task requires props if it has the RequiresPropsTrait.
            requires_props_trait = next(
                (
                    trait
                    for trait in getattr(task, "traits", [])
                    if isinstance(trait, RequiresPropsTrait)
                ),
                None,
            )

            if requires_props_trait:
                filtered_props = task.filter_props(props=props)
                kwargs["props"] = filtered_props

            if is_async:
                coroutine = func(self, *args, **kwargs)
                result_data = await coroutine
            else:
                # Run synchronous, blocking tasks in the event loop's default executor.
                # Use functools.partial to pass both args and kwargs to the function.
                p = functools.partial(func, self, *args, **kwargs)
                result_data = await self.loop.run_in_executor(None, p)
            ...
            if task.status == TaskState.success:
                ...
                if retry_trait:
                    backoff_delay = float(retry_trait.backoff())
                    ...
                    heapq.heappush(self.retries, (ready_at, self.retry_counter, task))
                    self.retry_counter += 1
                    if self._retry_scheduled_event:
                        self._retry_scheduled_event.set()
