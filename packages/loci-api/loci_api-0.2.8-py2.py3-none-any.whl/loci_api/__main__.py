...
def parse_args():
    ...
    func_insights_parser.add_argument("--filter", type=str, nargs='*', choices=['new', 'mod'],
                                      ...
                                      help="Filter functions by change type relative to the base version. " \
                                            "Ignored if --version-name-base is not provided, e.g. '--filter mod new' " \
                                            "will return insights for both new and modified functions. Default filter is 'mod'.")
    ...
    summary_parser.add_argument("repository", help="Full repository name, as 'owner/repo' e.g. auroralabs-loci/loci-action", type=str)
    summary_parser.add_argument("--pr", help="PR number", type=str, default="1")
...
def cmd_function_insights(project_name, version_name, version_name_base, perc_resp_limit, perc_thro_limit, perc_bott_limit, pairs, output, filters) -> int:
    ...
    version_id = api_helper.get_version_id(project_id, version_name)
    version_id_base = api_helper.get_version_id(project_id, version_name_base)
    ...
    insights = api_helper.get_function_insights(version_id, version_id_base, perc_resp_limit, perc_thro_limit, perc_bott_limit, pairs, _to_filter(filters))
    if insights is None:
        return 1
    ...
    report_id = api_helper.get_report_id(version_id, version_id_base)
    if report_id:
        report_data = api_helper.get_report_data(report_id)
        if report_data:
            diff_summary = {
                'target_total': report_data['message'][0][0],
                'base_total': report_data['message'][0][1],
                'new': report_data['message'][0][2],
                'modified': report_data['message'][0][3],
                'deleted': report_data['message'][0][4]
            }
...
def cmd_status(project_name, version_name, output) -> int:
    ...
    result = api_helper.get_project_status(project_id, version_id)
    if not result:
        return 1

    report_id = result['progress']['report_id']
    url = (
        f"{api_helper.BACKEND_HOST_URL.replace('api.', '', 1)}/#/main/{project_id}/report-list/{report_id}"
        if report_id
        else ""
    )

    statuses = result['progress']['statuses']
    formatted_result = {
        "status": result['progress']['status'],
        "url": url,
        "total": len(result['progress']['containers']),
        "counts": {
            "passed": sum(1 for s in statuses if s == 0),
            "failed": sum(1 for s in statuses if s > 0),
            "pending": sum(1 for s in statuses if s == -1),
        },
        "updated_at": result['progress']['end_dt'],
        "analysis": []
    }
    for idx, container in enumerate(result['progress']['containers']):
        formatted_result["analysis"].append({
            "binary": container,
            "step": result['progress']['actions'][idx],
            "status": result['progress']['statuses'][idx],
            "updated_at": result['progress']['action_timestamps'][idx]
        })
    return output_success_result(formatted_result, output)


def cmd_summary(project_name, version_name, version_name_base, repository, pr_number, output) -> int:
    project_id, _ = api_helper.get_project_id(project_name)
    if not project_id:
        return 1
    ...
    res = api_helper.get_function_insights_summary(project_id, version_id, version_id_base, repository, pr_number)
    if res is None:
        return 1

    return output_success_result(res["message"], output)
