...
def AndOperator(
    ...
    tuple:
        constraint1: pyo.Constraint
            The pyomo constraint representing A >= B + C - 1
    ...
    if relationshipBaseName is None:
        ...
        Bname = str(B)
        Cname = str(C)
        relationshipBaseName = f"{Aname}_{Bname}_{Cname}_AndOperator"
    ...
    if itrSet is None:
        setattr(model, constraint1Name, pyo.Constraint(expr=A >= B + C - 1))
