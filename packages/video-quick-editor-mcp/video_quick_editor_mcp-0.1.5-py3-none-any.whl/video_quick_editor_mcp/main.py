...
@mcp.tool()
def add_simple_b_roll(main_video_path: str, broll_video_path: str, insert_time: str, 
                      output_video_path: str, duration: float = None, 
                      position: str = 'fullscreen', scale: float = None) -> str:
    """在主视频上添加单个 B-roll 画面（简化版）。

    Args:
        main_video_path: 主视频文件路径。
        broll_video_path: B-roll 视频文件路径。
        insert_time: 插入时间（秒或 'HH:MM:SS' 格式）。
        output_video_path: 输出视频文件路径。
        duration: 叠加时长（秒，默认使用 B-roll 视频完整时长）。
        position: 叠加位置，可选：'fullscreen'、'top-left'、'top-right'、'bottom-left'、'bottom-right'、'center'，默认 'fullscreen'。
        scale: 缩放系数（0.1-1.0，默认全屏1.0，其他位置0.5）。
    ...
    try:
        if not os.path.exists(broll_video_path):
            raise RuntimeError(f"Error: B-roll video file not found at {broll_video_path}")
        
        main_props = _get_media_properties(main_video_path)
        broll_props = _get_media_properties(broll_video_path)
        
        if not main_props['has_video'] or not broll_props['has_video']:
            raise RuntimeError("Error: Both main video and B-roll must have video streams.")
        
        # 解析插入时间
        start_time = _parse_time_to_seconds(insert_time)
        if duration is None:
            duration = broll_props['duration']
        ...
        temp_dir = tempfile.mkdtemp()
        try:
            temp_broll = os.path.join(temp_dir, "processed_broll.mp4")
            
            # 缩放和处理 B-roll
            if position == 'fullscreen':
                scale_filter = f"scale={main_props['width']}:{main_props['height']}"
            else:
            ...
            if position == 'fullscreen':
                # 全屏模式：在指定时间替换画面
                filter_complex = f"[0:v][1:v]overlay=enable='between(t,{start_time},{start_time + duration})'[v]"
            else:
                # 叠加模式：在指定位置叠加
                filter_complex = f"[0:v][1:v]overlay=x={overlay_x}:y={overlay_y}:enable='between(t,{start_time},{start_time + duration})'[v]"
            ...
            subprocess.run(cmd, check=True, capture_output=True)
            return f"B-roll added successfully at {insert_time}. Output: {output_video_path}"
        ...
        error_message = e.stderr.decode('utf8') if e.stderr else str(e)
        raise RuntimeError(f"Error adding B-roll: {error_message}")
    except Exception as e:
        raise RuntimeError(f"An unexpected error occurred: {str(e)}")


# 原复杂版本的 add_b_roll 函数（已注释，保留参考）
"""
