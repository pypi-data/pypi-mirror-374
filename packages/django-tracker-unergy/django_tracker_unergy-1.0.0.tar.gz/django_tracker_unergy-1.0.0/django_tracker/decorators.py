            ...
            def new_save(self, *args, **kwargs):
                ...
                try:
                    if is_creation and audit_config.audit_creates:
                        ...
                        logger.info(
                            f"[AUDIT] Creación: {self.__class__.__name__}({self.pk}) por usuario '{get_current_user()}'. Campos: {audit_config.tracked_fields or 'todos'}."
                        )
                        ...
                        logger.info(
                            f"[AUDIT] Actualización: {self.__class__.__name__}({self.pk}) por usuario '{get_current_user()}'. Campos: {audit_config.tracked_fields or 'todos'}."
                        )
        ...
        if audit_config.audit_deletes:
            ...
            def new_delete(self, *args, **kwargs):
                ...
                try:
                    ...
                    logger.info(
                        f"[AUDIT] Eliminación: {self.__class__.__name__}({self.pk}) por usuario '{get_current_user()}'."
                    )
                except Exception as e:
