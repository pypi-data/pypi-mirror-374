...
from mcp_server_browser.mcp_server.types import (BrowserNavigateToolReturn, GetWindowsAndTabsToolReturn, BrowserGoBackOrForwardToolReturn,
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    try:
        ...
        yield AppContext(websocket_server=websocket_server)
...
mcp = FastMCP("XiaoJi", lifespan=app_lifespan)
...
async def browser_navigate(
    ...
    result: BrowserNavigateToolReturn = await call_tool(ctx.request_context.lifespan_context.websocket_server, tool_name="chrome_navigate",
                                                        tool_args={"url": url, "newWindow": new_window, "width": width, "height": height,
                                        "refresh": refresh})
    return result


@mcp.tool(name="get_windows_and_tabs", description="Get all currently open browser windows and tabs")
async def get_windows_and_tabs(ctx: Context[ServerSession, AppContext] = None) -> GetWindowsAndTabsToolReturn:
    result: GetWindowsAndTabsToolReturn = await call_tool(ctx.request_context.lifespan_context.websocket_server, tool_name="get_windows_and_tabs",
                             tool_args={})
    return result
...
async def browser_go_back_or_forward(
    ...
    result: BrowserGoBackOrForwardToolReturn = await call_tool(ctx.request_context.lifespan_context.websocket_server,
                             tool_name="chrome_go_back_or_forward",
...
async def browser_click_element(
    ...
    result: BrowserClickElementToolReturn = await call_tool(ctx.request_context.lifespan_context.websocket_server, tool_name="chrome_click_element",
...
async def browser_fill_or_select(
    ...
    result: BrowserFillOrSelectToolReturn = await call_tool(ctx.request_context.lifespan_context.websocket_server, tool_name="chrome_fill_or_select",
...
async def browser_get_elements(
        selector: str = Field(default="", description="CSS selector to filter elements. Takes precedence over query if both are provided."),
    ...
    result = await call_tool(ctx.request_context.lifespan_context.websocket_server,
...
async def chrome_keyboard(
    ...
    result: BrowserKeyboardToolReturn = await call_tool(ctx.request_context.lifespan_context.websocket_server, tool_name="chrome_keyboard",
...
async def browser_get_web_content(
    ...
    if not selector:
        need_html = True
    result: BrowserGetWebContentToolReturn = await call_tool(ctx.request_context.lifespan_context.websocket_server, tool_name="chrome_get_web_content",
                             tool_args={"url": url, "htmlContent": need_html, "textContent": need_text,
                                        "selector": selector})
    if not html_content:
...
async def browser_close_tabs(
    ...
    result: BrowserCloseTabsToolReturn = await call_tool(ctx.request_context.lifespan_context.websocket_server, tool_name="chrome_close_tabs",
                             tool_args={"tabIds": tab_ids or [], "url": url or ""})
    return result


@mcp.tool(name="browser_screenshot", description="Take a screenshot of the current page or a specific element(if you want to see the page, recommend to use chrome_get_web_content first)")
async def browser_screenshot(
        full_page: bool = Field(default=False,
                              description="Store screenshot of the entire page."),
        ctx: Context[ServerSession, AppContext] = None
) -> BrowserScreenshot:
    result: BrowserScreenshot = await call_tool(ctx.request_context.lifespan_context.websocket_server, tool_name="chrome_screenshot",
