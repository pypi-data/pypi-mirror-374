...
from raphson_mp.common.lyrics import LyricsLine, TimeSyncedLyrics, lyrics_to_text
...
from raphson_mp.server import album, cache, db, ffmpeg, settings
...
class FileTrack(Track):
    def __init__(self, conn: Connection, relpath: str):
        ...
        if settings.offline_mode and lyrics is None:
            ...
            # set lyrics in database so it is faster to look up next time
            if lyrics is not None:
                conn.execute('UPDATE track SET lyrics = ? WHERE path = ?', (lyrics, relpath))
