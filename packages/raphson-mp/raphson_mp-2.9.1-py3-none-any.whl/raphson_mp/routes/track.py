...
from raphson_mp.common.lyrics import Lyrics, PlainLyrics, TimeSyncedLyrics
...
# TODO remove after offline clients have updated
def _legacy_lyrics_dict(lyrics: Lyrics | None):
    if lyrics is None:
        return {"type": "none"}
    elif isinstance(lyrics, TimeSyncedLyrics):
        text = [{"start_time": line.start_time, "text": line.text} for line in lyrics.text]
        return {"type": "synced", "source": "", "text": text}
    elif isinstance(lyrics, PlainLyrics):
        return {"type": "plain", "source": "", "text": lyrics.text}
    else:
...
async def route_lyrics(request: web.Request, conn: Connection, _user: User):
    ...
    response = web.json_response(_legacy_lyrics_dict(lyr))
    if "mtime" in request.query:  # if cache busting mtime is present, we can serve with a long cache time
        response.headers["Cache-Control"] = "immutable, max-age=604800"
    return response
