...
def SubsonicArrayChild(conn: Connection, count: int, offset: int):
    return [
        SubsonicChild(conn, FileTrack(conn, relpath))
...
def SubsonicAlbumID3WithSongs(conn: Connection, album: Album) -> dict[str, Any]:
    ...
    data["song"] = [SubsonicChild(conn, FileTrack(conn, path), detailed=True) for path, in conn.execute(query, params)]
    return data
...
def SubsonicPlaylist(conn: Connection, name: str, include_tracks: bool = False) -> dict[str, Any]:
    ...
    if include_tracks:
        data["entry"] = [SubsonicChild(conn, track) for track in playlist.tracks(conn)]
...
async def getRandomSongs(request: web.Request, conn: Connection, _user: User):
    ...
    data = {
        "randomSongs": {
            "song": [
                SubsonicChild(conn, FileTrack(conn, relpath), detailed=True) for relpath, in conn.execute(query, params)
            ]
        }
    }
...
async def getSongsByGenre(request: web.Request, conn: Connection, _user: User):
    ...
    data = {
        "songsByGenre": {
            "song": [
                SubsonicChild(conn, FileTrack(conn, relpath), detailed=True)
                for relpath, in conn.execute(
                    f"""
                    SELECT track
                    FROM track_tag
...
async def getSimilarSongs2(request: web.Request, conn: Connection, _user: User):
    ...
    data = {"similarSongs2": {"song": [SubsonicChild(conn, FileTrack(conn, relpath)) for relpath, in rows]}}
    return response(request, data)
