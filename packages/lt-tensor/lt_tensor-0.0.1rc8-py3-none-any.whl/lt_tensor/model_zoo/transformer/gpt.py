...
class GPTLMHead(Model):
    ...
    def generate(
        ...
        # sampling check
        do_top_p = top_p not in [0.0, 1.0]
        do_top_k = top_k != 0
        do_temp = temperature not in [0.0, 1.1]
        # next token fn:
        if do_sample:
            next_fn = lambda x: torch.multinomial(x, num_samples=1)
        else:
            next_fn = lambda x: torch.argmax(x, dim=-1, keepdim=True)
