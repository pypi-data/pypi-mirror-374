...
class GPTConfig(ModelConfig):
    ...
    attention_head_version: int = 0
    pos_encoding: Literal["absolute", "rope", "alibi"] = "absolute"
    ...
    def __init__(
        ...
        attention_bias: bool = True,
        pos_encoding: Literal["absolute", "rope", "alibi"] = "absolute",
        attention_head_version: int = 0,  # 0 or 1
        *args,
        **kwargs,
    ):
        super().__init__(
            ...
            attention_head_version=attention_head_version,
        )
        ...
        self.blocks = nn.ModuleList(
            [
                DecoderLayer(
                    ...
                    attn_std=attn_std,
                    pos_encoding=self.pos_encoding,  # pass along
                )
...
class GPTLMHead(Model):
    ...
    def generate(
        ...
        # sampling check
        do_top_p = top_p not in [0.0, 1.0] and do_sample
        do_top_k = top_k != 0 and do_sample
        do_temp = temperature not in [0.0, 1.1] and do_sample
        _keep_tokens = self.cfg.max_seq_len - 2
        if do_sample:
            do_sample = any([do_temp, do_top_k, do_top_p])

        # next token fn:
        def _next_token(probs: Tensor):
            nxt = probs.softmax(-1)
            return torch.argmax(nxt, dim=-1, keepdim=True)

        for _ in range(max_new_tokens):
            logits = self.inference(generated)
            ...
            next_token = _next_token(logits)  # [1, 1]
            # add token into the pool
            generated = torch.cat((generated[..., -_keep_tokens:], next_token), dim=1)
