...
class Bit:
    ...
    def _record(self, name, error_message=None, annotations=None, ex=None, line_no=None, include_world=True):
        ...
        return BitHistoryRecord(
            ...
            world=deepcopy(self.world) if include_world else None,
            pos=deepcopy(self.pos) if include_world else None,
            orientation=self.orientation if include_world else None,
            annotations=deepcopy(annotations) if annotations is not None else None,
            filename=os.path.basename(filename),
            line_number=line_number
        )

    def _register(self, name, error_message=None, annotations=None, ex=None, line_no=None, include_world=True):
        self.history.append(
            self._record(name, error_message, annotations, ex, line_no=line_no, include_world=include_world))
        ...
        if error_message is None and self.state_counts[bit_state] >= 5:
            error_message = "Bit's been doing the same thing for a while. Is he stuck in an infinite loop?"
            raise BitInfiniteLoopException(error_message, annotations)

        elif error_message is None and len(self.history) > MAX_STEP_COUNT:
            error_message = "Bit has done too many things. Is he stuck in an infinite loop?"
            raise BitInfiniteLoopException(error_message, annotations)
    ...
    def _get_locals(self, frame):
        return {k: v for k, v in frame.f_locals.items() if v is not self}

    def _get_vars(self, local_vars):
        if not local_vars:
            return ''
        vars = ' [' + ', '.join(f'{k}={self._format_value(v)}' for k, v in local_vars.items()) + ']'
        return vars

    def _get_calling_line(self, frame):
        lineno = frame.f_lineno
        if frame.f_back and frame.f_back.f_code.co_filename == frame.f_code.co_filename:
            lineno = frame.f_back.f_lineno
        return lineno

    def _make_trace(self, filename):
        self._past_locals = None

        def trace(frame, event, arg):
            if frame.f_code.co_filename != filename:
                return trace

            local_vars = self._get_locals(frame)

            if event == 'call':
                vars = self._get_vars(local_vars)
                lineno = self._get_calling_line(frame)
                self._register('Calling function: ' + frame.f_code.co_name + vars, line_no=lineno, include_world=False)

            elif event == 'line':
                if local_vars != self._past_locals:
                    self._register(self._get_vars(local_vars), line_no=frame.f_lineno - 1, include_world=False)

                # vars = self._get_vars(local_vars)
                # if vars:
                #     self._register(vars, include_world=False)

            elif event == 'return':
                func_name = frame.f_code.co_name
                message = f'{func_name} returning'
                if arg is not None:
                    message += f' {self._format_value(arg)}'
                self._register(message, include_world=False)

                lineno = self._get_calling_line(frame)
                message = f'{func_name} returned'
                if arg is not None:
                    message += f' {self._format_value(arg)}'
                self._register(message, line_no=lineno, include_world=False)

            self._past_locals = local_vars
    ...
    def _evaluate(
        ...
        try:
            filename, _ = _get_caller_info()
            sys.settrace(self._make_trace(filename))
