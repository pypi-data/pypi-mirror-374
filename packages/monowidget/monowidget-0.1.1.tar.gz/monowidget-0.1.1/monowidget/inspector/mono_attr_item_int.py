...
# 优先从 _utils 导入
try:
    from monowidget._utils import QIntSlider, MONO_FONT
...
class QMonoAttrItemInt(QMonoAttrItemBase):
    """整数类型专用组件"""
    ...
    def _create_type_specific_ui(self):
        ...
        if self.ad.get('enum'):
            ...
            # 创建范围滑块
            self._qsl = self._create_range_slider(0, 100, 1, QIntSlider)
            
            # 设置范围
            if self.ad.get('range'):
                start, stop, step = self.ad['range'].start, self.ad['range'].stop, self.ad['range'].step
                start, stop, step = start or 0, stop, step or 1
