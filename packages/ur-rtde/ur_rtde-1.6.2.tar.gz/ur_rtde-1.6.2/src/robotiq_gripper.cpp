...
RobotiqGripper::RobotiqGripper(const std::string& Hostname, int Port, bool verbose)
    : hostname_(Hostname), port_(Port), verbose_(verbose), deadline_(io_context_)
{
...
void RobotiqGripper::connect(uint32_t timeout_ms)
{
  ...
  //boost::asio::ip::basic_resolver_query<boost::asio::ip::tcp> query(hostname_, std::to_string(port_));
