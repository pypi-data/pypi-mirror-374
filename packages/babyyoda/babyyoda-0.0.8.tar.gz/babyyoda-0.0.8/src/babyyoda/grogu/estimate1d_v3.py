...
@dataclass
class GROGU_ESTIMATE1D_V3(GROGU_ANALYSIS_OBJECT, UHIAnalysisObject):
    """A GROGU Estimate1D V3 object representing uncertainty estimates for histogram bins."""

    @dataclass
    class Bin:
        """A bin for Estimate1D containing value and error estimates."""
    ...
    def __post_init__(self) -> None:
        ...
        if "Type" not in self.d_annotations:
            self.d_annotations["Type"] = "Estimate1D"
    ...
    def num_edges(self) -> int:
        """Get number of edges."""
        return len(self.d_edges)

    @classmethod
    def from_string(cls, file_content: str) -> "GROGU_ESTIMATE1D_V3":
        """Parse Estimate1D V3 from string content."""
        lines = file_content.strip().splitlines()

        # Extract key from first line
        key = ""
        if find := re.search(r"BEGIN YODA_ESTIMATE1D_V3 (\S+)", lines[0]):
            key = find.group(1)
        ...
        for tline in lines:
            ...
            if line.startswith("BEGIN YODA_ESTIMATE1D_V3"):
                continue
            if line.startswith("END YODA_ESTIMATE1D_V3"):
                break
            ...
            if line.startswith("Edges(A1):"):
                ...
                edges = [float(s) for s in edge_strings if s]
                continue
    ...
    def to_string(self) -> str:
        """Convert Estimate1D V3 to string representation."""
        header = (
            f"BEGIN YODA_ESTIMATE1D_V3 {self.d_key}\n"
            f"{GROGU_ANALYSIS_OBJECT.to_string(self)}"
            "---\n"
        )

        # Format edges
        edge_str = ", ".join(f"{edge:.6e}" for edge in self.d_edges)
        edges = f"Edges(A1): [{edge_str}]\n"
        ...
        footer = "END YODA_ESTIMATE1D_V3"

        return f"{header}{edges}{error_labels}{header_line}{bin_data}\n{footer}"
