...
def run_server_sync(quiet: bool = False) -> None:
    """
    동기 래퍼 함수 (CLI에서 직접 호출 가능)
    
    Args:
        quiet: True일 때 INFO 로그 출력 억제 (Claude Desktop 호환성)
    
    anyio.run() 사용으로 안전한 실행
    """
    # quiet 파라미터가 True이거나 GREEUM_QUIET 환경변수가 설정된 경우 로깅 레벨 조정
    global QUIET_MODE
    if quiet or QUIET_MODE:
        # 기존 로거의 레벨을 WARNING으로 변경하여 INFO 로그 억제
