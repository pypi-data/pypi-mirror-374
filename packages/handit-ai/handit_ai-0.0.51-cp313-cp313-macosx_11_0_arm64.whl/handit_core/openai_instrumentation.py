    ...
    def wrapped_create(self, **kwargs):  # type: ignore[no-untyped-def]
        ...
        try:
            ...
            try:
                ...
                print(f"DEBUG: OpenAI result dir: {dir(result)}")
                print(f"DEBUG: Has choices: {hasattr(result, 'choices')}")
                if hasattr(result, 'choices'):
                    print(f"DEBUG: Choices value: {result.choices}")
                    print(f"DEBUG: Choices type: {type(result.choices)}")
                    if result.choices:
                        print(f"DEBUG: Choices length: {len(result.choices)}")
                        print(f"DEBUG: First choice type: {type(result.choices[0])}")
                        print(f"DEBUG: First choice dir: {dir(result.choices[0])}")
