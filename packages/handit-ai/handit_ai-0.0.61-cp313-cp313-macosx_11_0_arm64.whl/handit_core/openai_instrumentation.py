...
def patch_openai() -> None:
    ...
    print("DEBUG: patch_openai() called")
    try:
        import openai
        from openai.resources.chat import completions
    except Exception:
        print("DEBUG: Failed to import openai")
        return
    
    # Prevent double patching
    if getattr(completions.Completions.create, "_handit_patched", False):
        print("DEBUG: OpenAI already patched")
        return
    
    print("DEBUG: Patching OpenAI completions.create")
    
    # Also patch LangChain's structured output calls
    try:
        from langchain_openai import ChatOpenAI
        from langchain_core.language_models.chat import BaseChatModel
        print("DEBUG: Patching LangChain structured output on BaseChatModel")
        _patch_base_chat_model_structured_output()
    except ImportError:
        print("DEBUG: LangChain not available for structured output patching")
        pass
    
    # Try to patch specific chain objects that use structured output
    try:
        _patch_schema_inferencer_chain()
    except Exception as e:
        print(f"DEBUG: Failed to patch schema inferencer chain: {e}")
    ...
    def wrapped_create(self, **kwargs):  # type: ignore[no-untyped-def]
        ...
        print(f"DEBUG: Messages in request: {len(messages)} messages")
        for i, msg in enumerate(messages):
            print(f"DEBUG: Message {i}: {type(msg)} - {msg}")
        
        try:
            params_json = to_json_string(params_dict)
            print(f"DEBUG: Successfully serialized params, length: {len(params_json)}")
        except Exception as e:
            print(f"DEBUG: Failed to serialize params: {e}")
            # Fallback serialization for problematic messages
            try:
                safe_params = {
                    "model": model,
                    "message_count": len(messages),
                    "message_types": [str(type(msg)) for msg in messages],
                    "serialization_error": str(e)[:100]
                }
                params_json = json.dumps(safe_params)
                print(f"DEBUG: Using fallback params serialization")
            except Exception:
                params_json = '{"error": "Failed to serialize OpenAI parameters"}'

        print(f"DEBUG: About to call _native_on")
        _native_on(sid, func_name, module_name, file_name, line_no, t0, params_json)
        print(f"DEBUG: _native_on call completed")
        ...
        try:
            ...
            # Calculate timing immediately after the call, before serialization
            t1 = time.time_ns()
            dt_ns = t1 - t0
...
def _patch_base_chat_model_structured_output():
    """Patch BaseChatModel's with_structured_output method to capture structured calls"""
    print("DEBUG: _patch_base_chat_model_structured_output() called")
    try:
        from langchain_core.language_models.chat import BaseChatModel
        print(f"DEBUG: BaseChatModel imported: {BaseChatModel}")
        
        # Prevent double patching
        if hasattr(BaseChatModel, '_structured_output_patched'):
            print("DEBUG: BaseChatModel structured output already patched")
            return
            
        print("DEBUG: Getting original BaseChatModel.with_structured_output method")
        original_with_structured_output = BaseChatModel.with_structured_output
        
        def patched_with_structured_output(self, schema, **kwargs):
            """Patched version of with_structured_output"""
            print(f"DEBUG: with_structured_output called with schema: {type(schema)}")
            
            # Create the structured LLM normally
            structured_llm = original_with_structured_output(self, schema, **kwargs)
            print(f"DEBUG: Created structured LLM: {type(structured_llm)}")
            
            return structured_llm
        
        BaseChatModel.with_structured_output = patched_with_structured_output
        BaseChatModel._structured_output_patched = True
        
    except Exception as e:
        print(f"DEBUG: Failed to patch structured output: {e}")
        pass

def _patch_schema_inferencer_chain():
    """Try to patch the schema_inferencer chain directly"""
    print("DEBUG: Attempting to patch schema_inferencer chain")
    
    # Use a delayed import approach - patch when the chain is first accessed
    import sys
    import types
    
    # Create a custom module finder that patches chains when they're imported
    class ChainPatcher:
        def __init__(self):
            self.original_import = __builtins__.__import__
            __builtins__.__import__ = self.patched_import
            
        def patched_import(self, name, globals=None, locals=None, fromlist=(), level=0):
            # Call original import
            module = self.original_import(name, globals, locals, fromlist, level)
            
            # Check if this is importing the document_inference module
            if name.endswith('document_inference') or (fromlist and 'schema_inferencer' in fromlist):
                print(f"DEBUG: Found document_inference import, attempting to patch")
                try:
                    # Try to get the schema_inferencer from the module
                    if hasattr(module, 'schema_inferencer'):
                        schema_inferencer = module.schema_inferencer
                        print(f"DEBUG: Found schema_inferencer: {type(schema_inferencer)}")
                        
                        # Patch its invoke method
                        if hasattr(schema_inferencer, 'invoke') and not hasattr(schema_inferencer.invoke, '_handit_patched'):
                            original_invoke = schema_inferencer.invoke
                            
                            def traced_chain_invoke(input, **invoke_kwargs):
                                print(f"DEBUG: schema_inferencer.invoke called")
                                ...
                                if sid and _native:
                                    ...
                                    if _native_on and _return_on:
                                        func_name = "schema_inferencer"
                                        module_name = "graph.chains.document_inference"
                                        file_name = "<schema-inference>"
                                        line_no = 1
                                        t0 = time.time_ns()
                                        
                                        try:
                                            params_json = json.dumps({"input_type": str(type(input))})
                                        except:
                                            params_json = '{"input": "schema_inference_input"}'
                                        
                                        print(f"DEBUG: Emitting schema inferencer call event")
                                        _native_on(sid, func_name, module_name, file_name, line_no, t0, params_json)
                                        
                                        try:
                                            # Make the actual call
                                            result = original_invoke(input, **invoke_kwargs)
                                            
                                            # Calculate timing immediately after the call
                                            t1 = time.time_ns()
                                            dt_ns = t1 - t0
                                            ...
                                            try:
                                                if hasattr(result, 'model_dump'):
                                                    ...
                                                    result_json = json.dumps(str(result)[:500])
                                            except:
                                                result_json = f'{{"schema_result": "{str(result)[:500]}"}}'
                                            
                                            print(f"DEBUG: Emitting schema inferencer return event")
                                            _return_on(sid, func_name, t1, dt_ns, result_json)
                                            return result
                                            
                                        except Exception as e:
                                            # Emit return event for exceptions
                                            t1 = time.time_ns()
                                            dt_ns = t1 - t0
                                            error_json = f'{{"error": "{str(e)[:500]}"}}'
                                            try:
                                ...
                                # Fallback if no session or native
                                return original_invoke(input, **invoke_kwargs)
                            
                            schema_inferencer.invoke = traced_chain_invoke
                            schema_inferencer.invoke._handit_patched = True
                            print(f"DEBUG: Successfully patched schema_inferencer.invoke")
                        
                except Exception as patch_error:
                    print(f"DEBUG: Error patching schema_inferencer: {patch_error}")
            
            return module
    
    # Install the patcher
    try:
        patcher = ChainPatcher()
        print("DEBUG: Chain patcher installed")
    except Exception as e:
        print(f"DEBUG: Failed to install chain patcher: {e}")
