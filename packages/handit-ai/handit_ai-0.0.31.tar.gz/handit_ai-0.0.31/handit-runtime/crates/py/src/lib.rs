                    ...
                    if let Ok(name) = names.get_item(i).unwrap().extract::<String>() {
                        if let Ok(value) = locals.get_item(&name) {
                            ...
                            let preview_val = if let Ok(type_name_bound) = value.get_type().name() {
                                ...
                                match type_name.as_ref() {
                                    "str" => {
                                        // For string args, capture full content if it's not too expensive
                                        if let Ok(py_str) = value.downcast::<pyo3::types::PyString>() {
                                            match py_str.extract::<String>() {
                                                Ok(s) => s,
                                                Err(_) => format!("<str-arg:{}>", type_name)
                                            }
                                        } else {
                                            format!("<str-arg:{}>", type_name)
                                        }
                                    },
                                    "int" | "float" | "bool" | "NoneType" => {
...
unsafe extern "C" fn profiler_callback(
    ...
    match what {
        PY_TRACE_CALL => {
            ...
            // Build args preview (ultra-lightweight to avoid blocking)
            let args_preview = build_fast_args_preview(&frame_any);
        ...
        PY_TRACE_RETURN => {
            ...
            if let Some(t0) = t0 {
                // Fast return tracking - capture timing without expensive serialization in profiler callback
                // For non-blocking operation, we'll capture the full content later via a separate mechanism
                let ret_preview = if !arg.is_null() {
                    let arg_any: Bound<'_, pyo3::types::PyAny> = Bound::from_borrowed_ptr(py, arg as *mut ffi::PyObject);
                    
                    // Ultra-fast preview: avoid any expensive string conversions in profiler callback
                    let preview_value = if let Ok(type_name_bound) = arg_any.get_type().name() {
                        let type_name = type_name_bound.to_string_lossy();
                        match type_name.as_ref() {
                            "str" => {
                                // For string returns, try fast extraction without blocking
                                if let Ok(py_str) = arg_any.downcast::<pyo3::types::PyString>() {
                                    // Use extract instead of to_string_lossy which might be faster
                                    match py_str.extract::<String>() {
                                        Ok(s) => serde_json::Value::String(s),
                                        Err(_) => serde_json::Value::String("<string-extract-error>".to_string())
                                    }
                                } else {
                                    serde_json::Value::String("<string-downcast-error>".to_string())
                                }
                            },
                            "int" | "float" | "bool" | "NoneType" => {
                                // Only convert simple types that are guaranteed to be fast
                                match arg_any.str() {
                                    Ok(s) => serde_json::Value::String(s.to_string_lossy().to_string()),
                                    Err(_) => serde_json::Value::String(format!("<{}>", type_name)),
                                }
                            ...
                            _ => {
                                // For complex objects, just capture type to avoid any blocking
                                serde_json::Value::String(format!("<{}>", type_name))
                            }
                        }
                    } else {
                        serde_json::Value::String("<unknown-type>".to_string())
                    };
