...
class ArticleMetadata(BaseModel):
    ...
    study_type: Optional[str] = Field(description="Inferred study type")
    ...
    if name == "intelligent_search_pubmed":
        try:
            ...
            if article_ids:
                ...
                # Parse XML and extract article metadata (simplified for demo)
                for pmid in article_ids:
                    # In a full implementation, parse XML to extract real metadata
                    article_data = {"title": f"Article {pmid}", "pubdate": "2023"}
                    relevance_score = QueryIntelligence.calculate_relevance(article_data, context)
                    
                    articles.append(ArticleMetadata(
                        pmid=pmid,
                        title=f"Article {pmid} (detailed parsing would extract real title)",
                        authors=["Author extraction from XML"],
                        journal="Journal extraction from XML", 
                        pub_date="2023",
                        relevance_score=relevance_score,
                        key_concepts=["concept1", "concept2"]  # Would extract from abstract
                    ))
