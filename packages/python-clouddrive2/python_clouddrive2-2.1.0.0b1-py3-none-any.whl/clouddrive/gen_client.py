...
CRE_message_finditer = re_compile(
    "(?P<comment>(?:^//.*\n)*)^(?P<type>message|enum) (?P<name>\\w+)", MULTILINE
).finditer

CRE_rpc_finditer = re_compile(
    "(?P<comment>(?:^[ \t]*//.*\n)*)^[ \t]*rpc[ \t](?P<method>\\w+)\\((?:(?P<argspec>\\w+)[ \t]+)?(?P<argtype>[^)]+)\\)[^(]+\\((?:(?P<retspec>\\w+)[ \t]+)?(?P<rettype>[^)]+)\\);",
    MULTILINE,
).finditer
CRE_field_finditer = re_compile(
    r"^[ \t]*(?:(?P<spec>\w+)\s+)?(?P<type>[^=;]+)\s+(?P<name>\w+)(?:\s*=\s*(?P<value>\d+))?\s*;",
    MULTILINE,
).finditer
...
def extract_messages(proto_code):
    ...
    for n in it:
        yield m["name"], {
            **m.groupdict(),
            "define": proto_code[m.start() : n.start()].rstrip(),
        }
        m = n
...
rpcs = {
    m["method"]: {**m.groupdict(), "define": dedent(m[0])}
    for m in CRE_rpc_finditer(proto_code)
}
...
if async_:
    ...
    return None"""
            ...
            if argspec:
                method_body = (
                    f"arg = [to_message(clouddrive.pb2.{argtype}, a) for a in arg]"
                )
            else:
...
if async_:
    ...
    method_map[name] = "{%s}" % ", ".join(
        f'"{k}": {v}'
        for k, v in (("argument", arg_anno), ("return", ret_anno))
        if v != "None"
    )
    method_doc_parts = [
