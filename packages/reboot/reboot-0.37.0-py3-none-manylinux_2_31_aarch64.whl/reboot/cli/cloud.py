...
def parse_revisions(
    ...
    for revision in revisions.split(","):
        ...
        operator: LogsRequest.RevisionFilter.Operator = LogsRequest.RevisionFilter.Operator.EQUAL
        ...
        filters.append(
            LogsRequest.RevisionFilter(
                operator=operator,
                right_hand_revision_number=right_hand_revision_number,
                right_hand_latest=right_hand_latest,
            )
        )
...
async def _cloud_logs(
    ...
    proto_sources = (
        [Source.CONSENSUS] if source == SourceType.CONSENSUS else
        ...
        try:
            ...
            async for response in application.Logs(
                ...
                for record in response.records:
                    ...
                    line += record.text
                    print(line)
            if source == SourceType.CONSENSUS and printed_lines == 0:
