...
async def maleo_exception_handler(
    ...
    if exc.operation_type is OperationType.REQUEST and isinstance(
        exc.operation,
        (
            CreateFailedRequestOperation,
            ReadFailedRequestOperation,
        ...
        response_context, final_response = await ResponseContext.async_from_response(
            final_response
        )
        exc.operation.response_context = response_context

    exc.operation.log(logger, level=Level.ERROR)
