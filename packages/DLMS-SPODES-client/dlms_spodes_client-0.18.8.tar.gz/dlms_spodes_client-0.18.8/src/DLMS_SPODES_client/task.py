...
class TransferImage(StrictOK):
    ...
    async def transfer_block(self, c: Client, i: int, size: int):
        ...
        await c.execute_method2(
            ...
            mip=ImageBlockTransfer((
                ...
                cdt.OctetString(bytearray(self.image[offset: offset + size]))
            ))
        )

    async def exchange(self, c: Client) -> result.StrictOk | result.Error:
        """ update image if blocks is fulls ver 3"""
        current_status: ImageTransferStatus
        previous_status: Optional[i_t_status.ImageTransferStatus] = None
        activate_info: cdt.OctetString
        c.log(logL.INFO, F"start update by {self.obj}")
        res = result.StrictOk()
        if (_block_size := res.propagate_err(await ReadObjAttr(self.obj, 2).exchange(c))) is result.NULL:
            return res.as_error()
        ...
        if (current_status := res.propagate_err(await get_status.exchange(c))) is result.NULL:
            return res.as_error()
        if (activate_info := res.propagate_err(await get_to_activate_info.exchange(c))) is result.NULL:
            return res.as_error()
        if (
            ...
            if (_n_t_b := res.propagate_err(await get_not_transferred_block.exchange(c))) is result.NULL:
                return res.as_error()
            self.n_t_b = int(_n_t_b)
                ...
                match current_status:
                    ...
                    case i_t_status.TRANSFER_INITIATED:
                        while self.n_t_b < self.n_blocks:
                            await self.transfer_block(c, self.n_t_b, block_size)
                    ...
                    case i_t_status.VERIFICATION_FAILED:
                        if (_valid := res.propagate_err(await get_transferred_blocks_status.exchange(c))) is result.NULL:
                            return res.as_error()
                        valid = tuple(_valid)
                        c.log(logL.INFO, F"Got blocks[{len(self.obj.image_transferred_blocks_status)}]: {self.obj.image_transferred_blocks_status}")
                        ...
                        for index in filter(lambda it: valid[it] == b'\x00', range(len(valid))):
                            await self.transfer_block(c, index, block_size)
                    ...
                    case i_t_status.VERIFICATION_SUCCESSFUL:
                        ...
                        if any(map(lambda it: it == b'\x00', valid)):
                            ...
                            return res.as_error(exc.DLMSException(F"Got {self.obj.image_first_not_transferred_block_number} not transferred block"))
                            ...
                            return res.as_error(exc.DLMSException(message))
                        else:
                    ...
                    case err:
                        return res.as_error(DLMSException(f"Unknown image transfer status: {err}"))
