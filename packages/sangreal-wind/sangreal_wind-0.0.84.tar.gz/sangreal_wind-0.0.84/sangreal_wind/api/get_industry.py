...
def get_industry_all_windhk(level=1):
    ...
    adjust {bool} -- [由于中信变更行业分类，是否调整兼容之前的代码] (default: {True})
    """
    clss = WIND_DB.hkstockwindindustriesmembers
    ind_code = WIND_DB.hkstockindustriescode
    df = WIND_DB.query(
        clss.S_INFO_WINDCODE, clss.ENTRY_DT, clss.REMOVE_DT, ind_code.INDUSTRIESNAME
    ).filter(ind_code.LEVELNUM == level, ind_code.industriesclass == "Wind行业分类")
    try:
        df = df.filter(
            func.substring(clss.WIND_IND_CODE, 1, 2 + 2 * level)
            == func.substring(ind_code.INDUSTRIESCODE, 1, 2 + 2 * level)
        ).to_df()
    except:
        df = df.filter(
            func.substr(clss.WIND_IND_CODE, 1, 2 + 2 * level)
            == func.substr(ind_code.INDUSTRIESCODE, 1, 2 + 2 * level)
        ).to_df()
    df.columns = ["sid", "entry_dt", "out_dt", "ind"]
    # 去除行业中的罗马数字
...
def get_industry_windhk(trade_dt, sid=None, level=1):
    ...
    trade_dt = dt_handle(trade_dt)
    df = get_industry_all_windhk(level=level)
    if sid is not None:
        sid = {sid} if isinstance(sid, str) else set(sid)
        df = df[df["sid"].isin(sid)].copy()
