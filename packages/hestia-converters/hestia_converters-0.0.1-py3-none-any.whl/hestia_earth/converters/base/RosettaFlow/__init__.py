...
from .file_shemas import (
    ...
    list_of_flow_mappings, list_of_flow_mappings, template_csv_file_headers
)
...
class ProductFlowMapperError(MapperError):
    """Base exception for flow mapping Errors replated to product flows"""
...
class MetaData(BaseModel):
    stepping_stones: List[ResultNode | None] = Field(default=None, exclude=True,
                                                     description="List of nomenclatures traversed to find candidate",
                                                     examples=[[{'list_name': 'Ecoinvent 3.11', 'match_condition': "=",
...
class FlowMap:
    ...
    def map_flow_to_destination_id(self, source_flow: dict, check_reverse=False) -> str | None:
    ...
    def map_flow(self, source_flow: dict,
                 target_nomenclature: str | list = None,
                 target_context: str | list = None,
                 check_reverse=True,
    ...
    def map_flow_raw(self, source_flow: dict, check_reverse=False) -> List[FlowmappingFields] | None:
        if self.all_flow_mappings.empty:
            logger.warning("No mappings loaded.")
            ...
            if not explored_paths.empty:
                ...
                if not distant_results.empty and target_context:
                    distant_results = distant_results.loc[
                        distant_results['TargetFlowContext_stepping_stone'] == target_context]
            ...
            [
                (hasattr(c, required_field) and
                 (
                         ...
                         (isinstance(required_value, str) and required_value in getattr(c, required_field))
                 )
                 )
