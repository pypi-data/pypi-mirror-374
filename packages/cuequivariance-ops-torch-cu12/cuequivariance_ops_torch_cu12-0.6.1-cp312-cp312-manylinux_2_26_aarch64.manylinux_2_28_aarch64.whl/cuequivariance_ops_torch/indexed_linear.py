...
@torch.library.custom_op(
    "cuequivariance_ops::indexed_linear_B",
    mutates_args=(),
    device_types="cuda",
)
def _(
...
@torch.library.custom_op(
    "cuequivariance_ops::indexed_linear_C",
    mutates_args=(),
    device_types="cuda",
)
def _(
...
def indexed_linear(
    ...
    if subscripts == ("u", "v", "uv"):
        return torch.ops.cuequivariance_ops.indexed_linear_C(
            A, B, counts, Z, C, u, v, coefficient
        )
    elif subscripts in [("u", "uv", "v"), ("u", "vu", "v")]:
        transpose_B = subscripts == ("u", "vu", "v")
        return torch.ops.cuequivariance_ops.indexed_linear_B(
...
def indexed_linear_B_backward(ctx, grad_output):
    ...
    if ctx.needs_input_grad[0]:  # grad wrt A
        ...
        grad_A = torch.ops.cuequivariance_ops.indexed_linear_B(
    ...
    if ctx.needs_input_grad[1]:  # grad wrt B
        ...
        if transpose_B:
            ...
            grad_B = torch.ops.cuequivariance_ops.indexed_linear_C(
...
def indexed_linear_C_backward(ctx, grad_output):
    ...
    if ctx.needs_input_grad[1]:  # grad wrt B
        ...
        grad_B = torch.ops.cuequivariance_ops.indexed_linear_B(
