...
class Env:
    ...
    def init(self, rng: Array) -> Tuple[Obs, State]:
        state = init_fn(self, rng, self.cfg.size)  # without jit this takes forever
...
# %% Functions
def init_fn(env: Env, rng: Array, size) -> State:
    pos = spawn(env, rng, size)
    return State(pos=pos, hp=jnp.float32(env.health[env.types]))


def spawn(env, rng, size) -> Array:
    loc = jnp.zeros_like(env.map).at[jnp.arange(size // 4), jnp.arange(size // 4)].set(1)
    aux = lambda x, loc: random.choice(rng, jnp.arange(size**2), (x,), True, ((1 - env.map) & loc).flatten())  # noqa
    flat = jnp.concatenate((aux(env.blu_num, loc), aux(env.red_num, jnp.flip(loc).T)))
    pos = jnp.float32(jnp.column_stack((flat // size, flat % size)))
    return pos
