...
class MySQLTable:
    ...
    def _single_insert(self, data: Dict[str, Any]) -> int:
        """插入单条数据"""
        logger.debug("插入数据到表 {}，数据: {}", self._table_name, data)
    ...
    def _batch_insert(self, data_list: List[Dict[str, Any]]) -> List[int]:
        ...
        if len(data_list) <= 3:  # 少量数据时显示具体内容
            logger.debug("批量插入数据详情: {}", data_list)
        else:
            logger.debug("批量插入数据示例（前3条）: {}", data_list[:3])
    ...
    def select(self, where: Optional[Dict] = None, fields: Optional[List[str]] = None,
        ...
        Returns:
            List[Dict]: 查询结果列表
        """
        logger.debug("查询表 {}，条件: {}，字段: {}，排序: {}，限制: {}",
                    self._table_name, where, fields, order_by, limit)
        ...
        try:
            ...
            if results and len(results) <= 5:  # 少量数据时显示具体内容
                logger.debug("查询结果: {}", results)
            return results
