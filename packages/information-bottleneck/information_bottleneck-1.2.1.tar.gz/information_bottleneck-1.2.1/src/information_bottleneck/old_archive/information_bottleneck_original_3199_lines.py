...
class InformationBottleneck:
    ...
    def _compute_compression_term_exact(self, X: np.ndarray) -> float:
        ...
        # FIXME: Critical Research Issues in Compression Term Computation Based on Tishby et al. (1999)
        #
        # 1. MISSING THEORETICAL JUSTIFICATION FOR DISCRETE X ASSUMPTION
        #    - Current implementation assumes discrete X with uniform empirical p(x) = 1/n
        #    - Paper discusses both continuous and discrete cases but doesn't specify this approach
        #    - For continuous X, should use differential mutual information or proper discretization
        #    - Solutions:
        #      a) Add proper discretization scheme for continuous X: histogram binning or k-NN
        #      b) Implement differential entropy for continuous case: I(X;Z) = h(Z) - h(Z|X)
        #      c) Add adaptive binning based on data distribution characteristics
        #    - Paper context: Examples use both continuous (Gaussian) and discrete variables
        #    - Example for continuous X:
        #      ```python
        #      def _compute_differential_compression(self, X):
        #          # Estimate differential entropy h(Z) and h(Z|X)
        #          h_z = self._differential_entropy(self.p_z_given_x.mean(axis=0))
        #          h_z_given_x = -np.mean([self._entropy(row) for row in self.p_z_given_x])
        #          return h_z - h_z_given_x
        #      ```
        #
        # 2. NUMERICAL STABILITY ISSUES IN LOG COMPUTATIONS
        #    - Current threshold 1e-12 may be too small for practical applications
        #    - Should use more robust numerical methods from information theory literature
        #    - Missing proper handling of zero probabilities in log computations
        #    - Solutions:
        #      a) Use log-sum-exp trick for numerical stability
        #      b) Implement proper regularization: add small ε to all probabilities
        #      c) Use natural logarithm consistently, convert to bits only at end
        #    - Information theory best practice: always regularize probability estimates
        #
        # 3. INEFFICIENT NESTED LOOPS FOR LARGE DATASETS
        #    - O(n×k) computation could be vectorized for better performance
        #    - Paper examples work with moderate dataset sizes, but modern applications are larger
        #    - Solutions:
        #      a) Vectorize using numpy broadcasting: np.sum(p_x * p_z_given_x * np.log(p_z_given_x / p_z))
        #      b) Use batch processing for very large datasets
        #      c) Implement sampling-based approximation for computational efficiency
    ...
    def _compute_prediction_term_exact(self, Y: np.ndarray) -> float:
        ...
        # FIXME: Critical Research Issues in Prediction Term Computation Based on Tishby et al. (1999)
        #
        # 1. INCORRECT HANDLING OF CONTINUOUS Y VARIABLES (Paper Sections 3-4)
        #    - Current implementation assumes Y is discrete with integer labels
        #    - Paper discusses both discrete classification and continuous regression cases
        #    - For continuous Y, should use differential mutual information I(Z;Y) = h(Y) - h(Y|Z)
        #    - Solutions:
        #      a) Auto-detect Y type: if len(np.unique(Y)) > 50, treat as continuous
        #      b) Implement differential entropy estimation for continuous Y
        #      c) Add proper discretization for continuous Y using quantiles or k-means
        #    - Paper example: Gaussian Y requires differential entropy computation
        #    - Example:
        #      ```python
        #      if self._is_continuous_target(Y):
        #          # Differential mutual information for continuous Y
        #          h_y = self._differential_entropy(Y)
        #          h_y_given_z = self._conditional_differential_entropy(Y, self.p_z_given_x)
        #          return h_y - h_y_given_z
        #      ```
        #
        # 2. INEFFICIENT EMPIRICAL PROBABILITY COMPUTATION
        #    - Computing p(y) inside nested loop is O(n×k×|Y|) instead of O(n)
        #    - Should precompute empirical Y distribution once
        #    - Missing proper error handling for degenerate Y distributions
        #    - Solutions:
        #      a) Precompute p_y_empirical = np.bincount(Y) / len(Y) once outside loop
        #      b) Use vectorized operations for mutual information computation
        #      c) Add validation for minimum Y class frequencies
        #    - Computational efficiency critical for large datasets
        #
        # 3. MISSING VALIDATION OF BAYES RULE CONSISTENCY (Equation 17)
        #    - Paper's equation 17 requires: p(y|z) = Σ_x p(y|x)p(x|z) with p(x|z) from Bayes rule
        #    - Current implementation doesn't validate this consistency
        #    - Should verify p(y|z) satisfies normalization: Σ_y p(y|z) = 1 ∀z
        #    - Solutions:
        #      a) Add consistency check: validate Σ_y p(y|z) = 1.0 for all clusters
        #      b) Implement proper Bayes rule update as specified in equation 17
        #      c) Add warning if p(y|z) distributions become degenerate
        #    - Research accuracy: This is core to Information Bottleneck theory
    ...
    def _blahut_arimoto_update(self, X: np.ndarray, Y: np.ndarray, temperature: float = 1.0):
        ...
        # FIXME: Critical Research Accuracy Issues Based on Actual Tishby et al. (1999) Paper
        #
        # 1. INCORRECT BLAHUT-ARIMOTO EQUATION IMPLEMENTATION (Equation 31)
        #    - Paper specifies exact iterative algorithm: p^(t+1)(z|x) = p^(t)(z)g^(t)(z,x) / Z(x,β)
        #    - Current implementation uses simplified KL divergence instead of full equation
        #    - Missing proper g(z,x) computation: g(z,x) = exp[-β·D_KL[p(y|x)||p^(t)(y|z)]]
        #    - Solutions:
        #      a) Implement exact equation 31: g(z,x) = exp[-β·D_KL[δ(y-y_x)||p^(t)(y|z)]]
        #      b) Compute proper partition function: Z(x,β) = Σ_z p^(t)(z)g^(t)(z,x)
        #      c) For discrete Y, D_KL[δ(y-y_x)||p(y|z)] = -log p(y_x|z)
        #    - Paper reference: "the iterative algorithm amounts to the Blahut-Arimoto algorithm"
        #    - Example from paper:
        #      ```python
        #      # Exact Tishby Blahut-Arimoto (equation 31)
        #      for x in range(n_samples):
        #          y_x = Y[x]  # observed y for sample x
        #          Z_x_beta = 0.0
        #          # Compute partition function
        #          for z in range(n_clusters):
        #              g_z_x = np.exp(-self.beta * (-np.log(self.p_y_given_z[z, y_x])))
        #              Z_x_beta += self.p_z[z] * g_z_x
        #          # Update p(z|x)
        #          for z in range(n_clusters):
        #              g_z_x = np.exp(-self.beta * (-np.log(self.p_y_given_z[z, y_x])))
        #              self.p_z_given_x[x, z] = (self.p_z[z] * g_z_x) / Z_x_beta
        #      ```
        #
        # 2. DETERMINISTIC ANNEALING NOT FOLLOWING PAPER PROTOCOL (Section 6)
        #    - Paper emphasizes "deterministic annealing" as critical for avoiding poor local minima
        #    - Current temperature parameter not properly integrated with β schedule
        #    - Missing proper cooling schedule: β should increase gradually, not temperature decrease
        #    - Solutions:
        #      a) Implement proper β-annealing: start β near 0, increase to final value
        #      b) Follow paper's prescription: "β is slowly increased from 0 to β_max"
        #      c) Remove temperature scaling, use direct β increase instead
        #    - Research insight: "deterministic annealing is particularly effective"
        #
        # 3. MISSING CONVERGENCE CRITERIA FROM PAPER
        #    - Paper doesn't specify exact convergence criteria, but implies stability
        #    - Current single-iteration update insufficient for true convergence
        #    - Should iterate self-consistent equations until stable fixed point
        #    - Solutions:
        #      a) Add convergence check: |L^(t+1) - L^(t)| < tolerance
        #      b) Monitor information plane trajectory stability
        #      c) Implement multiple iterations within each β value
        #    - Paper insight: equations are "iterated until convergence"
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray, use_annealing: bool = True, 
        ...
        # FIXME: Critical Research Accuracy Issues Based on Actual Tishby et al. (1999) Paper
        #
        # 1. MISSING CORE INFORMATION BOTTLENECK FUNCTIONAL (Equation 4)
        #    - Paper derives exact functional: L[p(z|x)] = I(X;Z) - βI(Z;Y)
        #    - Current implementation doesn't explicitly compute this Lagrangian
        #    - Should track L value throughout optimization for convergence monitoring
        #    - Solutions:
        #      a) Implement exact functional: L = H(Z) - H(Z|X) - β[H(Y|Z) - H(Y)]
        #      b) Track L trajectory to detect convergence and phase transitions
        #      c) Report L value at each iteration for research reproducibility
        #    - Paper quote: "minimize the functional L[p(z|x)]" (equation 4)
        #    - Example:
        #      ```python
        #      def _compute_ib_functional(self, X, Y):
        #          H_Z = self._entropy(self.p_z)
        #          H_Z_given_X = self._conditional_entropy_z_given_x()
        #          H_Y_given_Z = self._conditional_entropy_y_given_z(Y)
        #          H_Y = self._entropy_y(Y)
        #          I_X_Z = H_Z - H_Z_given_X
        #          I_Z_Y = H_Y - H_Y_given_Z
        #          return I_X_Z - self.beta * I_Z_Y
        #      ```
        #
        # 2. DETERMINISTIC ANNEALING PROTOCOL INCORRECT (Section 6)
        #    - Paper specifies: "β is slowly increased from 0 to some final value"
        #    - Current annealing_schedule options don't follow paper's exact prescription
        #    - Missing proper phase transition detection at critical β values
        #    - Solutions:
        #      a) Implement 'tishby_protocol': β starts at 0, increases linearly to β_max
        #      b) Add phase transition detection: monitor I(X;Z) bifurcations
        #      c) Use multiple β values per phase transition point
        #    - Paper insight: "the annealing procedure finds the global minimum"
        #    - Research note: Different β ranges reveal different clustering granularities
        #
        # 3. SELF-CONSISTENT EQUATIONS NOT PROPERLY ITERATED (Equations 16-17)
        #    - Paper requires iteration: "self-consistent equations are iterated until convergence"
        #    - Current single update insufficient for achieving fixed point
        #    - Missing convergence criteria specific to information-theoretic measures
        #    - Solutions:
        #      a) Iterate equations 16-17 within each β step until |ΔL| < tolerance
        #      b) Monitor both p(z|x) and p(y|z) stability simultaneously
        #      c) Use information-theoretic convergence: |ΔI(X;Z)| + |ΔI(Z;Y)| < ε
        #    - Fixed point condition: p^*(z|x) = p^*(z)exp[-βD_KL[p(y|x)||p^*(y|z)]] / Z(x,β)
        #
        # 4. BLAHUT-ARIMOTO ALGORITHM IMPLEMENTATION GAPS (Equation 31)
        #    - Current implementation simplified compared to paper's exact specification
        #    - Missing proper alternating optimization between p(z|x) and p(y|z)
        #    - Should implement exact equation 31: iterative coordinate ascent
        #    - Solutions:
        #      a) Implement exact alternating updates as specified in equation 31
        #      b) Add proper convergence monitoring for Blahut-Arimoto iterations
        #      c) Use exact KL divergence computation for discrete Y case
        #    - Paper reference: "amounts to the Blahut-Arimoto algorithm"
        #      b) Add 'deterministic_annealing': T(t) cooling with β = 1/T
        #      c) Add validation-based adaptive annealing
        #    - Research basis: Proper annealing prevents local minima and enables phase transitions
        #
        # 3. MISSING CONVERGENCE DIAGNOSTICS
        #    - No monitoring of information plane dynamics I(X;Z) vs I(Z;Y)
        #    - Missing detection of phase transitions (critical points in β space)
        #    - Should track and visualize learning trajectory in information plane
        #    - Solutions:
        #      a) Compute and log I(X;Z), I(Z;Y) at each iteration
        #      b) Detect phase transitions: sudden changes in information coordinates
        #      c) Add early stopping based on information plane convergence
        #    - This provides crucial insights into representation learning dynamics
        #
        # 4. CATEGORICAL VARIABLE HANDLING INCOMPLETE
        #    - Information bottleneck designed for discrete X, Y variables
        #    - Current continuous variable discretization may lose information
        #    - Should use optimal binning strategies from information theory
        #    - Solutions:
        #      a) Implement equal-frequency binning for continuous X
        #      b) Use mutual information-based discretization for optimal bins
        #      c) Add continuous IB variant using kernel density estimation
        #    - Mathematical basis: Discrete IB requires proper probability estimates
        """
