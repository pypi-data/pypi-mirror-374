...
INIT_FILE="src/starch/__init__.py"
...
update_config_line() {
    ...
    local temp_file
    temp_file=$(mktemp)
    sed "s|VERSION_CONFIG: .*|VERSION_CONFIG: $new_config|" "$INIT_FILE" > "$temp_file"
...
update_version_line() {
    ...
    sed "s|__version__ = \".*\"|__version__ = \"$new_version\"|" "$INIT_FILE" > "$temp_file"
    mv "$temp_file" "$INIT_FILE"
}
...
show_current() {
    ...
    current_version=$(grep '__version__ = ' "$INIT_FILE" | sed 's/.*"\(.*\)".*/\1/')
...
bump_build() {
    ...
    current_config=$(grep "VERSION_CONFIG:" "$INIT_FILE" | sed 's/.*VERSION_CONFIG: //')

    local new_config
    new_config=$(echo "$current_config" | jq ".build = $new_build")
...
set_phase() {
    ...
    current_config=$(grep "VERSION_CONFIG:" "$INIT_FILE" | sed 's/.*VERSION_CONFIG: //')
    local new_config
    new_config=$(echo "$current_config" | jq ".phase = \"$phase\"")
...
set_base() {
    ...
    local new_config
    new_config=$(echo "$current_config" | jq ".base = \"$base\"")
...
reset_phase() {
    ...
    local new_config
    new_config=$(echo "$current_config" | jq '.phase = "a" | .build = 1')
...
set_build() {
    ...
    local new_config
    new_config=$(echo "$current_config" | jq ".build = $build")
