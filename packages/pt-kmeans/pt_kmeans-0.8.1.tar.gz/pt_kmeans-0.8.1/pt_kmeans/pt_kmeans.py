...
def _kmeans_plusplus_init(
    ...
    for i in tqdm(
        ...
        if i < n_clusters - 1:
            new_center_distances = compute_distance(
                x, centers[i : i + 1], distance_metric, x_pre_normalized=x_pre_normalized, chunk_size=chunk_size
            )
...
def initialize_centers(
    ...
    if method == "random":
        ...
        centers = _kmeans_plusplus_init(
            x, n_clusters, distance_metric, x_pre_normalized, show_progress, generator, chunk_size, device
        )
    else:
