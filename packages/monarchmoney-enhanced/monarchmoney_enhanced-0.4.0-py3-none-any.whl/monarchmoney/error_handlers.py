...
class RateLimitRecoveryStrategy(ErrorRecoveryStrategy):
    ...
    async def handle(self, error: Exception, context: ErrorContext) -> bool:
        ...
        logger.info("Handling rate limit", operation=context.operation, retry_count=context.retry_count)
...
class NetworkRecoveryStrategy(ErrorRecoveryStrategy):
    ...
    async def handle(self, error: Exception, context: ErrorContext) -> bool:
        ...
        logger.info("Handling network error", operation=context.operation, retry_count=context.retry_count)
...
class ServerErrorRecoveryStrategy(ErrorRecoveryStrategy):
    ...
    async def handle(self, error: Exception, context: ErrorContext) -> bool:
        ...
        logger.info("Handling server error", operation=context.operation, retry_count=context.retry_count)
...
class ErrorRecoveryManager:
    ...
    async def attempt_recovery(self, error: Exception, context: ErrorContext) -> bool:
        ...
        logger.warning("No recovery strategy succeeded", error_type=error.__class__.__name__)
        return False
        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                ...
                try:
                    ...
                    if not any(isinstance(error, exc_type) for exc_type in recoverable_exceptions):
...
class ErrorMessageFormatter:
    ...
    @staticmethod
    def format_error(error: Exception, context: Optional[ErrorContext] = None) -> Dict[str, Any]:
        """
