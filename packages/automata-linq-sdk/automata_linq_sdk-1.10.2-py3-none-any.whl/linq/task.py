...
from linq.schema.workflow_api import IntegerParameterDefinitionInput
...
Task = Union["ActionTask", "CodeTask", "Conditional", "SftpDataConnector"]
TaskLike = TypeVar("TaskLike", "ActionTask", "CodeTask", "Conditional", "SftpDataConnector")
...
class LabwareSource(SDKEntity[workflow_api.LabwareSourceInput, workflow_api.LabwareSource]):
    ...
    source_task: "ActionTask | ConditionalSourceTask | None" = None
    ...
    def _get_source_task_for_input(self) -> str | workflow_api.ConditionalSourceTaskInput | None:
        if isinstance(self.source_task, ActionTask):
...
_Input = TaskOutputReference | ReferenceData | ParameterReference | workflow_api.JsonSerializable
    ...
    def to_api_input(self) -> workflow_api.ActionTaskInput:
        ...
        return workflow_api.ActionTaskInput(
            ...
            time_estimate=(
                ...
                else ParameterReference.to_api_input(self.time_estimate)
            ),
