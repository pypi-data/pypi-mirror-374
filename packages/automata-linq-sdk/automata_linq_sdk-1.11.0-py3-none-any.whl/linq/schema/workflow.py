...
#   timestamp: 2025-08-21T14:38:35+00:00
...
ConditionalSourceTaskInput = ConditionalSourceTask
...
FailureResponse = Literal["abort", "pause", "continue"]
...
GrippingOffsetInput = GrippingOffset
...
GrippingWidthInput = GrippingWidth
...
HookParametersInput = HookParameters
...
ParameterReferenceInput = ParameterReference
...
PlanStatus = Literal["PENDING", "COMPLETED", "FAILED"]
...
PlannedLabwareLocation = Location
...
PythonReferenceFunctionInput = PythonReferenceFunction
...
ReferenceDataInput = ReferenceData
...
StoredLabwareInput = StoredLabware
...
TransportPathInput = TransportPath
...
ParameterValues = TypeAliasType(
    "ParameterValues",
    Annotated[
        StringParameterValueInput | IntegerParameterValueInput | FloatParameterValueInput | BooleanParameterValueInput,
        Discriminator("type"),
    ],
)
...
class CodeTaskInput(BaseModel):
    ...
    inputs: (
        dict[str, StaticDataInput | TaskOutputReferenceInput | ReferenceDataInput | ParameterReferenceInput] | None
    ) = Field(default=None, description="The inputs to the function", title="Inputs")
...
class DataConnectorInput(BaseModel):
    ...
    inputs: (
        dict[str, StaticDataInput | TaskOutputReferenceInput | ReferenceDataInput | ParameterReferenceInput] | None
    ) = Field(default=None, description="Input arguments that are passed to the instrument driver", title="Inputs")
    mock_behaviour: MockBehaviourInput | None = Field(
        default=None, description="Overrides for the behaviour of the task on mock drivers"
    )
...
class ExecutorOptions(BaseModel):
    ...
    behaviour_on_planner_failure: FailureResponse = Field(
        ...,
...
class ExecutorOptionsInput(BaseModel):
    ...
    behaviour_on_planner_failure: FailureResponse | None = Field(
        default=None,
        description="Behaviour of the system when a replan fails. Defaults to continue (ignore the failure and continue with the current plan)",
    )
    include_task_description_in_logs: bool = Field(
...
ParameterDefinition = TypeAliasType(
    "ParameterDefinition",
    Annotated[
        StringParameterDefinition | IntegerParameterDefinition | FloatParameterDefinition | BooleanParameterDefinition,
        Discriminator("type"),
    ],
)
