...
from .exceptions import APIError, AuthError, HubRestartCommandResponseNotFound
...
class Linq:
        ...
        if not credentials.is_configured():
            raise AuthError(
                message=AuthErrorMessage(
                    ...
                    description=(
                        "Validation error: Credentials are not set. "
                        "Please login by running 'linq auth login' command."
                    ),
                )
            )
            ...
            raise AuthError(
                message=AuthErrorMessage(
                    ...
                    description=(
                        ...
                        "Please login again by running 'linq auth login' command."
                    ),
                )
            )
    ...
    def _make_request(
        ...
        if response.status_code in [200, 201, 204, 422, 400, 404]:
            return response
    ...
    def get_plan_status(self, workflow_id: str, plan_id: str) -> workflow_api.WorkflowPlanStatus:
        ...
        if response.status_code != 200:
            error_response = WorkflowAPIErrorResponse.model_validate(response.json())
            raise APIError(f"Failed to get plan status '{workflow_id}': {error_response.detail}")

        return workflow_api.WorkflowPlanStatus.model_validate(response.json())

    def get_plan_result(self, workflow_id: str, plan_id: str) -> workflow_api.WorkflowPlanResult:
    ...
    def deploy_workflow(
        ...
        *,
        ...
        :param device_id: the ID of the device to deploy the workflow to
    ...
    def get_status(
        ...
        *,
        ...
        :param device_id: the ID of the device to get the status of
    ...
    def get_latest_error(
        ...
        *,
        ...
        device_state = backend_api.Device.model_validate(response.json())
        if device_state.error is not None:
            # NOTE: Abort is always available, but not returned from the API
            device_state.error.available_actions.append(backend_api.DeviceErrorAvailableActions.Abort)
    ...
    def start_workflow(
        ...
        *,
        ...
        :param device_id: the ID of the device to start the workflow on
    ...
    def pause_workflow(
        ...
        *,
        ...
        :param device_id: the ID of the device to pause the workflow on
    ...
    def stop_workflow(
        ...
        *,
        ...
        param device_id: the ID of the device to stop the workflow on
    ...
    def publish_workflow(
        ...
        *,
        ...
        response = self._make_request(
            ...
            parameters={"simulate_drivers": simulate_drivers, "simulate_time": simulate_time, "reason": reason},
            data={"parameter_values": [pv.model_dump(exclude_none=True, mode="json") for pv in parameter_values]},
        )
        if response.status_code != 200:
    ...
    def get_run_histories(
        ...
        *,
        ...
        """Fetch run histories ordered by start_time for a device."""
        parameters = {
