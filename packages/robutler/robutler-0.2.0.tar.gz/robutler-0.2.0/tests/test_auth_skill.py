"""
AuthSkill Unit Tests - Robutler V2.0

Comprehensive tests for Robutler Platform integration and authentication functionality.
"""
...
from robutler.agents.skills.robutler.auth import AuthSkill, AuthScope, AuthContext, AuthenticationError, AuthorizationError
...
def mock_robutler_client():
    """Create mock Robutler client for testing"""
    client = Mock()
    
    # Mock successful health check
    client.health_check = AsyncMock(return_value=ApiResponse(
        success=True,
        data={'status': 'healthy', 'service': 'robutler-platform'}
    ))
    
    # Mock successful user validation
    test_user = User(
        id='test-user-123',
        email='test@robutler.ai',
        name='Test User',
        role=UserRole.USER,
        total_credits=1000,
        used_credits=100
...
@pytest.fixture
def auth_skill(mock_robutler_client):
...
async def test_auth_skill_initialization(auth_skill, mock_robutler_client):
    """Test AuthSkill initialization with Robutler Platform client"""
    
    # Mock the RobutlerClient creation
    with patch('robutler.agents.skills.robutler.auth.skill.RobutlerClient', return_value=mock_robutler_client):
        agent = BaseAgent(
...
async def test_api_key_authentication(auth_skill, mock_robutler_client):
    """Test API key authentication with Robutler Platform"""
    ...
    with patch('robutler.agents.skills.robutler.auth.skill.RobutlerClient', return_value=mock_robutler_client):
        ...
        assert result['user']['email'] == 'test@robutler.ai'
...
async def test_request_authentication_hook(auth_skill, mock_robutler_client):
    ...
    with patch('robutler.agents.skills.robutler.auth.skill.RobutlerClient', return_value=mock_robutler_client):
        ...
        test_user = User(id='test-user-123', email='test@robutler.ai', role=UserRole.USER)
...
@pytest.mark.asyncio
async def test_user_tools(auth_skill, mock_robutler_client):
...
async def test_error_handling(auth_skill, mock_robutler_client):
    ...
    with patch('robutler.agents.skills.robutler.auth.skill.RobutlerClient', return_value=mock_failed_client):
        agent = BaseAgent(
            name="error-test",
            instructions="Test agent",
            skills={"auth": auth_skill}
...
@pytest.mark.asyncio
async def test_authorization_scopes(auth_skill, mock_robutler_client):
    ...
    with patch('robutler.agents.skills.robutler.auth.skill.RobutlerClient', return_value=mock_robutler_client):
        ...
        admin_user = User(id='admin-123', email='admin@robutler.ai', role=UserRole.ADMIN)
        ...
        user_auth = AuthContext(
            user=User(id='user-123', email='user@robutler.ai', role=UserRole.USER),
            authenticated=True,
            scope=AuthScope.USER,
            permissions=['read', 'write']
        )
