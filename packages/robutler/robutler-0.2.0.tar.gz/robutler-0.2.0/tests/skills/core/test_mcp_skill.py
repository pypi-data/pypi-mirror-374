...
# Test imports  
from robutler.agents.skills.core.mcp import MCPSkill, MCPServerConfig, MCPTransport, MCPExecution
...
class TestMCPSkillInitialization:
    ...
    async def test_mcp_skill_initialize_without_mcp_sdk(self):
        ...
        with patch('robutler.agents.skills.core.mcp.skill.MCP_AVAILABLE', False):
    ...
    async def test_mcp_skill_initialize_with_servers_config(self):
        ...
        with patch('robutler.agents.skills.core.mcp.skill.MCP_AVAILABLE', True):
            with patch('robutler.utils.logging.get_logger'):
...
class TestMCPServerConnection:
    ...
    async def test_connect_to_http_server_success(self):
        ...
        with patch('robutler.agents.skills.core.mcp.skill.create_http_client') as mock_create_http:
    ...
    async def test_connect_to_sse_server_success(self):
        ...
        with patch('robutler.agents.skills.core.mcp.skill.create_sse_client') as mock_create_sse:
...
class TestMCPDynamicToolRegistration:
    ...
    async def test_register_dynamic_tool_success(self):
        ...
        assert hasattr(registered_func, '_robutler_is_tool')
        assert registered_func._robutler_is_tool is True
        assert registered_func._tool_scope == "all"
        assert hasattr(registered_func, '_robutler_tool_definition')
        
        # Check OpenAI schema generation
        schema = registered_func._robutler_tool_definition
    ...
    async def test_register_dynamic_tool_exception(self):
        ...
        skill.logger.error.assert_called()
        assert "error-server_error_tool" in skill.dynamic_tools
...
class TestMCPToolExecution:
    ...
    async def test_execute_mcp_tool_complex_result(self):
        ...
        assert "❌ MCP tool execution error: can only concatenate str" in result
...
class TestMCPSkillTools:
    ...
    async def test_show_mcp_history_empty(self):
        ...
        assert "📝 No MCP operations recorded" in result
