...
async def initialized_mcp_skill(mock_agent, mock_logger):
    ...
    with patch('robutler.utils.logging.get_logger', return_value=mock_logger):
...
def mock_mcp_sdk_available():
    ...
    class MockMCPSDK:
        ...
        def __enter__(self):
            self.patcher = patch('robutler.agents.skills.core.mcp.skill.MCP_AVAILABLE', self.available)
