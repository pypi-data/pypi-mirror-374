...
Tests PaymentSkill with real Robutler Portal API and LiteLLM API integration.
Uses fixtures to procure payment tokens via Robutler API.
...
@pytest.fixture
async def test_payment_tokens(robutler_client):
    """Procure test payment tokens via Robutler API"""
    try:
...
@pytest.fixture
def payment_integration_config(robutler_api_key, robutler_api_url):
...
class TestPaymentSkillPlatformIntegration:
    """Test PaymentSkill integration with real Robutler Platform"""
    
    @pytest.mark.asyncio
    async def test_client_connection(self, payment_skill_integration, robutler_api_url, robutler_api_key):
        ...
        try:
            ...
            if health.success:
                print(f"✅ Connected to Robutler Platform: {skill.robutler_api_url}")
            else:
...
@pytest.mark.asyncio
async def test_payment_integration_configuration(robutler_api_key, robutler_api_url):
