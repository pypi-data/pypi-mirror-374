...
Tests DiscoverySkill with real Robutler Portal API integration.
Uses fixtures to procure test agents and data via Robutler API.
...
from robutler.agents.skills.robutler.discovery import (
    DiscoverySkill,
    AgentSearchResult,
    IntentRegistration,
    SearchMode
...
def robutler_api_key():
    """Get Robutler API key from environment"""
...
@pytest.fixture
def robutler_api_url():
    """Get Robutler API URL from environment"""
...
@pytest.fixture
async def robutler_client(robutler_api_key, robutler_api_url):
    """Create Robutler client for integration testing"""
    client = RobutlerClient(
        api_key=robutler_api_key,
        base_url=robutler_api_url
    )
...
@pytest.fixture
async def test_agents_data(robutler_client):
    """Procure test agents data via Robutler API"""
    try:
...
@pytest.fixture
def discovery_integration_config(robutler_api_key, robutler_api_url):
    ...
    return {
        ...
        'agent_url': f'{robutler_api_url}/test-discovery-agent'
    }
...
class TestDiscoverySkillPlatformConnection:
    """Test DiscoverySkill connection to real Robutler Platform"""
    
    @pytest.mark.asyncio
    async def test_platform_client_connection(self, discovery_skill_integration, robutler_api_url, robutler_api_key):
        """Test connection to real Robutler Platform"""
        skill = discovery_skill_integration
        
        assert skill.client is not None, "Should have initialized Robutler client"
        assert skill.robutler_api_url == robutler_api_url
        assert skill.robutler_api_key == robutler_api_key
        
        print(f"Connected to: {skill.robutler_api_url}")
    ...
    @pytest.mark.asyncio
    async def test_api_key_resolution_integration(self, robutler_api_key, robutler_api_url):
...
class TestAgentSearchIntegration:
    """Test agent search with real Robutler Platform API"""
...
class TestDiscoveryErrorHandlingIntegration:
    ...
    async def test_platform_unavailable_handling(self):
        ...
        skill = DiscoverySkill({
            'robutler_api_url': 'http://invalid-platform-url:9999',
...
@pytest.mark.asyncio
async def test_discovery_integration_configuration(robutler_api_key, robutler_api_url):
    """Test DiscoverySkill configuration with real environment variables"""
    
    print(f"Discovery Integration test configuration:")
    print(f"  ROBUTLER_API_URL: {robutler_api_url}")
