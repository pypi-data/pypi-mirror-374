...
@pytest.fixture
async def discovery_skill_with_client(discovery_config, mock_robutler_client, mock_agent):
    """DiscoverySkill instance with mocked client for testing"""
    with patch('robutler.agents.skills.robutler.discovery.skill.RobutlerClient') as mock_client_class:
        mock_client_class.return_value = mock_robutler_client
        
        # Mock successful health check
        mock_robutler_client.health_check.return_value = MockResponse(success=True)
        ...
        with patch('robutler.utils.logging.get_logger') as mock_logger:
...
class TestDiscoverySkillInitialization:
    ...
    def test_base_url_resolution(self):
        ...
        with patch.dict(os.environ, {'ROBUTLER_API_URL': 'http://env.localhost'}):
            skill = DiscoverySkill({'robutler_api_url': 'http://config.localhost'})
            assert skill.robutler_api_url == 'http://env.localhost'  # Environment wins
        
        # Test with config only
        skill = DiscoverySkill({'robutler_api_url': 'http://config.localhost'})
        assert skill.robutler_api_url == 'http://config.localhost'
        
        # Test with defaults
        skill = DiscoverySkill({})
        assert skill.robutler_api_url == 'http://localhost:3000'  # Default
    
    @pytest.mark.asyncio
    async def test_api_key_resolution_in_initialize(self, mock_robutler_client):
        ...
        with patch('robutler.agents.skills.robutler.discovery.skill.RobutlerClient') as mock_client_class:
            ...
            with patch.dict(os.environ, {'WEBAGENTS_API_KEY': 'env_api_key'}):
                agent_no_key = Mock()
...
class TestDiscoverySkillIntegration:
    ...
    async def test_discovery_tools_available(self, discovery_skill_with_client):
        ...
        assert hasattr(discovery_skill_with_client.search_agents, '_robutler_is_tool')
        assert hasattr(discovery_skill_with_client.discover_agents, '_robutler_is_tool')
        assert hasattr(discovery_skill_with_client.find_similar_agents, '_robutler_is_tool')
        assert hasattr(discovery_skill_with_client.publish_intents, '_robutler_is_tool')
        assert hasattr(discovery_skill_with_client.get_published_intents, '_robutler_is_tool')
