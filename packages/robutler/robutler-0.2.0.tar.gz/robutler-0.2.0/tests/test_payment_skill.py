...
from robutler.agents.skills.robutler.payments import (
    PaymentSkill, 
    PaymentContext, 
...
from robutler.agents.tools.decorators import tool, pricing, PricingInfo
...
def payment_config():
    ...
    return {
        ...
        'robutler_api_url': 'http://test.localhost',
        'robutler_api_key': 'test_api_key'
    }


@pytest.fixture
def payment_skill(payment_config, mock_robutler_client):
    """PaymentSkill instance for testing"""
    with patch('robutler.agents.skills.robutler.payments.skill.RobutlerClient') as mock_client_class:
        mock_client_class.return_value = mock_robutler_client
        
        # Mock health check to succeed
        mock_robutler_client.health_check.return_value = Mock(success=True)
        
        skill = PaymentSkill(payment_config)
        skill.logger = Mock()  # Mock logger to avoid setup issues
        
        # Mock initialize without full agent
        skill.client = mock_robutler_client
...
class TestPaymentSkillInitialization:
    ...
    def test_payment_skill_creation(self, payment_config):
        ...
        assert skill.robutler_api_url == 'http://test.localhost'
...
class TestPaymentTokenValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_payment_token_valid(self, payment_skill, mock_robutler_client):
        """Test successful payment token validation"""
        # Mock successful validation response
        mock_robutler_client.tokens.validate.return_value = True
        
        result = await payment_skill._validate_payment_token('valid_token')
        
        assert result == True
        mock_robutler_client.tokens.validate.assert_called_once_with('valid_token')
    
    @pytest.mark.asyncio
    async def test_validate_payment_token_invalid(self, payment_skill, mock_robutler_client):
        """Test failed payment token validation"""
        # Mock failed validation response
        mock_robutler_client.tokens.validate.return_value = False
        
        result = await payment_skill._validate_payment_token('invalid_token')
        
        assert result == False
        mock_robutler_client.tokens.validate.assert_called_once_with('invalid_token')
    
    @pytest.mark.asyncio
    async def test_validate_payment_token_with_balance_sufficient(self, payment_skill, mock_robutler_client):
        """Test payment token validation with sufficient balance"""
        # Mock successful balance check
        mock_robutler_client.tokens.validate_with_balance.return_value = {
            'valid': True, 
            'balance': 10.0
        }
        ...
        mock_robutler_client.tokens.validate_with_balance.assert_called_once_with('token_with_balance')
    
    @pytest.mark.asyncio
    async def test_validate_payment_token_with_balance_insufficient(self, payment_skill, mock_robutler_client):
        ...
        mock_robutler_client.tokens.validate_with_balance.assert_called_once_with('token_low_balance')
    
    @pytest.mark.asyncio
    async def test_validate_payment_token_with_balance_api_error(self, payment_skill, mock_robutler_client):
        ...
        mock_robutler_client.tokens.validate_with_balance.assert_called_once_with('token_api_error')
...
class TestPaymentContextSetup:
    ...
    @pytest.mark.asyncio
    async def test_setup_payment_context_with_valid_token(self, payment_skill, mock_context_with_payment_token, mock_robutler_client):
        """Test payment context setup with valid token and sufficient balance"""
        # Mock successful validation with sufficient balance
        mock_robutler_client._make_request.return_value = Mock(
            success=True,
    ...
    @pytest.mark.asyncio
    async def test_setup_payment_context_insufficient_balance(self, payment_skill, mock_context_with_payment_token, mock_robutler_client):
        ...
        from robutler.agents.skills.robutler.payments.exceptions import InsufficientBalanceError
    ...
    async def test_setup_payment_context_no_token_billing_enabled(self, payment_skill, mock_context_no_payment_token):
        ...
        from robutler.agents.skills.robutler.payments.exceptions import PaymentTokenRequiredError
...
class TestCostCalculation:
    ...
    async def test_calculate_llm_cost_with_completion_cost(self, payment_skill):
        ...
        with patch('robutler.agents.skills.robutler.payments.skill.completion_cost') as mock_completion_cost:
    ...
    async def test_calculate_llm_cost_fallback_to_cost_per_token(self, payment_skill):
        ...
        with patch('robutler.agents.skills.robutler.payments.skill.completion_cost') as mock_completion_cost, \
             patch('robutler.agents.skills.robutler.payments.skill.cost_per_token') as mock_cost_per_token:
    ...
    async def test_calculate_cost_from_tokens(self, payment_skill):
        ...
        with patch('robutler.agents.skills.robutler.payments.skill.cost_per_token') as mock_cost_per_token:
...
class TestPaymentCharging:
    ...
    @pytest.mark.asyncio
    async def test_charge_payment_token_success(self, payment_skill, mock_robutler_client):
        """Test successful payment token charging"""
        # Mock successful redemption
        mock_robutler_client.tokens.redeem.return_value = True
        
        result = await payment_skill._charge_payment_token('test_token', 0.50, 'Test charge')
        
        assert result == True
        mock_robutler_client.tokens.redeem.assert_called_once_with('test_token', 0.5)
    
    @pytest.mark.asyncio
    async def test_charge_payment_token_failure(self, payment_skill, mock_robutler_client):
        ...
        mock_robutler_client.tokens.redeem.side_effect = RobutlerAPIError("Failed to redeem token", 400, {})
        
        # Should raise PaymentChargingError
        from robutler.agents.skills.robutler.payments.exceptions import PaymentChargingError
        with pytest.raises(PaymentChargingError):
            await payment_skill._charge_payment_token('test_token', 0.50, 'Test charge')
    
    @pytest.mark.asyncio
    async def test_finalize_payment_with_token_charging(self, payment_skill, mock_robutler_client):
        ...
        mock_robutler_client.tokens.redeem.assert_called_once_with(
...
class TestPaymentSkillIntegration:
    ...
    @pytest.fixture
    def agent_with_payment_skill(self, payment_config, mock_robutler_client):
...
class TestStreamingVsNonStreamingUsageTracking:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_payment_flow_non_streaming(self, payment_skill, mock_robutler_client):
        ...
        mock_robutler_client._make_request.side_effect = [
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_payment_flow_streaming(self, payment_skill, mock_robutler_client):
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_402_error_insufficient_balance(self, payment_skill, mock_context_with_payment_token, mock_robutler_client):
    ...
    @pytest.mark.asyncio
    async def test_402_error_invalid_payment_token(self, payment_skill, mock_context_with_payment_token, mock_robutler_client):
    ...
    @pytest.mark.asyncio
    async def test_payment_charging_failure_handling(self, payment_skill, mock_robutler_client):
...
class TestPricingDecoratorIntegration:
    ...
    @pytest.fixture
    def payment_skill_with_pricing_tools(self, payment_config, mock_robutler_client):
    ...
    async def test_find_tool_function_success(self, payment_skill_with_pricing_tools):
        ...
        weather_pricing = getattr(weather_tool, '_robutler_pricing', None)
        analyze_pricing = getattr(analyze_tool, '_robutler_pricing', None)
        free_pricing = getattr(free_tool, '_robutler_pricing', None)
