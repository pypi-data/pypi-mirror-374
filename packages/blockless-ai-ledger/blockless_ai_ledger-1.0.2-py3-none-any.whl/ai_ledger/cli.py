...
def join(
    ...
    port: int = typer.Option(8001, "--port", help="Port to run on (auto-selects if busy)"),
    ...
    # Auto-select available port if the requested one is busy
    original_port = port
    port = find_available_port(port)
    
    if port != original_port:
        print_warning(f"Port {original_port} busy - using port {port} instead")
