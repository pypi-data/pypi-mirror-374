...
class ClaudeWrapperV3:
    def __init__(
        ...
        self.base_url = base_url or os.environ.get(
            "VICOA_BASE_URL", "https://agent-dashboard-mcp.onrender.com"
        )
                            ...
                            if not hasattr(self, "_permission_handled"):
                                ...
                                with self.send_message_lock:
                                    ...
                                    if options:
                                        ...
                                        self.log(
                                            f"[INFO] Permission prompt with {len(options)} options sent to Vicoa"
                                        )
                                    else:
...
def main():
    ...
    # Check if --continue or --resume in claude_args and bypass Vicoa
    if any(arg in ["--continue", "-c", "--resume", "-r"] for arg in claude_args):
        print(
            "\n⚠️  Warning: --continue and --resume flags are not yet fully supported by Vicoa.",
            file=sys.stderr,
        )
        print(
            "   The flags will be passed to Claude Code, but conversation history may not appear in the Vicoa dashboard.\n",
            file=sys.stderr,
        )
        try:
            claude_path = find_claude_cli()
