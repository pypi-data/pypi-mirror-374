...
def sparta_5e3b1a61d9():return str(uuid.uuid4())
def sparta_fc8e48493b(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_53e16f11da(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
...
def sparta_ab3e8c2986(json_data,userObj):
	A=json_data;logger.debug('CREATE RESOURCE');logger.debug(A);C=A[_B];D=sparta_53e16f11da(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_0f6223a0af(B)
	...
	return{_A:1}
def sparta_3e5942b27b(json_data,userObj):
	...
	return{_A:1}
def sparta_dbbcfaeed4(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_53e16f11da(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_e1629eaf4a(json_data,userObj):A=json_data[_B];B=qube_5a0182b6f5.get_folder_and_files_recursively_from_path(A);C={_A:1,'folderStructure':B};return C
def sparta_5934e69cde(json_data,userObj):
	A=json_data
	try:
...
def sparta_1ffd0c7c22(json_data,userObj):
	...
	return{_A:1}
def sparta_a28c498592(json_data,userObj):
	...
	return{_A:1}
def sparta_fe4aecf85c(json_data,userObj):
...
def sparta_13f86badf1(json_data,userObj):
	...
	return{_A:-1}
def sparta_27d69b5c57(json_data,userObj):
