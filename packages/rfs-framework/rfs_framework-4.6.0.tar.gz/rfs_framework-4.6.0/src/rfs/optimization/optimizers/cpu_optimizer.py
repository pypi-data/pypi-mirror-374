...
class TaskProfile:
    ...
    def get_stats(self) -> Dict[str, Any]:
        """통계 반환"""
        return {'name': self.name, 'task_type': self.task_type.value, 'avg_duration': sum(self.execution_times) / len(self.execution_times) if self.execution_times else 0, 'avg_cpu_usage': sum(self.cpu_usage) / len(self.cpu_usage) if self.cpu_usage else 0, 'success_rate': self.success_count / max(1, self.success_count + self.failure_count), 'total_executions': self.success_count + self.failure_count}
...
class ThreadPoolOptimizer:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {'max_workers': self.max_workers, 'active_tasks': self.active_tasks, 'completed_tasks': self.completed_tasks, 'failed_tasks': self.failed_tasks, 'avg_task_duration': avg_duration, 'success_rate': self.completed_tasks / max(1, self.completed_tasks + self.failed_tasks)}
...
class AsyncOptimizer:
    ...
    async def execute_with_optimization(self, coro: Coroutine[Any, Any, T], name: str=None) -> Result[T, str]:
        ...
        async with self.semaphore:
            ...
            try:
                ...
                self.task_registry = {**self.task_registry, task_name: weakref.ref(task)}
                result = await task
                duration = time.time() - start_time
                completed_tasks = completed_tasks + 1
                self.task_times = self.task_times + [duration]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {'active_tasks': active_tasks, 'completed_tasks': self.completed_tasks, 'failed_tasks': self.failed_tasks, 'avg_task_duration': avg_duration, 'success_rate': self.completed_tasks / max(1, self.completed_tasks + self.failed_tasks), 'semaphore_available': self.semaphore._value}
...
class ConcurrencyTuner:
    ...
    def __init__(self, config: CPUOptimizationConfig):
        ...
        self.current_settings = {'thread_workers': config.concurrency_level.value * 4, 'process_workers': config.concurrency_level.value * 2, 'async_semaphore': config.concurrency_level.value * 10}

    def analyze_performance(self, thread_stats: Dict, process_stats: Dict, async_stats: Dict) -> Dict[str, Any]:
        """성능 분석"""
        analysis = {'thread_efficiency': self._calculate_efficiency(thread_stats), 'process_efficiency': self._calculate_efficiency(process_stats), 'async_efficiency': self._calculate_efficiency(async_stats), 'cpu_utilization': psutil.cpu_percent(interval=1), 'memory_usage': psutil.virtual_memory().percent}
    ...
    def recommend_tuning(self) -> Dict[str, int]:
        ...
        if avg_cpu < 60:
            recommendations['thread_workers'] = {'thread_workers': min(32, self.current_settings['thread_workers'] + 2)}
            recommendations['async_semaphore'] = {'async_semaphore': min(200, self.current_settings['async_semaphore'] + 10)}
        elif avg_cpu > 85:
            recommendations['thread_workers'] = {'thread_workers': max(2, self.current_settings['thread_workers'] - 2)}
            recommendations['process_workers'] = {'process_workers': max(1, self.current_settings['process_workers'] - 1)}
...
class CPUOptimizer:
    ...
    async def _collect_cpu_stats(self) -> Result[CPUStats, str]:
        ...
        try:
            ...
            task_queue_size = thread_stats['active_tasks'] + process_stats.get('active_tasks') + async_stats.get('active_tasks')
            completed_tasks = thread_stats['completed_tasks'] + process_stats.get('completed_tasks') + async_stats.get('completed_tasks')
            failed_tasks = thread_stats['failed_tasks'] + process_stats.get('failed_tasks') + async_stats.get('failed_tasks')
            all_durations = []
            ...
            optimization_score = self._calculate_optimization_score(cpu_percent, task_queue_size, completed_tasks, failed_tasks)
            stats = CPUStats(usage_percent=cpu_percent, core_count=core_count, active_threads=threading.active_count(), active_processes=len(psutil.pids()), task_queue_size=task_queue_size, completed_tasks=completed_tasks, failed_tasks=failed_tasks, avg_task_duration=avg_task_duration, optimization_score=optimization_score)
            return Success(stats)
        except Exception as e:
            return Failure(f'Failed to collect CPU stats: {e}')

    def _calculate_optimization_score(self, cpu_percent: float, queue_size: int, completed: int, failed: int) -> float:
        """최적화 점수 계산 (0-100)"""
        score = 100.0
    ...
    def _update_task_profile(self, task_name: str, duration: float, cpu_usage: float, memory_usage: float, success: bool) -> None:
        """작업 프로파일 업데이트"""
        if task_name not in self.task_profiles:
            self.task_profiles = {**self.task_profiles, task_name: TaskProfile(task_name)}
        profile = self.task_profiles[task_name]
