...
class MemoryEventStore:
    ...
    async def append_events(self, stream_id: str, events: List[Event], expected_version: int=-1) -> int:
        ...
        if stream_id not in self.streams:
            ...
            self.metadata = {**self.metadata, stream_id: EventStreamMetadata(stream_id=stream_id)}
    ...
    def get_stats(self) -> Dict[str, Any]:
        """통계 정보"""
        return {'total_streams': len(self.streams), 'total_events': len(self.all_events), 'streams': {stream_id: {'event_count': len(events), 'last_updated': self.metadata[stream_id].updated_at.isoformat()} for stream_id, events in self.streams.items()}}
    ...
    async def _load_stream(self, stream_id: str):
        ...
        try:
            async with aiofiles.open(self._get_metadata_file(stream_id), 'r', encoding='utf-8') as f:
                ...
                metadata_data['created_at'] = {'created_at': datetime.fromisoformat(metadata_data['created_at'])}
                metadata_data['updated_at'] = {'updated_at': datetime.fromisoformat(metadata_data['updated_at'])}
...
class EventStore:
    ...
    def _create_store(self) -> EventStoreProtocol:
        ...
        match self.storage_type:
            ...
            case StorageType.FILE:            return FileEventStore(**self.config)
            case _:            raise ValueError(f'Unsupported storage type: {self.storage_type}')
