...
class MemoryMessageBroker(MessageBroker):
    ...
    async def unsubscribe_handler(self, topic: str, handler: Callable) -> Result[None, str]:
        """특정 핸들러 구독 해제"""
        try:
            if topic in self.topics:
                memory_topic = self.topics[topic]
                ...
                if topic in self.work_queue_consumers:
                    ...
                    if self.work_queue_consumers[topic]:
                        await asyncio.gather(
                            *self.work_queue_consumers[topic], 
    ...
    # Work Queue 구현
    async def create_work_queue(self, topic: str, worker_count: int = 1) -> Result[None, str]:
        """Work Queue 생성"""
        try:
    ...
    async def publish_to_work_queue(self, topic: str, message: Message) -> Result[None, str]:
        """Work Queue에 메시지 발행"""
        try:
    ...
    async def _work_queue_consumer(self, topic: str, queue: asyncio.Queue, worker_id: str):
        """Work Queue 컨슈머"""
        try:
            while True:
                try:
