...
class TaskResult:
    ...
    @classmethod
    def create_success(cls, task_id: str, task_name: str, task_type: TaskType, result_data: Any, start_time: datetime, end_time: datetime, context: Optional[TaskContext]=None, **kwargs) -> 'TaskResult':
        """성공 결과 생성"""
        return cls(task_id=task_id, task_name=task_name, task_type=task_type, status=ResultStatus.SUCCESS, start_time=start_time, end_time=end_time, result_data=result_data, context=context, **kwargs)

    @classmethod
    def create_failure(cls, task_id: str, task_name: str, task_type: TaskType, error_message: str, start_time: datetime, end_time: datetime, error_type: Optional[str]=None, error_traceback: Optional[str]=None, context: Optional[TaskContext]=None, **kwargs) -> 'TaskResult':
        """실패 결과 생성"""
        return cls(task_id=task_id, task_name=task_name, task_type=task_type, status=ResultStatus.FAILURE, start_time=start_time, end_time=end_time, error_message=error_message, error_type=error_type, error_traceback=error_traceback, context=context, **kwargs)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.context:
            ...
            data['context'] = {**data['context'], 'task_type': self.context.task_type.value}
            data['context'] = {**data['context'], 'execution_time': self.context.execution_time.isoformat()}
        return data

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'TaskResult':
        """딕셔너리에서 복원"""
        if 'start_time' in data and (hasattr(data['start_time'], '__class__') and data['start_time'].__class__.__name__ == 'str'):
            data['start_time'] = {'start_time': datetime.fromisoformat(data['start_time'])}
        if 'end_time' in data and (hasattr(data['end_time'], '__class__') and data['end_time'].__class__.__name__ == 'str'):
            data['end_time'] = {'end_time': datetime.fromisoformat(data['end_time'])}
        if 'status' in data and (hasattr(data['status'], '__class__') and data['status'].__class__.__name__ == 'str'):
            data['status'] = {'status': ResultStatus(data['status'])}
        if 'task_type' in data and (hasattr(data['task_type'], '__class__') and data['task_type'].__class__.__name__ == 'str'):
            data['task_type'] = {'task_type': TaskType(data['task_type'])}
        if 'context' in data and data['context']:
            ctx_data = data['context']
            if 'task_type' in ctx_data and (hasattr(ctx_data['task_type'], '__class__') and ctx_data['task_type'].__class__.__name__ == 'str'):
                ctx_data['task_type'] = {'task_type': TaskType(ctx_data['task_type'])}
            if 'execution_time' in ctx_data and (hasattr(ctx_data['execution_time'], '__class__') and ctx_data['execution_time'].__class__.__name__ == 'str'):
                ctx_data['execution_time'] = {'execution_time': datetime.fromisoformat(ctx_data['execution_time'])}
...
class TaskResultStore:
    ...
    def __init__(self, storage_path: Optional[str]=None, max_results: int=10000):
        ...
        self._results_by_status: Dict[ResultStatus, List[str]] = {status: [] for status in ResultStatus}
        self._task_ids_by_time: List[str] = []
        self._stats = {'total_results': 0, 'success_count': 0, 'failure_count': 0, 'average_execution_time_ms': 0.0}
    ...
    def get_results_in_range(self, start_time: datetime, end_time: datetime) -> List[TaskResult]:
        """시간 범위내 결과 조회"""
        results = []
    ...
    async def query_results(self, task_name: Optional[str]=None, status: Optional[ResultStatus]=None, start_time: Optional[datetime]=None, end_time: Optional[datetime]=None, tags: Optional[List[str]]=None, limit: int=100) -> List[TaskResult]:
        """복합 조건 결과 조회"""
    ...
    def _update_statistics(self, result: TaskResult) -> None:
        ...
        if result.status == ResultStatus.SUCCESS:
            self._stats = {**self._stats, 'success_count': self._stats['success_count'] + 1}
            self._stats = {**self._stats, 'success_count': self._stats['success_count'] + 1}
        elif result.is_failure():
            self._stats = {**self._stats, 'failure_count': self._stats['failure_count'] + 1}
            self._stats = {**self._stats, 'failure_count': self._stats['failure_count'] + 1}
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """통계 조회"""
        return {**self._stats, 'active_results': len(self._results), 'unique_tasks': len(self._results_by_name), 'success_rate': self._stats['success_count'] / max(1, self._stats['total_results']) * 100}
...
class ResultAnalyzer:
    ...
    def analyze_task_performance(self, task_name: str, days: int=7) -> Dict[str, Any]:
        ...
        results = [r for r in self.result_store.get_results_by_name(task_name) if start_time <= r.start_time <= end_time]
        if not results:
        ...
        return {'task_name': task_name, 'analysis_period_days': days, 'total_executions': total, 'success_count': successful, 'failure_count': failed, 'success_rate': successful / total * 100 if total > 0 else 0, 'average_execution_time_ms': avg_exec_time, 'min_execution_time_ms': min_exec_time, 'max_execution_time_ms': max_exec_time, 'recent_results': [r.to_dict() for r in results[-10:]]}

    def analyze_failure_patterns(self, days: int=7) -> Dict[str, Any]:
        """실패 패턴 분석"""
        end_time = datetime.now()
        start_time = end_time - timedelta(days=days)
        failed_results = [r for r in self.result_store.get_results_in_range(start_time, end_time) if r.is_failure()]
