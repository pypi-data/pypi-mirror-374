...
class TestTransactionConfig:
    ...
    def test_distributed_transaction_config(self):
        ...
        config = DistributedTransactionConfig(
            saga_id="user_registration",
            timeout_seconds=600,
            compensation_timeout=120
        )
...
class TestDistributedTransactionManager:
    ...
    async def test_rollback_distributed_transaction(self):
        ...
        # 롤백 (보상 트랜잭션)
        result = await self.manager.rollback_transaction(self.context, "Test compensation")
...
class TestTransactionDecorators:
    ...
    def setup_method(self):
        ...
        self.mock_redis_manager.execute_in_transaction.return_value = Success("redis_result")
        
        self.mock_distributed_manager = AsyncMock()
        self.mock_distributed_manager.execute_in_transaction.return_value = Success("saga_result")
        ...
        registry.register_manager(TransactionType.DISTRIBUTED, self.mock_distributed_manager)
...
class TestTransactionalContextManager:
    ...
    async def test_context_manager_success(self):
        ...
        async with transactional_context(TransactionType.DATABASE, config) as tx_context:
...
class TestPerformanceAndConcurrency:
    ...
    async def test_concurrent_transactions(self):
        ...
        mock_manager.execute_in_transaction.side_effect = lambda ctx, func, *args: Success(f"result_{args[0]}")
    ...
    async def test_transaction_decorator_overhead(self):
        ...
        mock_manager = AsyncMock()
        mock_manager.execute_in_transaction.side_effect = lambda ctx, func, *args: Success(func(*args))
        ...
        print(f"Performance: 1000 transactions in {total_time:.3f}s, avg: {avg_time:.6f}s per transaction")
