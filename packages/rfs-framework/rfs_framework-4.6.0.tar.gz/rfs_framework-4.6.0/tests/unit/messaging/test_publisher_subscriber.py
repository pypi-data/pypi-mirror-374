...
class TestPublisherSubscriber:
    ...
    async def test_publisher_batch_publish(self):
        ...
        async with broker_context() as broker:
            ...
            # 배치 메시지 생성
            messages = [
                Message(topic="batch_topic", data=f"batch_{i}")
                for i in range(10)
            ]
    ...
    async def test_publisher_with_priority(self):
        ...
        async with broker_context() as broker:
            ...
            for priority in priorities:
                ...
                await publisher.publish(message.data, topic="priority_topic", priority=priority)
    ...
    async def test_subscriber_backpressure(self):
        ...
        async with broker_context() as broker:
            ...
            print(f"Processed {len(slow_processed)} out of 20 messages with backpressure")