...
from tests.utils.async_helpers import create_memory_broker, broker_context
...
class TestMemoryMessageBroker:
    ...
    async def test_memory_broker_priority_handling(self):
        ...
        async with broker_context() as broker:
            ...
            # 다양한 우선순위의 메시지 발행
            messages = [
                Message(topic="priority_topic", data="low", priority=MessagePriority.LOW),
                Message(topic="priority_topic", data="urgent", priority=MessagePriority.URGENT),
                Message(topic="priority_topic", data="normal", priority=MessagePriority.NORMAL),
                Message(topic="priority_topic", data="high", priority=MessagePriority.HIGH),
            ]
