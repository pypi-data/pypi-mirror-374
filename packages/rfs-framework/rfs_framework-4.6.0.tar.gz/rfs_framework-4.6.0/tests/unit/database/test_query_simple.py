...
import pytest
import pytest
from rfs.database.query import Operator, SortOrder, Filter, Sort, Pagination, Query, QueryBuilder
...
class TestQueryBuilder:
    ...
    async def test_execute_count_query(self, query_builder):
        ...
        with patch.object(query_builder, '_execute_select') as mock_execute_select:
            from rfs.core.result import Success
            mock_execute_select.return_value = Success([TestModelForQuery(), TestModelForQuery()])
...
class TestAdvancedQueryBuilder:
    ...
    def test_left_join_method(self, advanced_query):
        ...
        result = advanced_query.left_join(TestModelForQuery, "user.id = profile.user_id")
    ...
    def test_right_join_method(self, advanced_query):
        ...
        result = advanced_query.right_join(TestModelForQuery, "order.customer_id = customer.id")
    ...
    def test_raw_method(self, advanced_query):
        ...
        with patch('rfs.database.query.logger') as mock_logger:
            ...
            mock_logger.warning.assert_called_once_with("Raw SQL은 ORM별로 구현이 필요합니다")
...
class TestTransactionalQueryBuilder:
    ...
    async def test_execute_with_transaction_manager(self, transaction_query):
        ...
        transaction_query.transaction_manager.transaction.return_value.__aenter__ = AsyncMock()
        transaction_query.transaction_manager.transaction.return_value.__aexit__ = AsyncMock()
        
        # Mock parent execute method
        from unittest.mock import patch
        
        with patch.object(transaction_query.__class__.__bases__[0], 'execute') as mock_execute:
            from rfs.core.result import Success
            ...
            transaction_query.transaction_manager.transaction.return_value = MockContextManager()
    ...
    async def test_execute_batch_with_failure(self, transaction_query):
        ...
        transaction_query.transaction_manager.transaction.return_value = MockTransaction()
