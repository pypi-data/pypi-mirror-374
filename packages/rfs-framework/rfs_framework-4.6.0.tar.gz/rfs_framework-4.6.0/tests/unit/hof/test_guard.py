...
from rfs.hof.guard import (
    Guard, GuardError, guard, guard_let, guarded,
    guard_type, guard_range, guard_not_empty,
    GuardContext, with_guards, _GuardReturn
)
...
class TestGuardedDecorator:
    ...
    def test_guarded_with_conditions(self):
        ...
        @guarded(
            lambda: is_valid[0],
            lambda: has_permission[0],
            else_return="Access denied"
        )
    ...
    def test_guarded_with_exception(self):
        ...
        @guarded(
            lambda: False,
            else_raise=PermissionError("Not allowed")
        )
        def restricted():
