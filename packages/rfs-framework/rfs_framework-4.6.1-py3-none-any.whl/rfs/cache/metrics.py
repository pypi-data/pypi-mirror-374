...
class CacheMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """딕셔너리로 변환"""
        return {'hits': self.hits, 'misses': self.misses, 'sets': self.sets, 'deletes': self.deletes, 'errors': self.errors, 'total_operations': self.total_operations, 'hit_rate': round(self.hit_rate, 4), 'miss_rate': round(self.miss_rate, 4), 'error_rate': round(self.error_rate, 4), 'avg_get_time': round(self.avg_get_time, 4), 'avg_set_time': round(self.avg_set_time, 4), 'max_get_time': round(self.max_get_time, 4), 'max_set_time': round(self.max_set_time, 4), 'memory_usage': self.memory_usage, 'memory_limit': self.memory_limit, 'connections_active': self.connections_active, 'connections_idle': self.connections_idle, 'hourly_stats': {'hits': self.hourly_hits, 'misses': self.hourly_misses}, 'key_analysis': {'patterns': self.key_patterns, 'hot_keys': self.hot_keys[:10]}}
...
class MetricsCollector(metaclass=SingletonMeta):
    ...
    def __init__(self):
        ...
        self.alert_thresholds = {'hit_rate_min': 0.8, 'error_rate_max': 0.01, 'response_time_max': 1.0}
    ...
    async def collect_cache_metrics(self, cache_name: str, cache_backend: CacheBackend) -> CacheMetrics:
        ...
        try:
            ...
            metrics = CacheMetrics(hits=stats.get('hits', 0), misses=stats.get('misses', 0), sets=stats.get('sets', 0), deletes=stats.get('deletes', 0), errors=stats.get('errors', 0))
            ...
            metrics.hourly_hits = {**metrics.hourly_hits, current_hour: stats.get('hits', 0)}
            metrics.hourly_misses = {**metrics.hourly_misses, current_hour: stats.get('misses', 0)}
    ...
    def _get_hot_keys(self, cache_name: str, limit: int=10) -> List[str]:
        """Hot keys 추출"""
        cache_keys = {k: v for k, v in self.key_access_count.items() if k.startswith(f'{cache_name}:')}
    ...
    async def _check_alerts(self, cache_name: str, metrics: CacheMetrics):
        ...
        if metrics.hit_rate < self.alert_thresholds['hit_rate_min']:
            alerts = alerts + [{'type': 'low_hit_rate', 'cache': cache_name, 'value': metrics.hit_rate, 'threshold': self.alert_thresholds['hit_rate_min'], 'message': f'캐시 히트율이 낮습니다: {metrics.hit_rate:.2%}'}]
        if metrics.error_rate > self.alert_thresholds['error_rate_max']:
            alerts = alerts + [{'type': 'high_error_rate', 'cache': cache_name, 'value': metrics.error_rate, 'threshold': self.alert_thresholds['error_rate_max'], 'message': f'캐시 에러율이 높습니다: {metrics.error_rate:.2%}'}]
        ...
        if max_response_time > self.alert_thresholds['response_time_max']:
            alerts = alerts + [{'type': 'high_response_time', 'cache': cache_name, 'value': max_response_time, 'threshold': self.alert_thresholds['response_time_max'], 'message': f'캐시 응답시간이 느립니다: {max_response_time:.3f}s'}]
        for alert in alerts:
    ...
    def generate_report(self, cache_name: str=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if cache_name:
                ...
                return {'cache_name': cache_name, 'metrics': metrics.to_dict(), 'history': self.get_metrics_history(cache_name, 1)[-10:], 'hot_keys': metrics.hot_keys}
            else:
...
def reset_cache_metrics(cache_name: str=None) -> Result[None, str]:
    ...
    try:
        ...
        collector = get_metrics_collector()
        if cache_name:
            metrics_history = {k: v for k, v in metrics_history.items() if k != 'cache_name, None'}
            operation_times = {k: v for k, v in operation_times.items() if k != 'cache_name, None'}
            keys_to_remove = [k for k in collector.key_access_count.keys() if k.startswith(f'{cache_name}:')]
