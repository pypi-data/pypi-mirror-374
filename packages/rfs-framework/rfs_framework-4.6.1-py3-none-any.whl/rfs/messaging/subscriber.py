...
class Subscriber:
        ...
        try:
            ...
            if topic in self._processing_tasks:
                ...
                if self._processing_tasks[topic]:
                    await asyncio.gather(*self._processing_tasks[topic], return_exceptions=True)
    ...
    async def _process_message(self, topic: str, message: Message, config: SubscriptionConfig):
        """메시지 처리"""
        # 동시 처리 제한
    ...
    async def _handle_message(self, topic: str, message: Message, config: SubscriptionConfig):
        ...
        try:
            ...
            if not before_result.is_success():
                ...
                await self._handle_processing_error(message, Exception(before_result.unwrap_err()), config)
                return
            
            # 메시지 처리
            try:
                if timeout:
                    result = await asyncio.wait_for(handler.handle(message), timeout=timeout)
                else:
                ...
                await self._handle_processing_error(message, asyncio.TimeoutError("처리 시간 초과"), config)
        ...
        try:
            ...
            if should_stop_retry or not message.should_retry:
                ...
                if config.enable_dlq:
                    ...
                    dlq_topic = config.dlq_topic or broker._make_dlq_topic(message.topic)
                    dlq_message = Message(
                        topic=dlq_topic,
                        data=message.data,
                        headers={
...
class WorkQueueSubscriber(Subscriber):
    ...
    def __init__(self, broker_name: str = None, config: SubscriptionConfig = None, worker_count: int = 4):
        super().__init__(broker_name, config)
