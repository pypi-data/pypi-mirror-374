...
class QueryAnalyzer:
    ...
    def _initialize_performance_rules(self) -> List[Dict[str, Any]]:
        """성능 규칙 초기화"""
        rules = [{'name': 'avoid_select_star', 'pattern': 'select_star', 'severity': 'medium', 'suggestion': 'Avoid SELECT * - specify only needed columns'}, {'name': 'missing_where_clause', 'pattern': 'no_where', 'severity': 'high', 'suggestion': 'UPDATE/DELETE without WHERE clause affects all rows'}, {'name': 'cartesian_product', 'pattern': 'cartesian_join', 'severity': 'critical', 'suggestion': 'Potential cartesian product - add JOIN conditions'}, {'name': 'function_on_indexed_column', 'pattern': 'function_in_where', 'severity': 'medium', 'suggestion': 'Functions on indexed columns prevent index usage'}, {'name': 'leading_wildcard_like', 'pattern': 'like_leading_wildcard', 'severity': 'medium', 'suggestion': 'LIKE with leading wildcard prevents index usage'}, {'name': 'or_condition_performance', 'pattern': 'or_condition', 'severity': 'low', 'suggestion': 'OR conditions may prevent index usage - consider UNION'}]
        return rules
    ...
    def _detect_performance_issues(self, query: str) -> List[Dict[str, Any]]:
        ...
        for rule in self.performance_rules:
            ...
            if pattern and pattern.search(query):
                issues = issues + [{'name': rule.get('name'), 'severity': rule.get('severity'), 'suggestion': rule.get('suggestion')}]
        return issues
    ...
    def _recommend_indexes(self, query: str) -> List[str]:
        ...
        if order_match:
            ...
            for col in order_columns:
                recommendations = recommendations + [f'Consider index for ORDER BY: {col.lower()}']
...
class ConnectionPoolOptimizer:
    ...
    def __init__(self, config: DatabaseOptimizationConfig):
        self.config = config
        self.pool_stats = ConnectionPoolStats(total_connections=0, active_connections=0, idle_connections=0, waiting_connections=0, total_acquisitions=0, avg_acquisition_time=0.0, timeout_count=0, error_count=0, pool_efficiency=0.0)
    ...
    def analyze_pool_performance(self) -> Dict[str, Any]:
        """풀 성능 분석"""
        analysis = {'current_stats': self.pool_stats, 'recommendations': [], 'optimal_pool_size': self._calculate_optimal_pool_size(), 'performance_score': self._calculate_performance_score()}
        if self.pool_stats.avg_acquisition_time > 0.1:
            analysis['recommendations'] = analysis.get('recommendations') + ['Connection acquisition time is high - consider increasing pool size']
        if self.pool_stats.pool_efficiency < 0.3:
            analysis['recommendations'] = analysis.get('recommendations') + ['Low pool efficiency - consider reducing pool size']
        elif self.pool_stats.pool_efficiency > 0.9:
            analysis['recommendations'] = analysis.get('recommendations') + ['High pool utilization - consider increasing pool size']
        if self.pool_stats.timeout_count > 0:
            analysis['recommendations'] = analysis.get('recommendations') + ['Connection timeouts detected - increase pool size or timeout duration']
        return analysis
...
class IndexOptimizer:
    ...
    def __init__(self, database_type: DatabaseType):
        self.database_type = database_type
        self.index_usage_stats: Dict[str, Dict[str, Any]] = defaultdict(lambda: {'scan_count': 0, 'lookup_count': 0, 'update_count': 0, 'size_mb': 0, 'last_used': None, 'efficiency_score': 0.0})
    ...
    def record_index_usage(self, index_name: str, usage_type: str, scan_cost: float=0) -> None:
        ...
        match usage_type:
            ...
            case "lookup":            stats['lookup_count'] = stats['lookup_count'] + 1
            case "update":            stats['update_count'] = stats['update_count'] + 1
        ...
        if total_usage > 0:
            ...
            stats['efficiency_score'] = {'efficiency_score': lookup_ratio * 100 + scan_ratio * 20}
    ...
    def _calculate_space_savings(self, unused_indexes: List[Dict]) -> Dict[str, float]:
        ...
        for index_info in unused_indexes:
            ...
            if index_name in self.index_usage_stats:
                total_size = total_size + self.index_usage_stats[index_name].get('size_mb', 0)
        return {'total_mb': total_size, 'estimated_performance_gain': min(total_size * 0.1, 20)}
    ...
    def generate_index_recommendations(self, query_stats: Dict[str, QueryStats]) -> List[str]:
        ...
        for stats in query_stats.values():
            if stats.avg_duration > 1.0 and stats.execution_count > 10:
                recommendations = recommendations + [f'Consider indexing for frequently slow query: {stats.query_hash[:8]}']
        return recommendations
...
class QueryOptimizer:
    ...
    def record_query_execution(self, query: str, duration: float, affected_rows: int=0, error: Optional[str]=None) -> None:
        """쿼리 실행 기록"""
        query_hash = self.query_analyzer.generate_query_hash(query)
        query_type = self.query_analyzer._detect_query_type(query)
        if query_hash not in self.query_stats:
            self.query_stats = {**self.query_stats, query_hash: QueryStats(query_hash=query_hash, query_type=query_type, execution_count=0, total_duration=0.0, avg_duration=0.0, min_duration=float('inf'), max_duration=0.0, affected_rows=0, index_usage=0.0, cache_hit_rate=0.0, error_count=0, last_execution=datetime.now())}
    ...
    def analyze_slow_queries(self, threshold_seconds: float=1.0) -> List[Dict[str, Any]]:
        ...
        for query_hash, stats in self.query_stats.items():
            if stats.avg_duration > threshold_seconds:
                ...
                slow_queries = slow_queries + [{'query_hash': query_hash, 'stats': stats, 'analysis': analysis.unwrap() if analysis.is_success() else {}, 'priority': self._calculate_optimization_priority(stats)}]
        slow_queries.sort(key=lambda x: (x.get('priority').value, -x.get('stats').execution_count * x.get('stats').avg_duration))
        return slow_queries
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {'cache_size': len(self.query_cache), 'cache_hits': self.cache_hits, 'cache_misses': self.cache_misses, 'hit_rate': hit_rate, 'total_queries_tracked': len(self.query_stats)}
...
class DatabaseOptimizer:
    ...
    async def _monitoring_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                self.performance_history = self.performance_history + [{'timestamp': datetime.now(), 'data': performance_data}]
    ...
    async def _collect_performance_data(self) -> Dict[str, Any]:
        """성능 데이터 수집"""
        return {'query_stats': dict(self.query_optimizer.query_stats), 'connection_stats': self.connection_pool_optimizer.pool_stats, 'cache_stats': self.query_optimizer.get_cache_stats(), 'index_stats': dict(self.index_optimizer.index_usage_stats)}

    async def _auto_optimize(self, performance_data: Dict[str, Any]) -> None:
        """자동 최적화 (주의: 프로덕션에서는 신중하게 사용)"""
        slow_queries = self.query_optimizer.analyze_slow_queries(self.config.thresholds.slow_query_seconds)
        if slow_queries:
            critical_queries = [q for q in slow_queries if q.get('priority') == OptimizationPriority.CRITICAL]
            if critical_queries:
        ...
        if pool_analysis.get('performance_score') < 50:
            print(f'WARNING: Connection pool performance score: {pool_analysis.get('performance_score')}')
    ...
    async def analyze_query_performance(self, query: str) -> Result[Dict[str, Any], str]:
        ...
        try:
            ...
            if query_hash in self.query_optimizer.query_stats:
                analysis['historical_stats'] = {'historical_stats': self.query_optimizer.query_stats[query_hash]}
            return Success(analysis)
        except Exception as e:
    ...
    def _generate_optimization_recommendations(self, slow_queries: List, connection_analysis: Dict, index_analysis: Dict, cache_stats: Dict) -> List[str]:
        """최적화 추천사항 생성"""
        recommendations = []
        critical_queries = [q for q in slow_queries if q['priority'] == OptimizationPriority.CRITICAL]
        if critical_queries:
            recommendations = recommendations + [f'URGENT: Optimize {len(critical_queries)} critical slow queries']
        high_priority_queries = [q for q in slow_queries if q['priority'] == OptimizationPriority.HIGH]
        if high_priority_queries:
            recommendations = recommendations + [f'Optimize {len(high_priority_queries)} high-priority slow queries']
        recommendations = recommendations + connection_analysis.get('recommendations', [])
        if index_analysis.get('unused_indexes'):
            recommendations = recommendations + [f'Consider dropping {len(index_analysis.get('unused_indexes'))} unused indexes']
        if index_analysis.get('inefficient_indexes'):
            recommendations = recommendations + [f'Review {len(index_analysis.get('inefficient_indexes'))} inefficient indexes']
        if cache_stats.get('hit_rate') < 0.5:
            recommendations = recommendations + ['Low query cache hit rate - consider cache optimization']
        return recommendations

    def _calculate_overall_performance_score(self, slow_queries: List, connection_analysis: Dict, index_analysis: Dict, cache_stats: Dict) -> float:
        """전체 성능 점수 계산 (0-100)"""
        score = 100.0
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {'timestamp': latest.get('timestamp'), 'total_queries_tracked': len(self.query_optimizer.query_stats), 'cache_hit_rate': self.query_optimizer.get_cache_stats()['hit_rate'], 'connection_pool_efficiency': self.connection_pool_optimizer.pool_stats.pool_efficiency, 'total_indexes_tracked': len(self.index_optimizer.index_usage_stats)}
