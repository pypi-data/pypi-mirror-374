...
class TransactionContext:
    ...
    config: Optional[Union[TransactionConfig, RedisTransactionConfig, DistributedTransactionConfig]] = None
...
class TransactionManager(ABC):
    ...
    @abstractmethod
    async def commit_transaction(self, context: TransactionContext) -> Result[None, str]:
        """트랜잭션 커밋"""
        pass
    
    @abstractmethod
    async def rollback_transaction(self, context: TransactionContext, reason: str = None) -> Result[None, str]:
        """트랜잭션 롤백"""
        pass
...
class DatabaseTransactionManager(TransactionManager):
    ...
    async def rollback_transaction(self, context: TransactionContext, reason: str = None) -> Result[None, str]:
        ...
        try:
            ...
            logger.debug(f"Database transaction {context.transaction_id} rolled back: {reason}")
            return Success(None)
    ...
    def _should_rollback(self, exception: Exception, config: Optional[TransactionConfig]) -> bool:
        """롤백 여부 결정"""
        if not config:
            return True
...
class RedisTransactionManager(TransactionManager):
    ...
    async def rollback_transaction(self, context: TransactionContext, reason: str = None) -> Result[None, str]:
        ...
        try:
            ...
            logger.debug(f"Redis transaction {context.transaction_id} rolled back: {reason}")
            return Success(None)
...
class DistributedTransactionManager(TransactionManager):
    ...
    async def rollback_transaction(self, context: TransactionContext, reason: str = None) -> Result[None, str]:
        ...
        try:
            ...
            if result.is_failure():
                logger.error(f"Compensation failed for saga {saga.saga_id}: {result.error}")
            ...
            logger.debug(f"Distributed transaction {context.transaction_id} rolled back: {reason}")
            return Success(None)
...
async def execute_with_retry(func: Callable[..., Any], 
                           context: TransactionContext,
                           *args, **kwargs) -> Result[Any, str]:
    """재시도 로직을 포함한 함수 실행"""
    last_error = None
    ...
    for attempt in range(1, context.config.retry_count + 1):
        ...
        try:
            ...
            if attempt < context.config.retry_count:
                delay = context.config.retry_delay_seconds * (2 ** (attempt - 1))  # Exponential backoff
                logger.warning(f"Transaction attempt {attempt} failed, retrying in {delay}s: {e}")
                await asyncio.sleep(delay)
            else:
                logger.error(f"Transaction failed after {attempt} attempts: {e}")
    
    return Failure(f"Transaction failed after {context.config.retry_count} attempts: {str(last_error)}")
...
if __name__ == "__main__":
    async def test_transaction_system():
        ...
        redis_config = RedisTransactionConfig(
            ttl_seconds=3600,
            pipeline_mode=True
        )
        ...
        redis_context = TransactionContext(
            transaction_type=TransactionType.REDIS,
            config=redis_config
        )
