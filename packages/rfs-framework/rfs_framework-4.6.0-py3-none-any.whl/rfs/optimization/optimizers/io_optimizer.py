    ...
    async def compress(self, data: bytes, method: str=None) -> Result[bytes, str]:
        ...
        try:
            ...
            match method:
                ...
                case CompressionStrategy.ZLIB.value:                compressed = zlib.compress(data)
    ...
    async def decompress(self, data: bytes, method: str) -> Result[bytes, str]:
        ...
        try:
            match method:
                ...
                case CompressionStrategy.ZLIB.value:                decompressed = zlib.decompress(data)
                case _:                decompressed = data
    ...
    def get_compression_stats(self) -> Dict[str, Any]:
        ...
        for method, records in self.compression_stats.items():
            if records:
                ...
                stats[method] = {method: {'operations': len(records), 'avg_compression_ratio': avg_ratio, 'avg_duration': avg_duration, 'total_bytes_saved': total_saved}}
        return stats
...
class NetworkIOOptimizer:
    ...
    async def _get_session(self, base_url: str=None) -> aiohttp.ClientSession:
        ...
        if session_key not in self.session_cache:
            ...
            connector = aiohttp.TCPConnector(limit=self.config.thresholds.max_concurrent_operations, limit_per_host=10, keepalive_timeout=60, enable_cleanup_closed=True)
            self.session_cache = {**self.session_cache, session_key: aiohttp.ClientSession(timeout=timeout, connector=connector)}
        ...
        for operation, records in self.operation_stats.items():
            if records:
                ...
                match operation:
                    ...
                    case "http_post":                    total_size = sum((r['request_size'] + r['response_size'] for r in records))
                    case _:                    total_size = 0
                avg_duration = sum((r['duration'] for r in records)) / total_operations
                avg_speed = sum((r['speed_mb_per_sec'] for r in records)) / total_operations
                stats[f'{operation}_stats'] = {'total_operations': total_operations, 'total_mb': total_size / 1024 / 1024, 'avg_duration': avg_duration, 'avg_speed_mb_per_sec': avg_speed}
...
class IOOptimizer:
    ...
    async def _collect_io_stats(self) -> Result[IOStats, str]:
        ...
        try:
            ...
            network_requests = network_stats.get('http_get_stats', {}).get('total_operations', 0) + network_stats.get('http_post_stats', {}).get('total_operations', 0)
            avg_network_time = (network_stats.get('http_get_stats', {}).get('avg_duration', 0) + network_stats.get('http_post_stats', {}).get('avg_duration', 0)) / 2
            ...
            if disk_cache_stats or network_cache_stats:
                ...
                total_requests = disk_cache_stats.get('hits', 0) + disk_cache_stats.get('misses', 0) + network_cache_stats.get('hits', 0) + network_cache_stats.get('misses', 0)
            ...
            optimization_score = self._calculate_optimization_score(avg_read_time, avg_write_time, avg_network_time, cache_hit_rate)
            stats = IOStats(disk_reads=disk_reads, disk_writes=disk_writes, network_requests=network_requests, bytes_read=bytes_read, bytes_written=bytes_written, avg_read_time=avg_read_time, avg_write_time=avg_write_time, avg_network_time=avg_network_time, cache_hit_rate=cache_hit_rate, compression_ratio=compression_ratio, optimization_score=optimization_score)
            return Success(stats)
        except Exception as e:
            return Failure(f'Failed to collect IO stats: {e}')

    def _calculate_optimization_score(self, avg_read_time: float, avg_write_time: float, avg_network_time: float, cache_hit_rate: float) -> float:
        """최적화 점수 계산 (0-100)"""
        score = 100.0
    ...
    async def optimize(self) -> Result[Dict[str, Any], str]:
        ...
        try:
            ...
            results = {'current_stats': current_stats, 'disk_stats': disk_stats, 'network_stats': network_stats, 'compression_stats': compression_stats, 'recommendations': recommendations}
    ...
    def _generate_recommendations(self, stats: IOStats) -> List[str]:
        ...
        if stats.avg_read_time > 1.0:
            recommendations = recommendations + ['Consider using larger buffer sizes for file reading']
            recommendations = recommendations + ['Enable caching for frequently accessed files']
        if stats.avg_write_time > 2.0:
            recommendations = recommendations + ['Consider batch writing for multiple small files']
            recommendations = recommendations + ['Use asynchronous writing for better performance']
        if stats.avg_network_time > 5.0:
            recommendations = recommendations + ['Consider connection pooling for HTTP requests']
            recommendations = recommendations + ['Enable compression for large data transfers']
        if stats.cache_hit_rate < 0.5:
            recommendations = recommendations + ['Increase cache size or improve cache strategy']
        if stats.compression_ratio > 0.8:
            recommendations = recommendations + ['Consider disabling compression for this data type']
        elif stats.compression_ratio == 0.0 and stats.bytes_written > 1024 * 1024:
            recommendations = recommendations + ['Consider enabling compression for large files']
