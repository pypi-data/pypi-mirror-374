...
class CircuitBreakerConfig:
    ...
    success_codes: List[int] = field(default_factory=lambda: [200, 201, 204])  # 성공 코드
...
class CircuitBreaker:
    ...
    def _record_failure(self, error: Optional[Exception] = None) -> None:
        ...
        with self.lock:
            ...
            # 실패 임계값 확인
            recent_failures = sum(1 for f in self.failure_window if f)
            failure_rate = recent_failures / len(self.failure_window) if self.failure_window else 0
            
            if self.state == CircuitState.CLOSED:
                # 실패 임계값 초과 시 OPEN으로
                if (recent_failures >= self.config.failure_threshold or 
