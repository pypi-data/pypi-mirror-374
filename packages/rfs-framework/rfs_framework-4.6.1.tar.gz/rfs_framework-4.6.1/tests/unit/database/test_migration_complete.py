...
from rfs.database.migration import (
    MigrationStatus, MigrationInfo, Migration, SQLMigration, 
    PythonMigration, MigrationManager
)
...
class TestSQLMigration:
    ...
    def test_sql_migration_with_files(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.sql', delete=False) as up_file:
            up_file.write("CREATE TABLE test (id INT);")
            up_file_path = up_file.name
            
        with tempfile.NamedTemporaryFile(mode='w', suffix='.sql', delete=False) as down_file:
...
""")
    ...
    async def test_run_migrations_with_dependencies(self):
        ...
        with patch.object(manager, 'discover_migrations', return_value=Success([migration1, migration2, migration3])):
