...
class TestMigrationManager:
    ...
    async def test_run_migration_with_error(self, manager):
        ...
        manager._database.execute = AsyncMock(
            return_value=Failure("SQL syntax error")
        )
        manager.add_migration(migration)
    ...
    def test_clear_history(self, manager):
        ...
        manager._history.extend([
            MigrationInfo("001", "first"),
            MigrationInfo("002", "second")
        ])
...
class TestAlembicMigrationManager:
    ...
    @pytest.mark.skipif(not pytest.importorskip("alembic", reason="Alembic not installed"),
