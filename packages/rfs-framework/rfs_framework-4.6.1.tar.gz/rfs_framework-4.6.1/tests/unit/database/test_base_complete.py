...
from rfs.database.base import (
    DatabaseType, ORMType, DatabaseConfig,
    ConnectionPool, Database,
    SQLAlchemyDatabase, TortoiseDatabase,
    DatabaseManager, get_database_manager, get_database,
...
class TestDatabaseConfig:
    ...
    def test_get_tortoise_config(self):
        ...
        assert tortoise_config["connections"]["default"]["engine"] == "tortoise.backends.asyncpg"
        
        # Test MySQL
        config.database_type = DatabaseType.MYSQL
        tortoise_config = config.get_tortoise_config()
        assert tortoise_config["connections"]["default"]["engine"] == "tortoise.backends.mysql"
        
        # Test SQLite
        config.database_type = DatabaseType.SQLITE
        config.url = "sqlite:///test.db"
        tortoise_config = config.get_tortoise_config()
        assert tortoise_config["connections"]["default"]["engine"] == "tortoise.backends.sqlite"
...
class TestConnectionPool:
    ...
    async def test_initialize_sqlalchemy_failure(self, pool):
        ...
        with patch('rfs.database.base.SQLALCHEMY_AVAILABLE', True):
            with patch('rfs.database.base.create_async_engine', side_effect=Exception("Connection failed")):
                result = await pool.initialize()
    ...
    async def test_initialize_exception(self, pool):
        ...
        with patch.object(pool, '_initialize_sqlalchemy', AsyncMock(side_effect=Exception("Unexpected error"))):
            result = await pool.initialize()
...
class TestDatabase:
    ...
    async def test_initialize_success(self, config):
        ...
        with patch.object(db.connection_pool, 'initialize', AsyncMock(return_value=Success(None))):
...
class TestSQLAlchemyDatabase:
    ...
    async def test_create_session(self, db):
        ...
        with patch.object(db.connection_pool, 'get_session_factory', return_value=mock_factory):
...
class TestTortoiseDatabase:
    ...
    async def test_execute_query_success(self, db):
        ...
        with patch('rfs.database.base.connections') as mock_connections:
            ...
            assert result.is_success()
            assert result.unwrap() == [{"id": 1}]
            mock_connection.execute_query.assert_called_once_with("SELECT * FROM test", [{"id": 1}])
...
class TestDatabaseManager:
    ...
    async def test_add_database_sqlalchemy(self, manager):
        ...
        with patch('rfs.database.base.SQLALCHEMY_AVAILABLE', True):
            with patch.object(SQLAlchemyDatabase.prototype, 'initialize', AsyncMock(return_value=Success(None))):
    ...
    async def test_add_database_exception(self, manager):
        ...
        with patch('rfs.database.base.SQLALCHEMY_AVAILABLE', True):
            with patch('rfs.database.base.SQLAlchemyDatabase', side_effect=Exception("Creation failed")):
                result = await manager.add_database("test_db", config)
