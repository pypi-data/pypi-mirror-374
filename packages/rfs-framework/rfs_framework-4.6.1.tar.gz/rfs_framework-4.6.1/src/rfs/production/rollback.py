...
class RollbackManager:
    ...
    def __init__(self, checkpoint_dir: str='/tmp/rfs_checkpoints', max_checkpoints: int=10, auto_rollback_enabled: bool=True):
    ...
    async def create_checkpoint(self, service_name: str, version: str, configuration: Dict[str, Any], environment_variables: Optional[Dict[str, str]]=None, dependencies: Optional[Dict[str, str]]=None, metadata: Optional[Dict[str, Any]]=None) -> Result[DeploymentCheckpoint, str]:
        """체크포인트 생성"""
        try:
            checkpoint_id = f'{service_name}_{version}_{int(time.time())}'
            checkpoint = DeploymentCheckpoint(checkpoint_id=checkpoint_id, service_name=service_name, version=version, timestamp=datetime.now(), configuration=configuration, environment_variables=environment_variables or {}, dependencies=dependencies or {}, metadata=metadata or {})
            ...
            if len(self.checkpoints[service_name]) > self.max_checkpoints:
                old_checkpoint = self.checkpoints[service_name] = {k: v for k, v in checkpoints[service_name].items() if k != 0}
            ...
            await self.event_bus.publish(Event(type='checkpoint.created', data={'checkpoint_id': checkpoint_id, 'service': service_name, 'version': version}))
            return Success(checkpoint)
        except Exception as e:
            logger.error(f'Failed to create checkpoint: {e}')
            return Failure(f'Failed to create checkpoint: {str(e)}')

    async def rollback(self, service_name: str, target_version: Optional[str]=None, trigger: RollbackTrigger=RollbackTrigger.MANUAL, reason: str='Manual rollback requested') -> Result[RollbackRecord, str]:
        """롤백 실행"""
        try:
            ...
            if target_version:
                target_checkpoint = next((cp for cp in reversed(checkpoints) if cp.version == target_version), None)
            ...
            rollback_record = RollbackRecord(rollback_id=rollback_id, service_name=service_name, from_version=current_checkpoint.version, to_version=target_checkpoint.version, trigger=trigger, reason=reason, start_time=datetime.now(), status=RollbackStatus.IN_PROGRESS)
            ...
            await self.event_bus.publish(Event(type='rollback.started', data={'rollback_id': rollback_id, 'service': service_name, 'from_version': current_checkpoint.version, 'to_version': target_checkpoint.version, 'trigger': trigger.value, 'reason': reason}))
            ...
            if type(result).__name__ == 'Success':
                ...
                await self.event_bus.publish(Event(type='rollback.completed', data={'rollback_id': rollback_id, 'duration': str(rollback_record.duration)}))
                ...
                await self.event_bus.publish(Event(type='rollback.failed', data={'rollback_id': rollback_id, 'error': result.error}))
    ...
    async def _execute_rollback(self, checkpoint: DeploymentCheckpoint) -> Result[bool, str]:
        """롤백 실행 로직"""
        try:
    ...
    async def can_rollback(self, service_name: str, target_version: Optional[str]=None) -> Result[bool, str]:
        """롤백 가능 여부 확인"""
        try:
    ...
    def get_rollback_history(self, service_name: Optional[str]=None, limit: int=10) -> List[RollbackRecord]:
        """롤백 히스토리 조회"""
    ...
    def get_checkpoints(self, service_name: str, limit: Optional[int]=None) -> List[DeploymentCheckpoint]:
        """체크포인트 목록 조회"""
        if service_name not in self.checkpoints:
            return []
        checkpoints = self.checkpoints[service_name]
    ...
    async def auto_rollback_check(self, service_name: str, metrics: Dict[str, float], thresholds: Dict[str, float]) -> Result[Optional[RollbackRecord], str]:
        """자동 롤백 체크 및 실행"""
        ...
        try:
            ...
            if trigger:
                ...
                result = await self.rollback(service_name=service_name, trigger=trigger, reason=reason)
                if type(result).__name__ == 'Success':
    ...
    def cleanup_old_checkpoints(self, service_name: Optional[str]=None, older_than_days: int=30) -> int:
        """오래된 체크포인트 정리"""
        ...
        for svc in services:
            if svc not in self.checkpoints:
                continue
            old_checkpoints = [cp for cp in self.checkpoints[svc] if cp.timestamp < cutoff_date]
