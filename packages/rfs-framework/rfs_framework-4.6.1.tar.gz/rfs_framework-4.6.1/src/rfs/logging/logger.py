...
class RFSLogger:
    ...
    def __init__(self, name: str, level: LogLevel=LogLevel.INFO, handlers: Optional[List[logging.Handler]]=None):
        self.name = name
    ...
    def _create_log_record(self, level: int, message: str, fields: Dict[str, Any]) -> logging.LogRecord:
        """로그 레코드 생성"""
        record = self.logger.makeRecord(self.name, level, '(unknown file)', 0, message, (), None)
    ...
    def with_fields(self, **fields) -> 'RFSLogger':
        """필드가 추가된 새 로거"""
        new_logger = RFSLogger(f'{self.name}[{','.join(fields.keys())}]', level=self.logger.level, handlers=self.logger.handlers)
        current = _log_context.get()
    ...
    def get_child(self, suffix: str) -> 'RFSLogger':
        """자식 로거 생성"""
        return RFSLogger(f'{self.name}.{suffix}', level=self.logger.level, handlers=self.logger.handlers)
...
def configure_logging(level: LogLevel=LogLevel.INFO, format: str='structured', handlers: Optional[List[logging.Handler]]=None, **kwargs):
    """
    로깅 설정
