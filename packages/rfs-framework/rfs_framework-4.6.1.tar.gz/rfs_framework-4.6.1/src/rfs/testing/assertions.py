...
def _format_assertion_message(expected: Any, actual: Any, message: Optional[str] = None) -> str:
    """어설션 메시지 포맷팅"""
...
def assert_not_equal(expected: Any, actual: Any, message: Optional[str] = None):
    ...
    if expected == actual:
        raise AssertionError(
            f"Values should not be equal: {expected}" + (f": {message}" if message else "")
        )
...
def assert_in(item: Any, collection: Collection, message: Optional[str] = None):
    ...
    if item not in collection:
        raise AssertionError(
            f"Expected {item} to be in {collection}" + (f": {message}" if message else "")
        )
...
def assert_not_in(item: Any, collection: Collection, message: Optional[str] = None):
    ...
    if item in collection:
        raise AssertionError(
            f"Expected {item} not to be in {collection}" + (f": {message}" if message else "")
        )
...
def assert_length(collection: Collection, expected_length: int, message: Optional[str] = None):
    """컬렉션의 길이 확인"""
...
def assert_result_value(result: Result, expected_value: Any, message: Optional[str] = None):
    """Result의 성공 값 확인"""
...
def assert_result_error(result: Result, expected_error: Any, message: Optional[str] = None):
    """Result의 오류 값 확인"""
...
# 비동기 어설션들
async def assert_eventually(condition: Callable[[], bool], timeout: float = 5.0,
...
async def assert_timeout(coro_or_callable: Union[Callable, Any], timeout: float,
...
# 숫자 어설션들
def assert_greater(value: Union[int, float], threshold: Union[int, float], 
                  message: Optional[str] = None):
    """값이 임계값보다 큰지 확인"""
    if value <= threshold:
        raise AssertionError(
...
def assert_greater_equal(value: Union[int, float], threshold: Union[int, float],
                        message: Optional[str] = None):
    """값이 임계값보다 크거나 같은지 확인"""
    if value < threshold:
        raise AssertionError(
...
def assert_less(value: Union[int, float], threshold: Union[int, float],
               message: Optional[str] = None):
    """값이 임계값보다 작은지 확인"""
    if value >= threshold:
        raise AssertionError(
...
def assert_less_equal(value: Union[int, float], threshold: Union[int, float],
                     message: Optional[str] = None):
    """값이 임계값보다 작거나 같은지 확인"""
    if value > threshold:
        raise AssertionError(
...
def assert_almost_equal(first: float, second: float, places: int = 7,
                       message: Optional[str] = None):
    """부동소수점 근사 비교"""
...
# 커스텀 어설션 생성 헬퍼
def create_assertion(name: str, condition_func: Callable[[Any], bool], 
...
class assert_raises_context:
    ...
    def __init__(self, expected_exception: Type[Exception], message: Optional[str] = None):
