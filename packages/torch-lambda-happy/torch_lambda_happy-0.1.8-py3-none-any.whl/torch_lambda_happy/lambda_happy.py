...
class LambdaHappy:
    ...
    Examples:
        ...
        >>> lambda_ = model.compute_many(m=10_000,
        ...
        >>> matX = torch.randn(1_000, 1_000)
        >>> model = LambdaHappy(X=matX, force_fastest=True, use_multigpu=True)
        >>> lambda_ = model.compute_agg(m=10_000,
    ...
    def _validate_args(self, m: int, device_type: str, dtype: torch.dtype):
    ...
    def _select_impl(
        ...
        if fn is None:
            raise RuntimeError(f"No implementation for ({device_type =}, {dtype =})")
        return fn
    ...
    def compute(
        ...
        self._validate_args(m, device_type, dtype)
        impl = self._select_impl(device_type, dtype)
        return impl(self, m)
