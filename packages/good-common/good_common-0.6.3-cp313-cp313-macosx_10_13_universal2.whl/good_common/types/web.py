...
class UrlParseConfig:
    ...
    embedded_redirect_params: set[str] = field(default_factory=lambda: {"redirect", "redirect_to", "url"})
    canonical_params: set[str] = field(default_factory=lambda: {"id", "q", "v", "chash", "action"})
    force_https: bool = True
    short_url_exception_domains: set[str] = field(default_factory=lambda: {
        "fec.gov",
        "archive.is",
        "archive.org",
        "archive.today",
...
class URL(str):
    ...
    def canonicalize(
        self, config: UrlParseConfig | None = None
    ) -> URL:
        if config is None:
...
def _filter_canonical_params(url: URL, config: UrlParseConfig) -> URL:
    ...
    if domain_rules.get("disable"):
        # logger.debug(f"Canonicalization disabled for domain {_host}")
        return url
    ...
    for key, value in url.query_params(format="plain", flat_delimiter=","):
