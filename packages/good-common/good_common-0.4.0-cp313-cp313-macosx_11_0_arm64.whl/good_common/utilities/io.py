    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
        ...
        for k, v in headers.items():
            if any([x in k for x in ("date", "modifiedtime")]):
                ...
                headers[k] = parsed_date.isoformat() if parsed_date else v
        return headers
...
async def get_file_metadata(
    ...
    try:
        ...
        if "Last-Modified" in head_response.headers:
            file_timestamp = dateparser.parse(head_response.headers["Last-Modified"])
        elif "Date" in head_response.headers:
            file_timestamp = dateparser.parse(head_response.headers["Date"])
...
def should_download_file(
    ...
    if file_timestamp and filepath.exists():
                    ...
                    if response.status_code != 206:  # 206 Partial Content
                        ...
                        if is_cloudfront_response(response):
                            ...
                            if (
                                ...
                                logger.error(
                                    f"CloudFront error content: {error_content}"
                                )
                            raise RangeNotSatisfiableError(
...
def safely_remove_file(filepath: Path, log_prefix: str = ""):
    ...
    if filepath.exists():
        try:
            ...
            logger.warning(f"{log_prefix}Could not remove file {filepath}: {str(e)}")
