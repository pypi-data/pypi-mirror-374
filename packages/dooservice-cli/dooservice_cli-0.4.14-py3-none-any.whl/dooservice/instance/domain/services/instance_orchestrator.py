...
class InstanceOrchestrator:
    ...
    def __init__(
        ...
        # Get message handler from global container
    ...
    def start_instance(self, config: InstanceConfig) -> None:
        ...
        self._msg.info("Starting instance '%s'", name)
        ...
        try:
            ...
            if config.deployment.docker.db:
                ...
                self._msg.debug("Starting database container: %s", db_container)
            ...
            if config.deployment.docker.web:
                ...
                self._msg.debug("Starting web container: %s", web_container)
    ...
    def stop_instance(self, config: InstanceConfig) -> None:
        ...
        self._msg.info("Stopping instance '%s'", instance_name)

        try:
            # Stop web service first, then database
            if config.deployment.docker.web:
                web_container = config.deployment.docker.web.container_name
                self._msg.debug("Stopping web container: %s", web_container)
                self._instance_repository.stop(web_container)

            if config.deployment.docker.db:
                db_container = config.deployment.docker.db.container_name
                self._msg.debug("Stopping database container: %s", db_container)
                self._instance_repository.stop(db_container)

            self._msg.success("Successfully stopped instance '%s'", instance_name)

        except Exception as e:
            self._msg.error("Failed to stop instance '%s': %s", instance_name, e)
            raise
    ...
    def delete_instance(
        ...
        try:
            ...
            try:
                ...
                self._msg.debug("Stop failed, continuing with deletion: %s", stop_error)

            # Delete containers
            if config.deployment.docker.web:
                web_container = config.deployment.docker.web.container_name
                self._msg.debug("Deleting web container: %s", web_container)
                self._instance_repository.delete(web_container)

            if config.deployment.docker.db:
                db_container = config.deployment.docker.db.container_name
                self._msg.debug("Deleting database container: %s", db_container)
            ...
            if remove_data:
                ...
                if data_removed:
                    self._msg.success("Removed data directory: %s", config.data_dir)
                else:
                    self._msg.warning("Data directory not found: %s", config.data_dir)

            self._msg.success("Successfully deleted instance '%s'", instance_name)

        except Exception as e:
            self._msg.error("Failed to delete instance '%s': %s", instance_name, e)
            raise
    ...
    def get_instance_logs(
        ...
        try:
            ...
            if service in ("web", "all") and config.deployment.docker.web:
                web_container = config.deployment.docker.web.container_name
                self._msg.debug("Fetching logs from web container: %s", web_container)
                logs_output.append(
            ...
            if service in ("db", "all") and config.deployment.docker.db:
                db_container = config.deployment.docker.db.container_name
                self._msg.debug("Fetching logs from db container: %s", db_container)
                logs_output.append(
                    self._instance_repository.logs(
    ...
    def exec_instance_command(
        ...
        try:
            if service in ("web", "all") and config.deployment.docker.web:
                web_container = config.deployment.docker.web.container_name
                self._msg.debug("Executing in web container: %s", web_container)
            ...
            if service in ("db", "all") and config.deployment.docker.db:
                db_container = config.deployment.docker.db.container_name
                self._msg.debug("Executing in db container: %s", db_container)
                _, output = self._instance_repository.exec_command(
