...
from sensory_data_client.repositories import (DocBulletsRepository,
                                            MetaDataRepository, 
                                            ImageRepository, 
                                            LineRepository,
                                            MinioRepository,
...
from sensory_data_client.db import DocType, RawLineORM, DocumentLineORM, ImageLineORM, TagORM, DocumentORM, StoredFileORM, UserORM, SubscriptionORM
...
class DataClient:
    ...
    def __init__(
        ...
        bullet_repo: DocBulletsRepository | None = None,
        ...
        DataClient._live.add(self)

    @classmethod
    def live_instances(cls):
        return list(cls._live)
    
    async def check_connections(self, timeout: float = 3.0) -> dict[str, str]:
        """
        Проверяет доступность PostgreSQL / MinIO / Elasticsearch.
        Никогда не пробрасывает исключения — всегда возвращает статусы.
        timeout — таймаут на каждый сервис (сек).
        """
        ...
        async def _safe(name: str, coro):
            ...
            try:
                ...
                statuses[name] = "failed: timeout"
            except (DatabaseError, MinioError, ESError) as e:
                statuses[name] = f"failed: {e}"
            except Exception as e:
                statuses[name] = f"failed: {e.__class__.__name__}: {e}"

        await _safe("postgres", self.metarepo.check_connection() if self.metarepo else None)
        await _safe("minio", self.minio.check_connection() if self.minio else None)
        await _safe("elastic", self.es.check_connection() if self.es else None)
