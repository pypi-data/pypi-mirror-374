...
from sensory_data_client.models.document import DocumentInDB, SummaryIn
...
class MetaDataRepository:
    ...
    async def save_summary(self, doc_id: UUID, payload: Dict[str, Any]) -> None:
        """
        Принимает dict (например, parsed.model_dump()), валидирует/нормализует
        и сохраняет:
          - колонки: language, authors(=autors), title, year, one_liner, extended_summary, conclusions
          - в metadata_.summaries только bibliography
        """
        try:
            clean = SummaryIn.model_validate(payload)
        except Exception as e:
            logger.error("Summary payload validation failed for %s: %s", doc_id, e)
            raise DatabaseError(f"Invalid summary payload: {e}") from e

        async with get_session(self._session_factory) as session:
            try:
                # Текущие метаданные под блокировкой
                cur_meta = await session.scalar(
                    select(DocumentORM.metadata_).where(DocumentORM.id == doc_id).with_for_update()
                )
                new_meta = dict(cur_meta or {})
                new_meta.setdefault("summaries", {})
                if clean.bibliography is not None:
                    new_meta["summaries"]["bibliography"] = clean.bibliography.model_dump()
                ...
                stmt = (
                    ...
                    .values(
                        ...
                        authors=clean.autors or None,        # TEXT[] колонка
                        title=(clean.title or None),
                        year=clean.year,
                        one_liner=(clean.one_liner or None),
                        extended_summary=(clean.extended_summary or None),  # TEXT колонка
                        conclusions=(clean.conclusions or None),            # TEXT колонка
                        metadata_=new_meta,
                        edited=func.now(),
                    )
                )
                ...
                await session.rollback()
                logger.error("save_summary failed for %s: %s", doc_id, e)
                raise DatabaseError(str(e)) from e