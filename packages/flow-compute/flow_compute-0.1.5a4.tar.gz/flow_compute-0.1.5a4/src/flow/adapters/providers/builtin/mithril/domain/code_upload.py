...
class CodeUploadService:
    ...
    def initiate_async_upload(self, task: Task, config: TaskConfig) -> None:
        ...
        try:
            ...
            if str(env_map.get("FLOW_DEV_VM", "")).strip().lower() == "true":
                self._logger.info(
                    "Skipping provider background code upload for dev VM (FLOW_DEV_VM=true)"
                )
                return
        except Exception:
            # Best-effort guard; fall through if env is unavailable
            pass
        manager = CodeTransferManager(provider=self._provider)
        # Determine upload source and destination to match runtime expectations
        try:
        ...
        transfer_cfg = CodeTransferConfig(
            source_dir=Path(code_root_val) if code_root_val else None,
        ...
        try:
            if console is not None:
                if result.bytes_transferred == 0 and result.files_transferred == 0:
                    ...
                    console.print(
                        f"[success]✓[/success] Upload complete: {result.files_transferred} files, {size_mb:.1f} MB → {(task.name or task.task_id)}:{target_dir} @ {result.transfer_rate}"
                    )
        except Exception:
