...
class ReservationsService:
    ...
    def __init__(self, flow: Flow | None = None) -> None:
        ...
        self._flow = flow or create_client(auto_init=True)
    ...
    def slot_duration_hours(cls, slot: dict[str, Any]) -> int:
        ...
        if not start or not end:
            return 0
        try:
            return max(0, int(round((end - start).total_seconds() / 3600)))
        except Exception:
            return 0
    ...
    def aggregate_slots(cls, slots: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for s in slots or []:
            try:
                ...
                et = cls.isoformat_utc_z(
                    s.get("end_time_utc") or s.get("end_time") or s.get("end")
                )
            except Exception:
