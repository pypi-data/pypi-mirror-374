...
class RunProgressManager:
    ...
    def start_timeline(self, params: RunParameters) -> None:
        ...
        self.timeline = StepTimeline(self.console, title="flow run", title_animation="auto")
        self.timeline.start()
    ...
    def _perform_upload(
        ...
        try:
            ...
            workdir = getattr(getattr(task, "config", None), "working_dir", "/workspace") or "/workspace"
            plan = _plan_for_run(source_dir=src, working_dir=workdir)
            target_dir = plan.remote_target
        except Exception:
            target_dir = getattr(getattr(task, "config", None), "working_dir", "/workspace") or "/workspace"
