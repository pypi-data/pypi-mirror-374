...
rsection_app = rsection.Application(port=9000)
...
rsection_app.calculate_all(skip_warnings=True)
rsection_filepath = os.path.join(os.path.abspath('.'), 'steel_section.rsc')
print(rsection_filepath)
rsection_app.save_model(path=rsection_filepath)
rsection_app.close_model(save_changes=False)
rsection_app.close_connection()

rstab_app = rstab.Application(port=9001)
