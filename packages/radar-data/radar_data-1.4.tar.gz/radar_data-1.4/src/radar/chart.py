...
class _ChartLayout:
    ...
    def _update_coordinate_data(self, xx, yy, sweep: dict, **kwargs):
    ...
    def set_data(self, sweep: dict, **kwargs):
        ...
        # Hash check to see if only data needs to be updated
        ...
        if sweep["sweepMode"] == "rhi":
            ...
            xx = np.outer(np.cos(ee), 1.0e-3 * rr)
            yy = np.outer(np.sin(ee), 1.0e-3 * rr)
            ...
            if self.overlay is None:
                ...
                extent = (0, 0, 1.0e-3 * rr[-1], np.degrees(ee[-1]))
                self.overlay = overlay.PolarGrid(extent=extent, ymax=h_max, s=self.s)
                self.overlay.load()
                for ax in self.ax:
                    self.overlay.draw(ax)

        elif sweep["sweepMode"] == "ppi":

            xx = np.outer(np.cos(ee) * np.sin(aa), 1.0e-3 * rr)
            yy = np.outer(np.cos(ee) * np.cos(aa), 1.0e-3 * rr)
            self._update_coordinate_data(xx, yy, sweep, **kwargs)
        ...
        if sweep["time"] is None and "title" not in kwargs:
            return
        sweepTime = datetime.datetime.fromtimestamp(sweep["time"], tz=datetime.timezone.utc)
        self.update_title(kwargs.get("title", sweepTime.strftime(r"%Y/%m/%d %H:%M:%S UTC")))
