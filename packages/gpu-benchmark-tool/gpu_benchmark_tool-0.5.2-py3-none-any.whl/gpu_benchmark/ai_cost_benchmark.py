...
class AICostBenchmark:
    ...
    def setup_monitoring(self):
        ...
        try:
            # Get the appropriate GPU backend and create a monitor
            backend = get_gpu_backend(backend_type="auto", device_id=self.device_id)
            if backend and backend.is_available():
                self.monitor = backend.create_monitor(self.device_id)
                print_success(f"GPU monitoring enabled for device {self.device_id} using {backend.__class__.__name__}")
            else:
                print_warning("No GPU backend available, monitoring disabled")
                self.monitor = None
        except Exception as e:
            print_warning(f"GPU monitoring not available: {e}")
            self.monitor = None
    ...
    def benchmark_resnet_training(self, config: ModelConfig) -> CostMetrics:
        ...
        # Calculate costs (only energy cost, no developer cost)
        energy_cost = training_energy * config.energy_cost_per_kwh
        total_cost = energy_cost  # Only energy cost
    ...
    def benchmark_transformer_inference(self, config: ModelConfig) -> CostMetrics:
        ...
        # For inference-only, training metrics are 0
        energy_cost = inference_energy * config.energy_cost_per_kwh
    ...
    def generate_cost_report(self, results: Dict[str, CostMetrics]) -> str:
        ...
        for model_name, metrics in results.items():
            ...
            report += f"Energy Cost: ${metrics.energy_cost:.2f}\n"
            report += f"Total Cost: ${metrics.total_cost:.2f}\n"
            report += f"Cost per Inference: ${metrics.cost_per_inference:.4f}\n"
            report += "-" * 50 + "\n\n"
