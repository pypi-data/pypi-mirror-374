...
from .ai_cost_benchmark import AICostBenchmark, create_standard_benchmarks, ModelConfig
...
def cmd_ai_cost_benchmark(args):
    ...
    try:
        ...
        if not selected_models:
            print_error("No valid models selected. Available: resnet, transformer")
            return 1
        ...
        if args.export is not None:
            ...
            for model_name, metrics in results.items():
                export_data[model_name] = {
                    ...
                    "training_energy_kwh": metrics.training_energy_kwh,
                    "inference_time_seconds": metrics.inference_time_seconds,
                    "inference_energy_kwh": metrics.inference_energy_kwh,
                    "time_to_accuracy": metrics.time_to_accuracy,
                    "performance_per_watt": metrics.training_time_seconds/metrics.training_energy_kwh if metrics.training_energy_kwh > 0 else 0
                }
        ...
        total_energy = sum(metrics.training_energy_kwh + metrics.inference_energy_kwh for metrics in results.values())
        print_info(f"\n⏱️ Total Time Across All Models: {total_time:.2f}s ({total_time/60:.2f} minutes)")
        print_info(f"⚡ Total Energy Across All Models: {total_energy:.4f} kWh")
