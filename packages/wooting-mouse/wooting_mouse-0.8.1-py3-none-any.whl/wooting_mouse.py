...
def wooting_mouse_handler():
    ...
    while 1:
        ...
        for command in commands:
            if command == "keyboard":
                payload = d0da.d0da_feature.activate_profile(0)
                device.send_feature(payload)
                time_of_day(device)

        payload = d0da.d0da_feature.get_current_keyboard_profile_index()
        current_profile = device.send_feature(payload)[5]
