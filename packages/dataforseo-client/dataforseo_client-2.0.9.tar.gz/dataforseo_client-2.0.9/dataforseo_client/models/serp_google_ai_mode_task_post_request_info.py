...
class SerpGoogleAiModeTaskPostRequestInfo(BaseModel):
    ...
    calculate_rectangles: Optional[StrictBool] = Field(default=None, description="calculate pixel rankings for SERP elements in advanced results. optional field. pixel ranking refers to the distance between the result snippet and top left corner of the screen;. Visit Help Center to learn more>>. by default, the parameter is set to false. Note: if set to true, the charge per task will be multiplied by 2")
