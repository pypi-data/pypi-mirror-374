...
class KeywordsDataGoogleAdsAdTrafficByKeywordsTaskGetResultInfo(BaseModel):
    ...
    ctr: Optional[StrictFloat] = Field(default=None, description="projected clickthrough rate (CTR) of the advertisement. number of clicks an ad is projected to receive divided by the number of ad impressions; the CTR is projected for the specified time period. if there is no data, then the value is null")
    average_cpc: Optional[StrictInt] = Field(default=None, description="the average cost-per-click value. represents the cost-per-click (USD) estimated for a keyword based on the specified time period and historical data;. if there is no data, then the value is null")
