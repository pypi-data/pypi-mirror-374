...
def _chi_tui_cached_binary() -> Optional[Path]:
...
def init_cmd(
    ...
    if not readme.exists() or force:
        readme.write_text(
            (
                ...
                'If you installed extras (`pip install "chi-sdk[tui]"`), run:\n\n'
    ...
    # Create minimal default nav (`.tui/main.yaml`) and a panel spec
    ...
    payload = {
        ...
        "notes": "Add .tui/bin to PATH or call wrapper via full path.",
    }
    if _json_mode(ctx):
...
def doctor_cmd(ctx, config_path: str, binary_name: Optional[str]):
    ...
    if not chi_tui:
        cached = _chi_tui_cached_binary()
        info["chi_tui_cache"] = str(cached) if cached else None
        if not cached:
            problems.append(
                "chi-tui not found on PATH. Install: pip install 'chi-sdk[tui]'"
            )
...
@cli.command("download", help="Ensure prebuilt TUI is installed (guidance)")
@click.pass_context
def download_cmd(ctx):
    ...
    try:
        ...
        guidance = "Install prebuilt TUI with: pip install 'chi-sdk[tui]' (GitHub Releases not reachable)"
        if _json_mode(ctx):
            emit_error(
    ...
    # Prefer raw binary asset if present
