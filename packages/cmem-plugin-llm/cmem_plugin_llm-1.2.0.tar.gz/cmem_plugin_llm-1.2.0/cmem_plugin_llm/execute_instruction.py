...
from openai import APIError, NotGiven, OpenAI
...
class ExecuteInstruction(WorkflowPlugin):
    ...
    def _call_openai_api(self, messages: list[dict], entity: Entity) -> str:
        """Call OpenAI API based on output format and return result"""
        try:
            result = ""
            ...
            match self.output_format.name:
                ...
                case OutputFormat.STRUCTURED_OUTPUT.name:
                    ...
                    result = (
                        structured_output.model_dump_json() if structured_output is not None else ""
                    )
        except APIError as api_error:
            return self._handle_api_error(api_error, entity)
        except (ValueError, TypeError, KeyError, AttributeError) as error:
            return self._handle_unexpected_error(error, entity)
        else:
            return result
    ...
    def _process_entities(
        ...
        for entity in entities.entities:
            ...
            result = self._call_openai_api(messages, entity)
