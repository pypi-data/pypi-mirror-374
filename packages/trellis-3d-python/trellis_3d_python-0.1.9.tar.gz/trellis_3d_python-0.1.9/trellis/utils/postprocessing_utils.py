...
from .render_utils import render_multiview, render_snapshot, render_snapshot_24
...
def rgb_to_rgba(rgb_img, bg_color=(0,0,0)):
    # PIL Image
    if not isinstance(rgb_img, np.ndarray):
        rgb_img = np.array(rgb_img)
    
    # rgb_img: H W 3 uint8
    mask = np.all(rgb_img == bg_color, axis=-1)
    h, w = mask.shape
...
def to_thumbnail(gaussian):
    ...
    snapshot = render_snapshot(gaussian)['color']
    image_rgb = np.array(Image.fromarray(snapshot[2]).resize((512, 512)))
    rgba_img = rgb_to_rgba(image_rgb, bg_color=(0,0,0))
...
def to_gif(gaussian):
    ...
    snapshot_frames = render_snapshot_24(gaussian)['color']
    gif_frames = []
    for frame in snapshot_frames:
        image = Image.fromarray(frame)
        rgba_img = rgb_to_rgba(image, bg_color=(0,0,0))
