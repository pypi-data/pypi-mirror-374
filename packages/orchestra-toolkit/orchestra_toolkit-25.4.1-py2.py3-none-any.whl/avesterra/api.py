...
from avesterra.parameters import Parameter
...
def coverings(
    ...
    frame.report_code = hgtp.Report.COVERINGS
    frame.entity = entity
    frame.authorization = authorization
    response = hgtp.post(frame=frame)
...
def fasteners(
    ...
    frame = hgtp.HGTPFrame()
    frame.command_code = hgtp.Command.REPORT
    frame.report_code = hgtp.Report.FASTENERS
    frame.entity = entity
    frame.authorization = authorization
    response = hgtp.post(frame=frame)
    return response.count
...
def subscribe(
    ...
    # There is a bug in AvesTerra v8.5 (25-04) where the SUBSCRIBE command
    # does't write the `authority` correctly.
    # During the first call, it will create the subscription with NULL authority
    # Calling it again will actually set the authority correctly
    # This is a temporary workaround as this bug will be fixed next release
    hgtp.post(frame=frame)
...
def wait(
    ...
    frame.outlet = outlet
    frame.timeout = timeout
    frame.parameter = Parameter.FALSE
    frame.authorization = authorization
...
def wait_sustained(
    ...
    frame.outlet = outlet
    frame.timeout = timeout
    frame.parameter = Parameter.TRUE
    frame.authorization = authorization
...
def subscription(
    ...
    return response.outlet, response.event_code, response.time, response.authorization
...
def entity_count(server: AvEntity = NULL_ENTITY) -> int:
    frame = hgtp.HGTPFrame()
    frame.command_code = hgtp.Command.INVOKE
    frame.method_code = AvMethod.AVESTERRA
    frame.parameter = Parameter.ENTITIES
    frame.entity = server
    response = hgtp.post(frame=frame)
    return int(response._bytes)
...
def reference_count(server: AvEntity = NULL_ENTITY) -> int:
    ...
    frame.command_code = hgtp.Command.INVOKE
    frame.method_code = AvMethod.AVESTERRA
    frame.parameter = Parameter.REFERENCES
    frame.entity = server
    response = hgtp.post(frame=frame)
    return int(response._bytes)
...
def knowledge_density(server: AvEntity = NULL_ENTITY) -> float:
    ...
    frame.command_code = hgtp.Command.INVOKE
    frame.method_code = AvMethod.AVESTERRA
    frame.parameter = Parameter.DENSITY
    frame.entity = server
    response = hgtp.post(frame=frame)
    return float(response._bytes)
