...
class BaseManager:
    ...
    def _create_resource(self, data: Dict[str, Any]) -> BaseResource:
        """Create resource instance with Active Record capabilities."""
        resource = self.resource_class.model_validate(data)
        self._attach_manager(resource)
        return resource

    def _attach_manager(self, resource: BaseResource):
    ...
    def _create_paginated_response(
        ...
        return PaginatedResponse[self.resource_class](
            payload=[self._create_resource(item) for item in data["payload"]],
            count=data["count"],
            next=data.get("next"),
...
class BaseSyncManager(BaseManager):
    ...
    def _make_multipart_request(
        ...
        if method.upper() == "POST":
            ...
            response = self._session.put(url, data=form_data, files=files_data)
        else:
    ...
    def get(self, id_: str) -> BaseResource:
        ...
        return self._create_resource(response.json())
