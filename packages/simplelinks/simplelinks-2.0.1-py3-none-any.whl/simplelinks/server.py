...
class SimpleLinksServer:
    ...
    async def register_client(self, websocket, message: dict) -> dict:
        ...
        try:
            ...
            self.logger.info(f"Calling db.register_client with machine_id={machine_id}, secret={subnet}, public_ip={public_ip_safe}")
            ...
            # v2.0: Include LAN peer information for internal routing
            lan_peers = self.get_lan_peers(public_ip_safe, private_ip_safe)
            ...
            success_response = {
                ...
                "subnet": subnet,
                "session_id": session_id,
                "lan_peers": lan_peers  # v2.0: Peer information for internal routing
            }
    ...
    def get_info(self) -> dict:
        ...
        return {
            ...
            "ssl": {
                "enabled": ssl_enabled,
                "cert_path": self.ssl_cert if ssl_enabled else None
            },
            "rate_limiting": {
    ...
    def get_lan_peers(self, client_public_ip: str, client_private_ip: str) -> List[dict]:
        """Get potential LAN peers for internal routing (v2.0)"""
        peers = []
        
        try:
            # Get all online clients
            all_clients = self.db.get_online_clients()
            
            for client in all_clients:
                # Skip clients without network information
                if not client.get('private_ip') or not client.get('virtual_ip'):
                    continue
                
                # Include clients that might be on the same network
                # This is a broad approach - client will test actual connectivity
                peer_info = {
                    'virtual_ip': client['virtual_ip'],
                    'private_ip': client['private_ip'],
                    'public_ip': client.get('public_ip'),
                    'hostname': client.get('hostname', 'unknown')
                }
                peers.append(peer_info)
            
            self.logger.debug(f"Providing {len(peers)} potential LAN peers for internal routing")
            return peers
            
        except Exception as e:
            self.logger.error(f"Error getting LAN peers: {e}")
            return []


def start(port: int = 20001, secret: str = None, host: str = "0.0.0.0", 
          ssl_cert: str = None, ssl_key: str = None, rate_limit: str = "512k",
          start_web_api: bool = True) -> None:
    """
    ...
    Args:
        ...
        start_web_api: Automatically start Web API (default: True)
    """
    ...
    if _server_instance.running:
        ...
        # Auto-start Web API if requested
        if start_web_api:
            try:
                ...
                if _server_instance.web_api and _server_instance.web_api.running:
                    ...
                    print(f"\n✓ Web API started automatically")
                    print(f"  Status page: {protocol}://{host if host != '0.0.0.0' else local_ip}:{api_port}/status.html")
                    print(f"  Access code: {_server_instance.server_secret}")
            except Exception as e:
                print(f"\n⚠️  Web API auto-start failed: {e}")
                print("   You can start it manually with: slink-server web-start")
    else:
...
def status() -> None:
    ...
    if clients:
        ...
        print(f"{'Virtual IP':<12} {'Hostname':<20} {'Status':<8} {'Last Seen'}")
        print("-" * 60)
        ...
        for client in clients:
            ...
            print(f"{client['virtual_ip']:<12} {hostname:<20} {status_str:<8} {last_seen}")
    else:
