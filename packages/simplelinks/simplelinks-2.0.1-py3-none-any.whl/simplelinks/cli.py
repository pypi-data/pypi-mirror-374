...
def server_cli():
    ...
    start_parser.add_argument(
        '--no-web-api', action='store_true',
        help='Do not automatically start Web API server'
    )
    ...
    if args.command == 'start':
        try:
            server.start(port=args.port, host=args.host, secret=args.secret,
                        ...
                        start_web_api=not args.no_web_api)
...
def client_cli():
    ...
    join_parser.add_argument(
        'host', help='Server IP address or hostname'
    )
    join_parser.add_argument(
        '--port', '-p', type=int, default=20001,
    ...
    join_parser.add_argument(
        '--no-internal-routing', action='store_true',
        help='Disable internal network routing optimization (v2.0)'
    )
    ...
    if args.command == 'join':
        try:
            ...
            internal_routing = not args.no_internal_routing
