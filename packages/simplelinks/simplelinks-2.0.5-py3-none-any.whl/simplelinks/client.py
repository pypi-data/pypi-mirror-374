...
class SimpleLinksClient:
    ...
    def __init__(self, host: str, port: int, server_secret: str, subnet: str = "default",
                 auto_reconnect: bool = True, internal_routing: bool = False):
        self.host = host
        self.port = port
        ...
        # Setup logging
        logging.basicConfig(level=logging.DEBUG)  # Changed to DEBUG for TUN troubleshooting
        ...
        # v2.0: Internal network routing
        self.internal_router = InternalRouter()
        self.internal_routing_enabled = internal_routing  # Can be disabled for compatibility
    ...
    async def register_with_server(self) -> bool:
        ...
        try:
            ...
            if response.get("success"):
                ...
                # v2.0: Schedule internal routing initialization for after registration completes
                # This prevents internal routing errors from affecting TUN device creation
                if self.internal_routing_enabled:
                    lan_peers = response.get("lan_peers", [])
                    if lan_peers:
                        # Schedule internal routing setup as a separate task
                        asyncio.create_task(self._setup_internal_routing_async(lan_peers))
                    else:
                        self.logger.info("No LAN peers available for internal routing")
    ...
    async def _setup_internal_routing_async(self, lan_peers: list) -> None:
        """Setup internal routing in a separate task to not interfere with TUN creation"""
        try:
            # Give TUN setup time to complete first
            await asyncio.sleep(2)
            
            self.logger.info("Initializing internal routing...")
            
            self.internal_router.local_virtual_ip = self.virtual_ip
            await self.internal_router.initialize_peer_routes(lan_peers)
            
            # Start internal communication listener
            asyncio.create_task(self.internal_router.start_internal_listener())
            
            # Setup packet handler callback
            self.internal_router.packet_handler = self.handle_internal_packet
            
            # Start route optimization
            asyncio.create_task(self.internal_router.periodic_route_optimization())
            
            self.logger.info("Internal routing initialized successfully")
            
        except Exception as e:
            self.logger.warning(f"Internal routing initialization failed: {e}")
            self.logger.warning("Continuing with server-only routing")
            self.internal_routing_enabled = False
    ...
    async def setup_linux_tun(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Virtual IP: {self.virtual_ip}, Network: {self.network}")
            
            # Clean up existing TUN device if any
            if hasattr(self, 'tun_fd') and self.tun_fd is not None:
                try:
                    self.logger.debug("Closing existing TUN device...")
                    os.close(self.tun_fd)
                except Exception as e:
                    self.logger.debug(f"Error closing existing TUN: {e}")
                self.tun_fd = None
            
            # Remove existing interface if it exists
            try:
                self.logger.debug("Removing existing slink0 interface...")
                os.system("ip link delete slink0 2>/dev/null")
            except Exception as e:
                self.logger.debug(f"Error removing existing interface: {e}")
            
            # Create TUN device
            self.logger.debug("Opening /dev/net/tun...")
            self.tun_fd = os.open("/dev/net/tun", os.O_RDWR)
            self.logger.debug(f"TUN fd opened: {self.tun_fd}")
            
            self.logger.debug("Creating TUN interface slink0...")
            ifr = struct.pack("16sH", b"slink0", IFF_TUN | IFF_NO_PI)
            ifname = fcntl.ioctl(self.tun_fd, TUNSETIFF, ifr)
            self.logger.debug("TUN interface created successfully")
            ...
            self.logger.debug(f"Network prefix length: {prefix_len}")
            
            # Configure IP address - use /32 to prevent automatic network routes
            loop = asyncio.get_event_loop()
            
            # Set IP with /32 prefix to avoid kernel creating broad network routes
            cmd1 = f"ip addr add {self.virtual_ip}/32 dev slink0"
            self.logger.debug(f"Executing: {cmd1}")
            result1 = await loop.run_in_executor(None, os.system, cmd1)
            self.logger.debug(f"IP address assignment result: {result1}")
            
            cmd2 = "ip link set slink0 up"
            self.logger.debug(f"Executing: {cmd2}")
            result2 = await loop.run_in_executor(None, os.system, cmd2)
            self.logger.debug(f"Interface up result: {result2}")
            
            # Add route for the VPN network excluding our own IP
            # This forces all VPN traffic through the TUN interface
            network_base = self.network.split('/')[0]  
            prefix_len = int(self.network.split('/')[1])
            
            # Calculate network range excluding our own IP
            import ipaddress
            network = ipaddress.IPv4Network(self.network, strict=False)
            
            # Add routes for individual IPs in the network (except our own)
            for ip in network.hosts():
                if str(ip) != self.virtual_ip:
                    cmd_route = f"ip route add {ip}/32 dev slink0"
                    self.logger.debug(f"Adding route: {cmd_route}")
                    await loop.run_in_executor(None, os.system, cmd_route + " 2>/dev/null")
            ...
            self.logger.error("TUN device creation failed. Common causes:")
            self.logger.error("1. Insufficient privileges (need root/sudo)")
            self.logger.error("2. TUN/TAP kernel module not loaded (modprobe tun)")
            self.logger.error("3. /dev/net/tun not accessible")
            self.logger.error("4. Network configuration conflicts")
            return False
    ...
    async def tun_reader(self):
        ...
        while self.running and self.connected:
            try:
                ...
                # v2.0: Intelligent routing - try internal first, fallback to server
                packet_sent = False
                
                # Try internal routing only if properly initialized
                if (self.internal_routing_enabled and 
                    hasattr(self, 'internal_router') and 
                    self.internal_router and
                    hasattr(self.internal_router, 'should_use_internal_route')):
                    
                    try:
                        if self.internal_router.should_use_internal_route(target_ip):
                            packet_sent = await self.internal_router.send_via_internal(target_ip, data.hex())
                            if packet_sent:
                                self.logger.debug(f"📡 Internal route: {target_ip} ({len(data)} bytes)")
                    except Exception as e:
                        self.logger.debug(f"Internal routing failed: {e}")
                        packet_sent = False
                
                if not packet_sent:
                    # Fallback to server routing (always works)
                    ...
                    self.logger.debug(f"🌐 Server route: {target_ip} ({len(data)} bytes)")
    ...
    async def handle_internal_packet(self, packet_data: str):
        """Handle packet received via internal network routing"""
        try:
            # Convert hex string back to bytes
            raw_data = bytes.fromhex(packet_data)
            
            # Write directly to TUN interface
            if hasattr(self, 'tun_fd') and self.tun_fd is not None:
                loop = asyncio.get_event_loop()
                await loop.run_in_executor(
                    None,
                    lambda: os.write(self.tun_fd, raw_data)
                )
                self.logger.debug(f"📡 Internal packet injected to TUN ({len(raw_data)} bytes)")
                
        except Exception as e:
            self.logger.error(f"Error handling internal packet: {e}")
    ...
    async def connect_to_server(self) -> bool:
        ...
        for protocol in protocols:
            try:
                ...
                tun_setup_success = await self.setup_network_interface()
                if not tun_setup_success:
                    self.logger.warning("Network interface setup failed, continuing with limited functionality")
                    self.logger.warning("You will only be able to use basic server communication, no local TUN interface")
                else:
                    self.logger.info("Network interface setup completed successfully")
...
def join(host: str, secret: str, subnet: str = "default", port: int = 20001,
         auto_reconnect: bool = True, internal_routing: bool = False) -> None:
    """
    ...
    Args:
        ...
        internal_routing: Enable internal network routing optimization (default: False - disabled for stability)
    """
    ...
    _client_instance = SimpleLinksClient(host=host, port=port, server_secret=secret, subnet=subnet,
                                         auto_reconnect=auto_reconnect, internal_routing=internal_routing)
