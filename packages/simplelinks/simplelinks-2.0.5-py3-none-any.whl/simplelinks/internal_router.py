...
class InternalRouter:
    ...
    async def start_internal_listener(self) -> None:
        ...
        try:
            ...
            while self.running:
                try:
                    # Use more compatible approach for UDP socket receiving
                    loop = asyncio.get_event_loop()
                    
                    # Check if sock_recvfrom is available (Python 3.7+)
                    if hasattr(loop, 'sock_recvfrom'):
                        data, addr = await loop.sock_recvfrom(self.direct_socket, 4096)
                    else:
                        # Fallback for older Python versions
                        data, addr = await loop.run_in_executor(
                            None, 
                            lambda: self.direct_socket.recvfrom(4096)
                        )
            ...
            self.logger.warning("Disabling internal routing due to listener failure")
            self.running = False
