...
class StablePositioner {
  ...
  async position() {
    ...
    const middleware = [offset(this.config.offset)];
  ...
  detectOscillation() {
    const now = Date.now();
    this.history = this.history.filter((h) => now - h.timestamp < 2e3);
    ...
    const hasOpposites = Object.entries(OPPOSITES).some(
      ([side, opposite]) => placements.some((p) => p.includes(side)) && placements.some((p) => p.includes(opposite))
    );
    if (hasOpposites) return true;
    const recent = this.history.slice(-3);
    const movement = recent.slice(1).reduce((sum, h, i) => sum + Math.abs(h.x - recent[i].x) + Math.abs(h.y - recent[i].y), 0);
  ...
  findBestPlacement() {
    ...
    if (space[base] > 0.8) return this.config.placement;
    const [best] = Object.entries(space).reduce((a, b) => b[1] > a[1] ? b : a);
    return align ? `${best}-${align}` : best;
  }
  lock(placement) {
  ...
  shouldUpdate(x, y, placement, last) {
    const threshold = Math.max(2, 3 * Math.sqrt(window.devicePixelRatio || 1));
    return Math.abs(x - last.x) > threshold || Math.abs(y - last.y) > threshold || placement !== last.placement;
  }
  reset() {
    this.history = [];
    this.lockUntil = 0;
...
const extract = (value) => typeof value === "string" ? value : value instanceof Set ? Array.from(value)[0] || "" : "";
const extractPlacement = (value) => {
  const str = extract(value) || "bottom";
  return VALID_PLACEMENTS.includes(str) ? str : "bottom";
};
var position_default = {
  type: "attribute",
  name: "position",
  keyReq: "starts",
  onLoad({ el, value, mods, startBatch, endBatch }) {
    const config = {
    ...
    const updatePosition = async () => {
      ...
      startBatch();
      try {
        positioner ?? (positioner = new StablePositioner(target, el, config));
