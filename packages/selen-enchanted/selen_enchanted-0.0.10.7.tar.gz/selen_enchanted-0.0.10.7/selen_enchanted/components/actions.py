...
class Actions:
    ...
    def human_like_mouse_move_to_element(
        self, driver, element, steps=30, jitter=3, delay=0.01
    ):
        ...
        for i in range(1, steps + 1):
            t = i / steps
            interp_x = (
                curr_x + (target_x - curr_x) * t + random.uniform(-jitter, jitter)
            )
            interp_y = (
                curr_y + (target_y - curr_y) * t + random.uniform(-jitter, jitter)
            )
