...
from ..constants import ENV_API_KEY, GEOCODE_BASE_URL, PLACES_BASE_URL
from ._auth import (
    AuthMode,
    BaseAuth,
    GoogleADCAuth,
...
class BaseGoogleMapsClient:
    ...
    def __init__(
        ...
        *,
        ...
        resolved_api_key = api_key or os.getenv(ENV_API_KEY)
        ...
        async def _do_send() -> httpx.Response:
            ...
            try:
                ...
                from rich.console import (
                    Console,  # TODO: Remove console prints before initial commit
                )
                from rich.json import JSON

                console = Console()
                console.print(
                    f"[bold red]HTTP {e.response.status_code}[/] for [cyan]{e.request.url}[/]"
                )
                if response_json:
