        ...
        if v is not None:
            ...
            for place_type in v:
                ...
                for _, types in PLACE_TYPES.items():
                    ...
                    raise ValueError(f"Invalid place type: {place_type}")
        return v
    ...
    def validate_included_type(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            if not v or not v.strip():
                raise ValueError("included_type must be a non-empty string")
            for _, types in PLACE_TYPES.items():
                if v in types:
                    return v
            raise ValueError(f"Invalid place type for included_type: {v}")
        return v
...
class DetailsRequest(CamelModel):
    ...
    place_id: str = Field(
        ...,
        description="A textual identifier that uniquely identifies a place, such as 'places/PLACE_ID'.",
    )
...
class ComponentFilter(BaseModel):
    ...
    def to_str(self) -> str:
        ...
        return f"{self.component.value}:{self.value}"


class ExtraComputation(str, Enum):
...
class GeocodingRequest(BaseModel):
    ...
    extra_computations: Optional[list[ExtraComputation]] = Field(
    ...
    def to_query_params(self) -> QueryParams:
        ...
        # The API expects repeated query parameters for multiple extra computations
        if self.extra_computations:
            # This part needs to be handled by the request-making library,
            # as dicts don't support duplicate keys. We'll return a list for this key.
            for ec in self.extra_computations:
                params = params.add("extra_computations", ec.value)
