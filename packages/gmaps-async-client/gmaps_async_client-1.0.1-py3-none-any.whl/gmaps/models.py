...
_custom_place_types: set[str] = set()
_custom_components: set[str] = set()
_custom_extra_computations: set[str] = set()

REGISTRY_MAP = {
    "place_types": _custom_place_types,
    "components": _custom_components,
    "extra_computations": _custom_extra_computations,
}

CustomField = Literal["place_types", "components", "extra_computations"]


def register_custom(field: CustomField, *values: str) -> None:
    """
    Register additional custom values to be accepted during validation.

    Args:
        field: The field type to register values for
        *values: Value strings to register as valid

    Examples:
        register_custom("place_types", "custom_restaurant", "special_shop")
        register_custom("components", "custom_area", "special_zone")
        register_custom("extra_computations", "CUSTOM_COMPUTATION")
    """
    if field == "place_types":
        _custom_place_types.update(values)
    elif field == "components":
        _custom_components.update(values)
    elif field == "extra_computations":
        _custom_extra_computations.update(values)
    else:  # pragma: no cover
        raise ValueError(f"Invalid field: {field}")


def get_custom(field: CustomField) -> set[str]:
    """
    Get all registered custom values for a field.

    Args:
        field: The field type to get values for

    Returns:
        Set of registered custom values

    Examples:
        get_custom("place_types")  # {'custom_restaurant', 'special_shop'}
    """
    return REGISTRY_MAP.get(field, set()).copy()


def clear_custom_registries() -> None:
    """Clear all custom registrations. Useful for testing."""
    _custom_place_types.clear()
    _custom_components.clear()
    _custom_extra_computations.clear()
...
def _components_allowlist() -> set[str]:
    """Get all allowed components (built-in enum values + custom registered ones)."""
    return _custom_components.copy()


def _extra_computations_allowlist() -> set[str]:
    """Get all allowed extra computations (built-in enum values + custom registered ones)."""
    return _custom_extra_computations.copy()


def validate_enum_or_allowlist(
    value: str | Enum,
    enum_class: type[Enum],
    allowlist_func: Callable[[], set[str]],
    field_name: str,
) -> str:
    ...
    if isinstance(value, str):
        ...
        try:
            enum = enum_class(value_stripped)
            return str(enum.value)
        except ValueError:
            if value_stripped in allowlist_func():
                return str(value_stripped)
    raise ValueError(
        f"Invalid {field_name} '{value}'. Use register_custom to allow it."
    )
...
class TextSearchRequest(CamelModel):
    ...
    language_code: Optional[str] = Field(
        default=None,
        description="The language code for returning results, e.g., 'en' or 'fr'.",
    )
...
class AutocompleteRequest(CamelModel):
    ...
    included_primary_types: Optional[list[str]] = Field(
        default=None,
        max_length=5,
        description="Restricts results to up to five primary types.",
    )
    include_pure_service_area_businesses: Optional[bool] = Field(
        default=False,
        description="Includes businesses that deliver to customers but lack a physical location.",
    ...
    language_code: Optional[str] = Field(
        default=None,
        description="The preferred language for results (IETF BCP-47 code).",
    )
...
class ComponentFilter(BaseModel):
    ...
    def validate_component(cls, v: ComponentStr) -> str:
        return validate_enum_or_allowlist(
            v, Component, _components_allowlist, "component"
        )
...
class GeocodingRequest(BaseModel):
    ...
    components: Optional[list[ComponentFilter]] = Field(
        default=None, description="A list of component filters to restrict results."
    )
    bounds: Optional[Viewport] = Field(
        default=None, description="The bounding box of the viewport to bias results."
    )
    ...
    extra_computations: Optional[list[ExtraComputationsStr]] = Field(
        default=None, description="Specifies additional features in the response."
    )
    output_format: Literal["json", "xml"] = Field(
        default="json", description="The format of the response."
    )
    ...
    def validate_extra_computations(
        ...
        if v is None:
            return v
        for ec in v:
            validate_enum_or_allowlist(
                ec,
                ExtraComputations,
                _extra_computations_allowlist,
                "extra_computation",
            )
        return v
