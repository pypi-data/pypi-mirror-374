...
class GeocodingClient(GeocodingBaseClient):
    ...
    geographic coordinates (geocoding), place IDs to addresses (place geocoding),
    ...
    - place_geocode_simple(): Get address from place ID
    ...
    Example:
        ...
        async with GeocodingClient() as client:
            ...
            response = await client.place_geocode_simple(
    ...
    async def geocode(
        ...
        **kwargs: Any,
        ...
        url_path = f"/geocode/{request.output_format}"
    ...
    async def place_geocode_simple(
        self,
        *,
        place_id: str,
        language: Optional[str] = None,
        ...
        **kwargs: Any,
        ...
        """
        Simplified place geocoding method for getting address from place ID.

        This is a convenience method that creates a GeocodingRequest internally
        for place geocoding use cases.

        Args:
            place_id: The place ID to get the human-readable address for
            language: Language for results (e.g., "en", "es")
            region: Region code for bias (e.g., "us", "fr")
            bounds: Viewport to bias geocode results more prominently
            output_format: Response format ("json" or "xml")
