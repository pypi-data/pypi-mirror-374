...
class Base(ClearSessionMixin, SlidgeTest):
    ...
    def get_participant(
        ...
        if presence_sent:
            participant.stored.presence_sent = True
        return participant
...
class TestMuc(Base):
    ...
    def test_correct_from_xmpp(self):
        muc = self.get_private_muc()
        muc.add_user_resource("gajim")
        self.recv(  # language=XML
            """
        ...
        with unittest.mock.patch("test_muc.Session.on_correct", return_value=None) as on_correct:
            self.recv(  # language=XML
                """
            <message from='romeo@montague.lit/gajim'
                     type='groupchat'
            ...
            assert on_correct.call_args[0][2] == "legacy-id"
    ...
    def test_join_room_avatar(self):
        ...
        with self.xmpp.store.session() as orm:
            for part in self.xmpp.store.participants.get_all(orm, muc.stored.id):
                if part.nickname.startswith("history-man"):
                    continue
                pres = self.next_sent()
                assert isinstance(pres, Presence), part.nickname
    ...
    def test_participant_avatar(self):
        ...
        self.run_coro(session.bookmarks.fill())
        muc = self.get_private_muc()
        muc.stored.participants_filled = True
        muc.commit()
    ...
    def test_bookmark_extension_preservation(self):
        ...
        patch = unittest.mock.patch(
            "slixmpp.plugins.xep_0356.privilege.uuid.uuid4", increment
        )
        patch.start()
        ...
        pinned_stanza = base_stanza.format(
            extension='<extensions><pinned xmlns="urn:xmpp:bookmarks-pinning:0" /></extensions>'
        )
        not_pinned_stanza = base_stanza.format(extension="")
        not_pinned_stanza_empty_extension = base_stanza.format(
        ...
        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=False, pin=None)
        )
        ...
        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=False, pin=True)
        )
        ...
        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=False, pin=False)
        )
        ...
        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=True, pin=None)
        )
        self.send(request_bookmark_stanza.format(counter=c), use_values=False)
        self.recv(  # language=XML
            bookmark_result_stanza.format(counter=c, extension=extension_not_pinned),
            use_values=False,
        )
        assert self.next_sent() is None

        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=True, pin=True)
        )
        c += 1
        self.send(request_bookmark_stanza.format(counter=c), use_values=False)
        self.recv(  # language=XML
        ...
        c += 1
        self.send(  # language=XML
            base_stanza.format(extension=extension_pinned).format(counter=c),
            use_values=False,
        )
        ...
        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=True, pin=False)
        )
        c += 1
        self.send(request_bookmark_stanza.format(counter=c), use_values=False)
        self.recv(  # language=XML
        ...
        c += 1
        self.send(  # language=XML
            base_stanza.format(extension=extension_not_pinned).format(counter=c),
            use_values=False,
        )
        self.recv(iq_confirmation_stanza.format(counter=c))
        ...
        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=True, notify="always")
        )
        c += 1
        self.send(request_bookmark_stanza.format(counter=c), use_values=False)
        self.recv(  # language=XML
            bookmark_result_stanza.format(counter=c, extension=extension_not_notify),
            use_values=False,
        )
        c += 1
        self.send(  # language=XML
            base_stanza.format(extension=extension_notify_always).format(counter=c),
            use_values=False,
        )
        self.recv(iq_confirmation_stanza.format(counter=c))
        ...
        self.send(request_bookmark_stanza.format(counter=c), use_values=False)
        self.recv(  # language=XML
            bookmark_result_stanza.format(counter=c, extension=extension_notify_always),
            use_values=False,
        )
        assert self.next_sent() is None

        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=True, notify="never")
        )
        c += 1
        self.send(request_bookmark_stanza.format(counter=c), use_values=False)
        self.recv(  # language=XML
        ...
        c += 1
        self.send(  # language=XML
            base_stanza.format(extension=extension_notify_never).format(counter=c),
            use_values=False,
        )
        self.recv(iq_confirmation_stanza.format(counter=c))
        ...
        self.send(request_bookmark_stanza.format(counter=c), use_values=False)
        self.recv(  # language=XML
            bookmark_result_stanza.format(
                counter=c, extension=extension_notify_always_mobile
            ),
            use_values=False,
        )
        c += 1
        self.send(  # language=XML
            base_stanza.format(extension=extension_notify_never_always_mobile).format(
                counter=c
            ),
            use_values=False,
        )
    ...
    def test_bookmark_chat_notification_setting(self):
        ...
        never_stanza = base_stanza.format(
            extension="<extensions><notify xmlns='urn:xmpp:notification-settings:0'><never /></notify></extensions>"
        )
        always_stanza = base_stanza.format(
            extension="<extensions><notify xmlns='urn:xmpp:notification-settings:0'><always /></notify></extensions>"
        )
        invite_stanza = (  # language=XML
            """
            <message xmlns="jabber:component:accept"
        ...
        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=False, notify="never")
        )
        ...
        self.xmpp.loop.create_task(
            muc.add_to_bookmarks(auto_join=False, preserve=False, notify="always")
        )
    ...
    async def __participants_as_list(self, muc: MUC):
        return [p async for p in muc.get_participants() if not p.is_user]
...
class TestJoinAway(Base):
    ...
    def test_away_contact_joins(self):
        ...
        assert self.next_sent() is None
        muc = self.muc
        self.muc.stored.participants_filled = True
        muc.commit()
...
class TestHats(Base):
    def test_hats(self):
        ...
        participant = self.run_coro(muc.get_participant("i-wear-hats"))
        participant.set_hats([Hat("uri1", "title1"), Hat("uri3", "title3")])
        self.send(  # language=XML
            """
            <presence from="room-private@aim.shakespeare.lit/i-wear-hats"
                      to="romeo@montague.lit/gajim">
