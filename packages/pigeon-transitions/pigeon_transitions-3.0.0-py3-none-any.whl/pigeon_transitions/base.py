    ...
    def callback(self, func, event_data):
        if isinstance(func, str):
            ...
            if hasattr(name, "__name__"):
                ...
                name = repr(name)
        with self._zipkin_tracer.zipkin_span(
            "pigeon-transitions", span_name=f"callback {name}"
        ):
            ...
            try:
                ...
                for frame in traceback.extract_tb(e.__traceback__)[::-1]:
                    ...
                    if PIGEON_TRANSITIONS_MODULE in parents or TRANSITIONS_MODULE in parents:
                        break
                    tb.insert(0, frame)
                self._logger.warning(f"An error was encountered while running callback \"{name}\":\n" + "\n".join(traceback.format_list(tb)))
