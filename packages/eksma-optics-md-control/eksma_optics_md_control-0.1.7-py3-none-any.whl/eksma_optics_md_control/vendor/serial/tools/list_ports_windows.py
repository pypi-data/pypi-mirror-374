...
DEVPKEY_Device_BusReportedDeviceDesc = DEVPROPKEY(
    (ctypes.c_byte * 16)(
        0x7E, 0x94, 0x0B, 0x54, 0x40, 0x8B, 0xBC, 0x45, 0xA8, 0xA2, 0x6A, 0x0B, 0x89, 0x4C, 0xBD, 0xA2
    ),
    4,
)
...
def get_parent_serial_number(child_devinst, child_vid, child_pid, depth=0, last_serial_number=None):
    ...
    m = re.search(r"VID_([0-9a-f]{4})(&PID_([0-9a-f]{4}))?(&MI_(\d{2}))?(\\(.*))?", parentHardwareID_str, re.I)
...
def iterate_comports():
    ...
    for index in range(len(GUIDs)):
        ...
        while SetupDiEnumDeviceInfo(g_hdi, index, ctypes.byref(devinfo)):
            ...
            RegQueryValueEx(
                hkey, "PortName", None, None, ctypes.byref(port_name_buffer), ctypes.byref(port_name_length)
            )
            RegCloseKey(hkey)
