...
class ProcessingRoutine:
            ...
            if self.file_type_dir:
                for target_file in input_dir.iterdir():
                    if target_file.stem == file.stem:
                        self.fill_file_type_dir(target_file)
        return out_list
    ...
    def move_file(self, step: ProcessingStep):
        """
        ...
        """
        new_path = self.output_dir.joinpath(step.new_name)
        shutil.copyfile(step.output_path.joinpath(step.new_name), new_path)
        self.fill_file_type_dir(new_path, copy=False)
    ...
    def fill_file_type_dir(self, file: Path, copy: bool = True):
        """
        ...
        """
        if self.file_type_dir:
            file_dir = self.file_type_dir.joinpath(file.suffix.strip("."))
