...
from scrapy_vectors import EmbeddingsLiteLLMPipeline


@pytest.fixture
def embedding_pipeline(monkeypatch):  # pylint: disable=unused-argument
    def mock_embedding(*args, **kwargs):  # pylint: disable=unused-argument
        return {
...
class TestEmbeddingsLiteLLMPipeline:
    ...
    def test_process_item(
        ...
        result = embedding_pipeline.process_item(item, None)
    ...
    def test_skip_invalid_items(
        self, embedding_pipeline, item, description
    ):  # pylint: disable=redefined-outer-name
