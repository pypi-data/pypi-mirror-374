...
from sugar.datastore.datastore import DSObject, DSMetadata, RawObject
...
class TestDSObject(unittest.TestCase):
    ...
    @patch('sugar.datastore.datastore._get_data_store')
...
class TestRawObject(unittest.TestCase):
    ...
    @patch('sugar.env.get_profile_path')
    def test_rawobject_get_file_path(self, mock_get_profile, mock_mktemp,
                                     mock_symlink, mock_makedirs, mock_exists):
        ...
        with patch('os.stat'):
            with patch('sugar.profile.get_color'):
                obj = RawObject('/test/file.txt')
                file_path = obj.file_path

                self.assertEqual(file_path, '/profile/data/tempfile')
                mock_symlink.assert_called_once_with('/test/file.txt', '/profile/data/tempfile')
...
class TestDatastoreFunctions(unittest.TestCase):
    ...
    def test_find_function(self, mock_get_ds):
        """Test find function."""
        mock_ds = Mock()
        mock_ds.find.return_value = (
            [{'uid': 'obj1', 'title': 'Object 1'}],
            1
        )
        mock_get_ds.return_value = mock_ds
    ...
    @patch('sugar.datastore.datastore._get_data_store')
    @patch('sugar.datastore.datastore.write')
...
class TestDatastoreSignals(unittest.TestCase):
    ...
    def test_signal_callbacks(self, mock_get_ds):
        ...
        with patch('sugar.datastore.datastore.created') as mock_created:
            datastore._datastore_created_cb('test_id')
            mock_created.send.assert_called_once()

        with patch('sugar.datastore.datastore.updated') as mock_updated:
            datastore._datastore_updated_cb('test_id')
            mock_updated.send.assert_called_once()

        with patch('sugar.datastore.datastore.deleted') as mock_deleted:
