...
class ActivityBundle(Bundle):
    ...
    def _parse_info(self, info_file):
        ...
        if cp.has_option(section, "bundle_id"):
            ...
            if cp.has_option(section, "service_name"):
                ...
                logging.error(
                    "ATTENTION: service_name property in the "
                    "activity.info file is deprecated, should be "
                    "changed to bundle_id"
                )
            else:
                raise MalformedBundleException(
                    "Activity bundle %s does not specify a bundle_id" % self.get_path()
                )
        ...
        if cp.has_option(section, "exec"):
            ...
            if cp.has_option(section, "class"):
                ...
                logging.error(
                    "ATTENTION: class property in the "
                    "activity.info file is deprecated, should be "
                    "changed to exec"
                )
            else:
        ...
        if cp.has_option(section, "icon"):
            ...
            logging.warning(
                "Activity bundle %s does not specify an icon" % self.get_path()
            )
        ...
        if not cp.has_option(section, "license"):
            logging.warning(
                "Activity bundle %s does not specify a license" % self.get_path()
            )
    ...
    def install_mime_type(self, install_path):
        ...
        if os.path.isfile(mime_path):
            ...
            os.spawnlp(
                os.P_WAIT, "update-mime-database", "update-mime-database", mime_dir
            )
        ...
        if mime_types is not None:
            ...
            for mime_type in mime_types:
                mime_icon_base = os.path.join(
                    install_path, "activity", mime_type.replace("/", "-")
                )
                svg_file = mime_icon_base + ".svg"
                info_file = mime_icon_base + ".icon"
                self._symlink(
                    svg_file,
                    os.path.join(installed_icons_dir, os.path.basename(svg_file)),
                )
                self._symlink(
                    info_file,
                    os.path.join(installed_icons_dir, os.path.basename(info_file)),
                )
    ...
    def _symlink(self, src, dst):
        ...
        if not os.path.islink(dst) and os.path.exists(dst):
            raise RuntimeError(
                "Do not remove %s if it was not installed by sugar" % dst
            )
    ...
    def uninstall(self, force=False, delete_profile=False):
        ...
        mime_dir = os.path.join(xdg_data_home, "mime")
        installed_mime_path = os.path.join(
            mime_dir, "packages", "%s.xml" % self._bundle_id
        )
