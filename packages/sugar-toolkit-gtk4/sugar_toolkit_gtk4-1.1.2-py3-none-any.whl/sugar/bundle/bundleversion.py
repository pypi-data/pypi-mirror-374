...
class NormalizedVersion(object):
    ...
    def _parse_version(self, version_string):
        ...
        if len(version_string) > 1 and version_string[0] == "0":
            raise InvalidVersionError(
                "Can not have leading zero in segment"
                " %s in %r" % (version_string, self._activity_version)
            )
    ...
    def _parse_extraversions(self, extraversion_string):
        ...
        for n in extraversion_string.split("."):
            if len(n) > 1 and n[0] == "0":
                raise InvalidVersionError(
                    "Can not have leading zero in "
                    "segment %s in %r" % (n, self._activity_version)
                )
            nums.append(int(n))
    ...
    def _cannot_compare(self, other):
        raise TypeError(
            "Can not compare %s and %s" % (type(self).__name__, type(other).__name__)
        )
