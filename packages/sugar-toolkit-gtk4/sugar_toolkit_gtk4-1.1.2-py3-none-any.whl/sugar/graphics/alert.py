...
class Alert(Gtk.Box):
    ...
    Use :func:`~sugar.graphics.window.Window.add_alert` and
    :func:`~sugar.graphics.window.Window.remove_alert` to add and
    remove an alert.  These methods are inherited by an
    :class:`~sugar.activity.activity.Activity` via superclass
    :class:`~sugar.graphics.window.Window`.
    ...
    def add_button(self, response_id, label, icon=None, position=-1):
        ...
        Args:
            ...
            icon (:class:`~sugar.graphics.icon.Icon` or \
...
class ConfirmationAlert(Alert):
    ...
    Args:
        **kwargs: parameters for :class:`~sugar.graphics.alert.Alert`
    """
