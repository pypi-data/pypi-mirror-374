...
class Ws:
    ...
    @staticmethod
    def append_rows(ws: TnWs, a_row: TyArr) -> TnCsWs:
        if ws is None:
            return ws
        for _row in a_row:
            ws.append(_row)
    ...
    @staticmethod
    def is_type(ws: TnCsWs, sheet_type: str) -> bool:
        if ws is None:
            return False
    ...
    def iter_sheet_lst(ws: TyWs, **kwargs):
        ...
        for row in ws.iter_rows(
            ...
            yield list(Ro.iter_cell_value(row, **kwargs))
    ...
    @staticmethod
    def sh_type(ws: TyWs) -> TnStr:
        if ws is None:
            return None
        return type(ws).__name__

    @staticmethod
    def sh_by_type(ws: TnCsWs, sheet_type: str) -> TnCsWs:
        if ws is None:
            return None
    ...
    @classmethod
    def sh_chartsheet(cls, ws: TnCsWs) -> TnCsWs:
        if cls.is_type(ws, 'Chartsheet'):
            return ws
        return None

    @classmethod
    def sh_worksheet(cls, ws: TnCsWs) -> TnCsWs:
