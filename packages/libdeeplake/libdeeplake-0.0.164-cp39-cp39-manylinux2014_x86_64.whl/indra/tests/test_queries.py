...
def test_uneven_tensors_query(tmp_datasets_dir):
    ...
    assert view_2.tensor_2[0].numpy() is None
...
def test_padded_json_tensors_bug(tmp_datasets_dir):
    ds = deeplake.empty(tmp_datasets_dir / "padded_tensors_ds", token=os.getenv(ENV_ACTIVELOOP_TOKEN), overwrite=True)

    ds.create_tensor("t", htype="generic")
    for i in range(1, 5):
        ds.t.append(i)
    ...
    dds = ds.query("select * where js['test'] == 1 or js['test'] == 3")
    assert (dds.t.numpy() == [[1], [3]]).all()

    dds = ds.query("select * where js['test'] in (1, 3)")
    assert (dds.t.numpy() == [[1], [3]]).all()


def test_partial_chunk_assertion():
    ds = api.tql.query("""select * from "hub://activeloop/coco-train" where images_meta['license'] == categories[0]""", token='')
