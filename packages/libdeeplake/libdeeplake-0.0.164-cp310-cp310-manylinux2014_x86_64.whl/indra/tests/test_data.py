...
def test_3d_polygon_data(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "test_3d_polygon_data", overwrite=True)
    with ds:
        ds.create_tensor(
            "polygon/geometry",
            dtype=np.float32,
        ...
        data = {"polygon/geometry": [[[i, i + 1, i + 2] for i in range(0, 1000, 3)]]}
        ds.append(data, append_empty=True)

    ids = api.dataset(str(tmp_datasets_dir / "test_3d_polygon_data"))
    dt = ids.tensors[0][0].numpy()
    dds = ds["polygon/geometry"][0].numpy()
    assert dt.shape == (1, 334, 3)
    assert np.all(dt == np.array(dds))
...
def test_empty_dataset(tmp_datasets_dir):
    ...
    assert np.all(ids.labels.numpy() == np.array([], dtype=np.uint32))
    assert ids.labels.bytes() == []
    assert np.all(view.labels.numpy() == np.array([], dtype=np.uint32))
