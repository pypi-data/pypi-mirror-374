...
class MultiProcessingIterator:
    ...
    def __del__(self):
        self.free_resources()
        try:
            atexit.unregister(self.pool.close)
    ...
    def start_processes(self):
        ...
        pool = self.loader_meta.context.Pool(
    ...
    def handle_exception(self, ex):
        ...
        if isinstance(ex, TransformExceptionWrapper):
            ...
            if self.ignore_errors:
                ...
                etype, value, tb = type(ex), ex, ex.__traceback__
    ...
    def send_stop_signals_to_subprocesses(self):
        if (
            ...
            and hasattr(self, "data_in_queues")
        ):
            try:
                for idx in range(self.num_workers):
                    if self.data_in_queues[idx] is not None:
                        self.data_in_queues[idx].put(StopChildProcess())
            except (EOFError, AttributeError, ImportError):
                # Ignore errors if queues are already closed or during interpreter shutdown
                pass
    ...
    def free_resources(self):
        """Free all resources in the correct order"""
        try:
            self.send_stop_signals_to_subprocesses()
        except Exception:
            pass
        ...
        if self.manager is not None:
            try:
                ...
                self.manager = None
                if hasattr(self, "data_in_queues"):
                    self.data_in_queues = None
                if hasattr(self, "data_out_queues"):
                    self.data_out_queues = None
