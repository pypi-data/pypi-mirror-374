...
def summarize_slots(
    ...
    *,
    ...
    if missing:
        raise ValueError(f"`df` must include columns: {missing}")
    ...
    for attr in required_attrs:
        if not hasattr(scheduler, attr):
            raise ValueError(f"Scheduler must have attribute `{attr}`.")
...
def _aggregate_until_fits(
    ...
    *,
    ...
    if freq not in allowed_freqs:
        raise ValueError(f"freq must be one of {allowed_freqs}, got {freq}")
...
def _histogram_with_threshold(values, bin_width, min_pct_threshold):
    """
    Compute histogram counts, bin edges, and filter bins by percentage threshold.
    Returns:
        valid_x, valid_counts, valid_percentages, edges
    """
    if values.empty:
        raise ValueError("No valid data to compute histogram.")

    bins = np.arange(values.min(), values.max() + bin_width, bin_width)
    counts, edges = np.histogram(values, bins=bins)
    percentages = (counts / len(values)) * 100
    valid_bins = [(x, count, pct) for x, count, pct in zip(edges[:-1], counts, percentages) if pct >= min_pct_threshold]
    if not valid_bins:
        raise ValueError(f"No bins meet the {min_pct_threshold}% threshold.")
    ...
    return valid_x, valid_counts, valid_percentages, edges
...
def plot_past_slot_availability(
    ...
    *,
    ...
    available = grouped.get(True, pd.Series(0, index=periods)).astype(float).values
    non_available = grouped.get(False, pd.Series(0, index=periods)).astype(float).values
    x = range(n)
    annotate = _should_annotate_labels(figsize[0], dpi, n, label_px_threshold)
...
def plot_monthly_appointment_distribution(df: pd.DataFrame) -> plt.Axes:
    ...
    # Extract month numbers and compute percentages
    month_counts = (
...
def plot_appointments_by_status(
    ...
    *,
    ...
    if df.empty:
        raise ValueError("No data available after filtering by reference date.")
...
def plot_appointments_by_status_future(
    ...
    *,
    ...
    if df.empty:
        raise ValueError("No future appointments available after reference date.")
...
def plot_status_distribution_last_days(
    ...
    *,
    ...
    if filtered_df.empty:
        raise ValueError(f"No data available in the last {days_back} days before {ref_date.date()}.")
...
def plot_status_distribution_next_days(
    ...
    *,
    ...
    if filtered_df.empty:
        raise ValueError(f"No data available in the next {days_ahead} days after {ref_date.date()}.")
...
def plot_scheduling_interval_distribution(
    ...
    *,
    ...
    if not valid_bins:
        raise ValueError(f"No intervals meet the {min_pct_threshold}% threshold.")
...
def plot_appointment_duration_distribution(df: pd.DataFrame) -> plt.Axes:
    ...
    if durations.empty:
        raise ValueError("No valid data available for 'appointment_duration'.")
...
def plot_waiting_time_distribution(df: pd.DataFrame) -> plt.Axes:
    ...
    if durations.empty:
        raise ValueError("No valid data available for 'waiting_time'.")
...
def plot_arrival_time_distribution(df: pd.DataFrame) -> plt.Axes:
    ...
    if attended_appointments.empty:
        raise ValueError("No attended appointments with valid check-in times.")
    ...
    if arrival_time_diff.empty:
        raise ValueError("No valid arrival time differences available.")
