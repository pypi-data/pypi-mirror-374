...
class AppointmentScheduler:
    ...
    ref_date : date-like, optional
        Reference date dividing past and future appointments. Each value may be ISO string 
        "YYYY-MM-DD", datetime.date, datetime.datetime, pandas.Timestamp or numpy.datetime64. 
    ...
    working_hours : list of tuple[int, int], default [(8, 18)]
        Working periods during the day, as (start_hour, end_hour).
    ...
    def _normalize_range(start: Union[str, date, datetime, pd.Timestamp, np.datetime64],
                        end:   Union[str, date, datetime, pd.Timestamp, np.datetime64]) -> tuple[datetime, datetime]:
        """Return a validated (start, end) datetime tuple. If start>=end and both
        are same-day midnights, widen to the end of day."""
        s = _to_datetime(start)
        e = _to_datetime(end)

        # Allow same-day ranges by expanding the end to 23:59 if needed
        if s.date() == e.date() and s.time() == time(0, 0) and e.time() == time(0, 0):
    ...
    def __init__(
        ...
        # ============================
        slot_lengths = {v: 60 // v for v in VALID_APPTS_PER_HOUR}
        ...
        if date_ranges is None:
            ...
            self.date_ranges: list[tuple[datetime, datetime]] = [
                (
                    datetime.combine(default_start, time(0, 0)),
                    datetime.combine(default_end, time(23, 59)),
                )
            ]
        else:
            ...
            for i, rng in enumerate(date_ranges):
                ...
                s, e = _normalize_range(rng[0], rng[1])
        ...
        # VALIDATION: ref_date
        # ============================
        overall_start: datetime = min(s for (s, _) in self.date_ranges)
        overall_end: datetime = max(e for (_, e) in self.date_ranges)

        if ref_date is None:
            # Default: set ref_date to the latest end date
            self.ref_date: datetime = overall_end
        else:
            ref_dt = _to_datetime(ref_date)
            self.ref_date = ref_dt

            # Must fall within at least one of the provided ranges
            if not any(start <= ref_dt <= end for start, end in self.date_ranges):
                raise ValueError(
                    f"`ref_date` must lie within at least one of the `date_ranges`. Got {ref_dt}."
                )
        ...
        # ============================
        # EXTEND date_ranges with booking_horizon
        # ============================
        if self.date_ranges:
            last_start, last_end = max(self.date_ranges, key=lambda r: r[1])
            extended_last: tuple[datetime, datetime] = (
                last_start,
                last_end + timedelta(days=self.booking_horizon),
            )
            self.date_ranges = [
                (s, e) if (s, e) != (last_start, last_end) else extended_last
        ...
        if working_hours is None:
            ...
            if not all(isinstance(h, tuple) and len(h) == 2 for h in working_hours):
                raise TypeError("`working_hours` must be a list of (start_hour, end_hour) tuples.")
            for start, end in working_hours:
                if not (0 <= start < 24 and 0 <= end <= 24 and start < end):
                    raise ValueError("`working_hours` must contain valid hours (0–23) with start < end.")

            ordered = sorted(working_hours, key=lambda t: (t[0], t[1]))
            for i in range(1, len(ordered)):
                prev_start, prev_end = ordered[i - 1]
                cur_start, cur_end = ordered[i]
                if cur_start < prev_end:
                    raise ValueError(
                        "`working_hours` blocks overlap: "
                        f"({prev_start}, {prev_end}) overlaps with ({cur_start}, {cur_end})."
                    )
            self.working_hours = ordered
        ...
        mw = dict(month_weights)  # accept MappingProxyType, dict, etc.
        ...
        # Build normalized weight maps for calendar sampling
    ...
    def _iter_working_dates(self) -> list[pd.Timestamp]:
        """Return all working dates across `date_ranges` filtered by `working_days`.

        Notes
        -----
        - The function preserves chronological order across ranges.
        - Ranges are assumed non-overlapping (validated in __init__).
        """
        dates: list[pd.Timestamp] = []
        for start_dt, end_dt in self.date_ranges:
            days = pd.date_range(start=start_dt, end=end_dt, freq="D")
            wk = days[days.weekday.isin(self.working_days)]
            if len(wk):
                # Extend as plain list of pd.Timestamp for speed in downstream product()
    ...
    def _daily_time_grid(self) -> list[time]:
        ...
        -------
        list[datetime.time]
            One entry per slot (e.g., every 15 minutes if appointments_per_hour=4).

        Notes
        -----
        - Uses `appointments_per_hour` to derive slot granularity.
        - Supports multiple working blocks in `working_hours` (non-overlapping).
        """
        grid: list[time] = []
        step = 60 // self.appointments_per_hour
        for start_hour, end_hour in self.working_hours:
            for hour in range(start_hour, end_hour):
                for minute in range(0, 60, step):
                    grid.append(time(hour=hour, minute=minute))
        return grid
    ...
    def _weighted_sample_past_slots(self, n: int) -> pd.DataFrame:
        ...
        -------
        pd.DataFrame
            Subset of `self.slots_df` with columns at least
            ['slot_id', 'appointment_date', 'appointment_time'].
        """
        if n <= 0:
            return pd.DataFrame(columns=["slot_id", "appointment_date", "appointment_time"])

        mask = (self.slots_df["appointment_date"] < self.ref_date) & (self.slots_df["is_available"])
        available = self.slots_df.loc[mask]
        if available.empty:
            return pd.DataFrame(columns=["slot_id", "appointment_date", "appointment_time"])

        weights = available["appointment_date"].apply(self._date_weight_raw)
        n = min(n, len(available))
        return available.sample(n=n, weights=weights, replace=False, random_state=self.seed)
    ...
    def _normalize_calendar_weights(self) -> None:
        ...
        # Guard against zero/NaN means (ya validamos antes que sean positivos)
    ...
    def generate_appointments(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Fallback defensivo por si en alguna variante antigua no existe el atributo:
        ...
        # Mark sampled past slots as no longer available
        ...
        for _, slot in booked_slots.iterrows():
            ...
            # Compute the permissible scheduling window (bounded by earliest_scheduling_date and horizon)
            ...
            if k.size == 0:
                # Fallback: schedule the day before, but keep inside bounds
    ...
    def assign_status(self, appointments_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Note: future rows keep 'scheduled'/'cancelled' from elsewhere in the pipeline.
    ...
    def assign_actual_times(self, appointments_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for appt_date in attended["appointment_date"].unique():
            ...
            for idx, appt in daily.iterrows():
                ...
                # Clinic opening bound for the day (first working block start)
                ...
                # Backlog: cannot start before previous appointment ends
                if previous_end is not None:
                    earliest_start = max(earliest_start, previous_end)

                # Small stochastic service delay (seconds), non-negative
                ...
                # Persist into the main DataFrame at the correct row index
    ...
    def generate_check_in_time(self, appointment_date: date, appointment_time: time) -> datetime:
        ...
        # Normal around the configured mean; std chosen to keep ~95% within ±20 minutes around the mean.
    ...
    def assign_patients(self) -> tuple[pd.DataFrame, pd.DataFrame]:
            ...
            if idx == 0:
                ...
                if num_new_patients > 0:
                    ...
                    for age in new_patients_df["age"]:
                        ...
                        # Keep within cutoffs without global resampling/unique enforcement
        ...
        # Remove temp columns from registry
        ...
        # Keep patients_df as an auxiliary registry; appointments_df is the primary table.
    ...
    def _normal_distribution(self, categories: List[str]) -> np.ndarray:
        ...
        n = len(categories)
        mean_idx = n / 2.0
        std_dev = max(n / 4.0, 1e-6)  # guard tiny std
    ...
    def to_csv(
        ...
        *,
        ...
        """
        import pandas as pd  # local import to keep module imports minimal
