...
class AppointmentScheduler:
    ...
    Parameters
    ----------
    date_ranges : list of tuple[datetime, datetime], optional
        List of (start_date, end_date) ranges for available appointment slots.
    ref_date : datetime, optional
        Reference date dividing past and future appointments.
    ...
    def __init__(
        ...
        if date_ranges is None:
            ...
            if not all(isinstance(r, tuple) and len(r) == 2 for r in date_ranges):
                raise TypeError("`date_ranges` must be a list of (start_date, end_date) tuples.")
            ...
            for i, (start_dt, end_dt) in enumerate(date_ranges):
                if not (isinstance(start_dt, datetime) and isinstance(end_dt, datetime)):
                    raise TypeError(
                        "`date_ranges` elements must be (datetime, datetime) tuples."
                    )
                if start_dt >= end_dt:
                    raise ValueError(
                        f"`date_ranges[{i}]` has start >= end ({start_dt} >= {end_dt})."
                    )
                validated_ranges.append((start_dt, end_dt))
        ...
        if ref_date is None:
            ...
            if not isinstance(ref_date, datetime):
                raise TypeError("`ref_date` must be a datetime object.")
            self.ref_date = ref_date

            # Must fall within at least one of the provided ranges
            if not any(start <= ref_date <= end for start, end in self.date_ranges):
                raise ValueError(
                    f"`ref_date` must lie within at least one of the `date_ranges`. Got {ref_date}."
                )
