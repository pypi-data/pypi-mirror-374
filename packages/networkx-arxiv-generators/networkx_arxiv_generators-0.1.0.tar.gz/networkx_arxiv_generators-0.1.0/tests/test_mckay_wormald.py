...
def test() -> None:
    ...
    test: nx.Graph = mckay_wormald_simple_graph(test_deg_seq, debug=False)
    nx.draw_random(test, with_labels=True)
    plt.show()

    test_deg_seq_rand = random_int_list_with_even_sum(30, 0, 10, seed=42)

    test_rand: nx.Graph = mckay_wormald_simple_graph(test_deg_seq_rand, debug=False)
    print(test_deg_seq_rand)
