...
from sklearn.utils import check_X_y, as_float_array  # , check_array
...
class EBLinearRegression(BayesianLinearRegression):
    ...
    def __init__(
        ...
        fit_intercept=True,
        normalize=True,
        perfect_fit_tol=1e-6,
        alpha=1,
        copy_x=True,
        verbose=False,
    ):
        super().__init__(n_iter, tol, fit_intercept, copy_x, verbose)
    ...
    def _center_data(self, X, y):
        ...
        if self.fit_intercept:
            ...
            X -= x_mean
            if self.normalize:
                X, x_std = f_normalize(X, axis=0, copy=False, return_norm=True)
            else:
                x_std = np.ones(X.shape[1], dtype=X.dtype)
            y_mean = np.average(y, axis=0)
            y = y - y_mean
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iter):
            ...
            if n_samples > n_features:
                ...
                mu = u * 1.0 / (dsq + alpha / beta)
                mu = np.dot(X.T, mu)
            mu = np.dot(mu, u_y)

            # precompute errors, since both methods use it in estimation
            error = y - np.dot(X, mu)
            sqd_err = np.sum(error**2)

            if sqd_err / n_samples < self.perfect_fit_tol:
            ...
            if self.optimizer == "fp":
                ...
                beta = (n_samples - gamma) / (sqd_err + np.finfo(np.float32).eps)
            else:
                # M-step, update parameters alpha and beta to maximize ML TYPE II
                eigvals = 1.0 / (beta * dsq + alpha)
                alpha = n_features / (np.sum(mu**2) + np.sum(1 / eigvals))
                beta = n_samples / (sqd_err + np.sum(dsq / eigvals))
            ...
            if converged or i == self.n_iter - 1:
                break
        eigvals = 1.0 / (beta * dsq + alpha)
        self.coef_ = beta * np.dot(vt.T * d * eigvals, u_y)
...
class VBLinearRegression(BayesianLinearRegression):
    ...
    def fit(self, X, y):
        ...
        u, d_, vt = svd(X, full_matrices=False)
        dsq = d_**2
        u_y = np.dot(u.T, y)
        ...
        for i in range(self.n_iter):
            ...
            mu_old = np.copy(mu)
            mu, eigvals = self._posterior_weights(
                e_beta, e_alpha, u_y, dsq, u, vt, d_, X
            )
            ...
            sqd_err = np.sum((y - np.dot(X, mu)) ** 2)
            xsx = np.sum(dsq * eigvals)
            d = self.d + 0.5 * (sqd_err + xsx)
        ...
        self.coef_, self.eigvals_ = self._posterior_weights(
            self.beta_, self.alpha_, u_y, dsq, u, vt, d_, X
        )
        self._set_intercept(x_mean, y_mean, x_std)
        self.eigvecs_ = vt.T
        return self

    def _posterior_weights(self, e_beta, e_alpha, u_y, dsq, u, vt, d, X):
        """
        Calculates parameters of approximate posterior distribution
        of weights
        """
        # eigenvalues of covariance matrix
        sigma = 1.0 / (e_beta * dsq + e_alpha)
        ...
        if n_samples > n_features:
            ...
            mu = u * 1.0 / (dsq + e_alpha / e_beta)  # + np.finfo(np.float64).eps)
            mu = np.dot(X.T, mu)
        mu = np.dot(mu, u_y)
        return mu, sigma
