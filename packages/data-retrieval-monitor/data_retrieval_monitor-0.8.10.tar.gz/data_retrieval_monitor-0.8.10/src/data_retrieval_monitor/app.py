...
def _chunk_badge_and_links(ch: dict):
    """
    Renders: [chunk-id badge]  p  l  📋
      - p: opens ch['proc'] if provided
      - l: opens /rawpath?p=<raw> (or the http(s) URL) in a new tab to easily view/copy the raw location
      - 📋: copies the filesystem path (scheme-stripped) directly via dcc.Clipboard
    """
    cid  = ch.get("id") or "c?"
    st   = (ch.get("status") or "waiting").lower()
    proc = ch.get("proc")
    raw  = ch.get("log_raw")  # original path or URL string
    ...
    # tiny 'l' link (log): open a helper page to show/copy the raw location
    if isinstance(raw, str) and raw.strip():
        raw_s = raw.strip()
        if raw_s.startswith("http://") or raw_s.startswith("https://"):
            # If raw is a URL, just open it directly.
            bits.append(html.A("l", href=raw_s, target="_blank", title="log", style={"marginRight":"6px"}))
        else:
            # Filesystem path → open /rawpath with the raw path embedded
            href = f"/rawpath?p={quote_plus(raw_s)}"
            bits.append(html.A("l", href=href, target="_blank", title="log path", style={"marginRight":"6px"}))

        # visible clipboard button (📋) that copies the path-only version
        copy_text = _path_only(raw_s)
        if copy_text:
            bits.append(
                dcc.Clipboard(
                    content=copy_text,
...
def route_rawpath():
    """
    Minimal page to show the raw log location and make it easy to copy.
    Opens in a new tab when clicking the tiny 'l' link beside each chunk.
    """
    raw_q = request.args.get("p", "", type=str)
    raw_path = raw_q  # keep exactly what was passed
    title = "Raw Log Location"
...
<html>
  ...
  <body>
    ...
    <input id="pathbox" type="text" readonly value="{html_escape(raw_path)}" />
    <div>
      <button id="copyBtn">Copy</button>
    </div>
    <div class="hint">Tip: the text is selectable; you can also press Ctrl/⌘+C.</div>
    <script>
      (function() {{
...
two_col_nowrap = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.Div(id="table-container", style={"flex":"0 0 auto", "paddingRight": "8ch"})
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=7050, debug=False)