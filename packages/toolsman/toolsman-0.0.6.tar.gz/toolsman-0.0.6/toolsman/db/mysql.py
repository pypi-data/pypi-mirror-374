...
class MysqlClient:
    ...
    def fetchone(self, sql: str, args: tuple | list | None = None) -> dict | None:
        """获取单条记录"""
        if not sql or not sql.strip():
            raise ValueError("SQL语句不能为空")
    ...
    def fetchall(self, sql: str, args: tuple | list | None = None) -> list[dict]:
        """获取所有记录"""
        if not sql or not sql.strip():
            raise ValueError("SQL语句不能为空")
    ...
    def fetchmany(self, sql: str, args: tuple | list | None = None, size: int = 1000) -> list[dict]:
        """获取多条记录"""
        if not sql or not sql.strip():
            raise ValueError("SQL语句不能为空")
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(sql, args)
                return cursor.fetchmany(size) or []
