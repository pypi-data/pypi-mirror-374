...
def running(interval: int, remind: str):
    """开启一个线程重复运行这个函数"""
...
def alive(interval=60, errback: Callable = None):
    """保持活跃。即一直在重复运行"""
        ...
        def inner(*args, **kwargs):
            while True:
                try:
                    ...
                    logger.info(
                        "{} 正常结束了，{}秒后重新启动".format(func.__name__, interval)
                    )
                finally:
                    time.sleep(interval)
