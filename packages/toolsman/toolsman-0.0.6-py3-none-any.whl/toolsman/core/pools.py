...
if __name__ == "__main__":
    import time, random
    from toolsman import printc, Timer


    def job(i):
        """模拟任务"""
        printc(f"{i} => Running...", "yellow")
        time.sleep(random.randint(2, 3))
        printc(f"{i} => Done", "green")


    # 测试1
    with Timer(name="PoolWait"):
        with PoolWait(5) as pool:
            for i in range(20):
                pool.add(job, i + 1)
    ...
    # 测试2
    with Timer(name="PoolMan"):
        with PoolMan(5) as pool:
            for i in range(20):
                pool.add(job, i + 1)
