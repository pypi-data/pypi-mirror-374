...
class TaurusMacroExecutorWidget(TaurusWidget):
    ...
    def __init__(self, parent=None, designMode=False):
        ...
        self.addToFavouritesAction = Qt.QAction(
            Qt.QIcon("emblems:emblem-favorite.svg"),
        ...
        addToFavouritesButton = Qt.QToolButton()
        addToFavouritesButton.setDefaultAction(self.addToFavouritesAction)
        self.addToFavouritesAction.setEnabled(False)
        actionsLayout.addWidget(addToFavouritesButton)
    ...
    def setHistoryBuffer(self, historyMacro):
        self._historyBuffer = historyMacro
...
#            door.runMacro(str(macroNode.name()), params)
    ...
    def onMacroStatusUpdated(self, data):
        ...
        if state == "start":
            ...
            self.macroProgressBar.setRange(int(range[0]), int(range[1]))
            self.playMacroAction.setEnabled(False)
            self.pauseMacroAction.setEnabled(True)
            self.stopMacroAction.setEnabled(True)
        ...
        self.macroProgressBar.setValue(int(step))
...
def createMacroExecutor(door=None):
    macroExecutor = TaurusMacroExecutor()
    macroExecutor.doorChanged.connect(macroExecutor.onDoorChanged)
    load_settings = True
    if door is not None:
        macroserver = get_macroserver_for_door(door)
        macroExecutor.setModel(macroserver)
        macroExecutor.doorChanged.emit(door)
...
def _main():
    taurus.warning(
        "`macroexecutor` script is deprecated since 3.4.0, "
        "use `sardanactl macroexecutor` instead."
    )
    ...
    if len(args) == 2:
        door = args[1]
    macroExecutor = createMacroExecutor(door)
    macroExecutor.show()
    sys.exit(app.exec_())


@click.command("macroexecutor")
@click.argument("door", required=False)
def macroexecutor_cmd(door):
    """GUI for executing macros.

    Provides intuitive macro parameter editors, history of executions
