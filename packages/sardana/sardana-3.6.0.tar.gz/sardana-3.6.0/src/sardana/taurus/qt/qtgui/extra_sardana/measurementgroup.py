...
from taurus.qt.qtgui.model import EditorToolBar, BaseToolBar
...
class MntGrpChannelEditor(TaurusBaseTableWidget):
    ...
    _simpleViewColumns = (ChannelView.Channel, ChannelView.Enabled,
                          ChannelView.Output, ChannelView.PlotType,
                          ChannelView.PlotAxes)
    ...
    def __init__(self, parent=None, designMode=False, with_filter_widget=True, perspective=None):
        ...
        self.setSimpleView(True)

    def isAdvancedView(self):
        return self._advancedView

    def setSimpleView(self, simpleview):
        self.setAdvancedView(not simpleview)

    def setAdvancedView(self, advancedview):
        if advancedview == self.isAdvancedView():
            return
        ...
        for i in range(self.getQModel().columnCount()):
            hide = not advancedview and (i not in columnIndexes)
            self.tableView().setColumnHidden(i, hide)
        self._advancedView = advancedview
    ...
    def createToolArea(self):
        ...
        advancedView_bar = AdvancedViewToolBar(self, self, on_change=self.setAdvancedView)
        ta.append(e_bar)
        ta.append(advancedView_bar)
        
        # Register configuration of advanced checkbox
        self.registerConfigProperty(
            advancedView_bar.checkboxAdvancedView.isChecked,
            advancedView_bar.checkboxAdvancedView.setChecked,
            "advanced_view"
        )
...
class AdvancedViewToolBar(BaseToolBar):
    """
    Internal widget to be placed in a _QToolArea providing a checkbox
    to switch between simple and advanced view
    """

    def __init__(self, view=None, parent=None, designMode=False, on_change=None):
        name="Toolbar checkbox"
        ...
        # Create QCheckbox
        self.checkboxAdvancedView = Qt.QCheckBox("Advanced view")
        self.checkboxAdvancedView.stateChanged.connect(on_change)
        self.checkboxAdvancedView.setChecked(False)

        # Add QCheckbox to the BaseToolBar
        self.addWidget(self.checkboxAdvancedView)
