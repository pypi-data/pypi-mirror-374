...
class TaurusSequencerWidget(TaurusWidget):
    ...
    def onAdd(self):
        ...
        for par in macroNode.params():
            try:
                if isinstance(par, RepeatParamNode):
                    for rpar in par.children():
                        for p in rpar.children():
                            p.setValue(p.defValue())
                else:
                    par.setValue(par.defValue())
            except AttributeError:
                self.warning("Could not populate default parameters")
