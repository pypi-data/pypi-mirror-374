...
class DiffracBasis(PseudoMotorController):
    ...
    ctrl_attributes = {'Crystal': {Type: str,
                       ...
                       'ComputeHKL': {Type: (float,),
                                            ...
                                            Access: ReadWrite},
                       'Ki': {Type: (float,),
                              Description: "Wavevector associated with the incident X-ray beam",
                              Access: ReadOnly},
                       'Kf': {Type: (float,),
                              Description: "Wavevector associated with the scattered X-ray beam",
                              Access: ReadOnly},
    }
    ...
    def setLoadCrystal(self, value: str) -> None:
    ...
    def getKi(self):
        ...
        UB = self.sample.UB_get()
        UB = [[UB.get(i, j) for j in range(3)] for i in range(3)]
        RUB = numpy.dot(R, UB)
        RUB_1 = numpy.linalg.inv(RUB)
    ...
    def getKf(self):
        ...
        RUB = numpy.dot(R, UB)
        RUB_1 = numpy.linalg.inv(RUB)
        kf = self.geometry.kf_get(self.detector)
        kf_rec = numpy.dot(RUB_1, kf.data)
        return kf_rec
