...
from sardana.pool.poolsynchronization import PoolSynchronization, SynchDescription
...
def test_synch_description_to_dial():
    synch = SynchDescription([{
        SynchParam.Delay: {SynchDomain.Time: .1,
                           SynchDomain.Position: .1},
    ...
    dial_synch = synch.to_dial(sign=-1, offset=2)
    group = synch[0]
    dial_group = dial_synch[0]
    for param in (SynchParam.Delay,
                  SynchParam.Active,
                  SynchParam.Total):
        assert dial_group[param][SynchDomain.Time] == \
            group[param][SynchDomain.Time], "param {} had changed".format(param)
    assert dial_group[SynchParam.Delay][SynchDomain.Position] == -.1
    assert dial_group[SynchParam.Initial][SynchDomain.Position] == 2
    assert dial_group[SynchParam.Active][SynchDomain.Position] == -.1
    assert dial_group[SynchParam.Total][SynchDomain.Position] == -0.2
