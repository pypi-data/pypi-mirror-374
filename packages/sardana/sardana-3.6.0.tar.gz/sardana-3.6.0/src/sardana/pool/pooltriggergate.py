...
class PoolTriggerGate(PoolElement):
    ...
    def get_index_attribute(self) -> sardana.sardanaattribute.SardanaAttribute:
    ...
    def has_moveable_on_input(self):
        return self._moveable_on_input is not None

    def get_moveable_on_input(self):
        return self._moveable_on_input

    def set_moveable_on_input(self, moveable_on_input, propagate=1):
        self._moveable_on_input = moveable_on_input
        if propagate > 0:
            self.fire_event(
                EventType("moveable_on_input", priority=propagate),
                moveable_on_input
            )

    moveable_on_input = property(
        get_moveable_on_input, 
        set_moveable_on_input, 
        doc="motor moveable_on_input configuration (none, single or map)"
    )
