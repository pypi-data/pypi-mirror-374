...
__all__ = ["addctrllib", "addmaclib", "commit_ctrllib", "defctrl", "defelem",
           "defm", "defmeas", "definstr", "edctrlcls", "edctrllib", "prdef",
           "reconfig", "relctrlcls", "relctrllib", "rellib", "relmac",
           "relmaclib", "send2ctrl", "udefctrl", "udefelem", "udefmeas",
           "sar_info"]
...
from sardana.macroserver.macro import Macro, Type, Table, LibraryError, macro, Optional
from sardana.taurus.core.tango.sardana.pool import Fault
from sardana.tango.core.util import get_server_name, get_server_version
...
class defm(Macro):
    ...
    def run(self, name, controller, axis):
        ...
        self.print("Created %s in %s" % (str(elem), pool.name))
...
class defmeas(Macro):
    ...
    def run(self, name, channel_list):
        ...
        if mg is None:
            error_message = (
                "Measurement group %s could not be created.\n" 
                "Check logs or try to create it with 'expconf'" % name
            )
            self.error(error_message)
        else:
            self.print("Created %s in %s" % (str(mg), pool.name))
...
class udefmeas(Macro):
    ...
    def run(self, mntgrps):
        for mntgrp in mntgrps:
            ...
            self.print("Deleted %s from %s" % (mntgrp.getName(), pool.name))
...
class defelem(Macro):
    ...
    def run(self, name, ctrl, axis):
        ...
        if elem is None:
            raise RuntimeError("Error when trying to create %s" % name)
        self.print("Created %s in %s" % (str(elem), pool.name))
...
class renameelem(Macro):
    ...
    def run(self, elem, new_name):
        ...
        self.print("Renamed %s to %s (in pool %s)" %
                   (old_name, new_name, pool.name))
...
class udefelem(Macro):
    ...
    def run(self, elements):
        for element in elements:
            ...
            self.print("Deleted %s from %s" % (element.getName(), pool.name))
    ...
    def run(self, controllers):
        for controller in controllers:
            ...
            try:
                ...
                self.error(msg)
                raise
            self.print("Deleted %s from %s" % (ctrl_name, pool.name))


@macro([["fullname", Type.String, None, "instrument full name"],
        ["class", Type.String, "NXinstrument",
         "NeXus class to be used in data file (HDF5, NeXus)"],
        ["pool", Type.Pool, Optional,
         "Pool where the instrument will be added"]])
def definstr(self, fullname, class_, pool):
    """Creates a new instrument.

    Use ``udefelem`` to delete an already existing instrument.
    """
    if pool is None:
        pool = self.macro_server.get_default_pool()
    pool.createInstrument(fullname, class_)
    self.print("Created %s in %s" % (fullname, pool.name))
...
class edctrllib(Macro):
    ...
    param_def = [
        ...
        ["pool", Type.Pool, Optional, "Pool where the controller was created"]
    ]
...
class relctrllib(Macro):
    ...
    def run(self, ctrl_library):
        ...
        self.print("Reloaded %s from %s" % (ctrl_library.name, pool.name))
...
class addctrllib(Macro):
    ...
    param_def = [["ctrl_library_name", Type.String, None,
                 ...
                 ["pool", Type.Pool, Optional,
                  "Pool where the controller lib will be added"]]

    def run(self, ctrl_library_name, pool):
        if pool == None:
            targetPool = self.macro_server.get_default_pool()
        else:
            targetPool = pool

        targetPool.ReloadControllerLib(ctrl_library_name)
        self.print("Added %s in %s" % (ctrl_library_name, targetPool.name))
...
class relctrlcls(Macro):
    ...
    def run(self, ctrl_class):
        ...
        self.print("Reloaded %s from %s" % (ctrl_class.name, pool.name))
...
class reconfig(Macro):
    ...
    def _reconfig_element(self, element):
        element.reconfig()
        name = element.getName()
        state = element.stateObj.read(cache=False).rvalue
        msg = "{} is in {} state".format(name, str(state.name).capitalize())
        if state == Fault:
            self.warning(msg)
            self.info(element.information())
            raise Exception("reconfig of {} failed".format(name))
        self.info(msg)

    def run(self, element):
        if get_server_version(element) < 941:
            raise RuntimeError(
                "`reconfig` is not supported with Tango <9.4.1"
                " (memorized attributes will be lost after a reconfig using Tango <9.4.1)"
            )
        if get_server_version(element) < 941 and get_server_name() == "Sardana":
            raise RuntimeError(
                "`reconfig` is not supported in Sardana server architecture used with Tango <9.4.1"
                " (only in separate Pool and MacroServer architecture)"
            )
        type_ = element.getType().lower()
        if type_ == "instrument":
            raise ValueError("Instruments can not be reconfigured")
        self._reconfig_element(element)
        if type_ == "controller":
            ctrl = element
            used_axes = ctrl.getUsedAxes()
            for axis in used_axes:
                element = ctrl.getElementByAxis(axis)
                self._reconfig_element(element)
