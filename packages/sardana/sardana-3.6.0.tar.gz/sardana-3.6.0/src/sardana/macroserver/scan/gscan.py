...
class CScan(GScan):
    ...
    @property
    def apply_overshoot_correction(self):
        return self._apply_overshoot_correction

    @apply_overshoot_correction.setter
    def apply_overshoot_correction(self, value: bool):
        self._apply_overshoot_correction = value
...
def generate_timestamps(synch_description: TypingList[Dict], 
...
def generate_positions(labels: TypingList[str],
                       starts: np.ndarray,
                       finals: np.ndarray,
                       nb_interv: int,
                       do_last_point: bool = True
                       ) -> Dict[int, Dict[str, float]]:
    ret = dict()
...
class CTScan(CScan, CAcquisition):
    ...
    def _go_through_waypoints(self):
        ...
        for i, waypoint in waypoints:
            ...
            dt_timestamp = time.time() - self._env['startts']
            theoretical_timestamps = generate_timestamps(synch, dt_timestamp)

            # Update initial_data with theoretical positions and time
            for index, data in theoretical_positions.items():
                ...
                # the best practice
                self.data.initial_data[index + self._index_offset] = data
            ...
            try:
                ...
                if not macro.isStopped():
                    state = measurement_group.stateObj.read(cache=False).rvalue
...
class TScan(GScan, CAcquisition):
    ...
    def _create_synch_description(self, active_time, repeats, latency_time=0):
        delay_time = 0
        mg_latency_time = self.measurement_group.getLatencyTime(force=True)
    ...
    def scan_loop(self):
        ...
        theoretical_timestamps = \
            generate_timestamps(synch_description, dt_timestamp)
