...
class Parameterizable(object):
    ...
    def _build_parameter(self, param_def):
        ...
        for p in param_def:
            ...
            if isinstance(t, collections.abc.Sequence) and not isinstance(t, str):
                ret_p = {'min': 1, 'max': None}
                if isinstance(t[-1], collections.abc.Mapping):
                    ret_p.update(t[-1])
...
class MacroFunction(SardanaFunction, Parameterizable):
    ...
    def to_parameter_definition(self):
        param_def = []
        args, varargs, varkw, defaults, *_ = inspect.getfullargspec(self.function)
