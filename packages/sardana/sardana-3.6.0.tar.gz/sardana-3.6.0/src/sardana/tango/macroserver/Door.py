...
class Door(SardanaDevice):
    ...
    def GetMacroEnv(self, argin):
        """GetMacroEnv(<macro name> [, <env var. name>]) -> [<env var name>, <value>]
        """
        macro_name = argin[0]
        if len(argin) > 1:
            macro_env = argin[1:]
        else:
            macro_env = self.door.get_macro(macro_name).get_env()
        ret = []
        for var in macro_env:
            val = self.door.get_env(var, macro_name=macro_name)
            if val:
                enc = json.JSONEncoder(default=str)
                jsonval = enc.encode(val)
                ret.extend((var, jsonval))
        return ret
