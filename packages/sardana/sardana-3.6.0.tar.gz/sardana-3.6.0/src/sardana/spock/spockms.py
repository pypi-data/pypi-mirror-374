...
from taurus.core import TaurusEventType, TaurusDevState
...
class GUIViewer(BaseGUIViewer):
    ...
    def show_scan(self, scan_nb=None, scan_history_info=None, directory_map=None):
            ...
            if not isinstance(scan_file, str):
                ...
                for fname in scan_files:
                    if _is_h5_file_valid(os.path.join(scan_dir, fname)):
                        ...
                        bfile, ext = os.path.splitext(fname)
                        fname = "%s_%05d%s" % (bfile, scan["serialno"], ext)
                        if _is_h5_file_valid(os.path.join(scan_dir, fname)):
                            scan_file = fname
                            break
        ...
        locations = [scan_dir]
        local_file = None
        if directory_map is None or scan_dir not in directory_map:
        ...
        entry_names = [
            "entry%d" % scan["serialno"],
            "scan_%05d" % scan["serialno"],
            "scan",
            "entry",
            "entry_%05d" % scan["serialno"],
        ]
        data_names = [
            "measurement",
            "data",
        ...
        try:
            enames = getattr(
                sardanacustomsettings, 'NXENTRY_NAMES', [])
            if not isinstance(enames, list):
                enames = [str(enames)]
            enames = [nm.format(ScanID=scan["serialno"])
                      for nm in enames]
            entry_names = enames + entry_names
        except Exception as e:
            print(str(e))

        try:
            dtnames = getattr(
                sardanacustomsettings, 'NXDATA_NAMES', [])
            if not isinstance(dtnames, list):
                dtnames = [str(dtnames)]
            dtnames = [nm.format(ScanID=scan["serialno"])
                          for nm in dtnames]
            data_names = dtnames + data_names
        except Exception as e:
            print(str(e))
        ...
        for entry_name in entry_names:
            for data_name in data_names:
                measurement_name = "%s/%s" % (entry_name, data_name)
        ...
        if measurement_index is None:
            for entry_name in entry_names:
                try:
                    measurement_name = "%s" % (entry_name)
                    windowTitle = scan_file + "[" + entry_name + "]"
                    title_name = "%s/title" % entry_name
                    measurement_index = taurus_nexus_widget.findNodeIndex(
                        local_file, entry_name)
                    break
                except Exception:
                    measurement_index = None
        if measurement_index is None:
               ...
               not isinstance(e.args, str):
                ...
                if reason == 'MissingParam':
                    ...
                    self._updateState(self._old_sw_door_state,
                                      TaurusDevState.Undefined, silent=True)
                    print("Unable to run macro: No connection to door '%s'" %
                          self.getSimpleName())
                else:
