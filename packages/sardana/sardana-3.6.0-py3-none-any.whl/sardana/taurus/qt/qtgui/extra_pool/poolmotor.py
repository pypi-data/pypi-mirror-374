...
def eval_hw_limit(limit: Limit, value: Optional[bool]) -> LimitInfo:
...
def eval_sw_limit(limit: Limit, position_attr: taurus.core.tango.TangoAttribute) -> LimitInfo:
...
class PoolMotorTVReadWidget(TaurusWidget):
    ...
    def setFormat(self, format):
        """Reimplemented to call setFormat of the position label
        """
        TaurusWidget.setFormat(self, format)
        lbl_read = getattr(self, "lbl_read", None)
        if lbl_read is not None:
            lbl_read.setFormat(format)
    ...
    def _config_limit(self, limit: Limit, hw_lim: LimitInfo, sw_lim: Any) -> None:
