...
if jupyter_client is not None:
    jupyter_client_compat = True
    if Version(jupyter_client.__version__) > Version("6.1.12"):
        jupyter_client_compat = False

msg = ""
if qtconsole is None:
    msg += "qtconsole is not installed; "
if jupyter_client is None:
    msg += "jupyter_client is not installed; "
if not jupyter_client_compat:
    msg += "jupyter_client version ({}) is not compatible (must be <= 6.1.12)".format(
...
if not app:
    app = Qt.QApplication(["test_qtspock"])
