...
from sardana.pool.poolsynchronization import MultiSynchDescription
...
class PoolElement(BaseElement, TangoDevice):
    ...
    def waitFinish(self, timeout: Optional[float] = None, id: Optional[Sequence[float]] = None):
        ...
        try:
            # TODO: Temporary fix for Taurus < 5.2 (see !2027, #1982)
            # Multiple states in waitForEvent don't evaluate correctly with
            # equal=False. Update to the following line when Taurus >= 5.2
            # is set as a required dependency:
            # evt_wait.waitForEvent((DevState.MOVING, DevState.RUNNING),
            #    after=id, equal=False, timeout=timeout, reactivity=reactivity)
            evt_wait.waitForEvent((DevState.MOVING, ), after=id, equal=False,
                                  timeout=timeout, reactivity=reactivity)
            evt_wait.waitForEvent((DevState.RUNNING, ), after=id+0.00001, equal=False,
                                  timeout=timeout, reactivity=reactivity)
        finally:
...
class Controller(PoolElement):
    ...
    def getLastUsedAxis(self) -> Optional[int]:
...
class ExpChannel(PoolElement):
    ...
    def getValueRefObj(self) -> TangoAttributeEG:
    ...
    def go(self, *args: Any, **kwargs: Any) -> tuple:
...
class MGConfiguration(object):
    ...
    def _setCtrlsSynchronizer(self, synchronizer, ctrls=None, apply_cfg=True):
        """Set the synchronizer for the indicated controllers. In case of
        empty ctrls value it will be applied to all the controllers

        :param synchronizer: <str> string indicating the synchronizer name
...
class MeasurementGroup(PoolElement):
    ...
    def getSynchDescription(self) -> MultiSynchDescription:
        """Get the current :ref:`synchronization description <sardana-measurementgroup-api-synchdescription>`  of the measurement group.
        
        :return: MultiSynchDescription object (dictionary-like) that describes the synchronization 
          description associated to each synchronizer present in the measurement 
          group.
        """
        synch_description_json = self._getAttrValue('SynchDescription')
        synch_description = MultiSynchDescription.from_json(synch_description_json)
        return synch_description

    def setSynchDescription(self, synch_description: Union[List[Dict], Dict[str, List[Dict]]]) -> None:
        """Set the current :ref:`synchronization description <sardana-measurementgroup-api-synchdescription>` of the measurement group.
        
        :param synch_description: Either a single :class:`~sardana.pool.poolsynchronization.SynchDescription`
          represented as a list or a :class:`~sardana.pool.poolsynchronization.MultiSynchDescription` represented as a dictionary,
          where keys are the synchronizer names as a string and values are the corresponding
          :class:`~sardana.pool.poolsynchronization.SynchDescription` as a list.
        """
        codec = CodecFactory().getCodec('json')
    ...
    def setOutput(self, output: bool, *elements: str, apply: bool = True) -> None:
    ...
    def getLatencyTime(self, force=False):
        return self._getAttrValue('LatencyTime', force=force)
