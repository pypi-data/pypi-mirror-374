...
def memorize_write_attribute(write_attr_func: Callable) -> Callable:
    ...
    def write_attr_wrapper(self, attribute):
        ...
        if store is not None:
            ...
            if store_value:
                attr_values['__value'] = str(store[0])
...
def run(prepare_func, args=None, tango_util=None, start_time=None, mode=None,
    ...
    # Remove motor groups.
    # They are intended to be temporary, and will be re-created when needed.
    # Keeping them around indefinitely can cause problems when removing or renaming
    # elements, and there may eventually be some performance impact.
    from sardana import sardanacustomsettings
    if getattr(sardanacustomsettings, "CLEANUP_MOTOR_GROUPS"):
        if name in {"Pool", "Sardana"}:
            motor_groups = db.get_device_name("/".join(args[:2]), "MotorGroup")

            for motor_group in motor_groups:
                db.delete_device(motor_group)
