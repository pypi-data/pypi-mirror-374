...
from sardana.pool import AcqSynchType, AcqMode, SynchDomain, SynchParam
...
from sardana.pool.test.util import (ActionEvent, set_obj_attrs,
    get_marker_kwargs, get_marker_attrs)
...
class BaseAcquisition(object):
    ...
    def consecutive_acquisitions(self, pool, config, synch_description):
        ...
        self.pmg.set_configuration_from_user(mg_conf)
        # Set the synch_description again since the synchronizers
        # in the measurement group may have changed
        self.pmg.set_synch_description(synch_description)
        repetitions = 0
        for group in synch_description:
            repetitions += group[SynchParam.Repeats]
...
@pytest.fixture()
def mntgrp02(create_measurement_group, ct01, twod01, tg01, tg02, request):
    """Return a `~sardana.pool.poolmeasurementgroup.PoolMeasurementGroup` object
    handling counter timers `ct01` and `twod01` hardware synchronized by 
    `tg01` and `tg02` trigger gate synchronizers respectively.
    It resides in the pool object returned by the `pool` fixture.
    """
    kwargs = {"name": "mntgrp02"}
    kwargs.update(get_marker_kwargs(request))
    mntgrp = create_measurement_group([ct01, twod01], kwargs)
    config = [[('ct01', 'tg01', AcqSynchType.Trigger), ],
             [('twod01', 'tg02', AcqSynchType.Trigger), ]]
    pool = mntgrp.pool
    mg_conf, channel_ids, channel_names = \
            createMGUserConfiguration(pool, config)
    mntgrp.set_configuration_from_user(mg_conf)
...
def test_mntgrp_single_synch(mntgrp01):
    synch_description = SynchDescription(
        [
            {
                SynchParam.Delay: {SynchDomain.Time: 0},
                SynchParam.Active: {SynchDomain.Time: 0.1},
                SynchParam.Total: {SynchDomain.Time: 0.1},
                SynchParam.Repeats: 10,
            }
        ]
    )
    acq_event = ActionEvent(mntgrp01)
    mntgrp01.set_synch_description(synch_description)
    mntgrp01.prepare()
    mntgrp01.start_acquisition()
    acq_event.started.wait(1)            
    acq_event.done.wait(2.1)
    assert mntgrp01.state == State.On


def test_mntgrp_multi_synch(mntgrp02):
    multi_synch_description = {
        "tg01":[
            {
                SynchParam.Delay: {SynchDomain.Time: 0},
                SynchParam.Active: {SynchDomain.Time: 0.2},
                SynchParam.Total: {SynchDomain.Time: 0.3},
                SynchParam.Repeats: 10,
            }
        ],
        "tg02":[
            {
                SynchParam.Delay: {SynchDomain.Time: 0},
                SynchParam.Active: {SynchDomain.Time: 0.1},
                SynchParam.Total: {SynchDomain.Time: 0.5},
                SynchParam.Repeats: 14,
            }
        ]
    }
    acq_event = ActionEvent(mntgrp02)
    attr_listener = AttributeListener()
    ct01 = mntgrp02.get_element(name='ct01')
    twod01 = mntgrp02.get_element(name='twod01')
    ct01.add_listener(attr_listener.event_received)
    twod01.add_listener(attr_listener.event_received)
    ct01_event = ActionEvent(ct01)
    twod01_event = ActionEvent(twod01)
    mntgrp02.set_synch_description(multi_synch_description)
    mntgrp02.prepare()
    mntgrp02.start_acquisition()
    acq_event.started.wait(1)
    ct01_event.started.wait(1)
    ct01_event.done.wait(3.5)
    assert ct01.state == State.On      
    acq_event.done.wait(4.5)
    assert mntgrp02.state == State.On
    assert len(attr_listener.data["ct01"]) == 10
    assert len(attr_listener.data["twod01"]) == 14
