...
class DiscretePseudoMotorController(PseudoMotorController):
    ...
    Example of Sardana configuration
    (https://sardana-controls.org/users/configuration/index.html)::
    
      dpmctrl01:
        type: PseudoMotor
        python_class: DiscretePseudoMotorController
        python_module: DiscretePseudoMotorController.py
        physical_roles:
          ContinuousMoveable: mot03
        elements:
          discretepm01:
            axis: 1
            attributes:
              Configuration:
                OUT:  # label describing the discrete position
                  pos: 1  # discrete position
                  set: 3.0  # continuous position
                  min: 2.5  # minimum continuous position indicating the discrete position
                  max: 3.5  # maximum continuous position indicating the discrete position
                IN:
                  pos: -1
                  set: -3.0
                  min: -3.5
