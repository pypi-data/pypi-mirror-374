...
class PoolAcquisition(PoolAction):
    ...
    * 0D
        ...
        if name == "start":
            ...
            if self._sw_acq_args is not None:
                if not self._sw_acq._wait(timeout):
                    ...
                    args = list(self._sw_acq_args.args)
                    args.append(index)
            ...
            if self._0d_acq_args is not None:
                if not self._0d_acq._wait(timeout):
                    ...
                    args = list(self._0d_acq_args.args)
                    args.append(index)
    ...
    def prepare(self, config, acq_mode, value, multi_synch_description=None,
        ...
        hw_acq_params = []
        for synchronizer, current_synch_description in multi_synch_description.items():
            current_value = current_synch_description.integration_time
            repetitions = current_synch_description.repetitions
            latency = current_synch_description.passive_time
            ...
            ctrls = config.get_timerable_ctrls(acq_synch=acq_sync_hw, enabled=True, synchronizer=synchronizer)
            if len(ctrls) > 0:
                ctrls_hw = get_timerable_ctrls(ctrls, acq_mode)
                hw_acq_params.append((ctrls_hw, current_value, repetitions, latency))
                set_pars(ctrls)
                self._prepare_ctrls(ctrls_hw, current_value, repetitions, latency,
                                    nb_starts)

        if hw_acq_params:
            hw_args = [hw_acq_params]
            hw_kwargs = dict(kwargs)
            self._hw_acq_args = ActionArgs(hw_args, hw_kwargs)

        sw_synch_description = multi_synch_description.get("software")
        ...
        if len(ctrls) > 0:
            ...
            repetitions = sw_synch_description.repetitions
            latency = sw_synch_description.passive_time
        ...
        if len(ctrls) > 0:
            ...
            self._sw_start_acq_args = ActionArgs(sw_start_args,
                                                 sw_start_kwargs)
            set_pars(ctrls_sw_start)
            self._prepare_ctrls(ctrls_sw_start, value, repetitions, latency,
                                nb_starts)
        ...
        synch_args = (ctrls_synch, multi_synch_description)
        synch_kwargs = {
            'moveable': moveable,
        ...
        for synch in config.get_synchs(enabled=True):
            ...
            if moveable_on_input is None:
                continue
            # multiplexor mode
            elif isinstance(moveable_on_input, dict):
                if moveable is None:
                    input_id = None
                else:
                ...
                synch.controller.set_axis_par(
                    synch.axis,
                    "active_input",
                    input_id
                )
            # coupled mode
            elif moveable_on_input not in (moveable.full_name, moveable.name):
                raise PoolException(
                        "moveable {} is not configured on {} input".format(
                            moveable, synch.name)
                    )
    ...
    def add_element(self, element: sardana.pool.poolelement.PoolElement) -> None:
    ...
    def remove_element(self, element: sardana.pool.poolelement.PoolElement) -> None:
    ...
    def get_elements(self, copy_of: bool = False) -> Sequence[sardana.pool.poolelement.PoolElement]:
    ...
    def read_value(self, ret: Optional[Dict] = None, serial: bool = False) -> Dict[sardana.pool.poolelement.PoolElement, sardana.sardanavalue.SardanaValue]:
...
class PoolAcquisitionTimerable(PoolAcquisitionBase):
    ...
    def read_value_ref(self, ret: Optional[Dict] = None, serial: bool = False) -> Dict[sardana.pool.poolelement.PoolElement, Union[Any, Exception, None]]:
    ...
    def raw_read_value_ref(self, ret: Optional[Dict] = None, serial: bool = False) -> Dict[sardana.pool.poolelement.PoolElement, sardana.sardanavalue.SardanaValue]:
    ...
    def in_acquisition(self, states: Dict[PoolElement, State]) -> bool:
    ...
    @DebugIt()
    def start_action(self, ctrls: List[AcqController],
                     value: float,
                     master: Optional[AcqConfigurationItem],
                     repetitions: int,
                     latency: float,
                     index: Optional[int],
                     acq_sleep_time: Optional[float],
                     nb_states_per_value: Optional[int],
                     **kwargs) -> None:
        """
        ...
        :return:
        """
        self._set_acquisition_variables(index, acq_sleep_time, nb_states_per_value)
        self._sort_ctrls(ctrls, master)
        self._start_ctrls(ctrls, value, repetitions, latency, **kwargs)

    def _set_acquisition_variables(self, index: Optional[int],
                                   acq_sleep_time: Optional[float],
                                   nb_states_per_value: Optional[int]) -> None:
        pool = self.pool
        self._aborted = False
        self._stopped = False
    ...
    def _sort_ctrls(self, ctrls: List[AcqController],
                    master: Optional[AcqConfigurationItem] = None) -> None:
    ...
    def _start_ctrls(self, ctrls: List[AcqController], value: float,
                     repetitions: int, latency: float, **kwargs) -> None:
    ...
    def _set_pool_ctrl_dict_loop(self, ctrls: List[AcqController]):
        ctrl_channels = {}
        for ctrl in ctrls:
            pool_channels = []
            pool_ctrl = ctrl.element
        ...
        if self._pool_ctrl_dict_loop is None:
            self._pool_ctrl_dict_loop = ctrl_channels
        else:
            self._pool_ctrl_dict_loop.update(ctrl_channels)

    def _split_ctrl(self, ctrls: List[AcqController]):
        ctrl_channels_value = {}
        ctrl_channels_ref = {}
        for ctrl in ctrls:
        ...
        if self._pool_ctrl_dict_value is None:
            self._pool_ctrl_dict_value = ctrl_channels_value
        else:
            self._pool_ctrl_dict_value.update(ctrl_channels_value)

        if self._pool_ctrl_dict_ref is None:
            self._pool_ctrl_dict_ref = ctrl_channels_ref
        else:
            self._pool_ctrl_dict_ref.update(ctrl_channels_ref)
...
class PoolAcquisitionHardware(PoolAcquisitionTimerable):
    ...
    def start_action(self, acq_params: List[Any],
                     acq_sleep_time: Optional[float] = None, 
                     nb_states_per_value: Optional[int] = None,
                     **kwargs):
        """
        Prepares controllers as in PoolAcquisitionTimerable.start_action with 
        the difference that it does it for every synchronization inside acq_params
        :param acq_params: List of unique synchronization parameters.
        :param acq_sleep_time: sleep time between state queries
        :param nb_states_per_value: how many state queries between readouts
        :param kwargs:
        :return:
        """
        PoolAcquisitionTimerable._set_acquisition_variables(
            self, None, acq_sleep_time, nb_states_per_value
        )
        for ctrls, value, repetitions, latency in acq_params:            
            PoolAcquisitionTimerable._sort_ctrls(
                self, ctrls
            )
            PoolAcquisitionTimerable._start_ctrls(
                self, ctrls, value, repetitions, latency, **kwargs
            )
