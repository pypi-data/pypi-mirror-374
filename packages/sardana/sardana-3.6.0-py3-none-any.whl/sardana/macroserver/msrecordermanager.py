...
class RecorderManager(MacroServerManager):
    ...
    def getRecorderLibs(self, filter=None):
        if filter is None:
            return self._modules
        expr = re.compile(filter, re.IGNORECASE)
        ret = {}
        for name, recorder_lib in self._modules.items():
            if expr.match(name) is None:
                continue
            ret[name] = recorder_lib
        return ret

    def getRecorderLib(self, name):
        ...
        if ret is None:
            raise UnknownRecorderLibrary("Unknown recorder library %s" % module_name)
        return ret

    def getRecorderMetaClass(self, recorder_name: str) -> RecorderClass:
    ...
    def getRecorderMetaClasses(self, filter: Optional[Type[DataRecorder]] = None,
                               extension: Optional[str] = None
                               ) -> Dict[str, Type[RecorderClass]]:
    ...
    def getRecorderClasses(self, filter: Optional[Type[DataRecorder]] = None,
                           extension: Optional[str] = None) -> Dict[str, Callable[[DataRecorder], Any]]:
    ...
    def getRecorderClass(self, klass_name: str) -> Optional[Type[DataRecorder]]:
        """ Return the Recorder class for the given class name.
        :param klass_name: Name of the recorder class.
        :return:  a :obj:`class` class of recorder or None if it does not exist
        """
