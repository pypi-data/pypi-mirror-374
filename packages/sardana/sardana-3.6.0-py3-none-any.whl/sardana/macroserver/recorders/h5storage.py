    ...
    def _createPreScanSnapshot(self, env):
        ...
        for dd in self.preScanSnapShot:
            ...
            if dtype in self.supported_dtypes:
                _snap.create_dataset(
                    label,
                    data=pre_scan_value,
                    compression=self._compression(dd.shape)
                )
                # link to this dataset also from the measurement group
                # if enabled in the sardanacustomsettings
                enable_softlinks = getattr(
                    sardanacustomsettings,
                    'NXSCANH5_RECORDER_LINK_PRE_SCAN_SNAPSHOT',
                    False
                    )
                if enable_softlinks and (label not in meas_keys):
                    pre_scan_path = posixpath.join('/',
                                                   self.entryname,
                                                   'measurement',
                                                   'pre_scan_snapshot',
                                                   label)
                    _meas[label] = h5py.SoftLink(pre_scan_path)
            else:
