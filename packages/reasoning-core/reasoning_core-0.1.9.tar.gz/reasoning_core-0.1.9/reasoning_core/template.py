...
class Config:
    ...
    """
    c: float = 1.0
    # CHANGE 1: `_level` is now `level` so it's part of the __init__ signature.
    level: int = 0
    ...
    def __post_init__(self):
        ...
        # CHANGE 2: Exclude 'level' from stochastic rounding.
        self._stochastic_fields = {
            f.name for f in fields(self) 
            if f.type is int and not f.name.startswith('_') and f.name != 'level'
        }
        ...
        # CHANGE 3: Apply updates if initialized with level > 0.
        if self.level > 0:
