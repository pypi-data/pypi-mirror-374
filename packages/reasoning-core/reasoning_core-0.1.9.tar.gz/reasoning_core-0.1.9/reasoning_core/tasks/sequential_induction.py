...
class SequentialInduction(Task):
    def __init__(self, config=SequenceConfig()):
        ...
        self.filters = [lambda s: filter_2_outof(s, length_check=self.config.n_visible_terms),
                        lambda s: filter_max_terms_len(s,max_terms_len=self.config.max_terms_len,
    ...
    def generate(self) -> Problem:
        ...
        S = Sequence(formula)
        while not all([ self.filters[i](S, length_check = self.config.n_visible_terms) for i in range(len(self.filters)) ]): #keep generating until the sequence fulfill all requirements (filters)
            formula = self.one_shot_sympy_generate()
            S = Sequence(formula)
