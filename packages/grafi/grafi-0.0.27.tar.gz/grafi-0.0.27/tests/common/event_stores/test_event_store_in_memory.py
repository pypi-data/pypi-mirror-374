...
class TestEventStoreInMemory:
    ...
    def test_record_single_event(self, event_store, sample_event):
    ...
    def test_record_multiple_events_separately(self, event_store, multiple_events):
    ...
    def test_record_events_batch(self, event_store, multiple_events):
        """Test recording multiple events in batch."""
        event_store.record_events(multiple_events)
    ...
    def test_get_event_by_id(self, event_store, sample_event):
        """Test getting an event by ID."""
        event_store.record_event(sample_event)

        retrieved_event = event_store.get_event(sample_event.event_id)
        assert retrieved_event is not None
        assert retrieved_event.event_id == sample_event.event_id
        assert retrieved_event is sample_event

    def test_get_event_by_nonexistent_id(self, event_store):
        """Test getting an event by non-existent ID."""
        result = event_store.get_event("nonexistent-id")
        assert result is None

    def test_get_conversation_events(self, event_store, multiple_events):
        """Test filtering events by conversation ID."""
        for event in multiple_events:
            event_store.record_event(event)

        # Get events for conv-0
        conv_0_events = event_store.get_conversation_events("conv-0")
        ...
        conv_1_events = event_store.get_conversation_events("conv-1")
        ...
        no_events = event_store.get_conversation_events("conv-999")
        assert no_events == []

    def test_get_agent_events(self, event_store, multiple_events):
        """Test filtering events by assistant request ID."""
        for event in multiple_events:
            event_store.record_event(event)

        # Get events for assist-0
        assist_0_events = event_store.get_agent_events("assist-0")
        ...
        assist_1_events = event_store.get_agent_events("assist-1")
        ...
        assist_2_events = event_store.get_agent_events("assist-2")
    ...
    def test_get_topic_events(self, event_store, topic_events):
        """Test filtering events by topic name and offsets."""
        for event in topic_events:
            event_store.record_event(event)

        # Get events for topic-0 with specific offsets
        topic_0_events = event_store.get_topic_events("topic-0", [0, 2])
        ...
        topic_1_events = event_store.get_topic_events("topic-1", [1])
        ...
        no_events = event_store.get_topic_events("non-existent", [0])
        assert no_events == []

    def test_clear_events(self, event_store, multiple_events):
        """Test clearing all events from the store."""
        for event in multiple_events:
            event_store.record_event(event)

        assert len(event_store.get_events()) == 5
        event_store.clear_events()
        assert len(event_store.get_events()) == 0

    def test_event_persistence_in_memory(self, event_store, sample_event):
        ...
        event_store.record_event(another_event)
    ...
    def test_empty_filters(self, event_store):
        """Test filtering on empty event store."""
        assert event_store.get_conversation_events("any") == []
        assert event_store.get_agent_events("any") == []
        assert event_store.get_topic_events("any", [0]) == []
        assert event_store.get_event("any") is None

    def test_event_ordering_preserved(self, event_store):
    ...
    def test_get_events_returns_copy(self, event_store, sample_event):
        """Test that get_events returns a copy, not the original list."""
        event_store.record_event(sample_event)

        events1 = event_store.get_events()
        events2 = event_store.get_events()
    ...
    def test_mixed_event_types(self, event_store, sample_event, topic_events):
        """Test handling mixed event types."""
        # Record both regular events and topic events
        event_store.record_event(sample_event)
        event_store.record_events(topic_events)
