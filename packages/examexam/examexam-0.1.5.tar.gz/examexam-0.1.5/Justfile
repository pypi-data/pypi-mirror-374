...
# Run safety check
safety:
    @echo "Not running safety check"
#    export -f requirements.txt --output requirements.txt --without-hashes
...
pylint: isort black
    ...
    {{venv}} pylint {{project}} --rcfile=.pylintrc --fail-under 9.5 --ignore-paths=test_TODO
    {{venv}} pylint scripts --rcfile=.pylintrc_scripts --fail-under 8.5 --ignore-paths=test_TODO
    {{venv}} pylint {{test_folder}} --rcfile=.pylintrc_tests --fail-under 9.5 --ignore-paths=test_TODO
