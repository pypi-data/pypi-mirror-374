"""
Serialization utilities for kgforge objects.
"""
...
class KGForgeJSONEncoder(json.JSONEncoder):
    """Custom JSON encoder for kgforge objects."""
...
def serialize_resource(resource: Resource, format: str = "json") -> Union[str, bytes]:
    ...
    if format.lower() == "json":
        return json.dumps(resource.to_dict(), cls=KGForgeJSONEncoder, ensure_ascii=False, indent=2)
...
def deserialize_resource(data: Union[str, bytes], format: str = "json") -> Resource:
    ...
    try:
        ...
        raise KGForgeError(f"Failed to deserialize resource: {e}")


def serialize_dataset(dataset: Dataset, format: str = "json") -> Union[str, bytes]:
    """Serialize a dataset to various formats."""
    if format.lower() == "json":
        return json.dumps(dataset.to_dict(), cls=KGForgeJSONEncoder, ensure_ascii=False, indent=2)
    elif format.lower() == "pickle":
...
def deserialize_dataset(data: Union[str, bytes], format: str = "json") -> Dataset:
    ...
    try:
        ...
        raise KGForgeError(f"Failed to deserialize dataset: {e}")
...
def save_to_file(obj: Union[Resource, Dataset], path: Union[str, Path], format: str = "auto") -> None:
    ...
    try:
        ...
        raise KGForgeError(f"Failed to save to file: {e}")
...
def load_from_file(path: Union[str, Path], obj_type: str, format: str = "auto") -> Union[Resource, Dataset]:
    ...
    try:
        ...
        raise KGForgeError(f"Failed to load from file: {e}")
