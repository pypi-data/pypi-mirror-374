"""
Unit tests for core kgforge components.
"""
...
from kgforge.core.context import Context
from kgforge.core.forge import KGForge
from kgforge.core.exceptions import ValidationError, KGForgeError
...
class TestKGForge:
    """Test KGForge main class functionality."""
    
    def test_initialization(self):
        """Test KGForge initialization."""
        kg = KGForge(backend="networkx")
        assert kg.context.graph.backend == "networkx"
    
    def test_graph_lifecycle(self, kg_forge):
        """Test complete graph lifecycle."""
        # Create graph
        dataset = kg_forge.create_graph("test-graph", description="Test graph")
        assert dataset.name == "test-graph"
        assert kg_forge.current_graph is not None
        ...
        added_resource = kg_forge.add_resource(resource)
        assert added_resource.id == resource.id
        
        # Query
        retrieved = kg_forge.get_resource(resource.id)
        assert retrieved is not None
        assert retrieved.id == resource.id
        
        # Add edge
        resource2 = kg_forge.add_resource(Resource(labels={"TestEntity2"}))
        edge = kg_forge.add_edge(resource, resource2, "RELATES_TO")
        assert edge.source == resource.id
        assert edge.target == resource2.id
        
        # Statistics
        stats = kg_forge.stats()
