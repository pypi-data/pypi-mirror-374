...
from kgops.utils.data_ingestion import FileIngester, create_resource_from_dataframe, create_resources_from_dataframe_rows
...
class KGOps:
    ...
    def ingest_file(self, 
        ...
        """
        Ingest data from various file formats and optionally add to the current graph.
        ...
        Args:
            ...
            add_to_graph: If True, automatically add resources to current graph
        ...
        Raises:
            ValidationError: If no active graph and add_to_graph is True
            KGOpsError: If ingestion fails
        """
        if add_to_graph and self._current_dataset is None:
            raise ValidationError("No active graph. Create or load a graph first, or set add_to_graph=False.")
        
        try:
            ingester = FileIngester()
            resources = ingester.ingest_file(
                file_path=file_path,
                labels=labels,
            ...
            self.logger.info(f"Ingested {len(resources)} resources from {file_path}")
            return resources
            
        except Exception as e:
            raise KGOpsError(f"Failed to ingest file {file_path}: {e}")
    ...
    def ingest_csv(self, 
                  ...
                  **kwargs) -> List[Resource]:
        """
        Convenience method to ingest CSV files.
        
        Args:
            file_path: Path to CSV file
            labels: Labels to apply to resources
            resource_per_row: If True, create one resource per row
            id_column: Column to use as resource ID
            label_column: Column containing labels
            property_mapping: Dict to rename columns
            add_to_graph: If True, add resources to current graph
    ...
    def ingest_json(self,
                   ...
                   **kwargs) -> List[Resource]:
        """
        Convenience method to ingest JSON files.
        
        Args:
            file_path: Path to JSON file
            labels: Labels to apply to resources
            resource_per_row: If True and JSON is array, create one resource per item
            id_column: Field to use as resource ID
            label_column: Field containing labels
            property_mapping: Dict to rename fields
            add_to_graph: If True, add resources to current graph
    ...
    def ingest_dataframe(self,
        ...
        Args:
            ...
            property_mapping: Dict to rename columns
            add_to_graph: If True, add resources to current graph
            **kwargs: Additional properties to add to resources
        ...
        try:
            if resource_per_row:
                resources = create_resources_from_dataframe_rows(
                    df=df,
                    labels=labels,
                    id_column=id_column,
                    label_column=label_column,
                ...
                resource = create_resource_from_dataframe(
                    df=df,
                    labels=labels,
                    **kwargs
                )
            ...
            self.logger.info(f"Ingested {len(resources)} resources from DataFrame")
            return resources
            
        except Exception as e:
            raise KGOpsError(f"Failed to ingest DataFrame: {e}")
