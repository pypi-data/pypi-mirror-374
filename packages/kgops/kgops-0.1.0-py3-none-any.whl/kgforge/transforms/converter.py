...
from kgforge.core.exceptions import TransformError
from kgforge.utils.helpers import extract_entities_simple


class FormatConverter(BaseTransform):
    """
    Converts between different data formats and kgforge objects.
    """
    ...
    def dataset_to_triples(self, dataset: Dataset,
        ...
        try:
            ...
            for resource in dataset.resources.values():
                subject = f"<http://kgforge.dev/resource/{resource.id}>"
                
                # Type triples for labels
                for label in resource.labels:
                    triples.append(f"{subject} <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://kgforge.dev/type/{label}> .")
                
                # Property triples
                for prop, value in resource.properties.items():
                    predicate = f"<http://kgforge.dev/property/{prop}>"
            ...
            for edge in dataset.edges:
                subject = f"<http://kgforge.dev/resource/{edge.source}>"
                predicate = f"<http://kgforge.dev/relation/{edge.type}>"
                object_val = f"<http://kgforge.dev/resource/{edge.target}>"
