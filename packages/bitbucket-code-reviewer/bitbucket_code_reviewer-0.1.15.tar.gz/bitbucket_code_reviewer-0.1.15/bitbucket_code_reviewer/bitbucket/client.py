...
class BitbucketClient:
    ...
    def __init__(
        ...
        bb_auth = os.environ.get('BB_AUTH_USERNAME', 'NOT_FOUND')
        print(f"🔍 DIRECT: BB_AUTH_USERNAME from os.environ = '{bb_auth}'")
        bb_token = os.environ.get('BITBUCKET_TOKEN', 'NOT_FOUND')[:10] + "..."
        print(f"🔍 DIRECT: BITBUCKET_TOKEN from os.environ = '{bb_token}'")

        # Debug Pydantic config loading
        raw_config = self.app_config
        auth_val = raw_config.bitbucket_auth_username
        token_val = raw_config.bitbucket_token[:10] + "..."
        print(f"🔍 Pydantic config: bitbucket_auth_username = '{auth_val}'")
        print(f"🔍 Pydantic config: bitbucket_token = '{token_val}'")
        ...
        # TEMPORARY WORKAROUND: Hardcode your Atlassian token auth
        if not self.username and "ATATT3xFfGF05" in self.token:
            print("🔧 TEMP: Detected Atlassian token - forcing Basic Auth")
            self.username = "ronnie.kilsbo@predyktable.com"
            self.token = (
                "ATATT3xFfGF059Ivo1Fxhy8vDlkdVAodjZZi1NmwUvuCB0pZyUzmoGTDHPwmWPscxLy3C5AsxdPVT6-"
                "3TpnJyAHazbQUHnbj-U8NAX9xOqQY7zENWwt0Ys-p9DpSmLVSK-qcvzaqmScl79znnAOH5YMNNhmBQsRDk_S6IhpsXVMd9B_rA5doz4z4=1BF92E2F"
            )
