...
class AppConfig(BaseSettings):
    ...
    bitbucket_token: str = Field(..., description="Bitbucket Repository Access Token")
    ...
    default_model_name: str = Field(default="gpt-4", description="Default model name")
    default_temperature: float = Field(
        default=0.1, description="Default temperature for LLM"
    )
...
def create_review_config(
    ...
    # Use CLI values if provided, otherwise fall back to environment/defaults
    config = ReviewConfig(
        llm_provider=llm_provider or app_config.default_llm_provider,
        model_name=model_name or app_config.default_model_name,
        temperature=temperature or app_config.default_temperature,
        max_tokens=max_tokens,
        language=language,
        max_tool_iterations=max_tool_iterations or 10,
...
def _validate_api_keys(llm_provider: LLMProvider, app_config: AppConfig) -> None:
    ...
    if llm_provider == LLMProvider.GROK and not app_config.grok_api_key:
        raise ValueError(
...
def get_app_config() -> AppConfig:
    ...
    Returns:
        AppConfig object with environment variables loaded
    """
    return AppConfig()
