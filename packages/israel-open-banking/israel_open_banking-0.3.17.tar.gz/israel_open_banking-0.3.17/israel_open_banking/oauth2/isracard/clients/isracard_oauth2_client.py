...
from israel_open_banking.oauth2.isracard.clients.isracard_oauth2_manager import IsracardOAuth2Manager
from israel_open_banking.oauth2.isracard.services.isracard_auth_service import IsracardAuthService
...
class IsracardOAuth2Client(OAuth2BaseClient):
    """PSD2‑compliant AIS adapter for Isracard Bank."""

    def __init__(self, service: IsracardAuthService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: IsracardAuthService = service

        # plug‑in concrete managers
        self.oauth2: IsracardOAuth2Manager = IsracardOAuth2Manager(self)

    def refresh_access_token(self) -> TokenResponse:
        """Retrieves the refresh token for the Isracard Bank API."""
        return self.oauth2.refresh_access_token()

    # Expose the raw service for internal helpers ---------------------------------
    @property
    def svc(self) -> IsracardAuthService:
        return self._svc
