...
from israel_open_banking.psd2_savings_and_loans.loans.pepper.clients.pepper_loans_account_manager import \
    PepperLoansAccountManager
from israel_open_banking.psd2_savings_and_loans.loans.pepper.clients.pepper_loans_balance_manager import \
    PepperLoansBalanceManager
from israel_open_banking.psd2_savings_and_loans.loans.pepper.clients.pepper_loans_transaction_manager import \
    PepperLoansTransactionManager
from israel_open_banking.psd2_savings_and_loans.loans.pepper.services.pepper_loans_bank_service import \
    PepperBankLoansService
...
class PepperAISLoansClient(AISLoansBaseClient):
    """PSD2‑compliant AIS adapter for Pepper Bank."""

    def __init__(self, service: PepperBankLoansService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: PepperBankLoansService = service

        # plug‑in concrete managers
        self.accounts: PepperLoansAccountManager = PepperLoansAccountManager(self)
        self.transactions: PepperLoansTransactionManager = PepperLoansTransactionManager(self)
        self.balances: PepperLoansBalanceManager = PepperLoansBalanceManager(self)
    ...
    @property
    def svc(self) -> PepperBankLoansService:
        return self._svc
