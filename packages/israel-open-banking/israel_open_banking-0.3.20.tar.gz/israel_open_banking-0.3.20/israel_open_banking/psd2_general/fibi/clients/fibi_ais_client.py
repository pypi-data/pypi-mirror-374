...
from israel_open_banking.core.ais import (
    ...
    TransactionDetails,
    BookingStatus, )
from israel_open_banking.psd2_general.fibi.clients.fibi_account_manager import FibiAccountManager
from israel_open_banking.psd2_general.fibi.clients.fibi_balance_manager import FibiBalanceManager
from israel_open_banking.psd2_general.fibi.clients.fibi_transaction_manager import FibiTransactionManager


class FibiAISClient(AISBaseClient):
    """PSD2‑compliant AIS adapter for Fibi Bank."""

    def __init__(self, service: FibiBankService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: FibiBankService = service

        # plug‑in concrete managers
        self.accounts: FibiAccountManager = FibiAccountManager(self)
        self.transactions: FibiTransactionManager = FibiTransactionManager(self)
        self.balances: FibiBalanceManager = FibiBalanceManager(self)
