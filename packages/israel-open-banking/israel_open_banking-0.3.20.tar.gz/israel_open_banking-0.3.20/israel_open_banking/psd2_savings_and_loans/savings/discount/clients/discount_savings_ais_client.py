...
from israel_open_banking.psd2_savings_and_loans.savings.discount.clients.discount_savings_account_manager import \
    DiscountSavingsAccountManager
from israel_open_banking.psd2_savings_and_loans.savings.discount.clients.discount_savings_balance_manager import \
    DiscountSavingsBalanceManager
from israel_open_banking.psd2_savings_and_loans.savings.discount.clients.discount_savings_transaction_manager import \
    DiscountSavingsTransactionManager
from israel_open_banking.psd2_savings_and_loans.savings.discount.services.discount_savings_bank_service import \
    DiscountBankSavingsService
...
class DiscountAISSavingsClient(AISSavingsBaseClient):
    """PSD2‑compliant AIS adapter for Discount Bank."""

    def __init__(self, service: DiscountBankSavingsService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: DiscountBankSavingsService = service

        # plug‑in concrete managers
        self.accounts: DiscountSavingsAccountManager = DiscountSavingsAccountManager(self)
        self.transactions: DiscountSavingsTransactionManager = DiscountSavingsTransactionManager(self)
        self.balances: DiscountSavingsBalanceManager = DiscountSavingsBalanceManager(self)
    ...
    @property
    def svc(self) -> DiscountBankSavingsService:
        return self._svc
