...
from israel_open_banking.psd2_savings_and_loans.loans.isracard.clients.isracard_loans_account_manager import \
    IsracardLoansAccountManager
from israel_open_banking.psd2_savings_and_loans.loans.isracard.clients.isracard_loans_balance_manager import \
    IsracardLoansBalanceManager
from israel_open_banking.psd2_savings_and_loans.loans.isracard.clients.isracard_loans_transaction_manager import \
    IsracardLoansTransactionManager
from israel_open_banking.psd2_savings_and_loans.loans.isracard.services.isracard_loans_bank_service import \
    IsracardLoansService
...
class IsracardAISLoansClient(AISLoansBaseClient):
    """PSD2‑compliant AIS adapter for Isracard Bank."""

    def __init__(self, service: IsracardLoansService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: IsracardLoansService = service

        # plug‑in concrete managers
        self.accounts: IsracardLoansAccountManager = IsracardLoansAccountManager(self)
        self.transactions: IsracardLoansTransactionManager = IsracardLoansTransactionManager(self)
        self.balances: IsracardLoansBalanceManager = IsracardLoansBalanceManager(self)
    ...
    @property
    def svc(self) -> IsracardLoansService:
        return self._svc
