...
from israel_open_banking.psd2_savings_and_loans.savings.leumi.clients.leumi_savings_account_manager import \
    LeumiSavingsAccountManager
from israel_open_banking.psd2_savings_and_loans.savings.leumi.clients.leumi_savings_balance_manager import \
    LeumiSavingsBalanceManager
from israel_open_banking.psd2_savings_and_loans.savings.leumi.clients.leumi_savings_transaction_manager import \
    LeumiSavingsTransactionManager
from israel_open_banking.psd2_savings_and_loans.savings.leumi.services.leumi_savings_bank_service import \
    LeumiBankSavingsService
...
class LeumiAISSavingsClient(AISSavingsBaseClient):
    """PSD2‑compliant AIS adapter for Leumi Bank."""

    def __init__(self, service: LeumiBankSavingsService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: LeumiBankSavingsService = service

        # plug‑in concrete managers
        self.accounts: LeumiSavingsAccountManager = LeumiSavingsAccountManager(self)
        self.transactions: LeumiSavingsTransactionManager = LeumiSavingsTransactionManager(self)
        self.balances: LeumiSavingsBalanceManager = LeumiSavingsBalanceManager(self)
    ...
    @property
    def svc(self) -> LeumiBankSavingsService:
        return self._svc
