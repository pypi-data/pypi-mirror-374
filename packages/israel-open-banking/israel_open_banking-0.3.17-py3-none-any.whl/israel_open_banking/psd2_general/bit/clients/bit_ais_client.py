...
from israel_open_banking.core.ais import (
    ...
    TransactionDetails,
    BookingStatus, )
from israel_open_banking.psd2_general.bit.clients.bit_account_manager import BitAccountManager
from israel_open_banking.psd2_general.bit.clients.bit_balance_manager import BitBalanceManager
from israel_open_banking.psd2_general.bit.clients.bit_transaction_manager import BitTransactionManager


class BitAISClient(AISBaseClient):
    """PSD2‑compliant AIS adapter for Bit Bank."""

    def __init__(self, service: BitService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: BitService = service

        # plug‑in concrete managers
        self.accounts: BitAccountManager = BitAccountManager(self)
        self.transactions: BitTransactionManager = BitTransactionManager(self)
        self.balances: BitBalanceManager = BitBalanceManager(self)
