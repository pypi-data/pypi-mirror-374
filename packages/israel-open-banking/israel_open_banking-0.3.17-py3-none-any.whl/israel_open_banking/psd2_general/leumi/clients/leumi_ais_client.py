...
from israel_open_banking.psd2_general.leumi.services.leumi_bank_service import LeumiBankService
...
from israel_open_banking.core.ais import (
    ...
    TransactionDetails,
    BookingStatus, )
from israel_open_banking.psd2_general.leumi.clients.leumi_account_manager import LeumiAccountManager
from israel_open_banking.psd2_general.leumi.clients.leumi_balance_manager import LeumiBalanceManager
from israel_open_banking.psd2_general.leumi.clients.leumi_transaction_manager import LeumiTransactionManager


class LeumiAISClient(AISBaseClient):
    """PSD2‑compliant AIS adapter for Leumi Bank."""

    def __init__(self, service: LeumiBankService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: LeumiBankService = service

        # plug‑in concrete managers
        self.accounts: LeumiAccountManager = LeumiAccountManager(self)
        self.transactions: LeumiTransactionManager = LeumiTransactionManager(self)
        self.balances: LeumiBalanceManager = LeumiBalanceManager(self)
