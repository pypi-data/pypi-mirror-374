...
from israel_open_banking.core.api_endpoints import LOAN_ACCOUNT_TRANSACTIONS_ENDPOINT, LOAN_ACCOUNTS_ENDPOINT, \
    LOAN_ACCOUNT_BY_ID_ENDPOINT, \
...
class FibiBankLoansService:
    def __init__(self, consent_id: str, access_token: str, psu_id: str, aspsp: Aspsp) -> None:
        ...
        self.ca_cert = config.FIBI_CA_CERT_PATH
        self.url_builder = BankUrlBuilder(
            api_base_url=config.FIBI_SAVINGS_AND_LOANS_API_URL
        )
    ...
    def _signed_get(self, endpoint: str, params: Optional[dict] = None) -> requests.Response:
        ...
        # 3. Sign the request
        headers, _ = build_headers(self.aspsp,
                                   {},
                                   extra_headers={
                                       "psu_id": self.psu_id,
                                       ...
                                       "psu_ip": '127.0.0.1',
                                       "psu_id_type": self._get_psu_id_type(self.aspsp),
                                   })
    ...
    def _get_psu_id_type(self, aspsp: Aspsp) -> str:
        if aspsp == Aspsp.MASSAD:
            return 'MASSAD'
        elif aspsp == Aspsp.FIBI_OTZAR:
            return 'FIBI-OTZAR'
        elif aspsp == Aspsp.FIBI_UBANK:
            return 'FIBI-UBANK'
        elif aspsp == Aspsp.FIBI_BNL:
            return 'FIBI-BNL'
        elif aspsp == Aspsp.FIBI_PAGI:
            return 'FIBI-PAGI'
