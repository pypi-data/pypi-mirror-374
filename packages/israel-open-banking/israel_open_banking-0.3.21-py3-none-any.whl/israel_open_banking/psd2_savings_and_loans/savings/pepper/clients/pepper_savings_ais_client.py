...
from israel_open_banking.psd2_savings_and_loans.savings.pepper.clients.pepper_savings_account_manager import \
    PepperSavingsAccountManager
from israel_open_banking.psd2_savings_and_loans.savings.pepper.clients.pepper_savings_balance_manager import \
    PepperSavingsBalanceManager
from israel_open_banking.psd2_savings_and_loans.savings.pepper.clients.pepper_savings_transaction_manager import \
    PepperSavingsTransactionManager
from israel_open_banking.psd2_savings_and_loans.savings.pepper.services.pepper_savings_bank_service import \
    PepperBankSavingsService
...
class PepperAISSavingsClient(AISSavingsBaseClient):
    """PSD2‑compliant AIS adapter for Pepper Bank."""

    def __init__(self, service: PepperBankSavingsService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: PepperBankSavingsService = service

        # plug‑in concrete managers
        self.accounts: PepperSavingsAccountManager = PepperSavingsAccountManager(self)
        self.transactions: PepperSavingsTransactionManager = PepperSavingsTransactionManager(self)
        self.balances: PepperSavingsBalanceManager = PepperSavingsBalanceManager(self)
    ...
    @property
    def svc(self) -> PepperBankSavingsService:
        return self._svc
