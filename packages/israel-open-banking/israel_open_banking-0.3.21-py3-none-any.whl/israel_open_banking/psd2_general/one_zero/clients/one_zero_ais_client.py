...
from israel_open_banking.psd2_general.one_zero.services.one_zero_bank_service import OneZeroBankService

"""OneZero‑Bank concrete implementation of the PSD2 AIS abstraction layer.

This file wires the bank‑specific HTTP wrapper (``OneZeroBankService``)
...
from israel_open_banking.core.ais import (
    ...
    TransactionDetails,
    BookingStatus, )
from israel_open_banking.psd2_general.one_zero.clients.one_zero_account_manager import OneZeroAccountManager
from israel_open_banking.psd2_general.one_zero.clients.one_zero_balance_manager import OneZeroBalanceManager
from israel_open_banking.psd2_general.one_zero.clients.one_zero_transaction_manager import OneZeroTransactionManager


class OneZeroAISClient(AISBaseClient):
    """PSD2‑compliant AIS adapter for OneZero Bank."""

    def __init__(self, service: OneZeroBankService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: OneZeroBankService = service

        # plug‑in concrete managers
        self.accounts: OneZeroAccountManager = OneZeroAccountManager(self)
        self.transactions: OneZeroTransactionManager = OneZeroTransactionManager(self)
        self.balances: OneZeroBalanceManager = OneZeroBalanceManager(self)
    ...
    @property
    def svc(self) -> OneZeroBankService:
        return self._svc
