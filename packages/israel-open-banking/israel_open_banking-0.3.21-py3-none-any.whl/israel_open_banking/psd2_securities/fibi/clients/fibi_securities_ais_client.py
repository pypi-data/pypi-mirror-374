...
from israel_open_banking.core.ais.securities.models import SecuritiesAccountList, SecuritiesAccountDetails, \
    ...
    SecuritiesOrder
from israel_open_banking.psd2_securities.fibi.clients.fibi_securities_account_manager import \
    FibiSecuritiesAccountManager
from israel_open_banking.psd2_securities.fibi.clients.fibi_securities_order_manager import \
    FibiSecuritiesOrderManager
from israel_open_banking.psd2_securities.fibi.clients.fibi_securities_position_manager import \
    FibiSecuritiesPositionManager
from israel_open_banking.psd2_securities.fibi.clients.fibi_securities_transaction_manager import \
    FibiSecuritiesTransactionManager
from israel_open_banking.psd2_securities.fibi.services.fibi_securities_bank_service import \
    FibiBankSecuritiesService
...
class FibiAISSecuritiesClient(AISSecuritiesBaseClient):
    """PSD2‑compliant AIS adapter for Fibi Bank."""

    def __init__(self, service: FibiBankSecuritiesService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: FibiBankSecuritiesService = service

        # plug‑in concrete managers
        self.accounts: FibiSecuritiesAccountManager = FibiSecuritiesAccountManager(self)
        self.positions: FibiSecuritiesPositionManager = FibiSecuritiesPositionManager(self)
        self.transactions: FibiSecuritiesTransactionManager = FibiSecuritiesTransactionManager(self)
        self.orders: FibiSecuritiesOrderManager = FibiSecuritiesOrderManager(self)
    ...
    @property
    def svc(self) -> FibiBankSecuritiesService:
        return self._svc
