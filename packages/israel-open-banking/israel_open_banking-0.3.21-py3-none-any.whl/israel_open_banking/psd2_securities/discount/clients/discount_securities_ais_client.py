...
from israel_open_banking.core.ais.securities.models import SecuritiesAccountList, SecuritiesAccountDetails, \
    ...
    SecuritiesOrder
from israel_open_banking.psd2_securities.discount.clients.discount_securities_account_manager import \
    DiscountSecuritiesAccountManager
from israel_open_banking.psd2_securities.discount.clients.discount_securities_order_manager import \
    DiscountSecuritiesOrderManager
from israel_open_banking.psd2_securities.discount.clients.discount_securities_position_manager import \
    DiscountSecuritiesPositionManager
from israel_open_banking.psd2_securities.discount.clients.discount_securities_transaction_manager import \
    DiscountSecuritiesTransactionManager
from israel_open_banking.psd2_securities.discount.services.discount_securities_bank_service import \
    DiscountBankSecuritiesService
...
class DiscountAISSecuritiesClient(AISSecuritiesBaseClient):
    """PSD2‑compliant AIS adapter for Discount Bank."""

    def __init__(self, service: DiscountBankSecuritiesService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: DiscountBankSecuritiesService = service

        # plug‑in concrete managers
        self.accounts: DiscountSecuritiesAccountManager = DiscountSecuritiesAccountManager(self)
        self.positions: DiscountSecuritiesPositionManager = DiscountSecuritiesPositionManager(self)
        self.transactions: DiscountSecuritiesTransactionManager = DiscountSecuritiesTransactionManager(self)
        self.orders: DiscountSecuritiesOrderManager = DiscountSecuritiesOrderManager(self)
    ...
    @property
    def svc(self) -> DiscountBankSecuritiesService:
        return self._svc
