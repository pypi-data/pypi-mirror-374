...
from israel_open_banking.core.ais import (
    ...
    TransactionDetails,
    BookingStatus, )
from israel_open_banking.psd2_general.discount.clients.discount_account_manager import DiscountAccountManager
from israel_open_banking.psd2_general.discount.clients.discount_balance_manager import DiscountBalanceManager
from israel_open_banking.psd2_general.discount.clients.discount_transaction_manager import DiscountTransactionManager


class DiscountAISClient(AISBaseClient):
    """PSD2‑compliant AIS adapter for Discount Bank."""

    def __init__(self, service: DiscountBankService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: DiscountBankService = service

        # plug‑in concrete managers
        self.accounts: DiscountAccountManager = DiscountAccountManager(self)
        self.transactions: DiscountTransactionManager = DiscountTransactionManager(self)
        self.balances: DiscountBalanceManager = DiscountBalanceManager(self)
    ...
    @property
    def svc(self) -> DiscountBankService:
        return self._svc
