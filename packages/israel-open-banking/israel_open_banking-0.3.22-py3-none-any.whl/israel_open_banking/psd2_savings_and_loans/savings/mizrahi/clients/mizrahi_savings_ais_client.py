...
from israel_open_banking.psd2_savings_and_loans.savings.mizrahi.clients.mizrahi_savings_account_manager import \
    MizrahiSavingsAccountManager
from israel_open_banking.psd2_savings_and_loans.savings.mizrahi.clients.mizrahi_savings_balance_manager import \
    MizrahiSavingsBalanceManager
from israel_open_banking.psd2_savings_and_loans.savings.mizrahi.clients.mizrahi_savings_transaction_manager import \
    MizrahiSavingsTransactionManager
from israel_open_banking.psd2_savings_and_loans.savings.mizrahi.services.mizrahi_savings_bank_service import \
    MizrahiBankSavingsService
...
class MizrahiAISSavingsClient(AISSavingsBaseClient):
    """PSD2‑compliant AIS adapter for Mizrahi Bank."""

    def __init__(self, service: MizrahiBankSavingsService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: MizrahiBankSavingsService = service

        # plug‑in concrete managers
        self.accounts: MizrahiSavingsAccountManager = MizrahiSavingsAccountManager(self)
        self.transactions: MizrahiSavingsTransactionManager = MizrahiSavingsTransactionManager(self)
        self.balances: MizrahiSavingsBalanceManager = MizrahiSavingsBalanceManager(self)
    ...
    @property
    def svc(self) -> MizrahiBankSavingsService:
        return self._svc
