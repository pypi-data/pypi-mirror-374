...
from israel_open_banking.psd2_savings_and_loans.loans.fibi.clients.fibi_loans_account_manager import \
    FibiLoansAccountManager
from israel_open_banking.psd2_savings_and_loans.loans.fibi.clients.fibi_loans_balance_manager import \
    FibiLoansBalanceManager
from israel_open_banking.psd2_savings_and_loans.loans.fibi.clients.fibi_loans_transaction_manager import \
    FibiLoansTransactionManager
from israel_open_banking.psd2_savings_and_loans.loans.fibi.services.fibi_loans_bank_service import \
    FibiBankLoansService
...
class FibiAISLoansClient(AISLoansBaseClient):
    """PSD2‑compliant AIS adapter for Fibi Bank."""

    def __init__(self, service: FibiBankLoansService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: FibiBankLoansService = service

        # plug‑in concrete managers
        self.accounts: FibiLoansAccountManager = FibiLoansAccountManager(self)
        self.transactions: FibiLoansTransactionManager = FibiLoansTransactionManager(self)
        self.balances: FibiLoansBalanceManager = FibiLoansBalanceManager(self)
    ...
    @property
    def svc(self) -> FibiBankLoansService:
        return self._svc
