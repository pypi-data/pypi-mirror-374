...
from israel_open_banking.psd2_savings_and_loans.loans.discount.clients.discount_loans_account_manager import \
    DiscountLoansAccountManager
from israel_open_banking.psd2_savings_and_loans.loans.discount.clients.discount_loans_balance_manager import \
    DiscountLoansBalanceManager
from israel_open_banking.psd2_savings_and_loans.loans.discount.clients.discount_loans_transaction_manager import \
    DiscountLoansTransactionManager
from israel_open_banking.psd2_savings_and_loans.loans.discount.services.discount_loans_bank_service import \
    DiscountBankLoansService
...
class DiscountAISLoansClient(AISLoansBaseClient):
    """PSD2‑compliant AIS adapter for Discount Bank."""

    def __init__(self, service: DiscountBankLoansService) -> None:
        super().__init__(client=None)  # We do not use a generic OpenBankingClient
        self._svc: DiscountBankLoansService = service

        # plug‑in concrete managers
        self.accounts: DiscountLoansAccountManager = DiscountLoansAccountManager(self)
        self.transactions: DiscountLoansTransactionManager = DiscountLoansTransactionManager(self)
        self.balances: DiscountLoansBalanceManager = DiscountLoansBalanceManager(self)
    ...
    @property
    def svc(self) -> DiscountBankLoansService:
        return self._svc
