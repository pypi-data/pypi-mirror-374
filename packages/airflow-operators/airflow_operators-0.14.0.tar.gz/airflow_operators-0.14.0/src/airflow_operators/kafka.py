...
class ConsumeMessageDataXcomOperator(CustomConsumeFromTopicOperator):
...
# Для совместимости
class ConsumeMessageDataOperator(ConsumeMessageDataXcomOperator):
    ...
...
class ConsumeMessageDataS3Operator(CustomConsumeFromTopicOperator):
    ...
    @classmethod
    def messages_process(cls, messages: Sequence[Any], context) -> None:
        s3_key = cls._generate_s3_key(
            task_id=context.get('task').task_id,
            dag_id=context.get('dag').dag_id,
            run_id=context.get('dag_run').run_id,
            key=cls.MESSAGES_KEY,
            map_index=0,
            suff="",
        )
        values = [json.loads(message.value().decode('utf-8')) for message in messages]

        context.get('ti').xcom_push(key=cls.S3_PATH_KEY, value=s3_key)
        context.get('ti').xcom_push(key=cls.MESSAGES_COUNT_KEY, value=str(len(values)))

        s3_hook = S3Hook(aws_conn_id=cls.S3_CONNECTION_ID)
        s3_hook.load_string(
            string_data=json.dumps(values),
            bucket_name=cls.S3_BUCKET,
            key=s3_key,
            replace=True,
        )
